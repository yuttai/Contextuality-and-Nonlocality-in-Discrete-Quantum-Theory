...
\NewDocumentEnvironment{suggest}{m m}{#1{TEXT \# #2 would go here.}}{#1{TEXT \# #2 end here.}}
...
\begin{suggest}{\andy}{0}
  In this investigation, we explore the implications of extending
  point-like quantum-mechanical probability measures to finite
  intervals. 
\end{suggest}
...
We start our investigation with the concept of $\delta$-determinism, where~$\delta$
...
%% ajh kill "In more detail"
\begin{suggest}{\andy}{1}
quantum-mechanical contextuality continues to hold, maintaining the 
Kochen-Specker result, but at a   \emph{certain fixed value} of
$\delta$, there is a sharp transition to   \emph{non-contextuality},
parallel in character to, say, a  phase transition.
...
between what appeared previously to be irreconcilable aspects
of contextuality.
\end{suggest}
...
Gleason's theorem. 
\begin{suggest}{\andy}{2}
We then demonstrate that all QIVPMs in this class are
consistent with non-empty ``balls'' of quantum states whose radii
correspond to the lengths of the intervals characterizing the
uncertainties.  As the sizes of the intervals go to zero, these balls
of quantum states converge to a dense, continuous, set of points
representing the unique states consistent with the Born rule and
Gleason's theorem.
\end{suggest}
%% ajh: above starting at "We then demonstrate" LONG SUGGESTED TEXT #2
%% would go here: use PLURAL, ball -> balls, and then, to me, it would
%% be better as "..converges to a dense set of points."
...
%% ajh: Inverted logic: LONG SUGGESTED 
\begin{suggest}{\yutsung}{3}
...
system.  Specifically, for each mixed states $\rho =
\sum_{j=1}^{N}q_{j}\proj{\phi_{j}}$, where $\ket{\phi_j} \in {\cal H}$, $q_j >
0$, and $\sum_{j=1}^{N}q_{j}=1$, the Born rule
induces a probability measure~$\muB_{\rho}$ defined as
$\muB_{\rho}\left(P\right) = \Tr\left(\rho P\right) =
\sum_{j=1}^{N}
\melem{\phi_{j}}{P}{\phi_{j}}$~\citep{Born1983bibTeX,Mermin2007,Jaeger2007}.
\end{suggest}
\begin{suggest}{\andy}{3}
If all the conditions are satisfied, and one knows all the
  $\ket{\phi_j} = \sum_{k=1}^{d}\alpha_{k}\ket{k}$ representing
the current state $\rho$ of the quantum system, then a conventional
probability measure can easily be
constructed~\citep{Born1983bibTeX,Mermin2007,Jaeger2007},  taking the 
form
\[ \muB_{\rho}\left(P\right) = \Tr\left(\rho P\right) =
\sum_{j=1}^{N}  q_{j}\muB_{\phi_{j}}\left(P\right) \ . \]
\end{suggest}
A conventional quantum probability measure can easily be constructed
using the Born rule if one knows the current state of the quantum
...
%% ajh: big problems here: LONG SUGGESTED 
\begin{suggest}{\andy}{5}
The above construction assumes a mathematical idealization
having known quantum state and infinitely precise measuring
instruments.  In an actual experimental setup with an ensemble of
quantum states that would ideally be identical, but are not actually
identically prepared, with imperfections and inaccuracies in the
...
\end{suggest}
%% To
%% begin with, "axioms" is COMPLETELY undefined, appears NOWHERE in
%% the text except here.  So either NAME and LIST (instead of inline
%% text)  the several statements around Eq. (6) as axioms, or use
%% another term.  In addition the english "state of the quantum" is
%% wrong here.  Including fixing that, I would replace the first two
%% sentences by TEXT # 5.  It's really strangely written.
...
quantum state is known and involving infinitely precise
...
next.
%% ajh; previous sentence: I am aware of no previous definition of the
%% concept of "the constraints needed to maintain coherence" or what
%% that possibly means in definable sense. I do not know how to fix
%% it, or even if anyone else has trouble with this phrase as I did.
...
%% ajh: inverted logic and phrasing.  I would start at "To formalize
%% this ..." and replace all the way to Eq (9) by LONG SUGGESTED 
\begin{suggest}{\andy}{6}
...
projector~$P$ if the interval $\bar{\mu}(P)$ contains the exact
probability calculated by the Born
rule~\citep{Born1983bibTeX,Mermin2007,Jaeger2007} , i.e, if:

 To formalize this connection, we first note that in contrast to
the case with conventional quantum probability spaces, there is
generally no unique state~$\rho$ consistent with a QIVPM. We say that
a QIVPM $\bar{\mu}$ is \emph{consistent} with a state $\rho$ and
projector~$P$ if the interval $\bar{\mu}(P)$ contains the exact
probability calculated by the Born rule,
  \begin{equation}
...
%%ajh: This story order is incomprehensible.  In my suggested
%% replacement: LONG SUGGESTED 
\begin{suggest}{\andy}{7}
...
%% ajh - inverted narrative order, suggested fix in 
%%   LONG SUGGESTED 
\begin{suggest}{\andy}{8}
