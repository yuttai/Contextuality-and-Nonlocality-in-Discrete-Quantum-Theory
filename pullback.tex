...
\documentclass[11pt,ngerman,english]{article}
...
The means by which quantum computing\footnote{We use the phrase \textquotedblleft conventional quantum theory\textquotedblright{}
where necessary to distinguish the usual quantum theory and quantum
computing paradigm using (continuous) complex numbers from discrete
quantum theory. Alternative terminology in the literature includes
\textquotedblleft actual,\textquotedblright{} \textquotedblleft standard,\textquotedblright{}
and \textquotedblleft ordinary\textquotedblright{} quantum theory. } extends the capacity of classical computing frameworks deeply involves
both the laws of physics and the mathematical principles of computation.
Richard Feynman and \foreignlanguage{ngerman}{Rolf Landauer} \cite{Feynman1982Simulating,Landauer1996188},
among others, have strongly advocated the careful study of quantum
computation to understand its mechanisms and the source of its computational
features. To tease apart a number of subtle properties of quantum
computing, we previously replaced complex continuous numbers by finite
fields to study how uncomputable numbers affect quantum theory \cite{geometry2013,DQT2014},
and replaced real-valued probability measures by interval-valued probability
measures to study the effect of experimental imperfections and finite
precision measurements~\cite{THOS2018}. This time we try to decompose
a quantum probability space as a family of classical probability spaces
to see which part of quantum probability space has a classical counterpart,
to have a unified notation for classical and quantum probability for
this part, and to separate ``quantum-ness'' from the classical part
in a quantum probability description.\footnote{We might also put ``equivalence principle'' as one of the motivation.
Equivalence principle, gravitational and inertial forces are indistinguishable,
is the foundation to build relativity, and we are stating that in
some situation classical and quantum random process are indistinguishable...}
...
Since this abstraction doesn't specify the process to generate the
outcomes, we could use the same abstraction to describe both classical
and quantum processes, including tossing a coin and one Stern-Gerlach
type experiment \cite{:/content/aip/journal/jmp/21/1/10.1063/1.524312,peres1995quantum,schwinger2001quantum}. 

Although a quantum process could be described by a probability space,
the probability spaces describing a family of quantum experiments
might contain some redundant information which could be used to glue
these probability spaces together to define a quantum probability
...
$\mu\left(\Omega\right)=1$, and $\mu\left(\Omega\backslash E\right)=\mu\left(\overline{E}\right)=1-\mu\left(E\right)$,
where $\Omega\backslash E$ is the complement of $E$, and we may
simply write $\Omega\backslash E$ as $\overline{E}$ if the sample
space~$\Omega$ is clear from the context.  Moreover, we require
...
\footnote{Should find a ``simple'' example comparing the classical and quantum
process... Maybe Stern-Gerlach experiment with classical particle?}
...
Stern-Gerlach type experiment \cite{:/content/aip/journal/jmp/21/1/10.1063/1.524312,peres1995quantum,schwinger2001quantum}.
...
outcomes is $\Omega_{z}=\{\ket{0},\ket{1},\ket{2}\}$. All possible
...
and $\Omega_{z}$. Each time we perform this experiment, it induces
a classical probability measure $\mu_{z}\colon2^{\Omega_{z}}\rightarrow\left[0,1\right]$.
Moreover, if we prepare the particles in the ``same'' way as the
particle inducing $\mu_{z}$, they will induce the same classical
probability measure $\mu_{z}$.
...
as $\ket{2}$. Although the sample space $\Omega_{x}=\left\{ \ket{\ps},\ket{\ms},\ket{2}\right\} $
and the probability measure $\mu_{x}\colon2^{\Omega_{x}}\rightarrow\left[0,1\right]$
...
same, i.e., 
\begin{equation}
\mu_{z}\left(\left\{ \ket{2}\right\} \right)=\mu_{x}\left(\left\{ \ket{2}\right\} \right)\,.\label{eq:mu0-equal-mu1}
\end{equation}


\subsection{The Source of Probability and Contextuality}

One of reason why we need probability is because we don't know the
detail to generate the outcomes. For example, a number might be deterministically
generated by a random number generator, but looks random for whom
don't know the seed and the algorithm. One might calculate whether
a flipped coin will be head up or tail up according to its initial
condition, but flipping coin looks like a random process for whom
don't know that. Whether the randomness of a quantum process is also
due to lack of knowledge was asked by Einstein, Podolsky, and Rosen
(EPR)~\cite{EPR1935,peres1995quantum,544199,Mermin2007}. In the
language of our previous examples, they guess there might be some
unknown physical laws such that if we know the initial condition of
the particle we could predict with certain that one of the outcomes
$\omega_{z}\in\Omega_{z}$ will happen.

EPR's question apparently looks hard, but could be explained if we
assume the predictions among these processes are correlated and consider
more experimental processes. To simplify the discussion, we will represent
the certain outcome $\omega_{z}$ as its characteristic function $\chi_{z}:\Omega_{z}\rightarrow\left\{ 0,1\right\} $
defined by
\begin{equation}
\chi_{z}\left(\omega\right)=\begin{cases}
1 & \textrm{if }\omega=\omega_{z}\,;\\
0 & \textrm{if }\omega\ne\omega_{z}\,,
\end{cases}
\end{equation}
and we will represent the hypothetical certain prediction of experiment
$\mu_{x}$ as another characteristic function $\chi_{x}:\Omega_{x}\rightarrow\left\{ 0,1\right\} $.
To have an idea how these characteristic functions could be correlated,
we find their probabilities of event $\left\{ \ket{2}\right\} $ are
the same no matter which process generates it as in Eq.~(\ref{eq:mu0-equal-mu1}).
We may generalize this idea to assume that the characteristic functions
satisfying a similar condition,
\begin{equation}
\chi_{z}\left(\ket{2}\right)=\chi_{x}\left(\ket{2}\right)\,,\label{eq:equality-among-chi}
\end{equation}
i.e., whether the hypothetical certain prediction is either $\ket{2}$
or not $\ket{2}$ are the same for two experiments $\mu_{z}$ and
$\mu_{x}$\footnote{To justify this assumption, we might need to assume that if we factor
an experiment as the composition of partial experiments~\cite{peres1995quantum},
the certain outcome of the original experiment is also the composition
of the certain outcome of the partial experiments... etc. However,
this kind of assumption is not as obvious as other assumptions we
discussed in Sec.~\ref{subsec:Classical-and-Quantum}.}. 

Since the common part of characteristic functions are the same, it
might be easier to glue these characteristic functions together and
consider $\chi:\Omega_{z}\cup\Omega_{x}\rightarrow\left\{ 0,1\right\} $
defined by
\begin{equation}
\chi\left(\omega\right)=\begin{cases}
\chi_{z}\left(\omega\right) & \textrm{if }\omega\in\Omega_{z}\,;\\
\chi_{x}\left(\omega\right) & \textrm{if }\omega\in\Omega_{x}\backslash\Omega_{z}\,.
\end{cases}
\end{equation}
$\chi$ is the natural generalization of $\chi_{z}$ and $\chi_{x}$
because if we restrict the function $\chi$ in a smaller domain $\Omega_{z}$,
the resulting function $\chi\restriction_{\Omega_{z}}$ is exactly
the same as $\chi_{z}$. Also, $\chi\restriction_{\Omega_{x}}=\chi_{x}$.

We could further extend this idea to consider more experiments with
different set of outcomes $\left\{ \Omega_{i}\right\} _{i=1}^{M}$.
If we could predict with certain that one of the outcomes $\omega_{i}\in\Omega_{i}$
will happen for every experiment, there is a characteristic function
$\chi:\bigcup_{i=1}^{M}\Omega_{i}\rightarrow\left\{ 0,1\right\} $
such that 
\begin{equation}
\sum_{\omega\in\Omega_{i}}\chi\left(\omega\right)=1\label{eq:characteristic_one}
\end{equation}
 for every set of outcomes $\Omega_{i}$. In contrast, if there is
no characteristic function $\chi:\bigcup_{i=1}^{M}\Omega_{i}\rightarrow\left\{ 0,1\right\} $
satisfying Eq.~(\ref{eq:characteristic_one}), then there is no certain
prediction consistent with the outcomes $\left\{ \Omega_{i}\right\} _{i=1}^{M}$
of all the experiments. 

After considering multiple experiments, we observe that the original
Kochen-Specker theorem asserts that the characteristic function doesn't
exists when we consider enough experiments.\footnote{Then we may put the 20-vector proof of the Kochen-Specker theorem
here if we want...}
...
\cite{peres1995quantum,schwinger2001quantum}, we will then postulate
any normalized complex vector spanned by $\ket{0}$, $\ket{1}$, and
$\ket{2}$ could be a measurement outcome, any orthonormal basis form
a sample space, and sending a particle to this orthonormal basis inducing
a classical probability measure. In general, as long as sending the
same particle, the probability of the same event in different experiments
should always be the same, i.e., giving two induced classical probability
...
\begin{equation}
\mu_{z}\left(\left\{ \ket{0},\ket{1}\right\} \right)=1-\mu_{z}\left(\left\{ \ket{2}\right\} \right)=1-\mu_{x}\left(\left\{ \ket{2}\right\} \right)=\mu_{x}\left(\left\{ \ket{\ps},\ket{\ms}\right\} \right)\,,
\end{equation}
