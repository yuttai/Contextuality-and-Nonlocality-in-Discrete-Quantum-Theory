...
defined by precomposition is a classical probability measure because
\begin{subequations}\label{eq:CPM-constraints}
\begin{eqnarray}
 &  & \mu\left(\varphi(\Omega)\right)=\mu\left(\mathbb{1}\right)\overset{\textrm{Eq.~(\ref{eq:QuantumProbability})}}{=}1\,,\label{eq:CPM-necess}\\
 &  & \mu\left(\varphi\left(\overline{E}\right)\right)=\mu\left(\mathbb{1}-\varphi\left(E\right)\right)\overset{\textrm{Eq.~(\ref{eq:QuantumProbability})}}{=}1-\mu\left(\varphi\left(E\right)\right)\,,\label{eq:CPM-complement}\\
...
\end{eqnarray}
\end{subequations}
and for each pair of disjoin $E_{0}$ and $E_{1}$: 
\begin{equation}
...
\overset{\textrm{Eq.~(\ref{eq:pullback-properties-basic})}}{=}{} & \mu\left(\varphi\left(E_{0}\right)+\varphi\left(E_{1}\right)-\left(\mathbb{1}-\mathbb{1}\right)\right)\\
...
\end{equation}
Since that ``precomposition'' is called ``pullback'' in differential
...
call $\varphi^{*}\mu\colon2^{\Omega}\rightarrow\left[0,1\right]$
