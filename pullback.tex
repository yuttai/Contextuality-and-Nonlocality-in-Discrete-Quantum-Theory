...
a flipped coin will be head up or tail up, but flipping coin looks
like a random process for whom don't know the initial condition. Whether
the randomness of a quantum process is also due to lack of knowledge
was asked by Einstein, Podolsky, and Rosen (EPR)~\cite{EPR1935,peres1995quantum,544199,Mermin2007}.
In the language of our previous examples, they guess there might be
some unknown physical laws such that if we know the initial condition
of the particle we could predict with certain that one of the outcomes
...
more experimental processes. To simplify the later discussion, we
will represent the certain outcome $\omega_{z}$ as its characteristic
function $\chi_{z}:\Omega_{z}\rightarrow\left\{ 0,1\right\} $ defined
by
\begin{equation}
...
Notice that Eq.~(\ref{eq:mu0-equal-mu1}) states the probability
of event $\left\{ \ket{2}\right\} $ should be the same no matter
which process to generate the outcomes. We may generalize this idea
to assume the characteristic functions of the common outcomes to be
the same, i.e., 
\begin{equation}
