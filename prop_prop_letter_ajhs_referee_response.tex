...
and the expectation value of every observable is the product of the
other two as well. In the $j=2$ column, we have instead that $\left(\sigma_{z}\otimes\sigma_{z}\right)\left(\sigma_{x}\otimes\sigma_{x}\right)=-\sigma_{y}\otimes\sigma_{y}$
and 
\begin{equation}
...
it is impossible to get the following measurement results because
the product of the expectation values in the $j=2$ column violate
Eq.~(\ref{eq:j_2_expectation_value}).
\begin{center}
\begin{tabular}{ccc}
\toprule 
\addlinespace
...
Since the experimental results are missing, they might come form the
