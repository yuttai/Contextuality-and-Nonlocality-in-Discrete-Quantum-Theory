...
of confidence. In contrast with the classical case, we found that
there may be no ``real'' infinitely-precise quantum state that is
being approximated when the approximating interval-value probability
measure is not sufficiently accurate. However, a coarse quantum interval-valued
probability measure might still be induced by many different quantum
states when we examine different part of its probability assignment.
Moreover, our examples and Gleason's theorem suggests that as the
measurement resources increase, interval-valued probability measures
could more and more likely be identified to a particular state as
shown in table~\ref{table_long}. 
\begin{table}[ht]
\center{%
