...
which is in $S^{2}$. In general, we could discuss the whole $\bar{f}^{-1}\left(\imposs\right)\subseteq\mathbb{C}S^{2}$.
However, restricting to $S^{2}$ enable us to visualize and imagine
easily. Also, if a interval-valued frame function~$\bar{f}$ has
an empty core when restricting to $S^{2}$, $\bar{\mu}$ has an empty
core on the original space as well. So restricting to $S^{2}$ still
provides valid results.} In contrast, if $\bar{f}^{-1}\left(\imposs\right)\cap S^{2}$ contains
...
would be smaller so that $\bar{f}$ is closer to the frame function
derived from the Born rule.
