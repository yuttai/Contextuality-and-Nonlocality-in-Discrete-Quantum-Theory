...
numbers~\cite{geometry2013,DQT2014}. In this proposal, our aim is to
...
of confidence. This raises an important philosophical question with
immense practical relevance in the context of quantum computing. The
question, which we could largely bypass in the classical case, is
whether there is, in the background of all the measurements and probabilities,
a ``real'' infinitely-precise quantum state that is being approximated
and that, in principle, can be more and more accurately described
given more and more resources. In particular, we could summarize our
examples and Gleason's theorem in table~\ref{table_long}. 
\begin{table}[ht]
\center{%
\begin{tabular}{c|cccc}
Measurement resources & Lowest & \multicolumn{2}{c}{$\longleftrightarrow$} & Highest\tabularnewline
\hline 
Number of intervals & $3$ & $4$ & \ldots{} & $\infty$\tabularnewline
\hline 
Maximum difference between ends of intervals & $1$ & $1/2$ & \ldots{} & $0$\tabularnewline
\hline 
Number of states in the core & 0 & 0 &  & 1\tabularnewline
\hline 
Number of states might corresponding to them & $3$ & $2$ & \ldots{} & $1$\tabularnewline
\hline 
How precise we could identify a state? & Coarse & \multicolumn{2}{c}{$\longleftrightarrow$} & Precise\tabularnewline
\end{tabular}}\caption{Summarize example~\ref{ex:three-dimensional-three-value} and \ref{ex:three-dimensional-4-value}
and Gleason's theorem}
\label{table_long}
\end{table}
Although the tend is clear, we still need more examples to fill the
blank and to prove our idea. Some research questions might be
\begin{description}
\item [{Research Question.}] Confirm that there is always an interval-valued
probability measure with an empty-core if the length of the interval
is bigger than zero. 
\item [{Research Question.}] In example~\ref{ex:three-dimensional-three-value}
and \ref{ex:three-dimensional-4-value}, we pick a possible candidate
corresponding to an interval-valued probability measure by only considering
the state whose interval-valued probability is $\necess$. Define
a general method which could be used to determine whether an interval-valued
probability measure is closer to the Born rule or not. 
...
\end{description}
Since the validity of Gleason's theorem in a world with finite-precision
measurements is a question, the natural follow-up questions is to
investigate the status of the theorems of Bell~\cite{BellBook1987,Redhead1987-REDINA,peres1995quantum,Jaeger2007}
and Kochen-Specker for quantum interval-valued probability measures
as well. Going further we actually have to concede not just the infinite
accuracy of the quantum state, but its very existence as a ``real''
entity that exists independently of measurements and probabilities.
The reasons for this second radical position are as follows. First
arguments embedded in the proof of the Kochen-Specker theorem essentially
rule out the existence of a sample space as the probabilities of events
cannot be consistently expressed using core assignments to elements
of the sample space. Second, the frequency interpretation of probability
