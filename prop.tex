...
Notice that the set of 1-dimensional projectors mapping to impossible,
$\bar{\mu}^{-1}\left(\imposs\right)=\left\{ \ket{\psi}\in\mathbb{C}^{3}~\middle|~\bar{\mu}(\proj{\psi})=\imposs\right\} $
provides a appealing way to suggest a possible state inducing $\bar{\mu}$.
Since for any two states~$\ket{\psi_{0}}$ and $\ket{\psi_{1}}\in\bar{\mu}^{-1}\left(\imposs\right)$,
we have $\bar{\mu}(\proj{\psi_{0}})=\bar{\mu}(\proj{\psi_{1}})=\imposs$
so that $\bar{\mu}$ might be induced by the state~$\ket{\psi_{0}}\times\ket{\psi_{1}}$\footnote{\yutsung{Just notice that I only focus on $\mathbb{R}^{3}$ instead
of $\mathbb{C}^{3}$. Although this might be just tedious details
in this approach, still need to verify carefully.}}. Notice that even if $\ket{\psi_{0}}$ and $\ket{\psi_{1}}$ are
normalized, when they are not orthogonal, $\ket{\psi_{0}}\times\ket{\psi_{1}}$
is not normalized. We will use $\ket{\psi_{0}}\times\ket{\psi_{1}}$
and the normalized $\ket{\psi_{0}}\times\ket{\psi_{1}}$ interchanged
when there is no confusion. There are 4 different cases.
\begin{enumerate}
...
of $\bar{\mu}$. Actually, $\bar{\mu}$ has an empty core. 
...
we could have some idea of the pure states which are in core.

As we did in Fig.~\ref{fig:three-dimensional-4-value}, 

\section{Subset of Computing Events}
