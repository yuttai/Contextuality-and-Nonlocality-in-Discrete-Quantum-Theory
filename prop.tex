...
in the Hilbert space of dimension 3.\footnote{Notice that we only discuss only discuss real 1-dimensional projectors
...
About the cross product, even if $\ket{\psi_{0}}$ and $\ket{\psi_{1}}$
are normalized, when they are not orthogonal, $\ket{\psi_{0}}\times\ket{\psi_{1}}$
is not normalized. We will use $\ket{\psi_{0}}\times\ket{\psi_{1}}$
and the normalized $\ket{\psi_{0}}\times\ket{\psi_{1}}$ interchanged
...
\begin{definition}\cite{gleason1957,CookeKeaneMoran1985,peres1995quantum,RichmanBridges1999,Hamhalter2013}
For any quantum probability measure~$\mu$ on the Hilbert space of
dimension 3, we can define a \emph{frame function}~$f:\mathbb{C}S^{2}\rightarrow\left[0,1\right]$
...
\begin{lemma}\cite{gleason1957,CookeKeaneMoran1985,peres1995quantum,RichmanBridges1999,Hamhalter2013}
A frame function~$f:\mathbb{C}S^{2}\rightarrow\left[0,1\right]$
satisfies the following properties:
\begin{itemize}
\item $f\left(\lambda\ket{\psi}\right)=f\left(\ket{\psi}\right)$ for any
$\ket{\psi}\in\mathbb{C}S^{2}$ and $\lambda\in\mathbb{C}S^{0}$.
\item $1=\sum_{i=1}^{3}f\left(\ket{\psi_{i}}\right)$ for any orthonormal
basis~$\left\{ \ket{\psi_{i}}\right\} _{i=1}^{3}$ of $\mathbb{C}^{3}$.
\end{itemize}
...
\item $1\in\sum_{i=1}^{3}\bar{f}\left(\ket{\psi_{i}}\right)$ for any orthonormal
basis~$\left\{ \ket{\psi_{i}}\right\} _{i=1}^{3}$ of $\mathbb{C}^{3}$.
\end{itemize}
\end{lemma}

Therefore, the set of 1-dimensional projectors mapping to impossible,
denoted by $\bar{\mu}^{-1}\left(\imposs\right)=\set{\ket{\psi}\in\mathbb{C}^{3}}{\bar{\mu}(\proj{\psi})=\imposs}$,
plays an important role to understand $\bar{\mu}$. There are 4 different
cases when analyzing $\bar{\mu}^{-1}\left(\imposs\right)$:
\begin{enumerate}
\item If another pair of states~$\ket{\psi_{0}'}$ and $\ket{\psi_{1}'}\in\bar{\mu}^{-1}\left(\imposs\right)$
...
might induce $\bar{\mu}$, but neither $\mu_{\ket{\psi_{0}}\times\ket{\psi_{1}}}^{B}$
nor $\mu_{\ket{\psi_{0}'}\times\ket{\psi_{1}'}}^{B}$ in the core
of $\bar{\mu}$ because $\mu_{\ket{\psi_{0}}\times\ket{\psi_{1}}}^{B}$
is inconsistent with $\bar{\mu}(\proj{\psi_{0}'})=\bar{\mu}(\proj{\psi_{1}'})=\imposs$
and $\mu_{\ket{\psi_{0}'}\times\ket{\psi_{1}'}}^{B}$ is inconsistent
with $\bar{\mu}(\proj{\psi_{0}})=\bar{\mu}(\proj{\psi_{1}})=\imposs$.
Actually, $\bar{\mu}$ has an empty core. 
\item If $\ket{\psi_{0}}\times\ket{\psi_{1}}$ and $\ket{\psi_{0}'}\times\ket{\psi_{1}'}$
always represent the same state for all $\ket{\psi_{0}'}$ and $\ket{\psi_{1}'}\in\bar{\mu}^{-1}\left(\imposs\right)$,
then $\ket{\psi_{0}}\times\ket{\psi_{1}}$ is the only pure state
which might induce $\bar{\mu}$. In another word, if $\mu_{\phi}^{B}\in\mathrm{core}\left(\bar{\mu}\right)$
for some pure state~$\ket{\phi}$, then $\ket{\phi}$ and $\ket{\psi_{0}}\times\ket{\psi_{1}}$
represent the same state. 
\item $\bar{\mu}^{-1}\left(\imposs\right)$ may contain only one state~$\ket{\psi_{0}}$.
In this case, $\ket{\psi_{0}}\times\ket{\psi_{1}}$ is undefined.
If $\mu_{\phi}^{B}\in\mathrm{core}\left(\bar{\mu}\right)$ for some
pure state~$\ket{\phi}$, $\ket{\phi}$ could be any state orthogonal
to $\ket{\psi_{0}}$.
\item $\bar{\mu}^{-1}\left(\imposs\right)=\emptyset$. If $\mu_{\phi}^{B}\in\mathrm{core}\left(\bar{\mu}\right)$
for some pure state~$\ket{\phi}$, we could say nothing about $\ket{\phi}$.
\end{enumerate}
In summary, if we have an idea of $\bar{\mu}^{-1}\left(\imposs\right)$,
...
$\bar{\mu}^{-1}\left(\imposs\right)$ in the above case 2 - 4 does
not contain any area in $S^{2}$. In contrast, if $\bar{\mu}^{-1}\left(\imposs\right)$
...
Therefore, the area of $\bar{\mu}^{-1}\left(\imposs\right)$ could
indicate how much $\bar{\mu}$ deviate from the Born rule probability
measures.

In order to simplify the discussion, let's set the center of $\bar{\mu}^{-1}\left(\imposs\right)$
as the north pole, and assume $\bar{\mu}$ maps every state whose
latitude is larger than $\theta_{0}$ to impossible. In order to simplify
the notation, let's give the following definition.
