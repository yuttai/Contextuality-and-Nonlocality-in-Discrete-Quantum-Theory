...
although quantum IVPMs have empty cores, we might think that the probability
measure~$\bar{\mu}$ is induced by some states. In example~\ref{ex:three-dimensional-three-value},
...
In example~\ref{ex:three-dimensional-4-value}, the probability measure
$\bar{\mu}$ might be induced by the state $\ket{2}$ because $\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps})=\imposs$.
...
\begin{definition}A function~$\bar{f}:S^{d-1}\rightarrow\mathscr{I}$
or $\bar{f}:\mathbb{C}S^{d-1}\rightarrow\mathscr{I}$ is called a
\emph{real} or \emph{complex interval-valued approximation frame function}\end{definition}
