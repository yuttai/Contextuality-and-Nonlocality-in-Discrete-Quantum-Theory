...
$\bar{\mu}^{-1}\left(\imposs\right)=\left\{ \ket{\phi}\in\mathbb{C}^{3}\middle|\bar{\mu}(\proj{\phi})=\imposs\right\} $
provides a appealing way to suggest a possible state inducing $\bar{\mu}$.
Since for any two states~$\ket{\phi_{0}},\ket{\phi_{1}}\in\bar{\mu}^{-1}\left(\imposs\right)$,
we have $\bar{\mu}(\proj{\phi_{0}})=\bar{\mu}(\proj{\phi_{1}})=\imposs$
so that $\bar{\mu}$ might be induced by the state $\ket{\phi_{0}}\times\ket{\phi_{1}}$\footnote{\yutsung{Just notice that I only focus on $\mathbb{R}^{3}$ instead
of $\mathbb{C}^{3}$. Although this might be just tedious details
in this approach, still need to verify carefully.}}.
