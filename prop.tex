...
\begin{lemma}An interval-valued frame function~$\bar{f}:\mathbb{C}S^{2}\rightarrow\mathscr{I}$
...
behaves classically when restricting the domain on $\events'$, i.e.,
consider $\bar{\mu}|_{\events^{\Omega}}$. In particular, if $\bar{\mu}$
is convex, each $\bar{\mu}|_{\events^{\Omega}}$ has a non-empty core.
However, when they are put together, there is no quantum probability
measure~$\mu:\events\rightarrow\left[0,1\right]$ whose restriction
$\mu|_{\events^{\Omega}}$ inside all the core of $\bar{\mu}|_{\events^{\Omega}}$.
Notice that every events in $\events^{\Omega}$ are commuting, which
consistent with the idea of the Kochen-Specker theorem, that each
triple of commuting observables could be colored well, but we cannot
color all observables which are not all commuting.\footnote{\yutsung{Since Bell's theorem only applies to entangled states, the
...
\begin{lemma}Given a quantum interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$,
if $\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$ is a classical
convex IVPM for any orthonormal basis~$\Omega$, then $\bar{\mu}$
is a quantum convex IVPM.\end{lemma}
\begin{proof}
...
and $P_{1}=\sum_{\ket{j}\in E_{1}}\proj{j}$. Then, 
\begin{eqnarray*}
...
or equal to 3, every classical convex IVPM~$\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$.\end{lemma}
\begin{proof}
...
quantum interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
is convex.\end{thm}
\begin{proof}
The direct consequence of the previous lemmas.
\end{proof}

\section{Properties of Quantum IVPMs Decided by Their Range}

\begin{lemma}Consider a collection of intervals~$\mathscr{I}$ with
a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ such that
$\iota\left(0\right)=\left[0,0\right]$, $\iota\left(1\right)=\left[1,1\right]$,
$\iota\left(1-x\right)=\left[1,1\right]-\iota\left(x\right)$, and
$\iota\left(\sum{}_{i=1}^{N}x_{i}\right)\subseteq\sum{}_{i=1}^{N}\iota\left(x_{i}\right)$.
...
by $\bar{\mu}(P)=\iota\left(\mu(P)\right)$.\end{lemma}
\begin{proof}
~
\begin{itemize}
...
\item For a set of mutually orthogonal projections $\left\{ P_{i}\right\} _{i=1}^{N}$,
we have $\bar{\mu}\left(\sum_{i=1}^{N}P_{i}\right)=\iota\left(\mu\left(\sum_{i=1}^{N}P_{i}\right)\right)=\iota\left(\sum_{i=1}^{N}\mu\left(P_{i}\right)\right)\subseteq\sum_{i=1}^{N}\iota\left(\mu\left(P_{i}\right)\right)=\sum_{i=1}^{N}\bar{\mu}\left(P_{i}\right)$. 
\end{itemize}
\end{proof}
\begin{lemma}Consider a collection of intervals~$\mathscr{I}$ with
a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ such that
...
$\iota\left(1-x\right)=\left[1,1\right]-\iota\left(x\right)$, and
$\iota\left(\sum{}_{i=1}^{N}x_{i}\right)\subseteq\sum{}_{i=1}^{N}\iota\left(x_{i}\right)$.
Then, there is a quantum interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
...
\end{proof}
\footnote{\yutsung{Examples of $\mathscr{I}$ with a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
such that $\iota\left(0\right)=\left[0,0\right]$, $\iota\left(1\right)=\left[1,1\right]$,
$\iota\left(1-x\right)=\left[1,1\right]-\iota\left(x\right)$, and
$\iota\left(\sum{}_{i=1}^{N}x_{i}\right)\subseteq\sum{}_{i=1}^{N}\iota\left(x_{i}\right)$.}}
