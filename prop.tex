...
of confidence. Despite the fact that a classical interval-value probability
measure could always correspond to some classical ``real'' infinitely-precise
probability measure, we found that there may be no ``real'' infinitely-precise
quantum state that is being approximated by a quantum interval-value
probability measure as a whole. However, if we break a quantum interval-valued
probability measure into pieces, each piece might still be induced
by some quantum states. Moreover, our examples and Gleason's theorem
suggests that as the measurement resources increase, an entire interval-valued
probability measure could more and more likely be identified to a
particular state as shown in table~\ref{table_long}. 
\begin{table}[ht]
\center{%
