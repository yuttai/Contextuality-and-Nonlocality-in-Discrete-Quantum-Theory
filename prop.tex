...
probability measure with an empty-core if the length of the interval is bigger
than zero.
\item[Research Question.] In example~\ref{ex:three-dimensional-three-value} and
\ref{ex:three-dimensional-4-value}, we pick a possible candidate corresponding
to an interval-valued probability measure by only considering the state whose
interval-valued probability is $\necess$. Define a general method which could be
used to determine whether an interval-valued probability measure is closer to
the Born rule or not.
\item[Research Question.] Confirm that as the number and
...
rule. 
\item[Research Question.] Investigate the status of the theorems
