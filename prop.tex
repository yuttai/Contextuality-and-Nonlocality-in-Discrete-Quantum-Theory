...
\begin{proof}
We first prove that a interval-valued frame function~$\bar{f}:\mathbb{C}S^{2}\rightarrow\mathscr{I}$
induced by a quantum IVPM~$\bar{\mu}$ satisfies the above properties:
\begin{itemize}
\item For any $\ket{\psi}\in\mathbb{C}S^{2}$ and $\lambda\in\mathbb{C}S^{0}$,
\[
\bar{f}\left(\lambda\ket{\psi}\right)=\bar{\mu}\left(\left(\lambda\ket{\psi}\right)\left(\lambda\ket{\psi}\right)^{\dagger}\right)=\bar{\mu}\left(\lambda\lambda^{*}\proj{\psi}\right)\overset{\lambda\in\mathbb{C}S^{0}}{=}\bar{\mu}\left(\proj{\psi}\right)=\bar{f}\left(\ket{\psi}\right)\textrm{ .}
\]
\item For any orthonormal basis~$\left\{ \ket{\psi_{i}}\right\} _{i=1}^{3}$,
we have 
\begin{eqnarray*}
 &  & \left[1,1\right]=\bar{\mu}(\mathbb{1})=\bar{\mu}\left(\sum_{i=1}^{3}\proj{\psi_{i}}\right)\subseteq\sum_{i=1}^{3}\bar{\mu}\left(\proj{\psi_{i}}\right)=\sum_{i=1}^{3}\bar{f}\left(\ket{\psi_{i}}\right)\\
 & \Rightarrow & 1\in\sum_{i=1}^{3}\bar{f}\left(\ket{\psi_{i}}\right)
\end{eqnarray*}
Also, 
\begin{eqnarray*}
 &  & \left[1,1\right]-\bar{f}\left(\ket{\psi_{i}}\right)=\left[1,1\right]-\bar{\mu}\left(\proj{\psi_{i}}\right)=\bar{\mu}\left(\mathbb{1}-\proj{\psi_{i}}\right)\\
 & = & \bar{\mu}\left(\proj{\psi_{j}}+\proj{\psi_{k}}\right)\subseteq\bar{\mu}\left(\proj{\psi_{j}}\right)+\bar{\mu}\left(\proj{\psi_{k}}\right)=\bar{f}\left(\ket{\psi_{j}}\right)+\bar{f}\left(\ket{\psi_{k}}\right)\textrm{ ,}
\end{eqnarray*}
where $i$, $j$, and $k$ are any permutations among $1$, $2$,
and $3$.
\end{itemize}
Given a function~$\bar{f}:\mathbb{C}S^{2}\rightarrow\mathscr{I}$
satisfies the above properties, consider $\bar{\mu}:\events\rightarrow\mathscr{I}$
defined by
\begin{itemize}
...
containing 2-dimensional projectors, the definition of $\bar{\mu}$
