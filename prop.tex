...
\begin{enumerate}
\item If another pair of states~$\ket{\psi_{0}'}$ and $\ket{\psi_{1}'}\in\bar{f}^{-1}\left(\imposs\right)$\footnote{\yutsung{{Check when }$\ket{\psi_{0}'}$ and $\ket{\psi_{1}'}$ are
complex?}} gives different state of cross product~$\ket{\psi_{0}'}\times\ket{\psi_{1}'}$,
...
how much $\bar{f}$ deviate from the Born rule probability measures.
