...
a real interval-valued frame function. Obviously, any results in section~\ref{sec:Interval-valued-Frame-Functions}
...
a corresponding condition. From now on, when we write an interval-valued
frame function~$\bar{f}$, we would always means a real interval-valued
frame function~$\bar{f}:S^{2}\rightarrow\mathscr{I}$ without other
specification.

If $\bar{f}$ has a non-empty core, $\bar{f}$ must be in the case
1 - 3 in the end of section~\ref{sec:Interval-valued-Frame-Functions}
...
must be coarse. Therefore, if the interval~$\mathscr{I}$ is finer,
then the area of $\bar{f}^{-1}\left(\imposs\right)$ would be smaller
so that $\bar{f}$ is closer to a Born rule frame function.
...
then for any $\ket{\psi_{0}}$ and $\ket{\psi_{1}}\in\angle^{-1}\left(\left[0,\theta_{0}\right]\right)$,
