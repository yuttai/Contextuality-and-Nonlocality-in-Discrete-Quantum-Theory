...
In general, each two 1-dimensional projectors~$\ket{\psi_{0}}$ and
$\ket{\psi_{1}}$ mapping to impossible suggests $\bar{\mu}$ might
be induced by the pure state~$\ket{\psi_{0}}\times\ket{\psi_{1}}$\footnote{Notice that even if $\ket{\psi_{0}}$ and $\ket{\psi_{1}}$ are normalized,
when they are not orthogonal, $\ket{\psi_{0}}\times\ket{\psi_{1}}$
is not normalized. We will use $\ket{\psi_{0}}\times\ket{\psi_{1}}$
and the normalized $\ket{\psi_{0}}\times\ket{\psi_{1}}$ interchanged
when there is no confusion.

\yutsung{Just notice that I only focus on $\mathbb{R}^{3}$ instead
of $\mathbb{C}^{3}$. Although this might be just tedious details
in this approach, still need to verify carefully.}}. Therefore, the set of 1-dimensional projectors mapping to impossible,
