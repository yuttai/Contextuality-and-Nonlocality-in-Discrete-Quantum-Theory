...
might help to regularize general QIVPMs, and when $\left\Vert \mathscr{I}\right\Vert $
...
By applying $\proj{\psi_{1}}+\proj{\psi_{2}}=\mathbb{1}-\proj{\psi_{0}}$
and Eq.~(\ref{eq:QIVPM-complement-states}) on the left-hand side,
...
the same and equal to $1$, Gleason's theorem assert that $\frameL$
...
know a little bit more, like its QIVPM is a composite function, to
...
model above QIVPMs, we might consider either manipulating the states
in the core of a QIVPM, or finding a way to manipulate QIVPMs directly.
However, both strategies are not straightforward. As we proved in
Thm.~\ref{thm:Non-extensible-of-Gleason's}, a QIVPM might have an
empty core which cannot be evolved over time. If we want to manipulate
and compute QIVPMs directly for multi-qubit algorithms, we need to
glue the QIVPM for each qubit or subsystem together to get the QIVPM
of the whole system, and this is not straightforward either.

A successful interval-valued theory might be further extended to the
quantum theory over finite fields, and consider the following quantum
interval-valued probability measures over finite fields.

\begin{definition}[Quantum Interval-valued Probability Measures
over Finite Fields]\label{def:QIVPMFF}Given a vector space~$\mathcal{H}$
of dimension~$D$ over the complexified field~$\Fpp$, the set of
events~$\events_{p^{2}}$ is defined as in Def.~\ref{def:QPMFF},
and $\mathscr{I}$ is a collection of intervals. Then, a quantum interval-valued
probability measure over finite field $\bar{\mu}:\events_{p^{2}}\rightarrow\mathscr{I}$
assigns a probability to each event (projection operator~$P$) subject
to $\bar{\mu}(\mathbb{0})=\imposs$, $\bar{\mu}(\mathbb{1})=\necess$,
...
and whether we could define any ``sensible'' Born upon them will
be the next natural extension for seeking a computable quantum theory.
