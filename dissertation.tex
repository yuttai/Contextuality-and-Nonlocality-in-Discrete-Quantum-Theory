...
me if there is any difficulty compile it in the other system. Besides,
to compile the PDF file from \LyX{} source, just finish step~\ref{enu:Clone-the-git}
...
only applies on the QIVPMs composted by interval maps $f:\left[0,1\right]\rightarrow\mathscr{I}$
...
shirking the maximum length of intervals itself could result QIVPMs
with non-empty cores. More precisely, we have the following question.

\begin{question}In a Hilbert space $\Hilb$ of dimension $D\geq3$,
given a QIVPM $\bar{\mu}:\events\rightarrow\mathscr{I}$ with $\alpha$
being the maximum length of intervals in $\mathscr{I}$, is there
an $\varepsilon>0$ such that $\coreBorn\left(\bar{\mu},\events\right)\ne\emptyset$
for $\alpha<\varepsilon$.\end{question}
...
\end{comment}
To better understand this question, we start from the dimension $D=3$.
This case is simpler than the general situation because any two-dimensional
projectors can be expressed as the complement of a one-dimensional
projector, $\mathbb{1}-\ket{\psi}$. Together with Eq.~\ref{eq:QIVPM-complement},
the interval-valued probability of any two-dimensional projectors,
$\bar{\mu}\left(\mathbb{1}-\ket{\psi}\right)$, can also be expressed
as the complement of the latter, $\necess-\bar{\mu}\left(\ket{\psi}\right)$.
Since the one-dimensional projectors are one-to-one corresponding
to the irreducible states in the complex projective space $\CP{2}$
as discussed in Sec.~\ref{subsec:Explicit-generalization-of}, QIVPMs
$\bar{\mu}:\events\rightarrow\mathscr{I}$ are one-to-one corresponding
to interval-valued frame functions $\bar{f}:\CP{2}\rightarrow\mathscr{I}$
according to the following lemma.

\begin{lemma}\label{thm:interval-valued-frame-function}Given a QIVPM
$\bar{\mu}:\events\rightarrow\mathscr{I}$, the function $\bar{f}:\CP{2}\rightarrow\mathscr{I}$
by $\bar{f}\left(\ket{\psi}\right)=\bar{\mu}\left(\proj{\psi}\right)$
are called an interval-valued frame function, and satisfies
\begin{equation}
\left[1,1\right]-\bar{f}\left(\ket{\psi_{0}}\right)\subseteq\bar{f}\left(\ket{\psi_{1}}\right)+\bar{f}\left(\ket{\psi_{2}}\right)\label{eq:interval-valued-frame-function}
\end{equation}
for any orthonormal basis $\left\{ \ket{\psi_{i}}\right\} _{i=0}^{2}$.
Conversely, if a function $\bar{f}:\CP{2}\rightarrow\mathscr{I}$
satisfies Eq.~\ref{eq:interval-valued-frame-function}, $\bar{f}$
must be an interval-valued frame function.\end{lemma}

\begin{proof}This lemma is the direct consequence of Lemma~\ref{thm:convex-3-1}.\end{proof}

Similar to the ultramodular functions in Sec.~\ref{subsec:Ultramodular-Functions},
we can also decompose an interval-valued frame function into its left-end
and right-end, where $\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]=\bar{f}\left(\ket{\psi}\right)$.
Then, Eq.~\ref{eq:interval-valued-frame-function} can be rephrased
as
\begin{equation}
\left[1,1\right]-\bar{f}\left(\ket{\psi_{0}}\right)\subseteq\bar{f}\left(\ket{\psi_{1}}\right)+\bar{f}\left(\ket{\psi_{2}}\right)\label{eq:two-ends-frame-function}
\end{equation}
