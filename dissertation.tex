...
Given a finite sample space~$\Omega$, and its power set $2^{\Omega}$
as the classical event space, a classical probability measure $\mu$
describe how likely every event~$E$ could happen by mapping $E$
to a number $\mu\left(E\right)\in\left[0,1\right]$.

is a function subject to the following constraints: $\mu\left(\emptyset\right)=0$,
...
given Hilbert space), subject to the following constraints:
\begin{align}
