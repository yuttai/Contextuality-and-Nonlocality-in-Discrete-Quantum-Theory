...
Conventional quantum theory is defined over the continuum of complex
numbers, but we cannot distinguish two arbitrary complex numbers without
unbounded resources. To explore alternative versions of quantum theory
incorporating our limitation of distinguishability, two types of discrete
quantum theories have been described in this thesis: quantum theories
and computing over finite fields and quantum interval-valued probability
measures. Examining the computational and physical consequences of
...
(Sec.~\ref{discretequantumtheoryI} to~\ref{discretequantumcomputingI}),
...
theory with cardinal probabilities is proposed (Sec.~\ref{sec:Discrete-Quantum-Theory-(II)}
...
define expectation values. One attempt to address this issue was to
switch to an axiomatic approach and following Gleason's theorem to
deduce a Born rule over finite fields (Sec.~\ref{sec:Toward-IVPM}).
However, quantum theories over finite fields looked unlikely to compatible
with infinitely precise real-valued probability; we thus extended
the classical imprecise interval-valued probability to quantum domain,
and defined quantum interval-valued probability measures (QIVPMs).

\begin{figure}[b]
\centering{}$\xymatrix{\textrm{Classical Probability Measure}\ar[d]\ar[r] & \textrm{Quantum Probability Measure}\ar[d]\\
\textrm{Classical IVPM}\ar[r] & \textrm{QIVPM}
}
$\caption{\label{fig:commutative-diagrams}Quantum IVPMs inherited from both
quantum probability measures and classical IVPMs. Since Quantum IVPMs
inherited from classical probability measures in two different paths,
it is natural to ask whether this diagram commutes.}
\end{figure}
As a natural extension of both convention quantum probability measures
and classical interval-valued probability measures (IVPMs) illustrated
in Fig.~\ref{fig:commutative-diagrams}, QIVPMs inherited definitions
and properties from the both sides. While the core and expectation
values of QIVPMs can be pulled back to the classical cases and consistent
...
extended to QIVPMs in subtle ways. \todo{Clean up until here\ldots}

Similar to gluing a family of classical probability spaces together
to form a conventional quantum probability space (Sec.~\ref{sec:fuzzy}),
a family of classical IVPMs can be glued to get a quantum IVPM. Although
most of structures above classical IVPMs can be extended to the quantum
case (Sec.~\ref{sec:Interval-Uncertainty}), .

....

Before discussing the further extension, let's summarize what we have
done previously. In Chapter~\ref{chap:Conventional-Quantum-Theory},
we reviewed what was established in CQT. In particular, Sec.~\ref{CQC1qubitBloch.sec}
studied the geometrical structure of quantum states to prepare replacing
the conventional Hilbert space~$\mathComplex^{D}$ or projective
Hilbert space~$\CP{D-1}$ by its discrete analog in Chapter~\ref{chap:Unrestricted-Finite-Fields}
and~\ref{chap:DQT=000026DQC}. In Chapter~\ref{chap:Unrestricted-Finite-Fields},
the field of complex numbers~$\mathComplex$ is replaced by unrestricted
finite fields~$\Fq$. Although this modal theory is

, and a valid pure state can be represented as any non-zero vector
in $\Fq^{D}$ and a measure scheme could be defined which only predicts
whether an measurement result is possible or not. \todo{Keep writing\ldots}
