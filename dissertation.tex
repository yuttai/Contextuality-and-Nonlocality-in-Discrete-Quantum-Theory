...
the corresponding family of classical probability space together to
define a quantum probability space. The glued quantum probability
space is well-behaved since not only we can define the expectation
values, but the quantum probability measure define on the whole space
can be simply induced by the Born rule according to Gleason's theorem
\cite{gleason1957,Redhead1987-REDINA,peres1995quantum,RichmanBridges1999,Hamhalter2013}.
...
likely one of the outcome in $E$ will actually happen. To maintain
...
where 
\begin{align}
\ket{\ps}=\frac{\ket{0}+\ket{1}}{\sqrt{2}}\,, &  & \ket{\ms}=\frac{\ket{0}-\ket{1}}{\sqrt{2}}\,.\label{eq:psms}
\end{align}
...
our discussion. This can be achieved by mapping a classical event~$E$
to the projector generated by $E$
\begin{equation}
...
\end{equation}
In general, if two classical events~$E$ and $E'$ mapped the same
projector, i.e., $\varphi\left(E\right)=\varphi\left(E'\right)$,
then the probability of $E$ is the same as the probability of $E'$.
Because of these good properties, the projector $\varphi\left(E\right)$
is called the quantum event corresponding to $E$, and the set of
all projectors on a given Hilbert space is called a quantum event
space~$\events$.

This function $\varphi$ also naturally sends the set structure to
the corresponding projector structure: $\varphi\left(\Omega\right)=\mathbb{1}$
and $\varphi\left(\overline{E}\right)=\mathbb{1}-\varphi\left(E\right)$.
Given two \emph{commuting} projectors $P_{0}$ and~$P_{1}$, there
exists a sample space $\Omega$ and a pair of event $E_{0}\subseteq\Omega$
and $E_{1}\subseteq\Omega$ such that $P_{0}=\varphi\left(E_{0}\right)$
and $P_{1}=\varphi\left(E_{1}\right)$. Conversely, given a pair of
event $E_{0}\subseteq\Omega$ and $E_{1}\subseteq\Omega$, their corresponding
quantum events $\varphi\left(E_{0}\right)$ and $\varphi\left(E_{1}\right)$
are commuting and satisfying the following properties: 
\begin{align}
...
Then, a quantum probability space can be defined by a quantum event
...
a state to construct quantum probability measures in CQT. However,
since the variants of quantum probability measures in Sec.~\ref{sec:Toward-IVPM}
and Chapter~\ref{chap:QIVPM} could not be constructed by the Born
rule easily, it is instructive to search for a Born rule here, and
we will actually find the unique one.

Although quantum probability measures constructed by gluing together
the classical ones looks complex, they could all be induced by operators
according to Gleason's theorem.
...
Although Gleason's theorem can correspond every quantum probability
measure to a unique mixed state~$\rho$, it doesn't explain whether
this state $\rho$ characterizing the state of particle sending to
a Stern-Gerlach type experiment. For example, in contrast with CQT,
if a quantum theory always ignore the input state and predict the
experimental outcomes are equally probable, even if this quantum theory
maps every input state to a quantum probability measure, it is still
not very sensible.
...
be the corresponding quantum probability measure of the resulting
events. To define a sensible correspondence, we hope that if the state~$\ket{\Phi}$
is one of the outcome of a quantum event~$P$, i.e., $P\ket{\Phi}=\ket{\Phi}$,
then the event~$P$ always happens, $\muB_{\Phi}\left(P\right)=1$
and vice versa. Moreover, since the physical phenomena exists and
should be the same no matter how we describe them, the probability
of an event should be invariant despite how we choose the basis. Because
changing to another basis is the same as applying an unitary map~$U$,
we should have$\muF_{U\ket{\Phi}}\left(UPU^{\dagger}\right)=\muF_{\Phi}\left(P\right)$.
It is easy to check the correspondence satisfying the conditions in
...
probability measure $\muB_{\rho}$ defined as 
\begin{equation}
...
quantum system being in the pure state $\ket{\ps}$ defined in Eq.~(\ref{eq:psms})
...
and precision of the particular experiment.\todo{Update the further
discussion chapter according to an updated Sec.~\ref{sec:fuzzy}.}
