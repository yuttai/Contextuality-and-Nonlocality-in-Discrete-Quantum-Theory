...
the map to the unit norm value $c=1\in\Fp$.
\begin{figure}
\noindent \centering{}\includegraphics[width=3.6in]{Fpp2Fp}\caption{\label{proof1.fig}Sketch of the map from $\Fpp$ to $\Fp$ using
...
under the application of $\Delta R$ as follows: %
\begin{comment}
...
and~\ref{prop:Born-unitary-invariant}. %
\begin{comment}
\todo{Notice that the notation here and previous is a little bit
different because we dealt with mixed states previously, but pure
states here. Besides, should we explain the physical meaning of these
propositions?}
\end{comment}
...
recovers the definition of classical IVPMs.  To see this, first,
$\events'$ is called a \emph{subspace} of the set of events~$\events$
...
Given a mutually commuting subspace~$\eventsC$ whose elements are
diagonalizable by a common orthonormal basis $\Omega=\left\{ \ket{0},\ket{1},\ldots,\ket{D-1}\right\} $,
the function $\varphi\colon2^{\Omega}\rightarrow\eventsC$ defined
in in Eq.~(\ref{eq:pullback-function}) maps any set~$E$ to the
sum of the projectors formed by elements in $E$, and preserve all
operations used to defined classical and quantum IVPMs as in Eqs.~(\ref{eq:pullback-properties-basic})
...
both quantum real expectation value and classical interval expectation
value, not only the property of quantum real expectation value could
be extended to the previous lemma, but the properties of classical
interval expectation value, like Eq.~(\ref{eq:classical-real-interval-Choquet})
and Thm.~\ref{thm:Rosenmuller}, could be extended to quantum interval
expectation value as well. In particular, Eq.~(\ref{eq:classical-real-interval-Choquet})
...
of Thm.~\ref{thm:Rosenmuller}. However, since Lemma~\ref{lemma:density-matrix-pullback}
