...
of circles (the paths of $\rme^{\rmi\frac{\theta}{2}}$), each corresponding
to a single point on the sphere in (a).\protect \\
\todo{{Since }the Hopf fibration{ is fibering out by} the circle
$\Sphere{1}${ }on the three-sphere $\Sphere{3}$ embedded in $\R^{4}$.
Need to confirm whether (b) is a figure in $\R^{4}$ and how to understand
the next sentence.}\protect \\
Points in (a) are color coded corresponding to circles in (b), e.g.,
one pole contains the red elliptical circle that would become an infinite-radius
circle in a slightly different projection, and the opposite pole corresponds
to the large perfectly round red circle at the equator.}
