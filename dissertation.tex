...
$\left\Vert \mathscr{I}\right\Vert $, is defined by the maximum length
...
only applies to the QIVPMs constructed by composing an interval map~$f$
with a conventional quantum probability measure~$\muB_{\rho}$, and
the states consistent with the composition $f\circ\muB_{\rho}$ collapses
to a single state as the maximum length of intervals in $\mathscr{I}$,
$\left\Vert \mathscr{I}\right\Vert $, shrinks to $0$. In contrast,
the globally inconsistent QIVPM defined in Table~\ref{tab:non-Born-QIVPM}
has the minimal sharp range~$\mathscr{I}_{0}$ with $\left\Vert \mathscr{I}_{0}\right\Vert =1$.
This suggests that even for general QIVPMs, shrinking the length $\left\Vert \mathscr{I}\right\Vert $
might help to regularize them. In another word, if $\left\Vert \mathscr{I}\right\Vert $
is shorter than a number, all QIVPMs map to $\mathscr{I}$ might have
an non-empty core.
...
for $\ket{\psi}\in\CP{2}$. According to Lemma~\ref{thm:convex-3-1},
$\bar{\mu}:\events\rightarrow\mathscr{I}$ is a QIVPM if and only
if Eqs.~(\ref{eq:QIVPM-constraints}) and
\begin{equation}
\bar{\mu}\left(\proj{\psi_{1}}+\proj{\psi_{2}}\right)\subseteq\bar{\mu}\left(\proj{\psi_{1}}\right)+\bar{\mu}\left(\proj{\psi_{2}}\right)\label{eq:interval-valued-inclusion}
\end{equation}
holds for any orthonormal basis $\left\{ \ket{\psi_{i}}\right\} _{i=0}^{2}$.
By applying Eq.~(\ref{eq:QIVPM-complement}) on the left-hand side,
Eq.~(\ref{eq:interval-valued-inclusion}) is equivalent to the following
interval-inclusion
\begin{eqnarray}
...
When the sum of $\frameL$ on each ``local'' classical probability
space, $\sum_{i=0}^{2}\frameL\left(\ket{\psi_{i}}\right)$, is \emph{exactly}
...
$\frameL$ might be as wild as we can imagine, and we might need to
know a little bit more, like the function is of the form $f\circ\muB_{\rho}$,
to deduce its global property.

\section{And Beyond\ldots}

After we build the quantum interval-valued probability models, we
might think how powerful a quantum computer could be based on this
model. Since the conventional quantum circuit model manipulates the
probability amplitudes of quantum states, to build a quantum computing
model above QIVPMs, we might either consider evolving the states in
the core of a QIVPM, or find a way to evolve QIVPMs directly. Both
strategies are not straightforward. As we proved in Thm.~\ref{thm:Non-extensible-of-Gleason's},
a QIVPM might have an empty core which cannot be evolved over time.
If we want to evolve and compute QIVPMs directly, we need to get the
QIVPM of the whole system from the QIVPM for each subsystem or qubit.
However, there is no straightforward way to define the tensor product
among QIVPMs because we cannot summarize a probability measure into
a state as we did in conventional quantum theory.
