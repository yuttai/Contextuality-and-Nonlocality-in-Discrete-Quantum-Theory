...
Before what we can do further based on this thesis, let's summarize
what we have done previously. In Chapter~\ref{chap:Conventional-Quantum-Theory},
...
and~\ref{chap:DQT=000026DQC}. Since the replaced state spaces are
discrete, the quantum computation model could be built upon them naturally.
In Chapter~\ref{chap:Unrestricted-Finite-Fields}, the field of complex
numbers~$\mathComplex$ is replaced by unrestricted finite fields~$\Fq$,
and a valid pure state can be represented as any non-zero vector in
$\Fq^{D}$ and a measure scheme could be defined which only predicts
