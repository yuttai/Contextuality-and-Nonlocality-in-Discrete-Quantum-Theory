...
require two different types of quantities. On one hand, the question
and answer of RSA are both discrete integers. On the other hand, even
if the quantities describing molecules, like their positions, are
considered as continuous, we want to argue that it is beyond our ability
to decide whether these quantities are ultimately discrete or continuous.
To better understand this issue without struggling with the quantum
theory, it is instructive to review how classical quantities are characterized
and computed. 
...
as their mathematical models, the physical quantities discussed in
the previous paragraph should not simply represented by real numbers.
Chuang-tzu's stick are made by molecules and cannot be cut in halves
endlessly. The input charge of an integrator must actually be an integer
multiple of the elementary charge. A prefect square cannot be draw
physically. Even we don't draw it, but merely asking whether it exists
in the space is still beyond our ability to answer because whether
the space itself is ultimately discrete or continuous is still an
unsolved question. Even if a prefect square exists physically, we
cannot decide whether it is really a prefect square or an almost prefect
one differing only by an arbitrarily small extent. Similar situation
applying to the length of a slide rule, it is beyond our ability to
decide whether its length is ultimately discrete or continuous. Even
if its length is continuous, identifying which real number represents
its length requires infinite precision, but the precision in reality
is limited generally three or four significant figures~\cite{wiki:AnalogComputer}.
As the results, even if analog computers really store real numbers,
they cannot be read, written, and used to branch the computation based
on their exact values. The opposite of the last statement is sometimes
assumed by real computational models. For example, Blum, Cucker, Shub,
and Smale modeled their BCSS machine to allow the operations of deciding
...
than the prediction based on this kind of theoretical models.

\begin{sidewaystable}
\caption{\label{tab:Comparison}Comparison between the classical quantities
and their mathematical models with their inspired quantum models.
\todo{Center entries vertically.}}
...
Not reliably, but after we have a better technology to manipulate the minimum units, we can predict the physical phenomenon and their computation model by a better discrete model. & 
Not reliably, because precision can never be high enough. \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
...
above them are summarized in Table~\ref{tab:Comparison} with their
inspired quantum models. The physical quantity manipulated by the
quantum circuit model, the most widely used model of quantum computing,
is the probability amplitudes of states. \todo{Introduction Cleaned
up until here. }Because we are agnostic about whether probability
amplitude is quantized or can really be valued in the continuum of
complex numbers, we tried to develop two different types of models:
the quantum theories and computing over finite fields are discrete
models, and the quantum interval-valued probability measures incorporate
the idea of measurement precision into the quantum theory. To distinguish
the modified quantum theories from the usual ones, the usual ones
will be called the ``conventional quantum theory'' where necessary
and it will be reviewed in Chapter~\ref{chap:Conventional-Quantum-Theory}~\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
...
\midrule
\addlinespace
Hadamard transformation & Included & May not be included & Included & Included &  & \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
