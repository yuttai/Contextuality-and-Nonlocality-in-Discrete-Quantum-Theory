...
an arbitrarily small extent \cite{Turing_1937,Gisin2017}, quantum
mechanics and computation based on the continuum of complex numbers
may not capture our ability of predicting quantum phenomenon and the
power of quantum computation. Being agnostic about whether the reality
is ultimately discrete or continuous, we considered two types of quantum
models to incorporate our limitation of distinguishability. By assuming
the quantum states are ultimately quantized and distinguishable, our
first type of quantum theories replaces the complex numbers by discrete
finite fields. We then focus on the limitation of distinguishability
...
After first suggesting by Richard Feynman 40 years ago~\cite{Feynman1982Simulating},
IBM, \textitalian{Rigetti}, Google, Microsoft, and many other companies
and countries start to build realistic quantum computers \cite{Shieber2018,Kahn2019},
and seek more efficient ways to simulate chemical molecules and to
break down RSA \cite{boneh1999twenty,wiki:RSAFactoring}. Despite
how we compute these two tasks, we can notice they require us to compute
two different types of quantities. While RSA requires discrete computation,
it is beyond our ability to distinguish whether the quantities required
by simulating molecules are discrete or not. Before discussing the
quantities characterizing and computing with quantum mechanics, it
is instructive to review how classical physical quantities are characterized
and computed. 

Discrete quantities can be easily characterized by whole numbers~\cite{ParkerBaldridge2004}.
Even if the proportion of whole numbers are fractions, the operations
among fractions is essentially the same as those among whole numbers
over the common denominator, and the computational results faithfully
represent the corresponding quantities. In general, discrete computation
can be reliably repeated with exact equivalence, and any digital computer
can faithfully simulate a Turing machine limited only by the available
memory and time~\cite{Turing_1937}. 

In contrast, the non-apparently discrete quantities confused many
cultures since very beginning. For example, Greek mathematicians in
the school of Pythagoras tried to apply their discrete idea into their
model, and believed every mathematical model must be ultimately discrete.
They believed so much that they in legend murdered Hippasus because
he found there is no minimum unit between a side of a square and its
diagonal. Comparing the idea that the length of a square's diagonal
is not a physical quantity or there is no square physically, working
with a mathematical model without minimum unit seems more appealing,
and their corresponding physical quantities are also assumed to be
continuous. An ancient Chinese philosopher Chuang-tzu (庄子) expressed
this belief as ``If a one-foot-long stick is cut into halves every
day, the cutting will never come to an end''\footnote{``一尺之棰，日取其半，萬世不竭。'' in Chinese \cite{Zhuangzi1999,Liu2018}.}.
Later, these continuous quantities becomes the basic component of
analog computers. After logarithm was discovered, a slide rule, also
known as a slipstick, is used to compute multiplication, division,
and more complex operations as a mechanical analog computer~\cite{wiki:SlideRule}.
Charge amplifiers is used to build integrators \cite{wiki:Integrator,wiki:ChargeAmplifier}. 
...
\addlinespace
\addlinespace
Contextuality & Yes & Supercontextuality &  &  &  & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Yes when measuring\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
precise enough\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular}\tabularnewline\addlinespace
\midrule
\addlinespace
...
\end{tabular}
\end{sidewaystable}
\todo{Introduction Cleaned up until here.}

\todo{Write a story so that physicists will also be interested in
this thesis. For example, quantum simulation problem and how to apply
these to other physicists work (say two more pages?). Check our previous
papers. Explicitly say what question we want to solve. In particular,
merging quantum mechanics and computer science is too broad for a
thesis. }

\todo{Maybe start with this kind of sentence “We want a more faithful
quantum theory not only because the current quantum theory don't take
into account of the measurement precision, but also because its computational
model doesn't fit the current development of computer science.”
}\todo{See if we can cite this paper~\cite{Gisin2017} for finite
precision measurement. }

\todo{新的大綱：主旨：probability amplitude到底是離散還是連續的？
\begin{itemize}
\item Some quantities are obvious discrete, but how to model other quantities
which we don't know?
\begin{itemize}
\item 一尺之槌，日取其半，萬世不竭。
\item Zeno 追龜的故事
\item $\sqrt{2}$是無理數謀殺案(物理量？)
\item Euclid?(物理量？)
\end{itemize}
當我們的儀器不足以分辨出最小單位時，假設沒有最小單位很合理，而且可以很好的解釋我們觀察到的現象。
\item 很多大家本來相信是連續的物理量最後被證明是離散的：
\begin{itemize}
\item “一尺之槌，日取其半，萬世不竭。”是錯的。
\item 光電效應
\item 角動量(角度？)
\end{itemize}
\end{itemize}
...
models and their physical realizations\todo{Put the third row in
the middle alignment.}}
\begin{tabularx}{\linewidth}{ccX}
\toprule 
\addlinespace
 & Discrete & Continuum\tabularnewline\addlinespace
\midrule
\addlinespace
\midrule
\addlinespace
Theoretical  Model & Turing machine & BCSS machine\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Physical Realization & Digital Computer & Analog Computer\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
How the models realize? & Reliably & Not Reliably:
\begin{itemize}
\item The quality might be quantized.
\item The precision of an analog computer is low.
\end{itemize}\tabularnewline\addlinespace
\bottomrule
\addlinespace
\end{tabularx}
\end{table}
