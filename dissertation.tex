...
amplitudes are assumed to be ultimately discrete, we choose to replace
the field of complex numbers by discrete finite fields as discussed
in Chapter~\ref{chap:Unrestricted-Finite-Fields} and \ref{chap:DQT=000026DQC}
for two reasons. On one hand, CQT is built upon the operations among
the probability amplitudes, and the probability amplitudes have the
required operations since the complex numbers form a field. On the
other hand, finite fields could be based to define the fraction-like
cardinal probability which could provide extremely small units when
the size of the field is extremely large. Alternatively, the probability
amplitudes might never be found quantized. In this situation, a better
model could take into account the precision of quantities. However,
instead of the precision of probability amplitudes, it is better to
consider the precision of their inducing probabilities because the
idea of ``imprecise probabilities'' have well-studied classically
\cite{Shafer1976,GilboaSchmeidler1994,Marinacci1999,JamisonLodwick2004,HuberRonchetti2009,Grabisch2016}.
\todo{Make sure whether quantum computers can branch its computation
based on its probability amplitudes or measured probabilities? If
they can only branch on measured probabilities, it will provide another
reason why we want to study the precision of the induced probabilities...
}In Chapter~\ref{chap:QIVPM}, this idea will be extended to quantum
...
integral \cite{Choquet1954,GilboaSchmeidler1994,Grabisch2016}. When
we extend classical IVPMs to QIVPMs, these known properties have their
natural correspondence for QIVPMs. In Sec.~\ref{sec:Kochen-Specker},
their expectation values will play a role in proving the Kochen-Specker
theorem \cite{BELL_1966,kochenspecker1967,Redhead1987-REDINA,peres1995quantum,Jaeger2007}
for ``almost'' certain QIVPMs, and establish bounds on its validity
in realistic experimental environments. In Sec.~\ref{sec:Gleason},
we are able to show that, while a QIVPM incorporating the effects
of finite precision might not be consistent with Gleason's unique
state~$\rho$ on all projectors defined on a Hilbert space~$\Hilb$
of dimension $d\ge3$, there is a mathematically precise sense in
which one recovers the original Gleason theorem asymptotically. Specifically,
it is possible to construct a class of QIVPMs representing bounded
resources that is parameterized by the size of the intervals.
...
and we present our conclusions in final Chapter~\ref{chap:Conclusion}.
