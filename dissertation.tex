...
an arbitrarily small extent \cite{Turing_1937,Gisin2017}, modeling
quantum mechanics and computation over the continuum of complex numbers
...
to incorporate the limitation of distinguishability. By assuming the
quantum states are quantized and distinguishable, instead of the conventional
theory over the continuum of complex numbers, we considered the quantum
theories and computing over finite fields. Shifting our attention
from the reality to the limitation of distinguishability itself, the
second model incorporate the finite precision measurement into the
quantum theory called quantum interval-valued probability measures.

Even before quantum mechanics is discovered, human tried to characterize
the physical quantities and controlled the quantities and operations
to simulate and predict unknown quantities. Discrete quantities can
be easily characterized by whole numbers~\cite{ParkerBaldridge2004}.
Even if the proportion of whole numbers are fractions but not whole
numbers, the operation among fractions can be considered as the operation
among whole numbers after we find the common denominator, and the
resulting numbers faithfully represent the quantities they represent.
In general, discrete computation can be reliably repeated with exact
equivalence, and any digital computer can faithfully simulate a Turing
machine limited only by the available memory and time~\cite{Turing_1937}. 

The non-apparently discrete quantities and their operations confused
many cultures from the very beginning. An ancient Chinese philosopher
Chuang-tzu (庄子) has expressed ``If a one-foot-long stick is cut into
