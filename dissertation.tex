...
amplitudes are assumed to be ultimately discrete, we need to replace
the field of complex numbers by some discrete structures. Finite fields
is chosen because CQT is built upon the operations among the probability
amplitudes, and the probability amplitudes have the required operations
since the complex numbers form a field. The quantum theories and computing
built upon finite fields are discussed in Chapter~\ref{chap:Unrestricted-Finite-Fields}
and \ref{chap:DQT=000026DQC}. Alternatively, the probability amplitudes
might never be found quantized. In this situation, a better model
could take into account the precision of quantities. However, instead
...
or not and to define the maximum entangled states.
...
of quantum computing. In this model, the states of a discrete $d$-dimensional
...
we extend classical IVPMs to QIVPMs, these known properties of classical
IVPMs all have their natural correspondence for QIVPMs. In Sec.~\ref{sec:Kochen-Specker},
...
for ``almost'' certain QIVPMs. In Sec.~\ref{sec:Gleason}, we are
able to show that, while a QIVPM incorporating the effects of finite
precision might not be consistent with Gleason's unique state~$\rho$
on all projectors defined on a Hilbert space~$\Hilb$ of dimension
$d\ge3$, there is a mathematically precise sense in which one recovers
the original Gleason theorem asymptotically. Specifically, it is possible
to construct a class of QIVPMs representing bounded resources that
is parameterized by the size of the intervals.
