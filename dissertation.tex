...
\begin{centering}
\hspace*{-0.5cm}\includegraphics[width=1.1\columnwidth]{../../../Public/Documents/StudyInIUB/Computer Science/201209-12.M800/Manuscripts/picC.png}
...
\begin{eqnarray}
\hspace*{-0.5cm}\Vec{X}=(|\alpha_{i}|^{2},...,\sqrt{2}\ \mathrm{Re}\ \alpha_{i}\alpha_{j}^{*},\ldots,\sqrt{2}\ \mathrm{Im}\ \alpha_{i}\alpha_{j}^{*},\ldots)\ ,
\end{eqnarray}
where 
\begin{eqnarray}
\Vec{X}\cdot\Vec{X}=\left(\sum_{i=0}^{D-1}\left|\alpha_{i}\right|^{2}\right)^{2}=1\ .
...
in $\Vec{X}$. Each set of $(p+1)$ redundant points is, geometrically
...
$\Vec{X}$ on the generalized $n$-qubit Bloch sphere. Dividing the
...
structure that map to the \textit{same point\/} in $\Vec{X}$, we
...
equivalent points that map to the same point $\Vec{X}$ on the generalized
...
Geometry Vol. 2} (Blaisdell Publishing Company), pp 55–57 
