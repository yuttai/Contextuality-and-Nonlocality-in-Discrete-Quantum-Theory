...
complex numbers. Although its predicting ability to quantum phenomenon
is limited by measurement precision, it can well describe the physical
reality together with error analysis. On the other hand, current computers
perform digital computation most of the time, except some analog chips
communicating with physical world~\cite{Camenzind2005}. While the
computability of digital parts is faithfully characterized by its
theoretical model, the Turing machine~\cite{Turing_1937}, the computability
of analog chips in reality is far weaker than the theoretical prediction
of real computation \cite{Siegelmann1998,Ziegler2007,weihrauch2012computable,blum2012complexity}.
One of the reason behind this gap is that our measurement precision
is always finite, and some of the difference cannot be compensated
by error analysis technique. All these problems for classical computation
could potentially apply to quantum computation. To address these issues,
we tried to develop two different types of quantum theories and computing.
Since classical discrete computing model faithfully represents digital
computers, we first tried to build discrete quantum theories and computing
by considering quantum theories and computing over finite fields in
Chapter~\ref{chap:DQT=000026DQC}. In order to , We then tried to
incorporate the idea of finite precision measurement with quantum
theories in Chapter~\ref{chap:QIVPM}.
