...
are resolved.
\end{enumerate}
The previous steps only tested in Microsoft Windows. Please report
me if there is any difficulty to compile it. Besides, to compile a
PDF file from the \LyX{} source, just finish step~\ref{enu:Clone-the-git}
and click the toolbar button \includegraphics[width=1em]{buffer-view}.\end{preface*}
...
future research.
\end{comment}
As we discussed in the previous section, the finite-precision extension
of the Gleason theorem, Thm.~\ref{thm:Finite-precision-Gleason},
only applies on the QIVPMs composed by interval maps $f:\left[0,1\right]\rightarrow\mathscr{I}$
and quantum probability measures $\muB_{\rho}:\events\rightarrow\left[0,1\right]$:
the states consistent with the composition $f\circ\muB_{\rho}$ converges
to $\rho$ as the maximum length of intervals in $\mathscr{I}$ shrinking
to $0$. In contrast, the wildest QIVPM defined in Table~\ref{tab:non-Born-QIVPM}
maps to $\mathscr{I}_{0}$ whose intervals' maximum length is $1$,
the longest possible length. This provides another possibility that
shirking the maximum length of intervals itself could result the QIVPMs
mapping to these intervals having non-empty cores. More precisely,
...
satisfying the maximum length of intervals in $\mathscr{I}$ less
than $\varepsilon$, $\bar{\mu}$ must have an non-empty core, i.e.,
...
which is largely simplified by the fact that any two-dimensional projectors
can be expressed as the complement of a one-dimensional projector,
and by Eq.~(\ref{eq:QIVPM-complement}) so does their interval-valued
probabilities, i.e., $\bar{\mu}\left(\mathbb{1}-\proj{\psi}\right)=\necess-\bar{\mu}\left(\proj{\psi}\right)$.
...
we can decompose interval-valued probabilities on these states into
its left-end and right-end, where $\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]=\bar{\mu}\left(\proj{\psi}\right)$.
...
\end{equation}
Since the maximum length of intervals in $\mathscr{I}$ is less than
$\varepsilon$, $\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]\in\mathscr{I}$
implies $\frameR\left(\ket{\psi}\right)-\frameL\left(\ket{\psi}\right)<\varepsilon$.
Then, the largest term in the previous inequality $\frameR\left(\ket{\psi_{1}}\right)+\frameR\left(\ket{\psi_{2}}\right)$
...
\end{equation}
for any $D$-dimensional projector~$P$, where $\ket{\Phi_{j}}\in\mathcal{H}$
are normalized, $q_{j}>0$, and $\sum_{j=1}^{N}q_{j}=1$. Our Question~\ref{question:approximation-Gleason}
ask what $\frameL$ would look like with a positive $\varepsilon$.
...
a manifold is glued by many local coordinates. When the curvature
of each small piece is ``exactly'' the same and positive, Killing-Hopf
...
$\frameL$ on each local classical probability space is ``exactly''
the same and equal to $1$, Gleason's theorem can assert $\frameL$
can be expressed as Eq.~\ref{eq:Gleason-for-CP2}. However, when
each local classical probability space becomes imprecise, a general
$\frameL$ might be as wild as we can image, and we might say little
about its global property.
