...
and the further extended questions are in final Chapter~\ref{chap:Further-Discussion}.
...
As we discussed in previous section, the finite-precision extension
of the Gleason theorem, Thm.~\ref{thm:Finite-precision-Gleason},
only applies on a class of QIVPMs inducing by composting interval
maps $f:\left[0,1\right]\rightarrow\mathscr{I}$ and quantum probability
measures $\muB_{\rho}:\events\rightarrow\left[0,1\right]$.

\begin{comment}
 Almost all QIVPMs in Chapter~\ref{chap:QIVPM} are closely related
to quantum probability measures. For example, given a $\delta$-deterministic
QIVPM, Lemma~\ref{lemma:delta2zero} provides a way to derive a $0$-deterministic
QIVPM $\barmuD:\events\rightarrow\left\{ \imposs,\necess\right\} $
