...
$\mu\left(\Omega\right)=1$, $\mu\left(\overline{E}\right)=1-\mu\left(E\right)$,
and $\mu\left(E_{0}\cup E_{1}\right)=\mu\left(E_{0}\right)+\mu\left(E_{1}\right)$
for each pair of disjoint events $E_{0}\subseteq\Omega$ and $E_{1}\subseteq\Omega$.

Since the previous abstraction doesn't specify how to generate the
outcomes, the generating process could well be a quantum experiment.
Let us prepare a beam of one kind of spin~$1$ particles with basis
vectors $\ket{0}$, $\ket{1}$, and $\ket{2}$. In principle, this
beam can be split by a Stern-Gerlach type experiment according to
the eigenvalues of the observable $\mathbf{O}_{0}=0\,\proj{0}+1\,\proj{1}+2\,\proj{2}$,
and the states corresponding to the split beam are $\ket{0}$, $\ket{1}$,
and $\ket{2}$ \cite{:/content/aip/journal/jmp/21/1/10.1063/1.524312,peres1995quantum}.
Instead of a beam, if only one particle is sent to the beam splitter,
the state after the experiment is one of $\ket{0}$, $\ket{1}$, and
$\ket{2}$, and each state has a probability. In the language of our
previous abstraction, the set of outcomes is $\Omega_{0}=\left\{ \ket{0},\ket{1},\ket{2}\right\} $.
...
and $\Omega_{0}$. This experiment defines a classical probability
...
similar to the previous one. While this experiment send exactly the
same particle, we use a beam splitter corresponding to the observable
$\mathbf{O}_{1}=0\,\proj{\ps}+1\,\proj{\ms}+2\,\proj{2}$, where $\ket{\ps}=\frac{\ket{0}+\ket{1}}{\sqrt{2}}$
and $\ket{\ms}=\frac{\ket{0}-\ket{1}}{\sqrt{2}}$. The outcome of
this experiment is $\Omega_{1}=\left\{ \ket{\ps},\ket{\ms},\ket{2}\right\} $,
and it defines another probability measure $\mu_{1}:2^{\Omega_{1}}\rightarrow\left[0,1\right]$.
Although $\mu_{0}$ and $\mu_{1}$ are different probability measures,
the probability of the same event are experimentally found to be the
same, i.e., $\mu_{0}\left(\emptyset\right)=\mu_{1}\left(\emptyset\right)$
and $\mu_{0}\left(\left\{ \ket{2}\right\} \right)=\mu_{1}\left(\left\{ \ket{2}\right\} \right)$.
In general, when the particle and the event are the same, so does
their probability no matter how we set up the beam splitters.

Since the probability induced by the different beam splitters are
correlated, it is more natural to define the set of quantum event~$\events$
as the union of all possible event spaces using different beam splitters.
Since we are focus on the probability instead of outcome, and the
probabilities of events $\left\{ \ket{0},\ket{1}\right\} $ and $\left\{ \ket{\ps},\ket{\ms}\right\} $
are always the same, it is reasonable to identify them as the same
event. To simplify the notation, given any classical event $E\subseteq\Omega$,
its corresponding quantum event is defined by 
\begin{equation}
\varphi\left(E\right)=\sum_{\ket{j}\in E}\proj{j}\,.\label{eq:pullback-function}
\end{equation}
Following this definition, it is easy to check that not only 
\begin{equation}
\varphi\left(\left\{ \ket{0},\ket{1}\right\} \right)=\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}=\varphi\left(\left\{ \ket{\ps},\ket{\ms}\right\} \right)
\end{equation}
but every classical event mapping the same projector must also have
the same probability. If we follow the convention $\varphi\left(\emptyset\right)=\mathbb{0}$,
