...
we describe product and entangled $n$-qubit states used in circuit
model, and the unitary matrices used to transform states as addressed
by by the property \ref{enu:unitary-matrix}. In Sec.~\ref{sec:fuzzy},
we introduces the important issues of \ref{enu:measurement-framework}
and the foundations of quantum probability space. To facilitate the
transition to DQT carried out in later sections, we concern ourselves
first with the properties of the simplest possible abstract state
object in CQT, the single qubit state.

\section{Geometrical Structure of States\label{CQC1qubitBloch.sec}}

\subsection{The System Described by a Two-dimensional Hilbert Space}
...
overall phase transformation $\rme^{\rmi\phi}$, that is, 
\begin{equation}
\left(\alpha_{0},\alpha_{1}\right)\sim\left(\rme^{\rmi\phi}\alpha_{0},\rme^{\rmi\phi}\alpha_{1}\right)\,.\label{A3.eq}
\end{equation}
...
them to 2 degrees of freedom by effectively removing $\rme^{\rmi\phi}$
...
\begin{figure}[htb]
\centerline{ \includegraphics[width=0.48\columnwidth]{hopfS2}\hspace{0.03in}
\includegraphics[width=0.48\columnwidth]{hopfFibers}} \centerline{\hspace{1in}
(a) \hfill{}(b) \hspace{1in}} \caption{(a) The sphere represented by \Eq{A7.eq}, which is the irreducible
...
of circles (the paths of $\rme^{\rmi\phi}$), each corresponding to
a single point on the sphere in (a). Points in (a) are color coded
corresponding to circles in (b), e.g., one pole contains the red elliptical
circle that would become an infinite-radius circle in a slightly different
...
\begin{figure}[thb]
\begin{centering}
\includegraphics[width=0.47\columnwidth]{blochSphere} \hspace{0.01in}
...
they differ only by an overall phase $\rme^{\rmi\phi}$. However,
in practice this is not a construction that is easy to realize in
a practical computation. We now outline an explicit algorithm for
...
ambiguity $\rme^{\rmi\phi}$ from the $n$-qubit states. If we take
a particular weighting of the elements of the density matrix $\rho$,
