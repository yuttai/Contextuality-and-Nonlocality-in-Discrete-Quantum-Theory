...
\includegraphics[viewport=200bp 0bp 1800bp 600bp,width=0.9\columnwidth]{picC}
\par\end{centering}
...
\end{equation}
In contrast, the irreducible pure states in $\ff{7^{2}}^{3}$ is not
unique. As we computed in Eq.~(\ref{eq:numberOfIrreducibleStates}),
there are 
\begin{equation}
\frac{7^{3-1}\left(7^{3}-\left(-1\right)^{3}\right)}{7+1}=2107\label{eq:numberOfIrreducibleStates-7-3}
\end{equation}
irreducible pure states in $\ff{7^{2}}^{3}$. Since we don't have
