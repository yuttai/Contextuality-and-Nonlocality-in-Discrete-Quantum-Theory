...
is experimentally found to be the same, i.e., $\mu_{0}\left(\left\{ \ket{2}\right\} \right)=\mu_{1}\left(\left\{ \ket{2}\right\} \right)$.
...
Because we are focus on the probability instead of outcome, and the
...
event. By observing the projectors generated by these two events are
the same
\begin{equation}
...
not only the events mapping to the same projector must have the same
probability, but this function $\varphi$ also naturally sends the
set structure to the corresponding projector structure: $\varphi\left(\Omega\right)=\mathbb{1}$
