...
space, a classical probability measure~$\mu$ maps every event~$E$
...
To understand Gleason's theorem, we can follow the Stern-Gerlach type
experiment interpretation in the previous section. Although Gleason's
theorem correspond every quantum probability measure~$\mu$ to a
unique mixed state~$\rho$, it doesn't specify whether this state
$\rho$ characterizing the state of particle sending to the quantum
experiment. Consider an extreme example: a quantum theory could ignore
the input state and predict the experimental outcomes are always equally
probable. Even if these predictions form a quantum probability measure,
this kind of prediction is so different from the prediction of CQT
that they can be easily distinguished experimentally.
...
is one of the outcomes of a quantum event~$P$, $P\ket{\Phi}=\ket{\Phi}$,
