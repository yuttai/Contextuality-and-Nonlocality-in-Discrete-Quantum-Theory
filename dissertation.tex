...
\end{pmatrix}\,.
\end{aligned}
\label{eq:MQT-dynamics}
\end{equation}
For example, 
\begin{equation}
\begin{aligned}S\ket{0} & =\begin{pmatrix}1 & 0\\
1 & 1
...
\end{pmatrix}=\begin{pmatrix}1\\
0
\end{pmatrix}=\ket{0}\,.\end{aligned}
\label{eq:MQT-dynamics-apply}
\end{equation}
...
has one occurrence $\ket{0}\ket{\overline{0}}$. This can be easily
proved as follows. Since each $a_{i}$ is either 0 or 1, then each
$S(a_{i})$ is either $+$ or $1$, and hence each $\mathsf{not}\left(S(a_{i})\right)$
is either~$+$ or~$0$. The result follows since any state with
a combination of $+$ and $0$, when expressed in the standard basis,
would consist of a superposition containing the state $\ket{0\ldots}$.
