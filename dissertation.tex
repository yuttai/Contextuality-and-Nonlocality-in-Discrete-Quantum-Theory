...
functions, which look like continuous at first sight. However, we
want to argue that either these quantities might later be discovered
to be discrete, or the exact values of these continuous quantities
may never be pinned down. Hence, modeling quantum computing based
on the exact values of continuous quantities cannot really describe
...
paths may be significantly different, and may not be compensated by
error analysis techniques easily like the butterfly effect. Therefore,
the computational power of analog computers might be far weaker than
the prediction based on this kind of theoretical models. \todo{Maybe
extended to consider probabilistic model? }
