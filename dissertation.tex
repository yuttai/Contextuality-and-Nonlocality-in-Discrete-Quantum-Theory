...
\toprule 
\addlinespace
Physical quantities & Discrete & Discrete with extreme small unit & Continuous, or no evidence to support they are discrete\tabularnewline\addlinespace
\addlinespace
\addlinespace
...
we first review the ``conventional quantum theory'' (CQT)\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
...
hard we try. Notice that the probability amplitudes is unlikely to
be accessed directly while the probability of an event can be deduced
after a series of experiments. To achieve a coherent formulation of
finite-resource quantum mechanics, we extend classical interval-valued
probability measures to quantum domain, and consider quantum interval-valued
probability measures (QIVPM) in Chapter~\ref{chap:QIVPM}.
...
in Sec.~\ref{sec:The-geometry-of} will be used to defined purity
which provides an easy way to decide whether a state is entangled
or not and to defined the maximum entangled states.
...
It is called modal quantum theory because it can only predict whether
a measure result is possible or impossible, but not its probability.
...
vector space over $\Fpp$, and can be reduced to a point in the discrete
complex projective space. Among these irreducible states, the product
states and entangled states can not only be identified as in CQT but
also be counted for different $p$. Although the state space is a
natural discrete analog to CQT, this model still only predicts whether
a measurement result is possible or impossible, because it is not
possible to define an inner product in the usual sense due to the
modular arithmetic~\cite{grove2002classical}. Under suitable conditions,
we can have deterministic quantum algorithms such as the algorithms
of Deutsch, Simon \cite{Simon:1994:PQC:1398518.1399019,Mermin2007,Jaeger2007},
and Bernstein-Vazirani \cite{Bernstein:1993:QCT:167088.167097,Mermin2007},
but this still leads to excessive computational power for partially
solving the $\usat$ problem.
...
to find a notion of probability for quantum theory over finite fields.
The cardinal probability is defined in Sec.~\ref{sec:Discrete-Quantum-Theory-(II)}
by restricting states in some local region of the vector spaces. Within
a local region, a notion of inner product and probability could be
recovered. In Sec.~\ref{discretequantumcomputingII}, this discrete
quantum theory is applied on two representative algorithms, the deterministic
Deutsch-Jozsa algorithm \cite{DeutschJozsa1992,Jaeger2007} and the
probabilistic Grover algorithm \cite{Grover:1996:FQM:237814.237866,Mermin2007,Jaeger2007}.
\begin{comment}
...
\end{comment}
Despite the great success of cardinal probabilities, lacking arithmetic
operations on them provides some difficulties to define expectation
values. By attempting to define expectation values, we consider quantum
probability measures over finite fields (QPMFF) in Sec.~\ref{sec:Toward-IVPM}.
However, we cannot find a discrete Born rule to satisfy the Gleason-like
conditions. \todo{Introduction Cleaned up until here. }
