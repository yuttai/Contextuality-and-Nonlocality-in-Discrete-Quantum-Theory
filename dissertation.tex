...
are resolved.
\end{enumerate}
The previous steps only tested in Microsoft Windows with MiK\TeX ~2.9.
Please report me if there is any difficulty to compile it. Besides,
...
of the inducing probabilities... }This idea will be extended to quantum
domain as quantum interval-valued probability measures (QIVPM).
...
to theories with such bizarre powers that they are probably unphysical.
...
quantum probability space. The expectation values defined in Sec.~\ref{sec:fuzzy}
...
a state to a quantum probability measure in CQT. However, since the
variants of quantum probability measures in Sec.~\ref{sec:Toward-IVPM}
...
to be orthogonal if $\ip{\Phi}{\Psi}=0$. This product satisfies conditions
\ref{enu:inner-product-complex-conjugate} and \ref{enu:inner-product-linear}
...
one... Moreover, Prof.~Ortiz wants to check Shor's algorithm in DQC(I).}
...
probability $\unknown$). Let $\left|\mathscr{I}\right|$ be the maximum
length of intervals in $\mathscr{I}$. We also need a notion of \emph{norm}
to quantify the distance between (pure or mixed) states. The norm
of a pure state $\rho=\proj{\psi}$ is defined as usual by $\left\Vert \psi\right\Vert =\sqrt{\ip{\psi}{\psi}}$.
...
then the norm of their difference is bounded by $\left|\mathscr{I}\right|$,
i.e., $\left\Vert \rho-\rho'\right\Vert \le\left|\mathscr{I}\right|$.\end{thm}
...
length of the intervals in $\mathscr{I}$ is $\left|\mathscr{I}\right|$,
it is also the upper bound of the difference: 
\begin{equation}
\left|\muB_{\rho'}\left(\proj{\psi}\right)-\muB_{\rho}\left(\proj{\psi}\right)\right|=\left|\melem{\psi}{\rho-\rho'}{\psi}\right|\le\left|\mathscr{I}\right|\,.
\end{equation}
...
Hence, $\left\Vert \rho-\rho'\right\Vert \le\left|\mathscr{I}\right|$.\end{proof}
...
future research.
\end{comment}
Recall in previous section, we denote the maximum length of intervals
in $\mathscr{I}$ as $\left|\mathscr{I}\right|$. The finite-precision
extension of the Gleason theorem, Thm.~\ref{thm:Finite-precision-Gleason},
only applies on the QIVPMs of the form $f\circ\muB_{\rho}$ and states
that as $\left|\mathscr{I}\right|$ shrinks to $0$, the composition
$f\circ\muB_{\rho}$ would be better in the sense that the states
consistent with the composition $f\circ\muB_{\rho}$ converges to
$\rho$. In contrast, the wildest QIVPM defined in Table~\ref{tab:non-Born-QIVPM}
has the range~$\mathscr{I}_{0}$ with $\left|\mathscr{I}_{0}\right|=1$,
the longest possible length. This suggests that shrinking the length
$\left|\mathscr{I}\right|$ might regularize QIVPMs mapping to $\mathscr{I}$
...
satisfying $\left|\mathscr{I}\right|<\varepsilon$, $\bar{\mu}$ must
have an non-empty core, i.e., $\coreBorn\left(\bar{\mu},\events\right)\ne\emptyset$.\end{question}
...
Schrödinger equation? And its relation with Grover's algorithm? }\todo{Notice
...
so does their interval-valued probabilities, i.e., $\bar{\mu}\left(\mathbb{1}-\proj{\psi}\right)=\necess-\bar{\mu}\left(\proj{\psi}\right)$.
...
space $\CP{2}$ as discussed in Sec.~\ref{subsec:Explicit-generalization-of}.
To further simplify the question, for any state $\ket{\psi}\in\CP{2}$,
we can decompose the interval-valued probabilities on its projectors
$\proj{\psi}$ into the left-end and right-end on $\ket{\psi}$, i.e.,
$\bar{\mu}\left(\proj{\psi}\right)=\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]$.
According to Lemma~\ref{thm:convex-3-1}, $\bar{\mu}:\events\rightarrow\mathscr{I}$
is a QIVPM if and only if the pair of functions $\frameL:\CP{2}\rightarrow\left[0,1\right]$
and $\frameR:\CP{2}\rightarrow\left[0,1\right]$ satisfying the following
interval-inclusion
\begin{equation}
\left[1,1\right]-\left[\frameL\left(\ket{\psi_{0}}\right),\frameR\left(\ket{\psi_{0}}\right)\right]\subseteq\left[\frameL\left(\ket{\psi_{1}}\right),\frameR\left(\ket{\psi_{1}}\right)\right]+\left[\frameL\left(\ket{\psi_{2}}\right),\frameR\left(\ket{\psi_{2}}\right)\right]\label{eq:interval-valued-frame-function}
\end{equation}
for any orthonormal basis $\left\{ \ket{\psi_{i}}\right\} _{i=0}^{2}$.
...
\end{equation}
Since $\left|\mathscr{I}\right|<\varepsilon$, the length of every
interval $\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]$
...
In another word, the left-end function satisfies the following inequalities
\begin{subequations}
\begin{eqnarray}
...
\end{equation}
for any state $\ket{\psi}\in\CP{2}$, where $\ket{\Phi_{j}}\in\mathcal{H}$
are normalized, $q_{j}>0$, and $\sum_{j=1}^{N}q_{j}=1$. Our Question~\ref{question:approximation-Gleason}
...
Gleason's theorem in general was asked by Sam Sanders in constructivenews~\cite{Sanders2013},
and there is no clear answer for his question as far as I understand.
To have an idea how hard this question could be, recall in Sec.~\ref{sec:fuzzy}
we state that a quantum probability space is glued by a family of
classical probability spaces. This is similar to the situation that
a manifold is glued by many local coordinates. When each small piece
has \emph{exactly} the same and positive curvature, the Killing-Hopf
theorem asserts this manifold is a sphere, but little can we say even
if the curvature has only small deviation from constant. Similar situation
might happen when approximating Gleason's theorem. Since the sum of
$\frameL$ on each ``local'' classical probability space, $\sum_{i=0}^{2}\frameL\left(\ket{\psi_{i}}\right)$,
is \emph{exactly} the same and equal to $1$, Gleason's theorem can
assert $\frameL$ can be expressed as Eq.~(\ref{eq:Gleason-for-CP2}).
...
say little about its global property.
