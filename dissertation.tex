...
even if the quantities describing molecules, like the domain of their
position wave functions or the probability amplitudes of a wave function,
are considered as continuous, we want to argue that these quantities
might later be discovered as discrete. Even if they were not discrete,
...
\end{tabularx}
\end{sidewaystable}
The situations for classical quantities, the computational power above
them, and their inspired quantum models are summarized in Table~\ref{tab:Comparison}.
Similar to classical analog computers, the quantum circuit model,
...
states. It is reasonable to expect the similar inconsistency between
classical classical quantities and their mathematical representation
applies to the quantum case as well. To address this issue, we will
start from the ``conventional quantum theory'' reviewed in Chapter~\ref{chap:Conventional-Quantum-Theory}\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
and ``ordinary'' quantum theory.}, where the probability amplitudes are in the field of complex numbers.
On one hand, if the probability amplitudes are assumed to be ultimately
discrete, we replace the field of complex numbers by discrete finite
fields and consider the quantum theories and computing over finite
fields. On the other hand, if the probability amplitudes are never
quantized no matter how hard we try, we still should not be able to
access their exactly values. This inaccessibility is essentially the
same as the measurement precision should only be finite, which will
be incorporated into the quantum theory by our quantum interval-valued
probability measures (QIVPM) framework.
