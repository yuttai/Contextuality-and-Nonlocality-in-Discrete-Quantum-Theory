...
over $\ff{3}$, $\ff{7}$, $\ff{11}$, $\ff{19}$, \ldots{}. We achieve
our goal by observing that any field in this family is extensible
to a field~$\ff{p^{2}}$ whose elements can be viewed as discrete
complex numbers with the real and imaginary parts in~$\ff{p}$. In
a complexified finite field $\Fpp$, the Frobenius automorphism that
maps $\alpha\in\Fpp$ to $\alpha^{p}\in\Fpp$ acts like complex conjugation
\cite{Artin1991,grove2002classical,DummitFoote2004}. For example,
in $\Fppx{3}$, we have $\left(2+\rmi\right)^{3}=8+12\rmi-6-\rmi=2+11\rmi$
...
to $a^{2}+b^{2}\in\Fp$, 
\begin{equation}
\fnorm{\alpha=a+\rmi b}=a^{2}+b^{2}\,.\label{fieldnorm.eq}
\end{equation}
