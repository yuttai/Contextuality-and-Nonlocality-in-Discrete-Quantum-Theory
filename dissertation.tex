...
amplitudes are assumed to be ultimately discrete, since their field
structure is essential to construct a quantum theory, we might assume
they are elements in discrete finite fields and build the quantum
theories and computing upon these finite fields in Chapter~\ref{chap:Unrestricted-Finite-Fields}
and \ref{chap:DQT=000026DQC}. Alternatively, the probability amplitudes
might never be found quantized. However, instead of considering the
precision of probability amplitudes, it is better to consider the
precision of their inducing probabilities because the probability
amplitudes is unlikely to be accessed directly while the probability
of an event can be deduced after a series of experiments. Therefore,
we extend the idea of ``imprecise probabilities'' into quantum domain,
and consider quantum interval-valued probability measures (QIVPM)
in Chapter~\ref{chap:QIVPM}.
...
will be used in Sec.~\ref{sec:The-geometry-of} to defined purity
...
We call it the modal quantum theory because it can only predict whether
...
states and entangled states could not only be identified as in CQT
but also be counted for different $p$. Despite this model's similarity
to CQT, it still only predicts whether a measurement result is possible
or impossible, because it is not possible to define an inner product
in the usual sense due to the modular arithmetic~\cite{grove2002classical}.
Under suitable conditions, we can have deterministic quantum algorithms
such as the algorithms of Deutsch, Simon \cite{Simon:1994:PQC:1398518.1399019,Mermin2007,Jaeger2007},
...
restricts states in some local region of the vector spaces. Within
a local region, a notion of inner product and probability could be
...
\end{comment}
Despite the great success of cardinal probabilities, it is difficulty
to define their arithmetic operations and expectation values. Since
these could be defined easily with real-valued probability measures
in CQT, we attempt to consider quantum probability measures over finite
fields (QPMFF) in Sec.~\ref{sec:Toward-IVPM}. In contrast to CQT,
our Gleason-like conditions over finite fields cannot deduce a discrete
Born rule in general. This impossible result suggests us that the
discrete and finite state spaces might not compatible with infinitely
precise real-valued probability. Thus, no matter whether the probability
amplitudes are ultimately discrete or not, 

it might be more natural to consider a discrete Born rule mapping
to finite number of intervals, and we will start the discussion in
Chapter~\ref{chap:QIVPM}.
...
and the expectation value is \todo{Introduction Cleaned up until
here. }
