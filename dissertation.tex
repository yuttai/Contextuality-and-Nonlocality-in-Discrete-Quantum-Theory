...
if the quantities describing molecules, like the domain of their position
wave functions or the probability amplitudes of a wave function, are
considered as continuous, we want to argue that these quantities might
later be discovered as discrete. Even if they are ultimately continuous,
this fact may never be proved and the exact value of these quantities
may never be pinned down. To better understand this issue without
struggling with the quantum theory, it is instructive to review how
...
asking whether it exists in the space is still beyond our ability
...
inspired quantum models. Similar to classical analog computers, the
quantum circuit model, the most widely used model of quantum computing,
manipulates a quantity which is not discrete apparently: the probability
amplitudes of quantum states, and it is reasonable to expect the similar
consistence between classical classical quantities and their mathematical
representation applies to the quantum case as well. To distinguish
...
and ``ordinary'' quantum theory.} On one hand, to address the assumption that the probability amplitudes
are ultimately discrete, we consider the quantum theories and computing
over finite fields because the conventional quantum theory is based
on the field of complex numbers, and it is essential to have a field
structure underlying a quantum theory. On the other hand, if the probability
amplitudes are contiuous, we want to incorporate the idea of measurement
precision into the quantum theory and consider the quantum interval-valued
probability measures.

\todo{Introduction Cleaned up until here. }Because we are agnostic
about whether probability amplitude is quantized or can really be
valued in the continuum of complex numbers, we tried to develop two
different types of models: the quantum theories and computing over
finite fields are discrete models, and the quantum interval-valued
