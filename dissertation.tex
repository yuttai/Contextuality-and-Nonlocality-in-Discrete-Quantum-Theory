...
\begin{preface*}
\begin{itemize}
\item The latest version of this thesis is in GitHub: \code{https://git.io/fxbuG}
...
Hence, when the quantities used to branch the computation is closed
to zero, the branch chosen by a BCSS machine cannot reliably predict
...
paths may be significantly different, and these difference may not
be compensated by error analysis techniques easily like the butterfly
...
\toprule 
\addlinespace
Physical quantities & Discrete & Discrete with extreme small unit & Continuous?\tabularnewline\addlinespace
\addlinespace
\addlinespace
Computational devices & Digital computer & Analog computer & Analog computer\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Mathematical representation & Integers & Real numbers & Real numbers\tabularnewline\addlinespace
\addlinespace
\addlinespace
...
\midrule
\addlinespace
How the models realize? & Reliably & 
...
\midrule
\addlinespace
Inspired quantum models &  & Quantum Theories and Computing over Finite Fields & Quantum Interval-Valued Probability Measures (QIVPM)\tabularnewline\addlinespace
\addlinespace
\bottomrule
\end{tabularx}
\end{sidewaystable}
The situations for classical quantities and the computational power
above them are summarized in Table~\ref{tab:Comparison}, and the
two possibilities dealing with real-valued models inspires us two
types of quantum models. Similar to classical analog computers, the
quantum circuit model, the most widely used model of quantum computing,
manipulates quantities which are not discrete apparently: the probability
amplitudes of quantum states. To address the similar inconsistency,
we first review the ``conventional quantum theory''\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
and ``ordinary'' quantum theory.} in Chapter~\ref{chap:Conventional-Quantum-Theory}, where the probability
amplitudes are in the field of complex numbers. If the probability
amplitudes are assumed to be ultimately discrete, we replace the field
of complex numbers by discrete finite fields and consider the quantum
theories and computing over finite fields in Chapter~\ref{chap:Unrestricted-Finite-Fields}
and \ref{chap:DQT=000026DQC}. Another possibilities could be that
the probability amplitudes would never be quantized no matter how
hard we try. Notice that the probability amplitudes can never be accessed
directly in an experiment, the accessible properties of a quantum
system is its measurement results and the probability of an event.
Hence, to incorporate the finite precision measurement into the quantum
theory, we consider a framework called quantum interval-valued probability
measures (QIVPM) in Chapter~\ref{chap:QIVPM}.
...
\end{comment}
The review of CQT in Chapter~\ref{chap:Conventional-Quantum-Theory}
starts from the geometrical structure of states in Sec.~\ref{CQC1qubitBloch.sec},
where a pure state is represented as a vector in the Hilbert space,
and an irreducible state is actually a point in the projective Hilbert
space \cite{MosseriDandoloff2001,Jaeger2007} also known as the complex
...
space, apply Gleason's theorem \cite{gleason1957,Redhead1987-REDINA,peres1995quantum,RichmanBridges1999,Hamhalter2013}
to recover the Born rule \cite{Born1983,Mermin2007,Jaeger2007}, and
define the expectation value of an observable. These expectation values
will be used to defined purity which provides an easy way to decide
whether a state is entangled or not and to defined the maximum entangled
states.
...
\end{comment}
When replacing the continuum of complex numbers by discrete finite
fields, we first doesn't restrict the chosen finite fields in Chapter~\ref{chap:Unrestricted-Finite-Fields}
\cite{Schumacher2012-SCHMQT,DQT2014,SchumacherWestmoreland2010}.
This theory is called modal quantum theory because it can only predict
...
can express simple algorithms such as quantum teleportation \cite{BennettBrassardEtAl1993,peres1995quantum,Mermin2007,Jaeger2007},
it is so weak that it cannot express Deutsch's algorithm \cite{Deutsch1985,Mermin2007}.
This quantum theory is, however, also so powerful that it can be used
to solve the $\usat$ problem~\cite{Papadimitriou1993}, which is
as hard as a general $\NP$-complete problem~\cite{Valiant198685}.
...
database search problem for certain database sizes.
\end{comment}
Next, in Sec.~\ref{discretequantumtheoryI} to \ref{sec:Geometry-of-Entangled-States},
...
discrete analogs that permit a great many of the standard requirements
of quantum computing to be preserved. In this model, the states of
a discrete $d$-dimensional system is a vector in the $d$-dimensional
vector space over $\Fpp$. \todo{Introduction Cleaned up until here.
} Because the states of a discrete $n$-qubit system are in principle
enumerable, we can count the number of states, and determine the proportions
of entangled and unentangled states. Under suitable conditions, we
have amplitude-based partitions of unity, unitary transformations,
and entanglement, as well as solutions to deterministic quantum algorithms
such as the algorithms of Deutsch, Simon \cite{Simon:1994:PQC:1398518.1399019,Mermin2007,Jaeger2007},
and Bernstein-Vazirani \cite{Bernstein:1993:QCT:167088.167097,Mermin2007},
though still with some bothersome shortcomings. Because of the modular
nature of arithmetic in the finite complex field, it is not possible
to define an inner product in the usual sense, and we show in Sec.~\ref{subsec:Partial-Algorithm}
that this leads to excessive computational power for the unstructured
database search problem for certain database sizes.
...
system are in principle enumerable, we can count the number of states,
and determine the proportions of entangled and unentangled states.
We tried define its measurement process in three different approaches.
For the model only predicting possible or impossible, we have solutions
to deterministic quantum algorithms such as Deutsch's algorithm, but
unnaturally strong computational power may still occur. In an improved
model, the cardinal probability of measurement results could be predicted
locally. In the local ordered region, the unnaturally computational
power is ruled out and the probabilistic Grover search algorithm can
be implemented. However, lacking arithmetic operations on cardinal
probabilities, expectation values cannot be defined easily. Since
arithmetic operations works with conventional probability measures,
we consider quantum probability measures over finite fields, but we
haven't found a consistent way to treat them in general.
...
\begin{sidewaystable}
\caption{Comparison among our models, where the upper half of the table lists
their quantum mechanical definitions and properties, and the lower
half of the table lists their computational power.}
...
\midrule
\addlinespace
Solve $\usat$ efficiently & Unlikely & Yes & Partially & Unlikely &  & \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
...
and precision of the particular experiment.\todo{Add a two-page conclusion
chapter which could include further discussion, but it can be only
a few sentences? }\todo{Discrete Schrödinger equation? And its relation
with Grover's algorithm? }\todo{Notice that in differential geometry,
Killing–Hopf theorem asserts that having positive constant curvature
guarantees a manifold is essentially a sphere, but say little things
even if the curvature has only small deviation from constant. Since
a quantum probability measure is glued by classical probability measures,
like a manifold. When their relations are exact, Gleason's theorem
asserts that a quantum probability measure can essentially induced
by the Born rule, but they can be wild when we move on to interval-valued
situation! }
