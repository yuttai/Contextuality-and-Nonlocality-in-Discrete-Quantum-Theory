...
an arbitrarily small extent \cite{Turing_1937,Gisin2017}, the quantum
mechanical model based on the continuum of complex numbers may not
capture our ability of predicting quantum phenomenon and the power
of realistic quantum computers. To incorporate our limitation of distinguishability,
but agnostic about whether the reality is ultimately discrete or continuous,
we considered two types of quantum models. By assuming the quantum
states are ultimately discrete and distinguishable, our first type
of quantum theories replaces the complex numbers by finite fields.
We then focus on the limitation of distinguishability itself, incorporate
the finite precision measurement into our model, and consider quantum
...
No matter whether we use classical or quantum computers to solve these
two tasks, they require two different types of quantities. The question
and answer of RSA are both discrete integers, but it is beyond our
ability to distinguish whether the quantities required by simulating
molecules are discrete or not. Before we dive into the discussion
of these quantities in quantum computation, it is instructive to review
how classical quantities are characterized and computed. 
...
model is continuous, their corresponding physical quantities are naturally
assumed to be continuous. Later, these continuous quantities becomes
the basic component of analog computers. After logarithm was discovered,
a slide rule, also known as a slipstick, is used to compute multiplication,
division, and more complex operations as a mechanical analog computer~\cite{wiki:SlideRule}.
Charge amplifiers are used to build electrical analog computers and
compute the integration in calculus \cite{wiki:Integrator,wiki:ChargeAmplifier}.

Although assuming physical quantities is as continuous as their mathematical
models are convenience, the physical quantities discussed in previous
paragraph are not simply real numbers. Chuang-tzu's stick are made
by molecules and cannot be cut in halves endlessly. The input charge
of an integrator an must actually be an integer multiple of the elementary
charge. A prefect square cannot be draw physically. It is beyond our
ability to decide whether a prefect square exists in the space because
whether space itself is ultimately discrete or continuous is still
an solved question. Even if it exists, human have no ability to distinguish
them from an almost prefect square differing only by an arbitrarily
small extent. Similar situation applying to the length of a slide
rule, it is beyond our ability to decide whether its length is ultimately
discrete or continuous. Even if it is continuous, there is no way
to identify which real number represents the length of a slide rule.
As the results, even if analog computers really store real numbers,
they cannot be read, write, and use to its exact value to branch the
computation. The opposite of the last condition sometimes is assumed
by real computational models. For example, Blum, Cucker, Shub, and
Smale modeled their BCSS machine to allow the operations of deciding
whether a number is greater or equal to zero over real numbers and
whether a number is exactly zero over complex numbers \cite{BSS1989,Ziegler2007,blum2012complexity}.
Therefore, the computational power of analog computers is far weaker
...
\toprule 
\addlinespace
Physical quantities & Discrete & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Discrete with\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
extreme small unit\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & We don't know\tabularnewline\addlinespace
\addlinespace
\addlinespace
...
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Mathematical\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
representation\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Integers & Real numbers & Real numbers\tabularnewline\addlinespace
\addlinespace
\addlinespace
...
\addlinespace
\addlinespace
How the models realize? & Reliably & Not reliably & Not reliably\tabularnewline\addlinespace
\bottomrule
\addlinespace
\end{tabular}
\end{table}
The situations for classical quantities and the computational power
above them are summarized in Table~\ref{tab:Comparison}, and similar
situations could potentially be applied to quantum mechanics and computation.
The physical quantity manipulated by the quantum circuit model, the
most widely used model of quantum computing, is the probability amplitudes
of states. \todo{Introduction Cleaned up until here. }Because we
are agnostic about whether probability amplitude is quantized or can
really be valued in the continuum of complex numbers, we tried to
develop two different types of models: the quantum theories and computing
over finite fields are discrete models, and the quantum interval-valued
probability measures incorporate the idea of measurement precision
into the quantum theory. To distinguish the modified quantum theories
from the usual ones, the usual ones will be called the ``conventional
...
\addlinespace
\addlinespace
States space & $\mathbb{C}^{D}$ & $\Fq^{D}$ & $\Fpp^{D}$ & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Local region\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
in $\Fpp^{D}$\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & $\Fpp^{D}$ & $\mathbb{C}^{D}$\tabularnewline\addlinespace
\addlinespace
\addlinespace
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Likelihood\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
of events is\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
predicted by\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Real-valued\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
probability\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Possible or\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
impossible\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
...
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Cardinal\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
probability\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
...
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
probability, but no\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
sensible Born rule\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Interval-valued\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
probability\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular}\tabularnewline\addlinespace
\addlinespace
\addlinespace
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Expectation\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
value\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Defined & Undefined & Undefined & Undefined &  & Defined\tabularnewline\addlinespace
\addlinespace
\midrule 
\addlinespace
...
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Hadamard\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
transformation\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Included & %
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
May not be\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
included\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Included & Included &  & \tabularnewline\addlinespace
\addlinespace
\addlinespace
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Solve\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
$\usat$\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
efficiently\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Unlikely & Yes & Partially & Unlikely &  & \tabularnewline\addlinespace
\addlinespace
\addlinespace
\begin{tabular}{c}
\noalign{\vskip\doublerulesep}
Grover search\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\noalign{\vskip\doublerulesep}
algorithm\tabularnewline[\doublerulesep]
\noalign{\vskip\doublerulesep}
\end{tabular} & Yes &  &  & Yes &  & \tabularnewline\addlinespace
\bottomrule
\addlinespace
\end{tabular}
\end{sidewaystable}
\todo{Beginning of Merge Left}Since conventional probability amplitude
is in the field of complex numbers, and the field structure is essential
to compute the probability from some probability amplitudes, we in
Chapter~\ref{chap:DQT=000026DQC} tried to build discrete quantum
theories and computing by considering them over finite fields. 
...
more faithful.\todo{End of Merge Left}

\todo{Beginning of Merge Right}We begin by replacing the continuum
of complex numbers by discrete finite fields in quantum theory. The
simplest theory, defined over unrestricted finite fields, is so weak
that it only predict whether the measure result is possible or impossible.
Its computational model is also weak that it cannot express Deutsch's
algorithm, but, paradoxically, is also so powerful that it can be
used to solve the $\usat$ problem, which is as hard as a general
$\NP$-complete problem. 
...
measures, but also allows us to establish bounds on the validity of
the Kochen-Specker and Gleason theorems in realistic experimental
environments.\todo{End of Merge Right}
...
\chapter{Quantum Theories and Computing over Unrestricted Finite Fields}
