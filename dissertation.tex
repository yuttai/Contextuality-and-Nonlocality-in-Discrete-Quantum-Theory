...
functions, which look like continuous at first sight. Indeed, either
...
paths may be significantly different. These difference may not be
compensated by error analysis techniques easily like the butterfly
...
hard we try, but we could never access their exactly values as well.
Notice that the probability amplitudes can never be accessed directly
in an experiment, the accessible properties of a quantum system is
its measurement results and the probability of an event. Hence, to
incorporate the finite precision measurement into the quantum theory,
we consider a framework called quantum interval-valued probability
