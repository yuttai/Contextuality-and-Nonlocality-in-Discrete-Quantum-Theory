...
in $\mathscr{I}$ as $\left\Vert \mathscr{I}\right\Vert $. The finite-precision
...
that as $\left\Vert \mathscr{I}\right\Vert $ shrinks to $0$, the
composition $f\circ\muB_{\rho}$ would be better in the sense that
the states consistent with the composition $f\circ\muB_{\rho}$ converges
to $\rho$. In contrast, the wildest QIVPM defined in Table~\ref{tab:non-Born-QIVPM}
has the range~$\mathscr{I}_{0}$ with $\left\Vert \mathscr{I}_{0}\right\Vert =1$,
the longest possible length. This suggests that shrinking the length
$\left\Vert \mathscr{I}\right\Vert $ might regularize QIVPMs mapping
to $\mathscr{I}$ as the following question.
...
satisfying $\left\Vert \mathscr{I}\right\Vert <\varepsilon$, $\bar{\mu}$
...
\end{equation}
Since $\left\Vert \mathscr{I}\right\Vert <\varepsilon$, the length
