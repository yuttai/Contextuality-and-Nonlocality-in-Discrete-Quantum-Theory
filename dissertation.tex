...
depending on the measurement bases, this hidden variable theory is
called \emph{contextual}. The measurement process itself in MQT depending
...
is a sphere, but little can we say even if the curvature has a small
deviation from constant. A similar situation might happen when approximating
Gleason's theorem, but this time the whole space is glued by ``local''
classical probability space defined by each orthonormal basis $\left\{ \ket{\psi_{i}}\right\} _{i=0}^{2}$.
