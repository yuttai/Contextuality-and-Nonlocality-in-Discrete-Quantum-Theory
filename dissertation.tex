...
like the way being acknowledged\ldots{} }I want to thank Prof.~Gerardo
...
Chapter~\ref{chap:QIVPM} is based on ``Quantum Interval-Valued
...
and passing my dissertation proposal Spring 2017. I also thank Prof.\ Dylan
Thurston for serving in my advisory committee and my Tier 3 committee,
with other advisory committee members passing my Computer Science
qualifying exam Spring 2016. Especially, my survey of real computation
in the qualifying exam inspired me the comparison of quantum computer
and analog computer in Chapter~\ref{chap:Introduction}. Chapter~\ref{chap:Introduction}
also contains the motivation in my application for Rethinking Foundations
of Physics 2017 workshop reviewed by Prof.\ Amr Sabry~(\textarabic{عمرو صبري}).
I want to thank Prof.\ Lawrence Moss for inviting me to present our
results in the interdisciplinary logic seminar and theory seminar
in Computer Science, co-chairing my Tier 3 committee, writing an assessment
letter for me, and drawing my attention on Prof.~Abramsky's paper~\cite{Abramsky2012},
...
Judge and Prof.\ Matthias Weber; directors of PhD. study in Computer
Science, especially Prof.\ Yuqing Melanie Wu (吴愈青) and Prof.\ \textturkish{Funda
Ergun}.

Thank Kin Wai Chan for suggesting me the correct direction for Proposition~\ref{appH.sec},
...
Thank all my friends for your supports, including:

\begin{itemize}

\item Meng-Wei Chen (陳孟瑋) as my roommate, AT\&T family plan member,
and emergency contact for all kinds of emergency and non-emergency
situations.

\item My other AT\&T family plan members, especially Kexin ``Casey''
Chen (谌可心), Yao-yu Chih (池耀宇), Xinyi ``Diana'' Gong (龚欣怡), and Nanxi
``Nancy'' Zhang (张楠茜), for you always pay our telephone bill on
time so that\ldots\ldots\todo{Maybe I don't have to thank them
as AT\&T family plan members, but I should thank some of them for
support and sharing information\ldots}

\item My other roommates,

\item Hao-Chun Lee (李浩君) and Hsien-Ching Kao (高憲慶) for discussing
what I did in the prospective of a general PhD. in Physics and working
...
Since we as human cannot distinguish quantities differing only by
...
better capture our ability of predicting quantum phenomenon and the
power of realistic quantum computers. Since we are agnostic about
whether the reality is ultimately discrete or continuous, we considered
two types of quantum models. By assuming the quantum states are ultimately
...
of distinguishability itself, incorporate the finite precision measurement
...
and countries start to build realistic quantum computers recently
\cite{Shieber2018,Kahn2019}, and seek more efficient ways to simulate
chemical molecules and to break down RSA \cite{boneh1999twenty,wiki:RSAFactoring}.
...
cultures since very beginning. For example, the ancient Chinese philosopher
Chuang-tzu (庄子) didn't believe these quantities have a minimum unit,
and expressed ``If a one-foot-long stick is cut into halves every
day, the cutting will never come to an end''\footnote{``一尺之棰，日取其半，萬世不竭。'' in Chinese \cite{Zhuangzi1999,Liu2018}.}.
...
become the basic component of analog computers. After logarithm was
discovered, a slide rule, also known as a slipstick, is used to compute
...
to build electrical analog computers and compute the integration in
calculus \cite{wiki:Integrator,wiki:ChargeAmplifier}.

Although it is convenient to assume physical quantities is as continuous
...
with a real number. Chuang-tzu's stick are made by molecules and cannot
be cut in halves endlessly. The input charge of an integrator must
be an integer multiple of the elementary charge. A prefect square
cannot be draw physically. Even we don't draw it, but merely ask whether
it exists in the space. This question is still beyond our ability
to answer because whether the space itself is ultimately discrete
or continuous is still an unsolved question. Even if a prefect square
exists physically, we may not able to decide whether the ``square''
we discussed is really prefect or almost prefect differing only by
...
precision, but the precision in reality is limited generally three
...
effect. Therefore, the computational power of analog computers might
be far weaker than the prediction based on this kind of theoretical
models.%
\begin{comment}
\todo{Maybe extended to consider probabilistic model? }
\end{comment}

\begin{sidewaystable}
\caption{\label{tab:Comparison}Comparison between the classical quantities
...
\toprule 
\addlinespace
Physical quantity & Discrete & Discrete with extreme small units & Continuous, or no evidence to support they are discrete\tabularnewline\addlinespace
\addlinespace
\addlinespace
Computational device & Digital computer & Analog computer & Analog computer\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Mathematical representation & Integer & Real number & Real number\tabularnewline\addlinespace
\addlinespace
\addlinespace
Computational model  & Turing machine & BCSS machine & BCSS machine\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
How does the theorical model predict the behavior of the Physical device? & 
Reliably & 
Not reliably, but after we have a better technology to manipulate the minimum units, we can predict the physical phenomenon and their computational power by a better discrete model. & 
Not reliably, because precision can never be high enough, and the difference between their computational power might not be able compensated by error analysis techniques. \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Inspired quantum model &  & Quantum theories and computing over finite fields & Quantum interval-valued probability measures (QIVPM)\tabularnewline\addlinespace
\addlinespace
\bottomrule
\end{tabularx}
\end{sidewaystable}
...
states. To address the similar inconsistency, we first review the
``conventional quantum theory'' (CQT)\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
and ``ordinary'' quantum theory.}, where the probability amplitudes are in the field of complex numbers.
If the probability amplitudes are assumed to be ultimately discrete,
...
``imprecise probabilities'' have well-studied classically \cite{Shafer1976,GilboaSchmeidler1994,Marinacci1999,JamisonLodwick2004,HuberRonchetti2009,Grabisch2016}.
...
of the inducing probabilities\ldots{} }This idea will be extended
to quantum domain as quantum interval-valued probability measures
(QIVPM).
...
whether a measure result is possible or impossible, but not its probability.
Although a version of quantum theory defined over the two-valued field
can express simple algorithms such as quantum teleportation \cite{BennettBrassardEtAl1993,peres1995quantum,Mermin2007,Jaeger2007,Schumacher2012-SCHMQT},
...
integer), the complex numbers have extremely compelling and natural
...
$p$. Despite this model's similarity to CQT, it still only predicts
whether a measurement result is possible or impossible, because it
is not possible to define an inner product in the usual sense due
to the modular arithmetic~\cite{grove2002classical}. Under suitable
conditions, we can have deterministic quantum algorithms such as the
algorithms of Deutsch, Simon \cite{Simon:1994:PQC:1398518.1399019,Mermin2007,Jaeger2007},
and Bernstein-Vazirani \cite{Bernstein:1993:QCT:167088.167097,Mermin2007},
...
this discrete quantum theory is applied on two representative algorithms,
the deterministic Deutsch-Jozsa algorithm \cite{DeutschJozsa1992,Jaeger2007}
and the probabilistic Grover algorithm \cite{Grover:1996:FQM:237814.237866,Mermin2007,Jaeger2007}.
\begin{comment}
...
a discrete Born rule in general. This impossible result suggests us
that the discrete and finite state spaces might not be compatible
with infinitely precise real-valued probability. Thus, no matter whether
...
we utilize the previous defined expectation values while proving the
finite precision variants of the Kochen-Specker theorem \cite{BELL_1966,kochenspecker1967,Redhead1987-REDINA,peres1995quantum,Jaeger2007},
...
a Hilbert space~$\Hilb$ of dimension $d\ge3$, there is a mathematically
precise sense in which one recovers the original Gleason theorem asymptotically.
Specifically, it is possible to construct a class of QIVPMs representing
bounded resources that is parameterized by the size of the intervals.
We then demonstrate that all QIVPMs in this class are consistent with
a non-empty “ball” of quantum states whose radius is defined
by the maximal length of the intervals characterizing the uncertainties.
As the size of the intervals goes to zero, this ball of quantum states
converges to a point representing the unique state consistent with
the Born rule and Gleason's theorem.

\begin{sidewaystable}
\caption{\label{tab:organization}Comparison among our models, where the upper
...
\toprule 
\addlinespace
 & Conventional & Modal & Discrete (I) & Discrete (II) & QPMFF & QIVPM\tabularnewline\addlinespace
\addlinespace
\midrule
\midrule 
\addlinespace
 & Chapter~\ref{chap:Conventional-Quantum-Theory} & Sec.~\ref{modalquantum} & Sec.~\ref{discretequantumtheoryI} to~\ref{sec:Geometry-of-Entangled-States} & Sec.~\ref{sec:Discrete-Quantum-Theory-(II)} & Sec.~\ref{sec:Toward-IVPM} & Chapter~\ref{chap:QIVPM}\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
States space & $\mathbb{C}^{D}$ & $\Fq^{D}$ & $\Fpp^{D}$ & Local region in $\Fpp^{D}$ & $\Fpp^{D}$ & $\mathbb{C}^{D}$\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Likelihood of events is predicted by & Real-valued probability & Possible or impossible & Possible or impossible & Cardinal probability & Real-valued probability, but no sensible Born rule & Interval-valued probability\tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Expectation value & Defined & Undefined & Formally defined & Undefined &  & Defined\tabularnewline\addlinespace
\addlinespace
\midrule
\midrule 
\addlinespace
 &  & Sec.~\ref{modalquantumcomputing} & Sec.~\ref{discretequantumcomputingI} & Sec.~\ref{discretequantumcomputingII} &  & \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Deutsch's algorithm & Yes & Maybe no & Yes & Yes &  & \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Efficiently solve $\usat$ & Unlikely & Yes & Partially & Unlikely &  & \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
Grover search algorithm & Yes &  &  & Yes &  & \tabularnewline\addlinespace
\addlinespace
\bottomrule
\end{tabularx}
\end{sidewaystable}
The organization of this thesis is summarized in Table~\ref{tab:organization},
and finally Chapter~\ref{chap:Further-Discussion} contains the further
extended questions.
...
circuits will be introduced later in Chapter~\ref{chap:DQT=000026DQC}
...
\par\end{centering}
}\hfill{} \label{hopfFibers.fig}\caption{(a) The two-sphere~$\Sphere{2}$ represented by \Eq{A7.eq}, which
...
\par\end{centering}
\label{blochSphere.fig}\caption{(a) The conventional Bloch sphere with a unique state represented
...
measure define on the whole space can be simply induced by the Born
...
a bad idea because two events might appear different but representing
the same situation. For example, if we take the complement on the
both sides of $\mu_{0}\left(\left\{ \ket{2}\right\} \right)=\mu_{1}\left(\left\{ \ket{2}\right\} \right)$,
we will have
\begin{equation}
...
event space~$\events$. \todo{Check the citation of this part. Did
von Neumann really think about this when he uses projectors to formulate
quantum theory?}
...
structure: $\varphi\left(\Omega\right)=\mathbb{1}$ and $\varphi\left(\overline{E}\right)=\mathbb{1}-\varphi\left(E\right)$.
...
space~$\events$ together with a quantum probability measure $\mu:\events\rightarrow\left[0,1\right]$
...
the domain of $\varphi$ on an classical event space $2^{\Omega}$,
the function $\varphi^{*}\mu:2^{\Omega}\rightarrow\left[0,1\right]$
defined by precomposition $\left(\varphi^{*}\mu\right)\left(E\right)=\mu\left(\varphi\left(E\right)\right)$
is a classical probability measure and called the pullback of $\mu$
by $\varphi:2^{\Omega}\rightarrow\events$.
...
random variable, and the expectation values are invariant.\todo{Add
interpretation!}
...
\todo{Put the definition of indicator function!}\end{definition}
...
convention to introduce the Born rule which is the only way corresponding
...
and Chapter~\ref{chap:QIVPM} could not be constructed by the Born
rule easily, searching for a Born rule step-by-step here might provide
a better idea what we should do in Sec.~\ref{sec:Toward-IVPM} and
Chapter~\ref{chap:QIVPM}. While the situations will become more
...
unique mixed state $\rho=\sum_{j=1}^{N}q_{j}\proj{\Phi_{j}}$ such
...
then the event~$P$ always happens, $\muB_{\Phi}\left(P\right)=1$,
...
changing to another basis is the same as applying an unitary map~$U$,
we should have $\muB_{U\ket{\Phi}}\left(UPU^{\dagger}\right)=\muB_{\Phi}\left(P\right)$.
...
also use to decide whether a state is entangled or not for multipartite
...
All possible choices of generators in $\ff{3^{2}}$ is listed in Table~\ref{tab:Generators-in-F9}.
\begin{table}
...
let $\fnorm*{\left\{ c\right\} }$ denote the set of elements whose
...
the Hopf fibration circle here\ldots ? }Since $\Fpp$ has $p^{2}-1$
...
for non-vanishing $\alpha$, including the map to the unit norm value
$c=1\in\Fp$.
\begin{figure}
\noindent \centering{}\includegraphics[width=3.6in]{Fpp2Fp}\label{proof1.fig}\caption{Sketch of the map from $\Fpp$ to $\Fp$ using $\fnorm{\alpha}$,
...
\par\end{centering}
\label{bloch.fig}\caption{Schematically normalized plots of the elements of the discrete Bloch
...
\section{Discrete Quantum Theory (I): Geometry of Entangled States\label{sec:Geometry-of-Entangled-States}}
...
\par\end{centering}
\label{statePlot3Unit.fig}\caption{Logarithmic plot of the number of discrete unnormalized states (top,
...
one\ldots{} Moreover, Prof.~Ortiz wants to check Shor's algorithm
in DQC~(I).}
...
$3$, \ldots , $2^{3n+2}$ are all quadratic residues. Hence the numbers
$-2^{n}$, \ldots , $2^{n}$ are all inside the ordered range $S_{0}\left(k\right)$.
On the other hand, if we choose any prime smaller than $p$, there
is a quadratic non-residue smaller than $k$, and we also know that
the least quadratic non-residue is a prime~\cite{numtheory.ref}.
...
\begin{equation}
\Delta=\begin{pmatrix}1-\frac{N}{2} & 1 & 1 & \cdots & 1\\
...
can be written as \todo{Verify the computation??}
\begin{equation}
\begin{pmatrix}a_{l}\\
b_{l}\\
\vdots\\
...
since we haven't found a way to manipulate the resulting cardinal
probabilities, we have little idea how to define the mixed states
and expectation values. To overcome these issues, we try a different
approach to define a ``conventional'' probability over finite fields
axiomatically.
...
\end{equation}
which is much more than one, the number of QPMFF\@. Since we don't
have enough QPMFF to correspond even pure states, there is no discrete
...
Like the previous paragraph, we now compute the QPMFF induced by $\ket{\Phi'}=U'\ket{\oplus}=\frac{2\alpha}{\gamma}\ket{0}+\frac{1}{\gamma}\ket{2}$,
...
we discussed the ``true'' probabilities, $\mu\left(E\right)$ and
$\mu\left(\overline{E}\right)$, can be anywhere in the range $\left[0.2,0.3\right]$
and $\left[0.7,0.8\right]$, respectively, as long as they satisfy
$\mu\left(E\right)+\mu\left(\overline{E}\right)=1$. These ``true''
probabilities can be summarized as a probability measure $\mu:2^{\Omega}\rightarrow\left[0,1\right]$
satisfying 
\begin{align}
...
\end{align}
we conclude the ``true'' probability of each event belongs to the
interval probability of the corresponding event. This conclusion gives
...
Like how we pull back classical ideas to the quantum world in Secs.~\ref{subsec:Definitions-of-IVPMs}
...
\end{equation}
In another word, if $\mu$ is in the core of $\bar{\mu}$, the expectation
value of $X$ belongs to $\left[1.7,1.8\right]$ which should be the
expectation value of $X$ with respect to $\bar{\mu}$.
...
Our interval-valued probability framework will allow us to state,
and prove, a stronger version of the theorem that holds even if the
observables have values that are only definite up to some precision
specified by a parameter $\delta$. Our approach provides a quantitative
realization of Mermin's intuition~\cite{Mermin1999}: 
\begin{quote}
...
extension of the KS theorem demonstrates contextuality (\textsf{C}).
...
Like how we proved Thm.~\ref{cor:Kochen-Specker-IVPM}, we need other
two lemmas to simplify the proof of the convexity condition.
...
some intersections, but not a subset relation, then $E_{0}\cap E_{1}$,
$E_{0}\backslash E_{1}$, and $E_{1}\backslash E_{0}$ are all non-empties.
...
future research.
\end{comment}
As we discussed in the previous section, Thm.~\ref{thm:Finite-precision-Gleason}
...
with the composite QIVPM collapses to a single state as the maximum
...
mapping to intervals shorter than $\varepsilon$ always have non-empty
cores.
...
such that for all QIVPM $\bar{\mu}:\events\rightarrow\mathscr{I}$
satisfying $\left\Vert \mathscr{I}\right\Vert \le\varepsilon$, $\bar{\mu}$
must have an non-empty core, i.e., $\coreBorn\left(\bar{\mu},\events\right)\ne\emptyset$
?\end{question}
...
that in differential geometry, Killing–Hopf theorem asserts that having
positive constant curvature guarantees a manifold is essentially a
sphere, but say little things even if the curvature has only small
...
can essentially induced by the Born rule, but they can be wild when
we move on to interval-valued situation! }
\end{comment}
...
of functions $\frameL:\CP{2}\rightarrow\left[0,1\right]$ and $\frameR:\CP{2}\rightarrow\left[0,1\right]$
defined by $\left[\frameL\left(\ket{\psi}\right),\frameR\left(\ket{\psi}\right)\right]=\bar{\mu}\left(\proj{\psi}\right)$
for any irreducible state $\ket{\psi}\in\CP{2}$. According to Lemma~\ref{thm:convex-3-1},
$\bar{\mu}:\events\rightarrow\mathscr{I}$ is a QIVPM if and only
if $\bar{\mu}$ satisfies Eqs.~(\ref{eq:QIVPM-constraints}) and
\begin{equation}
...
In another word, the left-end function~$\frameL$ satisfies the following
inequalities
\begin{subequations}
\begin{eqnarray}
...
given any function $\frameL:\CP{2}\rightarrow\left[0,1\right]$ satisfying
Eq.~(\ref{eq:left-end-frame-function}), there exists a unique mixed
state~$\rho$ such that 
\begin{equation}
...
then ask what $\frameL$ would look like with a positive $\varepsilon$.

With different settings, whether there is an approximate version of
Gleason's theorem was asked by Sam Sanders in constructivenews on
2013~\cite{Sanders2013}, and there is no clear answer for his question
as far as I understand. To have an idea how hard this question could
be, recall in Sec.~\ref{sec:fuzzy} we state that a quantum probability
space is glued by a family of classical probability spaces. This is
like the situation that a manifold is glued by many local coordinates.
...
can we say even if the curvature has only small deviation from constant.
Similar situation might happen when approximating Gleason's theorem,
...
need to know a little bit more, like its QIVPM is a composite function,
to deduce its global property.
...
probability amplitudes instead of the measured probability, a quantum
computing model above QIVPMs might either need to manipulate the states
in the core of a QIVPM or find a way to manipulate QIVPMs directly.
However, both strategies are not straightforward. On one hand, a QIVPM
might have an empty core which cannot be evolved over time as we proved
in Thm.~\ref{thm:Non-extensible-of-Gleason's}. On the other hand,
if we want to manipulate and compute QIVPMs directly for multi-qubit
algorithms, we need to glue the QIVPM for each qubit or subsystem
together to get the QIVPM of the whole system, and this is not straightforward
either.

A successful interval-valued theory might be further extended to the
quantum theory over finite fields as follow.
...
probability measure over finite field $\bar{\mu}:\events_{p^{2}}\rightarrow\mathscr{I}$
...
and whether we could define a ``sensible'' Born upon them combine
