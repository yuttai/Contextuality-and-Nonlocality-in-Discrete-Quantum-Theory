...
The abstract should said them separately and explicitly.'' }

Most quantum computing models are based on the continuum of real numbers,
while classical digital computers faithfully realize only discrete
computational models. Analog computers appear to be an option, but
in reality are far weaker than would be needed for computational models
requiring real numbers. One approach to resolving this conflict is
to find consistent mathematical ways to limit measurement precision
to computable contexts that do not require incomputable real numbers.
\todo{I don't understand the previous two sentences... Maybe simplify...
...
unrestricted finite fields, is so weak that it only predict whether
the measure result is possible or impossible. Its computational model
is also weak that it cannot express Deutsch's algorithm, but, paradoxically,
is also so powerful that it can be used to solve the $\usat$ problem,
which is as hard as a general $\NP$-complete problem. 
...
model, the probability of measurement results could be predicted locally. 

the probabilistic Grover search algorithm can be implemented in a
local region, we haven't found a consistent way to treat quantum probability
measures in general.
...
\tableofcontents{}\todo{Reorganize the chapters and sections to
fit the new abstract.}
