...
and the foundations of quantum probability space. The observables
and expectation values defined in Sec.~\ref{sec:fuzzy} will be used
to understand the geometry of entangled states in Sec.~\ref{sec:The-geometry-of}.
The property~\ref{enu:unitary-matrix} directly related to quantum
circuit will be introduced later in Chapter~\ref{chap:DQT=000026DQC}
...
coherence, we want $\mu$ subject to the following constraints. When
$E$ contains no outcome, $E$ is the least likely to happen, i.e.,
$\mu\left(\emptyset\right)=0$. When $E$ contains all outcomes, $E$
is the most likely to happen, i.e., $\mu\left(\Omega\right)=1$. If
we know the likelihood of an event, $\mu\left(E\right)$, it is natural
to assume the likelihood of the event’s complement is just the opposite
of $\mu\left(E\right)$, i.e., $\mu\left(\overline{E}\right)=1-\mu\left(E\right)$.
For each pair of disjoint events $E_{0}\subseteq\Omega$ and $E_{1}\subseteq\Omega$,
the likelihood of the union event is additive, $\mu\left(E_{0}\cup E_{1}\right)=\mu\left(E_{0}\right)+\mu\left(E_{1}\right)$.

Since the previous abstraction disregards the underlying process generating
the outcomes, this process might be a quantum measurement. Let us
prepare a beam of a kind of spin~$1$ particles. In principle, we
can preform a Stern-Gerlach type experiment to splits the beam according
to the eigenvalues of the observable $\mathbf{O}_{0}=0\,\proj{0}+1\,\proj{1}+2\,\proj{2}$
\cite{:/content/aip/journal/jmp/21/1/10.1063/1.524312,peres1995quantum}.
If we only send one particle instead of a beam, it could be in three
possible states after the experiment, i.e., the outcome of this process
is $\Omega_{0}=\left\{ \ket{0},\ket{1},\ket{2}\right\} $. All possible
...
and $\Omega$. Given any particle, it specifies the likelihood of
each event $\mu_{0}\left(E\right)$, and the function $\mu_{0}:2^{\Omega}\rightarrow\left[0,1\right]$
is a probability measure satisfying the conditions listed in previous
paragraph.

The different of the quantum world is that the probability of previous
experiment is correlated to the other experiment if we use the same
particle, but change the beam splitter. Consider a beam splitter corresponding
to the observable $\mathbf{O}_{1}=0\,\proj{\ps}+1\,\proj{\ms}+2\,\proj{2}$,
where $\ket{\ps}=\frac{\ket{0}+\ket{1}}{\sqrt{2}}$ and $\ket{\ms}=\frac{\ket{0}-\ket{1}}{\sqrt{2}}$.
The outcome of this process is $\Omega_{1}=\left\{ \ket{\ps},\ket{\ms},\ket{2}\right\} $.
All possible events are $\emptyset$, $\left\{ \ket{\ps}\right\} $,
$\left\{ \ket{\ms}\right\} $, $\left\{ \ket{2}\right\} $, $\left\{ \ket{\ps},\ket{\ms}\right\} $,
$\left\{ \ket{\ps},\ket{2}\right\} $, $\left\{ \ket{\ms},\ket{2}\right\} $,
and $\Omega_{1}$. When we can formulate the likelihood of each event
as another probability measure $\mu_{1}:2^{\Omega}\rightarrow\left[0,1\right]$,
we found the likelihood of the same event of these two experiments
are the same if we use the same particle, i.e., $\mu_{0}\left(\emptyset\right)=\mu_{1}\left(\emptyset\right)$
and $\mu_{0}\left(\left\{ \ket{2}\right\} \right)=\mu_{1}\left(\left\{ \ket{2}\right\} \right)$.

Since the probability is always the same if the particle and the event
are the same no matter which beam splitter is actually used. It is
more natural to define the set of quantum event~$\events$ as all
possible events using different beam splitters. Since we are focus
on the probability instead of outcome, and the probabilities of events
$\left\{ \ket{0},\ket{1}\right\} $ and $\left\{ \ket{\ps},\ket{\ms}\right\} $
are always the same, it is reasonable to identify them as the same
event. 

, and consider the quantum probability measure $\mu:\events\rightarrow\left[0,1\right]$
as a function assigning the likelihood to every quantum event
...
the set of all events is the set of all projectors on a given Hilbert
space denoted by $\events$. \todo{Use the same idea as the classical
probability measure to explain the meaning of $\mu\left(P\right)$.
Maybe move the structure preserving function $\varphi:2^{\Omega}\rightarrow\events$
defined in Eq.~\ref{eq:pullback-function} here.} each event~$P_{i}$
is mapped to a probability~$\mu\left(P_{i}\right)$ using a probability
measure $\mu:\events\rightarrow\left[0,1\right]$ subject to the following
constraints:
\begin{align}
...
\todo{Rewrite the beginng of this paragraph to explain the meaning
...
\subsection{Issues about Measurements and Contextuality}
...
\end{equation}
If we follow the convention $\varphi\left(\emptyset\right)=\mathbb{0}$,
this function $\varphi$ naturally sends the set operations to the
corresponding projector operations: $\varphi\left(\Omega\right)=\mathbb{1}$,
$\varphi\left(\overline{E}\right)=\mathbb{1}-\varphi\left(E\right)$,
and
\begin{align}
...
\end{align}
for each pair of set $E_{0}\subseteq\Omega$ and $E_{1}\subseteq\Omega$.
...
IVPM naturally. Moreover, if $\mu:\events\rightarrow\left[0,1\right]$
is a quantum probability measure satisfying Eqs.~(\ref{eq:QuantumProbability})
and (\ref{eq:QuantumProbability-Addition}), the pullback of $\mu$,
$\varphi^{*}\mu:2^{\Omega}\rightarrow\left[0,1\right]$, can also
be defined by precomposition $\left(\varphi^{*}\mu\right)\left(E\right)=\mu\left(\varphi\left(E\right)\right)$,
and is a classical probability measure as well.
...
of Thm.~\ref{thm:Rosenmuller}. Although we previously only pull
back probability measures, this time we also need to pull back observables.
...
If we follow Eq.~(\ref{eq:pullback-function}) to define $\varphi:2^{\Omega}\rightarrow\events$,
then the pullback of $\mathbf{O}$ by $\varphi$ is a random variable
$\varphi^{*}\mathbf{O}:\Omega\rightarrow\mathbb{R}$ defined by $\varphi^{*}\mathbf{O}=\sum_{i=0}^{D-1}\lambda_{i}\mathbf{1}_{\left\{ \ket{i}\right\} }$.\end{definition}

While both observables and probability measures can be pulled back
from quantum to classical, these pullbacks keep the expectation value
invariant.
...
defined by Eq.~(\ref{eq:pullback-function}).
\begin{itemize}
\item Given a quantum probability measure $\mu:\events\rightarrow\left[0,1\right]$,
the expectation value of $\mathbf{O}$ relative to $\mu$ is exactly
the expectation value of the pullback of $\mathbf{O}$ relative to
the pullback of $\mu$, i.e., 
\begin{equation}
...
\end{equation}
\item Given a QIVPM $\bar{\mu}:\events\rightarrow\mathscr{I}$, the expectation
value of $\mathbf{O}$ relative to $\bar{\mu}$ is exactly the expectation
...
\begin{proof}[Proof of Eq.~(\ref{eq:real-expectation-value-between-classical-quantum})]By
Eq.~(\ref{eq:quantum-expectation}), we have
\[
...
\begin{proof}[Proof of Eq.~(\ref{eq:interval-expectation-value-between-classical-quantum})]Consider
to express the observable~$\mathbf{O}$ in the spectral decomposition
specialized to compute the expectation value relative to a QIVPM,
$\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}P_{i}^{s}$,
where $\lambda_{N^{-}}^{-}<\cdots<\lambda_{1}^{-}<0\le\lambda_{1}^{+}<\cdots<\lambda_{N^{+}}^{+}$
and each $P_{i}^{s}$ is the projector onto the eigenspace of distinct
