...
actually be an integer multiple of the elementary charge. A prefect
square cannot be draw physically. Even we don't draw it, but merely
ask whether it exists in the space. This question is still beyond
our ability to answer because whether the space itself is ultimately
discrete or continuous is still an unsolved question. Even if a prefect
square exists physically, we may not able to decide whether the ``square''
...
paths may be significantly different, and these difference is unlikely
...
models. Similar to its classical counterparts, quantum circuit model,
the most widely used model of quantum computing, manipulates quantities
...
amplitudes might never be found quantized, taking into account the
precision of quantities could still result in a better model. However,
comparing to the precision of probability amplitudes, it is easier
to consider the precision of their inducing probabilities because
the idea of ``imprecise probabilities'' have well-studied classically
\cite{Shafer1976,GilboaSchmeidler1994,Marinacci1999,JamisonLodwick2004,HuberRonchetti2009,Grabisch2016}.
...
likely one of the outcomes in $E$ will actually happen. To maintain
the coherence, $\mu$ is subject to the following constraints: $\mu\left(\emptyset\right)=0$,
...
von Neumann really think about this when he use projectors to formulate
quantum theory?}
...
also used to decide whether a state is entangled or not for multipartite
...
\end{equation}
which is much more then one, the number of QPMFF\@. Since we don't
...
$\muF_{\Phi''}\left(P_{0}\right)=\muF_{U''\ket{\Phi'}}\left(U''P_{0}U''\right)=\muF_{\Phi'}\left(P_{0}\right)=\tfrac{2}{3}$
...
probabilities can actually be summarized as a probability measure
$\mu:2^{\Omega}\rightarrow\left[0,1\right]$ satisfying 
\begin{align}
...
equation? And its relation with Grover's algorithm? }\todo{Notice
...
similar to the situation that a manifold is glued by many local coordinates.
...
is \emph{exactly} the same and equal to $1$, Gleason's theorem assert
