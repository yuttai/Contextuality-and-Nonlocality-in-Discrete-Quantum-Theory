...
To have the current content of the dissertation, I would also like
to thank John Gardiner for some inspiring discussion about quantum
...
service, recommended IU libraries to purchase them.
...
I want to thank following people which haven't been listed previously:\footnote{I only provide the Chinese name for some of them for three reasons.
First, I might only know their Chinese names. Second, there is no
unique way to translate a Chinese name to English, and I am not familiar
with any of them. Third, all these methods translate infinitely different
Chinese names to the same English name so we loss some precision or
information if we only record their English name.}
...
\item My other friends have office in Lindley or \textportuges{Luddy}
...
Chandio}, Yuxiang Jiang (蒋宇翔), Pei-Ying Chen (陳珮瑩), Prof.~Qin Zhang
(张勤), Kaitlyn Owens, Praveen Narayanan, Liang Chen (陈亮), \textfrench{Ambrose
Bonnaire-Sergeant}, Ali Varamesh, Larry ``LP'' \textfrench{Ailes},
and Min-Chin Lin (林旻瑾), \ldots , etc.
...
(陳孟瑋), Yu ``Larry'' Chen (陈煜), Kenshin Reita (禮田謙信), Jing-Han Liou
(劉經翰), Shu-Ling Wang (王舒齡), and Wan-Ling ``Wynnie'' Chang (張婉鈴).
...
\item My friends lived in the Tulip Tree Apartments, including Chao-Hong
Chen (陳昭宏), Tsaiyi Wu (吳采奕), Cheng ``Freddie'' Shi (史程), Ossama
Abdel~Gawwad, and Cathryn ``Cathy'' Creger-Chambers, \textbahasai{Sary
Silvhiany}, Shaozhuan Li (李绍颛), and Chenwei Zhang (张晨薇), Shu-Nan ``Nancy''
...
\item Other friends, including Yu-Jung Lin (林宇容), Winnie Lou (何慧芳),
Yi-Rong Yang (楊宜蓉), Yi-Chu Chang (張逸竹), Jim \textgerman[variant=german,spelling=new,babelshorthands=true]{Zimmerly},
Davis Chen (陳鵬舟), Zihang Shao (邵子航), Prof.~Ying Ding (丁颖), Hung-Chun
Chao (趙竑鈞), Guang Zuo (左光), Prof.~Jung-Chao Ban (班榮超), Prof.~Jyh-Chyi
Gong (龔治齊), and 江靜宜, \ldots , etc.

\end{itemize}

\noindent This list could be on and on and never ends. If you have
help me in any form or provide any helpful information during my Ph.D.
study, but I forget to list you or your unit here. Please forgive
my bad memory.
