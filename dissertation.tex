...
complex numbers \cites{BSS1989}{Ziegler2007}{blum2012complexity}.
...
and its power set $2^{\Omega}$ as the classical event space, a classical
probability measure~$\mu$ maps an event~$E$ to a number $\mu\left(E\right)\in\left[0,1\right]$
specifying how likely one of the outcome~$\omega\in E$ actually
happens. To maintain the coherence, we want $\mu$ subject to the
following constraints: $\mu\left(\emptyset\right)=0$, $\mu\left(\Omega\right)=1$,
$\mu\left(\overline{E}\right)=1-\mu\left(E\right)$, and $\mu\left(E_{0}\cup E_{1}\right)=\mu\left(E_{0}\right)+\mu\left(E_{1}\right)$
...
space denoted by $\events$. After the measurement, if the system
transits to a state $\ket{\Phi}$ and we call an event $P\in\events$
happen when .
...
probability measure $\muB_{\rho}$ defined as \cites{Born1983}{Mermin2007}{Jaeger2007}
\begin{equation}
...
\begin{thm}[Shapley \cites{Shapley1971}{GilboaSchmeidler1994}{NgMoYeh1997Chinese}{Grabisch2016}]\label{thm:Shapley-classical}Every
...
exact probability calculated by the Born rule \cites{Born1983}{Mermin2007}{Jaeger2007},
i.e., 
\begin{equation}
