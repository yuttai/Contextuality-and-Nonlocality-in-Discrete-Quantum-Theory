...
No matter how, classical or quantum, to attack these two tasks, they
...
if the quantities describing molecules are considered as continuous,
we want to argue that it is beyond our ability to decide these quantities,
including the coordinate of the wave functions, are ultimately discrete
or continuous. To better understand this issue without struggling
with their meaning in the quantum theory, it is instructive to review
...
assumed to be continuous. Later, these continuous quantities become
...
previous paragraph may not simply represented by real numbers. Chuang-tzu's
stick are made by molecules and cannot be cut in halves endlessly.
The input charge of an integrator must actually be an integer multiple
of the elementary charge. A prefect square cannot be draw physically.
Even we don't draw it, but merely asking whether it exists in the
space is still beyond our ability to answer because whether the space
itself is ultimately discrete or continuous is still an unsolved question.
Even if it exists, it cannot be distinguished by human from an almost
prefect square differing only by an arbitrarily small extent. Similar
situation applying to the length of a slide rule, it is beyond our
ability to decide whether its length is ultimately discrete or continuous.
Even if it is continuous, identifying which real number represents
...
computation. The opposite of the last condition is sometimes assumed
...
and their mathematical models. \todo{Add the quantum case, and center
entries vertically.}}
...
Not reliably, but could be represented by a faithful discrete model when the minimum unit could be manipulated with a better technology. & 
