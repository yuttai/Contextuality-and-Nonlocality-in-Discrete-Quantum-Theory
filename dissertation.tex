...
these quantities might later be discovered to be discrete. Even if
they were not discrete, this fact may never be proved and the exact
value of these quantities may never be pinned down. To better understand
this issue without struggling with the quantum theory, it is instructive
to review how classical quantities are characterized and computed.
...
Hence, even if the inputs of an analog computer and a BCSS machine
are close to each other, their computational results might be completely
different, like the butterfly effect, and may not be compensated by
error analysis techniques. Therefore, the computational power of analog
computers is far weaker than the prediction based on this kind of
theoretical models.
...
we first review the ``conventional quantum theory'' in Chapter~\ref{chap:Conventional-Quantum-Theory}\footnote{Alternative terminology in the literature includes ``actual,'' ``standard,''
...
we replace the field of complex numbers by discrete finite fields
and consider the quantum theories and computing over finite fields.
Another possibilities could be that the probability amplitudes are
never quantized no matter how hard we try, and we could never access
their exactly values. Notice that the probability amplitudes can never
be accessed directly in an experiment, the accessible properties of
a quantum system is its measurement results and the probability of
an event. Hence, to incorporate the finite precision measurement into
the quantum theory, we consider a framework called quantum interval-valued
