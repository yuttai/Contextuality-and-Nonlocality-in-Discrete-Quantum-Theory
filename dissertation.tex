...
coherence, we want $\mu$ subject to the following constraints: $\mu\left(\emptyset\right)=0$,
...
Since the previous abstraction disregards the process generating the
outcomes, this process might be a quantum experiment. Let us prepare
a beam of one kind of spin~$1$ particles. In principle, we can preform
a Stern-Gerlach type experiment to splits the beam according to the
eigenvalues of the observable $\mathbf{O}_{0}=0\,\proj{0}+1\,\proj{1}+2\,\proj{2}$
\cite{:/content/aip/journal/jmp/21/1/10.1063/1.524312,peres1995quantum}.
If we only send one particle instead of a beam, the state after the
experiment could be $\ket{0}$, $\ket{1}$, or $\ket{2}$. In another
word, the outcome of this process is $\Omega_{0}=\left\{ \ket{0},\ket{1},\ket{2}\right\} $,
and all possible events are $\emptyset$, $\left\{ \ket{0}\right\} $,
$\left\{ \ket{1}\right\} $, $\left\{ \ket{2}\right\} $, $\left\{ \ket{0},\ket{1}\right\} $,
$\left\{ \ket{0},\ket{2}\right\} $, $\left\{ \ket{1},\ket{2}\right\} $,
and $\Omega_{0}$. Sending any particular particle specifies the likelihood
of each event $\mu_{0}\left(E\right)$, and the function $\mu_{0}:2^{\Omega}\rightarrow\left[0,1\right]$
...
similar to the previous one. This experiment send exactly the same
particle, but we use a beam splitter corresponding to the observable
...
this process is $\Omega_{1}=\left\{ \ket{\ps},\ket{\ms},\ket{2}\right\} $,
and all possible events are $\emptyset$, $\left\{ \ket{\ps}\right\} $,
$\left\{ \ket{\ms}\right\} $, $\left\{ \ket{2}\right\} $, $\left\{ \ket{\ps},\ket{\ms}\right\} $,
$\left\{ \ket{\ps},\ket{2}\right\} $, $\left\{ \ket{\ms},\ket{2}\right\} $,
and $\Omega_{1}$. When we formulate the likelihood of each event
as another probability measure $\mu_{1}:2^{\Omega}\rightarrow\left[0,1\right]$,
the likelihood of the same event of these two experiments are experimentally
found to be the same, i.e., $\mu_{0}\left(\emptyset\right)=\mu_{1}\left(\emptyset\right)$
...
as all possible events using different beam splitters. Since we are
focus on the probability instead of outcome, and the probabilities
of events $\left\{ \ket{0},\ket{1}\right\} $ and $\left\{ \ket{\ps},\ket{\ms}\right\} $
