...
where $\overline{E}$ is the complement of $E$.  Moreover, $\mu\left(E_{0}\cup E_{1}\right)=\mu\left(E_{0}\right)+\mu\left(E_{1}\right)$
...
Now the sample space is $\Omega_{1}=\left\{ \ket{\ps},\ket{\ms},\ket{2}\right\} $,
and another probability measure $\mu_{1}:2^{\Omega_{1}}\rightarrow\left[0,1\right]$
is defined. Although the sample spaces and the probability measures
are different, these two experiments may produce the same outcome
state. Moreover, the probability of the same event in different experiments
are experimentally found to be the same, i.e., $\mu_{0}\left(\emptyset\right)=\mu_{1}\left(\emptyset\right)$
...
of the same event in different experiments are always the same. This
fact is equivalent to the fact that commuting observables could be
measured simultaneous, and the latter fact is essential to define
contextuality which will be explained later.

Since the probability induced by the different beam splitters are
correlated, it is more natural to define the set of quantum events~$\events$
as the union of all possible event spaces using different beam splitters
on a given Hilbert space. Because we are focus on the probability
instead of outcome, and the probabilities of events $\left\{ \ket{0},\ket{1}\right\} $
and $\left\{ \ket{\ps},\ket{\ms}\right\} $ are always the same, it
is reasonable to identify them as the same event. By observing the
projectors generated by these two events are the same
\begin{equation}
\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}\,,
\end{equation}
we define the projector generated by any classical event~$E$ as
its quantum event 
\begin{equation}
\varphi\left(E\right)=\sum_{\ket{j}\in E}\proj{j}\in\events\,.\label{eq:pullback-function}
\end{equation}
If we follow the convention $\varphi\left(\emptyset\right)=\mathbb{0}$,
not only the same projector must have the same probability, but this
function $\varphi$ also naturally sends the set structure to the
corresponding projector structure: $\varphi\left(\Omega\right)=\mathbb{1}$
...
Moreover, $E_{0}$ and $E_{1}$ also satisfy the following properties:
\begin{align}
...
After we define the set of all events~$\events$ as the set of all
projectors on a given Hilbert space, a quantum probability space can
be defined by $\events$ together with a probability measure $\mu:\events\rightarrow\left[0,1\right]$
subject to the following constraints \cite{10.2307/2308516,gleason1957,Redhead1987-REDINA,Maassen2010,Abramsky2012}:
\begin{align}
