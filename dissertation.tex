...
a Stern-Gerlach type experiment. For example, a variant of quantum
probability measure has only one instance defined in Eq.~\ref{eq:QPMFF37}.
This instance predicts all outcomes to be equally probable which intuitively
corresponds to the mixed state $\frac{1}{3}\proj{0}+\frac{1}{3}\proj{1}+\frac{1}{3}\proj{2}$
in its space. However, ignoring the input state and always predicting
equally probable is not a sensible quantum theory.

Let $\ket{\Phi}$ characterize the state of particle sending to a
Stern-Gerlach type experiment, and $\muB_{\Phi}$ be the corresponding
quantum probability measure of the resulting events. 

To find a sensible correspondences between quantum probability measures
and the input states, 

\noindent , when sending a particle to a Stern-Gerlach type experiment,
we will get a quantum probability measure of the form $\Tr\left(\rho P\right)$
for some operator~$\rho$.

Recall in Sec.~\ref{CQC1qubitBloch.sec}, the state of the particle
can be characterized by a pure state $\ket{\Psi}\in\Hilb$ or more
generally a mixed state~$\rho'$. Although the function $\Tr\left(\rho P\right)$
is parametrized by 
