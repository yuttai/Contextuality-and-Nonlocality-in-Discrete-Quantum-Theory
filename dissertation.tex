...
state~$\ket{2}$, and the probability of the common event $\left\{ \ket{2}\right\} $
...
measured simultaneously, and the latter fact is essential to define
...
containing all possible event spaces using different beam splitters.
However, a simple union of all event spaces will contain duplicate
information. For example, if we take the complement on the both sides
of $\mu_{0}\left(\left\{ \ket{2}\right\} \right)=\mu_{1}\left(\left\{ \ket{2}\right\} \right)$,
we will have
\begin{equation}
...
and $\left\{ \ket{\ps},\ket{\ms}\right\} $ are always the same as
well. To simplify our discussion, it might be desirable to identify
two classical events $\left\{ \ket{0},\ket{1}\right\} $ and $\left\{ \ket{\ps},\ket{\ms}\right\} $
as one quantum event. Fortunately, the projectors generated by these
two events are the same, i.e., $\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}$.
Thus, given a classical event~$E$, we may define its corresponding
quantum event $\varphi\left(E\right)$ as the projector generated
by $E$,
\begin{equation}
\varphi\left(E\right)=\sum_{\ket{j}\in E}\proj{j}\label{eq:pullback-function}
\end{equation}
with the convention $\varphi\left(\emptyset\right)=\mathbb{0}$. Then,
a quantum event space $\events$is the set of all projectors on a
given Hilbert space. 

It is easy to check that defining quantum 

we not only want to consider all possible events with different beam
splitters as a whole, but also want to 

Because the projectors generated by these two events are the same,
i.e., $\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}$, 

if we want to consider all possible events with different beam splitters
as a whole, it might be easier to define a quantum 

we could simplify our discussion by mapping a classical event~$E$

To simplify our discussion on quantum probability, it would be better
to identify $\left\{ \ket{0},\ket{1}\right\} $ and $\left\{ \ket{\ps},\ket{\ms}\right\} $
as the same event. Fortunately, the projectors generated by these
two events are the same, i.e., $\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}$.
So does 

Because we are focus on the probability instead of outcome, 

Since the probability induced by the different beam splitters are
correlated, it is more natural to define the set of quantum events
as the union of all possible event spaces using different beam splitters.
Because we are focus on the probability instead of outcome, and ,
it is reasonable to identify them as the same event. By observing
the projectors generated by these two events are the same, i.e., $\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}$,
we define the projector generated by any classical event~$E$ as
its quantum event 

It is easy to verify that , not only the events mapping to the same
projector must have the same probability, but this function $\varphi$
also naturally sends the set structure to the corresponding projector
