...
position wave functions or the probability amplitudes of a wave function.
Even if these quantities look like continuous, we want to argue that
either these quantities might later be discovered to be discrete,
or the exact value of these continuous quantities may never be pinned
down. To better understand this issue without struggling with the
quantum theory, it is instructive to review how classical quantities
are characterized and computed.
...
the branch chosen by an analog computer. Since two branches of computations
may not have any relations, the later computational path may be completely
different like the butterfly effect, and this kind of difference cannot
be compensated by error analysis techniques easily. Therefore, the
computational power of analog computers might be far weaker than the
prediction based on this kind of theoretical models.

\begin{sidewaystable}
\caption{\label{tab:Comparison}Comparison between the classical quantities
and their mathematical models with the inspired quantum models. \todo{Center
entries vertically.}}
...
Not reliably, because precision can never be high enough, and the difference between their computational power might not be compensated by error analysis techniques. \tabularnewline\addlinespace
\addlinespace
\midrule
\addlinespace
...
theories and computing over finite fields. Another possibilities could
be that the probability amplitudes are never quantized no matter how
hard we try, and we could never access their exactly values. Notice
that the probability amplitudes can never be accessed directly in
an experiment, the accessible properties of a quantum system is its
measurement results and the probability of an event. Hence, to incorporate
the finite precision measurement into the quantum theory, we consider
a framework called quantum interval-valued probability measures (QIVPM).
...
fields, we first doesn't restrict our quantum theory and computing
over which finite fields in Chapter~\ref{chap:Unrestricted-Finite-Fields}
\cite{Schumacher2012-SCHMQT,DQT2014,SchumacherWestmoreland2010}.
This theory is called modal quantum theory because it is so weak that
it only predict whether the measure result is possible or impossible,
and even the expectation value of an observable is not able to be
defined. Although a version of quantum theory defined over a two-valued
field can express simple algorithms such as quantum teleportation
\cite{BennettBrassardEtAl1993,peres1995quantum,Mermin2007,Jaeger2007},
it is so weak that it cannot express Deutsch's algorithm \cite{Deutsch1985,Mermin2007}.
This quantum theory is, however, also so powerful that it can be used
...
\todo{Introduction Cleaned up until here. }Next, in Sec.~\ref{discretequantumtheoryI}
to \ref{sec:Geometry-of-Entangled-States}, we improve on this by
showing that for finite fields of order $p^{2}$, with the prime $p$
of the form $4\ell+3$ ($\ell$ a non-negative integer), the complex
numbers have extremely compelling and natural discrete analogs that
permit a great many of the standard requirements of quantum computing
...
algorithms of Deutsch, Simon, and Bernstein-Vazirani, though still
with some bothersome shortcomings. Because of the modular nature of
arithmetic in the finite complex field, it is not possible to define
an inner product in the usual sense, and we show in Sec.~\ref{subsec:Partial-Algorithm}
...
\midrule
\addlinespace
Expectation value & Defined & Undefined & Undefined & Undefined &  & Defined\tabularnewline\addlinespace
\addlinespace
\midrule
\midrule 
\addlinespace
