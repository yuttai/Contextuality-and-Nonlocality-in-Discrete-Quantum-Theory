...
where the maximum length of intervals in $\mathscr{I}$ is an important
. In this case, the states consistent with the composition $f\circ\muB_{\rho}$
is bounded by . While the QIVPM defined in Table~\ref{tab:non-Born-QIVPM}
is not in this class, it is too wild to have a non-empty core, and
the maximum length of its intervals $\mathscr{I}_{0}$ is $1$, the
longest length of all possible intervals. This suggests us to ask
if we shirk the maximum length of intervals alone, whether this ill-behaved
QIVPMs disappears. More precisely, we have the following question.
