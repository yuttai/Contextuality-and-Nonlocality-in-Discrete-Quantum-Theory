...
should be identified as the same quantum event to simplify our discussion.
Since the projectors generated by these two events are equal as operators,
i.e., $\proj{0}+\proj{1}=\proj{\ps}+\proj{\ms}$, given any classical
event~$E$, we can follow the same idea to define its quantum event
as the projector generated by $E$
\begin{equation}
\varphi\left(E\right)=\sum_{\ket{j}\in E}\proj{j}\label{eq:pullback-function}
\end{equation}
with the convention $\varphi\left(\emptyset\right)=\mathbb{0}$. This
function $\varphi$ well glues all classical event spaces into one
quantum event space~$\events$, the set of all projectors on a given
Hilbert space, because it is easy to inductively prove that if two
classical events~$E$ and $E'$ corresponding to the same quantum
event, i.e., $\varphi\left(E\right)=\varphi\left(E'\right)$, then
the probability of $E$ is the same as the probability of $E'$.

Moreover, this function $\varphi$ also naturally sends the set structure
to the corresponding projector structure: $\varphi\left(\Omega\right)=\mathbb{1}$
