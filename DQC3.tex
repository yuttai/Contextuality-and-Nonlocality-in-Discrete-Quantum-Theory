...
\usepackage{mathrsfs}
\usepackage{fullpage}
% \documentclass[english,prl,aps,showpacs,preprint,floatfix,superscriptaddress]{revtex4-1} 
...
\author{John Gardiner\\
 \affiliation{College of Physical and Mathematical Sciences, Brigham
  Young University, Provo, UT 84602, U.S.A}\\[.2in]
%% \author
Andrew J. Hanson\\
\affiliation{School of Informatics and Computing, Indiana University,
  Bloomington, IN 47405, U.S.A}\\[.2in]
%%\author
Gerardo Ortiz \\
...
$x^2+1$ has no root in the field, and we can adjoin $\ComplexI$ as a
...
\[
\mathbf{H}_{1}=\begin{pmatrix}-1 & -\ComplexI & 0\\
\ComplexI & 1 & -\ComplexI\\
0 & \ComplexI & 1
\end{pmatrix}
\]
...
The dimension of an eigenspace may be less than the multiplicity of
its eigenvalue. For example, consider a Hermitian matrix 
\[
\mathbf{H}_{2}=\begin{pmatrix}-1 & -1-\ComplexI\\
-1+\ComplexI & 0
\end{pmatrix}
\]
Over $\ff{9}$, its only eigenvalue is $1$ of multiplicity $2$.
The only eigenspace of $\mathbf{H}_{2}$ is spanned by $\begin{pmatrix}1\\
-1+\ComplexI
\end{pmatrix}$. The dimension of eigenspace is 1 less than 2 so that $\mathbf{H}_{2}$
...
$1$ with the post-measurement state $\begin{pmatrix}1\\
-1+\ComplexI
\end{pmatrix}$. However, only operator always producing $1$ is the identity operator
...
\[
\mathbf{H}_{3}=\begin{pmatrix}-1 & -1-\ComplexI\\
-1+\ComplexI & 1-
\end{pmatrix}
\]
Over $\ff{9}$, the eigenvalues and eigenvectors of $\mathbf{H}_{3}$
are $-1-\ComplexI$ with $\begin{pmatrix}1\\
-1-\ComplexI
\end{pmatrix}$ and $-1+\ComplexI$ with $\begin{pmatrix}1\\
1+\ComplexI
\end{pmatrix}$. It is easy to verify the eigenvalues are not in $\ff{3}$, and the
...
then 
\begin{eqnarray*}
-1\pm\sqrt{2} & \equiv & -1\pm\ComplexI\left(\bmod p\right)
\end{eqnarray*}
...
preserved when we map them into finite fields.
\end{enumerate}
...
\begin{align*}
  1&=\dpt\big(\ket{\Psi},\ket{0}\big)+ \dpt\big(\ket{\Psi},\ket{1}\big) + 
     \dpt\big(\ket{\Psi},\ket{2}\big)+ \dpt \big(\ket{\Psi},\ket{3}\big)\\
...
\begin{proof}
  ...
  with entries in $\left\{ 0,\pm1,\pm1\pm\ComplexI\right\} $. Then the
  ...
  ${+} \equiv (1+\ComplexI)$, ${-} \equiv (1-\ComplexI)$ \}, and
\[
