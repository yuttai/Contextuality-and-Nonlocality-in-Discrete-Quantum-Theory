...
, and $\left(\mathscr{L},0,\uplus\right)$ is an abelian group. By
the fundamental theorem of abelian group~\cite{DummitFoote2004},
$\mathscr{L}$ must be isomorphic to $\mathbb{Z}_{d_{1}}\oplus\ldots\oplus\mathbb{Z}_{d_{k}}\oplus\mathbb{Z}^{b}$,
where $\mathbb{Z}_{d_{1}},\ldots,\mathbb{Z}_{d_{k}}$ are finite cyclic
subgroups, $\mathbb{Z}^{b}$ is the free abelian group with rank $b$,
and $\oplus$ is the direct sum of the abelian groups. Moreover, we
want $\left(\mathscr{L},0,\uplus\right)$ to have a sense of probability. 
\begin{problem}
