...
\begin{eqnarray}
{\cal P}_{\rho}\left(\ket{i}\right)={\cal P}_{\rho}\left(\lambda_{i}\right) & = & \sum_{j}p_{j}{\cal P}_{\Psi_{j}}\left(\ket{i}\right)\label{prob1.eq-2}
\end{eqnarray}
...
is a probability, we can identify ${\cal P}$ to a mixed state $\rho$
by Gleason's theorem.
\begin{thm}
(Gleason's Theorem) 

For $d\ge3$, for any ${\cal P}:\C^{d*}\rightarrow\left[0,1\right]$,
if ${\cal P}$ is a state-function and probability, i.e.,
\begin{eqnarray}
 &  & \textrm{For any vector }\ket{i}\in\C^{d*}\textrm{, and }\gamma\in\C^{*}\textrm{ , }{\cal P}(\ket{i})={\cal P}(\gamma\ket{i})\textrm{ ,}\label{eq:parallel-DQT(I)-1}\\
 &  & \textrm{For any orthonormal basis }\{\ket{i}\}\textrm{ , }{\displaystyle \sum_{i=0}^{d-1}{\cal P}\left(\ket{i}\right)=1}\textrm{ ,}\label{eq:total-probability-1}
\end{eqnarray}
there is a density matrix $\rho$ such that ${\cal P}={\cal P}_{\rho}$~\cite{Redhead1987-REDINA,gleason1957,peres1995quantum}.\footnote{In order to simplify the statement, we consider consider the probability
on the state vectors instead of the projectors or closed subspaces
as in the Gleason's original paper.}
\end{thm}
In contrast, when $d=2$, consider the discontinuous function ${\cal P}:\C^{2*}\rightarrow\left[0,1\right]$
by 
\[
{\cal P}
\left(\begin{array}{c} \alpha_{0}\\ \alpha_{1} \end{array}\right)=
\cases{ 
1 & , if $\alpha_{0}\alpha_{1}>0$\\
\frac{1}{2} & , if $\alpha_{0}\alpha_{1}=0$\\
0 & , if $\alpha_{0}\alpha_{1}<0$ } 
\]
${\cal P}$ satisfies equation (\ref{eq:parallel-DQT(I)-1}) and (\ref{eq:total-probability-1})
because no matter the sign of $\alpha_{0}\alpha_{1}$, we always have
\[
...
is that it implies the Born rule (\ref{prob1.eq-6})~\cite{peres1995quantum,inun.606849520040101}.
...
way interpreted as a probability. Hence, everyone who tried to build
a quantum theory over finite fields struggled on how to define a reasonable
