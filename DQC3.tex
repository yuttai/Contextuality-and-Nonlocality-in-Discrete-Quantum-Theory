...
\yutsung{Does the sentences in this paragraph contradict to each
other? According to the postulates in page 206 and 207 in Peres~\cite{peres1995quantum},
this paragraph should read ``Quantum Physics is contextual, meaning
that if we predetermine definite outcomes for any measurement, then
the predetermined outcomes depend not only on the choice of the measured
observable(s) and on the system being measured, but also on the details
of the measurement process. To be more precise, as noted by Peres~\cite{peres1995quantum}
and Mermin~\cite{MerminPRL1990}, the result of measuring an observable
in a physical system must not depend on whether we measure the observable
alone, or along with arbitrary sets of mutually commuting observables.
However, if we try to predetermine definite measurement outcomes for
all observables on a system, the predetermined outcome of an observable
$\mathbf{O}$ will be changed by measuring other observables commuting
to $\mathbf{O}$.''}

\yutsung{Personal note about the reason why the quantum physics is
contextuality, and a bit of literature review. Don't know whether
or where to put them in our paper through.

In order to prove by contradiction, assume the quantum physics is
non-contextuality, i.e., 
\begin{enumerate}
\item \label{enu:non-contextuality}Given an observable $\mathbf{O}$, we
can predetermine a definite measurement outcome $v\left(\mathbf{O}\right)$
not depending on the choice of compatible observables that may be
preformed together with it. (postulate I in page 207 in \cite{peres1995quantum}).
\end{enumerate}
This leads to the premise of the Kochen-Specker theorem
\begin{enumerate}[resume]
\item \label{enu:premise}We can also define $v$ on states by $v\left(\ket{n}\right)=v\left(\proj{n}\right)$.
If $\left\{ \ket{n}\right\} $ forms a orthonormal basis, then $\sum_{n}v\left(\ket{n}\right)=1$.
(page 196 in \cite{peres1995quantum}).
\end{enumerate}
The Kochen-Specker theorem says Premise (\ref{enu:premise}) is impossible
to be satisfied so that Postulate (\ref{enu:non-contextuality}) fails,
and the quantum theory is contextual . 

Because lack of commuting observables in their theory, Schumacher
and Westmoreland~\cite{2010arXiv1010.5452S} uses ``$v$ is well-defined
on states'' instead of Postulate (\ref{enu:non-contextuality}) as
the definition of non-contextuality. However, comparing to Postulate
(\ref{enu:non-contextuality}), it is less clear why ``$v$ is well-defined
on states'' should be a postulate or not.

There is no solid reason why people assume Postulate (\ref{enu:non-contextuality})
as a postulate. Peres gives a ``psychological reason'' that Postulate
(\ref{enu:non-contextuality}) is true when the measurement outcome
is predictable. Another ``psychological reason'' might be: because
Postulate (\ref{enu:non-contextuality}) is true when we replace the
``predetermined definite outcome'' by the usual ``measurement result''
in CQT. However, in DQT, because commuting observables cannot be measured
simultaneously, it may not be surprising that Postulate (\ref{enu:non-contextuality})
is false.}
