...
\begin{enumerate}
\item \label{enu:non-contextuality}Given an observable $\mathbf{O}$, the
predetermined definite outcome $v\left(\mathbf{O}\right)$ does not
depend on the choice of compatible observables that may be preformed
together with it. (postulate I in page 207 in \cite{peres1995quantum}).
\end{enumerate}
This leads to the premise of the Kochen-Specker theorem
\begin{enumerate}[resume]
\item \label{enu:premise}For a set of commuting projection operator $\proj{n}$,
if $\left\{ \ket{n}\right\} $ forms a basis, then $\sum_{n}v\left(\proj{n}\right)=1$.
...
Schumacher and Westmoreland~\cite{2010arXiv1010.5452S} uses Premise
(\ref{enu:premise}) instead of Postulate (\ref{enu:non-contextuality})
as the definition of non-contextuality because lack of commuting observables
in their theory. Our paper can do better and use Assumption (\ref{enu:non-contextuality})
as non-contextuality. 
