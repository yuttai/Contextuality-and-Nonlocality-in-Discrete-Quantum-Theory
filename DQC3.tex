...
\begin{equation}
\textrm{For any orthonormal basis }\{\ket{i}\}\textrm{ and any state }\ket{\Psi}\textrm{ , }{\displaystyle \sum_{i=0}^{d-1}{\cal P}_{\rho}\left(\ket{i}\right)=1}\label{eq:total-probability}
\end{equation}


Conversely, if an assignment ${\cal P}:\C^{d*}\rightarrow\left[0,1\right]$
is a probability, i.e., satisfying equation (\ref{eq:total-probability}),
we can identify ${\cal P}$ to a mixed state $\rho$ by Gleason's
theorem.
\begin{thm}
(Gleason's Theorem) 
\begin{enumerate}
\item For $d\ge3$, for any probability ${\cal P}:\C^{d*}\rightarrow\left[0,1\right]$,
there is a density matrix $\rho$ such that ${\cal P}={\cal P}_{\rho}$~\cite{Redhead1987-REDINA,gleason1957}.\footnote{In order to simplify the statement, we consider consider the probability
...
\end{enumerate}
\end{thm}
Let's move on to Gleason's Theorem in DQT. Can we recover Gleason's
Theorem in DQT? or more precisely: 
\begin{problem}
Given an assignment $\hat{\pi}:\ffzd{p^{2}}\rightarrow\mathscr{L}_{2}$
in DQT, if $\hat{\pi}$ is a probability, i.e., 
\begin{equation}
...
about $\pi_{2}$ and $\pi_{3}$ because the semigroup$\left(\mathscr{L}_{2},\text{impossible},\vee\right)$
is not cancellative, i.e., $a\vee c=b\vee c$ does not imply $a=b$.
This suggests to study the range of the probability. Consider the
range has the structure $\left(\underline{\mathscr{L}},\mathscr{L},0,1,\oplus,<\right)$,
where $\left(\underline{\mathscr{L}},0,\oplus,<\right)$ is the underlying
commutative ordered semigroup~\cite{clifford1958}, $\mathscr{L}\subseteq\underline{\mathscr{L}}$
is the allowed range value, $1\in\mathscr{L}'\subseteq\mathscr{L}$
is another special value, and for all $x\in\mathscr{L}$, we have
$0<x<1$. For example, $\left(\mathscr{L}_{2},\mathscr{L}_{2},\text{impossible},\text{possible},\vee,<\right)$
and $\left(\mathbb{R},\left[0,1\right],0,1,+,<\right)$ are two possible
range of the probability, but we cannot pick $\underline{\mathscr{L}}=\ff{p^{2}}$
because we cannot define an inequality such that $\left(\ff{p^{2}},0,+,<\right)$
...
but continuity doesn't even make sense in DQT. This fact implies that
the structure of the domain of the probability, $\ffzd{p^{2}}$, is
also too weak. In order to think which structure we need, let's check
which the reasonable property ${\cal P}_{\Psi}:\C^{d*}\rightarrow\left[0,1\right]$
...
{\cal P}_{\Psi}\left(\ket{i}\right) & = & {\cal P}_{\Psi}\left(\gamma\ket{i}\right)\textrm{ ,}
\end{eqnarray}
...
\end{equation}
for any unitary matrix $U$. Because we consider ${\cal P}_{\Psi}$
and ${\cal P}_{U\Psi}$ together, the first part of Gleason's theorem
doesn't really make sense, but we can still ask whether there is an
unique ``Born rule'' satisfying all the conditions. If we add these
three conditions, our problem becomes
\begin{problem}
\label{prob:Gleason's}Is there a structure $\left(\underline{\mathscr{L}},\mathscr{L},0,1,\oplus,<\right)$
such that $\tilde{\pi}:\ffzd{p^{2}}\times\ffzd{p^{2}}\rightarrow\mathscr{L}$
...
\begin{eqnarray}
 ...
 &  & \textrm{ \ensuremath{{\displaystyle \bigoplus_{i=0}^{d-1}\tilde{\pi}\left(\ket{\Psi},\ket{i}\right)=1}} .}\nonumber \\
 & \bullet & \textrm{If }\braket{\Psi}{i}=0\textrm{ ,}\nonumber \\
 &  & \textrm{then }\tilde{\pi}\left(\ket{\Psi},\ket{i}\right)=0\textrm{ .}\label{enu:particular-vector-DQT(I)}\\
 ...
 &  & \textrm{i.e. , }U^{\dagger}U=I\textrm{, then }\tilde{\pi}(\ket{\Psi},\ket{i})=\tilde{\pi}(U\ket{\Psi},U\ket{i})\textrm{. }\nonumber \\
 & \bullet & \textrm{For any vector }\ket{\Psi},\ket{i}\in\ffzd{p^{2}}\textrm{, and }\gamma\in\ff{p^{2}}^{*}\textrm{, }\nonumber \\
 &  & \dpr(\ket{\Psi},\ket{\Phi})=\dpr(\gamma\ket{\Psi},\ket{\Phi})=\dpr(\ket{\Psi},\gamma\ket{\Phi})\textrm{. }\label{enu:parallel-DQT(I)}
\end{eqnarray}

\end{problem}
The no-go theorem in section \ref{sub:A-No-Go-Theorem} said the if
$\left(\underline{\mathscr{L}},\mathscr{L},0,1,\oplus,<\right)=\left(\mathbb{R},\left[0,1\right],0,1,+,<\right)$,
there is no $\tilde{\pi}:\ffzd{p^{2}}\times\ffzd{p^{2}}\rightarrow\mathscr{L}$
for $d\ge3$. It is easy to extend the no-go theorem and prove 
\begin{thm}
If $\left(\underline{\mathscr{L}},0,\oplus,<\right)$ is a ordered
group, then there is no $\tilde{\pi}:\ffzd{p^{2}}\times\ffzd{p^{2}}\rightarrow\mathscr{L}$
satisfying all conditions in problem \ref{prob:Gleason's} for $d\ge3$.
\end{thm}
When we consider $\left(\mathscr{L}_{2},\mathscr{L}_{2},\text{impossible},\text{possible},\vee,<\right)$,
