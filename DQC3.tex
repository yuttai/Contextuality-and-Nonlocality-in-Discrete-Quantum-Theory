...
same state if we normalize it or multiply it by a global phase. Second,
${\cal P}_{\rho}$ is a probability, and the total probability should
be one. 

Conversely, any ${\cal P}$ with these two properties can be identified
with a mixed state $\rho$ by Gleason's theorem~\cite{Redhead1987-REDINA,gleason1957,peres1995quantum}\footnote{In order to simplify the statement, we consider the probability on
...
invariant under scalar multiplication and is a probability, i.e.,
\begin{eqnarray}
 &  & {\cal P}(\ket{i})={\cal P}(\gamma\ket{i})\textrm{ for any vector }\ket{i}\in\C^{d*}\textrm{, and }\gamma\in\C^{*}=\C\backslash\left\{ 0\right\} \textrm{ ,}\label{eq:parallel-DQT(I)-1}\\
...
share the vector $\ket{0}$. In contrast, when $d=2$, two orthonormal
bases are either identical or completely different after we use equation
(\ref{eq:parallel-DQT(I)-1}) to identify ${\cal P}\left(\ket{i}\right)$
and ${\cal P}(\gamma\ket{i})$. Therefore, when we solve the system
of linear equation (\ref{eq:total-probability-1}), we actually solve
them individually and may get a discontinuous solution. For example,
consider 
\[
{\cal P}
\left(\begin{array}{c} \cos\theta\\ \sin\theta\rme^{\rmi\gamma} \end{array}\right)=
\cases{ 
...
\sin\left(\theta+\frac{\pi}{2}\right)\rme^{\rmi\gamma}
\end{array}\right)=1\textrm{ .}
\end{eqnarray*}
However, ${\cal P}_{\rho}$ is always continuous. Therefore, no density
matrix $\rho$ satisfies ${\cal P}={\cal P}_{\rho}$. 

Although the situation is different between $d=2$ and $d\ge3$ in
CQT, a probability in DQT cannot be identified with a state despite
the dimension of the system. 
\begin{thm}
\label{thm:GleasonDQT}For any $d\ge2$, there exists $\hat{\pi}:\ffzd{p^{2}}\rightarrow\mathscr{L}_{2}$
which is invariant under scalar multiplication and is a probability,
i.e., 
\begin{eqnarray}
 &  & \hat{\pi}(\ket{i})=\hat{\pi}(\gamma\ket{i})\textrm{ for any vector }\ket{i}\in\ffzd{p^{2}}\textrm{, and }\gamma\in\ff{p^{2}}^{*}\textrm{ ,}\label{eq:parallel-DQT}\\
 &  & {\displaystyle \bigvee_{i=0}^{d-1}\hat{\pi}\left(\ket{i}\right)=}\textrm{possible for any orthonormal basis }\{\ket{i}\}\textrm{ , }\label{eq:total-probability-DQT}
\end{eqnarray}
such that $\dpr_{\Psi}\ne\hat{\pi}$ for any $\ket{\Psi}\in\ffzd{p^{2}}$,
where $\dpr_{\Psi}\left(\ket{i}\right)=\dpr\left(\ket{\Psi},\ket{i}\right)$
is the discrete Born rule (\ref{eq:modality}).\end{thm}
\begin{proof}
...
\]
$\hat{\pi}$ satisfies equation (\ref{eq:parallel-DQT}) because we
have 
\begin{eqnarray*}
...
\end{eqnarray*}
for any $\gamma\in\ff{p^{2}}^{*}$. For equation (\ref{eq:total-probability-DQT}),
because $\textrm{impossible}\lor\textrm{possible}=\textrm{possible}$,
it is sufficient to prove the following claim.
\begin{description}
\item [{Claim}] For any $\ket{\Phi}\in\ffzd{p^{2}}$ such that $\hat{\pi}\left(\ket{\Phi}\right)=\textrm{impossible}$,
there is a $\ket{\Psi}\in\ffzd{p^{2}}$ such that $\ip{\Phi}{\Psi}=0$
and $\hat{\pi}\left(\ket{\Psi}\right)=\textrm{possible}$.\end{description}
\begin{casenv}
\item When $\ip{\Phi}{0}=0$, $\ket{0}$ is what we want because $\ip{0}{0}=1\ne0$
and $\ip{0}{d-1}=0$ implies $\hat{\pi}\left(\ket{0}\right)=\textrm{possible}$.
...
$\QED$
\end{proof}
It is easy to identify the reasons why theorem \ref{thm:GleasonDQT}
is different from Gleason's theorem in CQT, but it is hard to find
the correct conditions to get the corresponding Gleason's theorem
in DQT. Given ${\cal P}$ in CQT, we search for a mixed state $\rho$
such that ${\cal P}={\cal P}_{\rho}$, but given $\hat{\pi}$ in DQT,
we only search for a pure state $\ket{\Psi}$ such that $\dpr_{\Psi}=\hat{\pi}$.
...
$\dpr_{\Psi}=\hat{\pi}$ in DQT. 

Lacking the notion of mixed state is not the only reason why theorem
\ref{thm:GleasonDQT} is hold. Even if we find a way to define a mixed
state, if $\rho$ is a mixed state between $\ket{\Psi_{1}}$ and $\ket{\Psi_{2}}$,
then we still have 
\[
\left|\dpr_{\rho}^{-1}\left(\left\{ \textrm{impossible}\right\} \right)\right|\le\left|\dpr_{\Psi_{1}}^{-1}\left(\left\{ \textrm{impossible}\right\} \right)\right|<\left|\hat{\pi}^{-1}\left(\left\{ \textrm{impossible}\right\} \right)\right|\textrm{ .}
\]
We can always pick $\hat{\pi}$ such that $\left|\hat{\pi}^{-1}\left(\left\{ \textrm{impossible}\right\} \right)\right|$
is larger because we have $x\lor\textrm{possible}=\textrm{possible}$
no matter whether $x$ is possible or impossible. More precisely,
compare the following arithmetic equation and Boolean equation.
\begin{eqnarray}
...
a commutative semigroup without cancellative, i.e., $a\vee c=b\vee c$
does not imply $a=b$. 

If the problem is that the range of $\hat{\pi}$ is only a commutative
semigroup, we may want to consider $\hat{\pi}:\ffzd{p^{2}}\rightarrow\mathscr{L}$
, and $\left(\mathscr{L},0,\oplus\right)$ is an abelian group. By
the fundamental theorem of finitely generated abelian group~\cite{DummitFoote2004},
$\mathscr{L}$ must be isomorphic to $\mathbb{Z}_{d_{1}}\times\ldots\times\mathbb{Z}_{d_{k}}\times\mathbb{Z}^{b}$,
where $\mathbb{Z}_{d_{1}},\ldots,\mathbb{Z}_{d_{k}}$ are finite cyclic
subgroups, $\mathbb{Z}^{b}$ is the free abelian group with rank $b$,
and $\times$ is the direct product of the abelian groups. However,
if $\mathscr{L}$ has any finite cyclic subgroups, we may have $\hat{\pi}\left(\ket{0}\right)\ne0$,
but $d_{1}\hat{\pi}\left(\ket{0}\right)=0$. This contradicts the
idea that $\hat{\pi}$ can be understood as a probability. Therefore,
we must have $\mathscr{L}\cong\mathbb{Z}^{b}$.
\begin{problem}
\label{thm:GleasonDQT-group}Is there a group $\left(\mathscr{L},0,\oplus\right)\cong\mathbb{Z}^{b}$
...
\item $\hat{\pi}:\ffzd{p^{2}}\rightarrow\mathscr{L}$ satisfies 
\begin{eqnarray}
 &  & \hat{\pi}(\ket{i})=\hat{\pi}(\gamma\ket{i})\textrm{ for any vector }\ket{i}\in\ffzd{p^{2}}\textrm{, and }\gamma\in\ff{p^{2}}^{*}\textrm{ , }\label{eq:parallel-DQT-1}\\
 &  & {\displaystyle \bigoplus_{i=0}^{d-1}\hat{\pi}\left(\ket{i}\right)=}1\textrm{ for any orthonormal basis }\{\ket{i}\}\textrm{ . }\label{eq:total-probability-DQT-1}
\end{eqnarray}
For $d\ge3$, there is a mixed state $\rho$ such that $\hat{\pi}=\tilde{\pi}_{\rho}$;
for $d=2$, there is a $\hat{\pi}$ corresponding to no mixed state
$\rho$.
\end{itemize}
\end{problem}
For $d=2$, we can easily construct a $\hat{\pi}$ satisfying equation
(\ref{eq:zero-count}) and corresponding to no mixed state $\rho$.
When $d=3$ and $p=7$, we have verified the only $\hat{\pi}$ is
$\hat{\pi}\left(\ket{i}\right)=\hat{\pi}\left(\ket{j}\right)$ and
$3\hat{\pi}\left(\ket{i}\right)=1$ for all $\ket{i},\ket{j}\in\ffzd{p^{2}}$.
Therefore, we don't have enough $\hat{\pi}$ to corresponding to even
pure states. We believe the same situation happens for other $d\ge3$
and $p\ge7$. 

We don't know whether $\mathscr{L}$ can be a non-Boolean commutative
semigroup, but not yet a group. In summary, there are some difficulties
to establish corresponding Gleason's theorem in DQT no matter how
