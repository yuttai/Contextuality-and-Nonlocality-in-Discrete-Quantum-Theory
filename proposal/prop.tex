...
presented in greater detail than the other proposed activities. As
argued in the previous sections, given fixed finite resources, it is
only possible to estimate the quantum probabilities within an interval
of confidence. It is therefore natural to propose the notion of an
``interval-valued quantum probability measure'' that combines the
...
given an interval-valued quantum probability measure, i.e., given
...
constructed the following convex interval-valued quantum probability
measure with an empty core.

\begin{example}[Three-dimensional three-interval-valued quantum
...
\begin{example}[Three-dimensional four-interval-valued quantum
...
\end{enumerate}
It is straightforward but tedious to check that $\bar{\mu}$ is an
interval-valued quantum probability measure. Although we have not
...
precision of the intervals increase the interval-valued quantum
...
\paragraph*{Research Question.} the above is mostly about gleason's 
theorem; foundational; related theorem richer are bell; and KS; 

\paragraph*{Research Question.} add discrete theory to the story; do
not assume anything is infinitely precise not even the hypothetical state

\paragraph*{Research Question.} revisit Mermin-Meyer etc with 
mathematical framework we have with and without discrete theory 


%%%%%
\subsection{Proposed Activity IV: Computational Aspects}

mention aaronson three statements paradox. something is wrong;
investigate; investigate complexity; p=np; mention usat and mention
deutsch-josza improvement depends on precision not as dramatic
