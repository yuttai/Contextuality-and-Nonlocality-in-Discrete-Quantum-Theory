...
presented in great detail. As argued in the previous sections, given
fixed finite resources, it is only possible to estimate the quantum
probabilities within an interval of confidence. It is therefore
natural to propose the notion of an ``interval-valued quantum
probability measure'' that combines the definitions of conventional
quantum probability measures with classical interval-probabilities.
...
convex: if certain intervals exist then combinations of these
intervals must also exist so that we can add probabilities etc. in the
classical case ... theorem but in the quantum case we found a counterexample
...
and the Dempster-Shafer plausibility as the
right-end~\cite{Dempster1967,Shafer1976,GilboaSchmeidler1994,Goodman1997,Marinacci1999,Teran2014}.
Dempster-Shafer functions, also called \emph{completely} or
\emph{totally monotone}, play an important role in the theory of
non-additive belief.  Furthermore, convex interval-valued probability
measures have a good property that they can always be associated with
real-valued probability measures.
