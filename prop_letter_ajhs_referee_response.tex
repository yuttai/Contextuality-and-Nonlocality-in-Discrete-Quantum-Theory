...
We begin by presenting two lemmas to make the presentation more
modular. 
...
\begin{lemma}
  ...
  $\mu : \events \rightarrow \mathscr{I}$, it is sufficient to verify
  that for all orthogonal projectors $P_{0}$ and~$P_{1}$, we have
\begin{equation}
...
\begin{equation}
\begin{aligned}
& \bar{\mu}\left(P+P'-PP'\right)+\bar{\mu}\left(PP'\right) & (\mbox{because } P = PP' + P - PP') \\
=& \bar{\mu}\left(PP'+P(\mathbb{1}-P')+P'-PP'\right)+\bar{\mu}\left(PP'\right) \\
=& \bar{\mu}\left(P(\mathbb{1}-P')+P'\right)+\bar{\mu}\left(PP'\right) & (\mbox{because } P' = PP' + P' - PP') \\
=& \bar{\mu}\left(P(\mathbb{1}-P')+PP'+(\mathbb{1}-P)P'\right)+\bar{\mu}\left(PP'\right) & (\mbox{using Eq.~(2) twice}) \\
=& \bar{\mu}(P(\mathbb{1}-P'))+\bar{\mu}(PP')+\bar{\mu}((\mathbb{1}-P)P')+
    \bar{\mu}\left(PP'\right) & (\mbox{using Eq.~(2) twice}) \\
=& \bar{\mu}(P(\mathbb{1}-P')+PP')+\bar{\mu}((\mathbb{1}-P)P'+PP') \\
=& \bar{\mu}(P) + \bar{\mu}(P')
\end{aligned}
\end{equation}
\end{widetext}
...
\noindent The proof is by contradiction: Suppose there is a $\delta$-deterministic 
QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$. By the previous
lemma, we can construct a 0-deterministic QIVPM; however, by Thm.~2 in the
...
did). We simply attribute lack of certainty to the uncertain
results obtained through the use of a measuring instrument. Thus, the parameter~$\delta$ in our
...
the face of small nonvanishing $\delta$'s.
...
find the following probability measure that can verified to be a
...
see a set of experimental data with $\frac{1}{3}$ uncertain represented
...
uncertain $\frac{1}{4}$ of the measurement results of a hidden variable
theory cannot be consistent with any valid QIVPM, symmetrically uncertain
...
a deterministic hidden variable theory if enough records is uncertain.
The uncertain records for the Mermin-Peres ``magic square'' could
