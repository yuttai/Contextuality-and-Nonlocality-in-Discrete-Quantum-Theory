...
at length~$\left(2n\right)^{-1}$ at each end. Since these intervals
are overlapping, we define the non-overlapping interior part of a
interval according to the following rule.\begin{subequations}
\begin{eqnarray}
\textrm{Int}\left(\imposs\right) & = & \imposs\,,\\
\textrm{Int}\left(\left[0,\tfrac{1}{2n}\right]\right) & = & \left(0,\tfrac{0.5}{2n}\right]\,,\\
\textrm{Int}\left(\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\right) & = & \left[\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right]\nonumber \\
 &  & \textrm{for even }0<k<2n\,,\\
\textrm{Int}\left(\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\right) & = & \left(\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right)\nonumber \\
 &  & \textrm{for odd }k\,,\\
\textrm{Int}\left(\left[\tfrac{2n-1}{2n},1\right]\right) & = & \left[\tfrac{2n-0.5}{2n},1\right)\,,\\
\textrm{Int}\left(\necess\right) & = & \necess\,.
\end{eqnarray}
\end{subequations}Hence, given a real-valued probability~$x$, we
will replace it with an interval~$\iota_{n}\left(x\right)=[\ell,r]$
...
we can summarize the set of intervals~$\mathscr{I}_{2}$ and their
interior in Table~\ref{tab:I2}.
\begin{table}
\caption{\label{tab:I2}Intervals in $\mathscr{I}_{2}$ and their interior.}
\begin{tabular}{cc}
\toprule 
$[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$\tabularnewline
\midrule
\midrule 
$\imposs$ & $\imposs$\tabularnewline
\midrule 
$\left[0,\tfrac{1}{4}\right]$ & $\left(0,\tfrac{1}{8}\right]$\tabularnewline
\midrule 
$\left[0,{\textstyle \tfrac{1}{2}}\right]$ & $\left(\tfrac{1}{8},\tfrac{3}{8}\right)$\tabularnewline
\midrule 
$\left[\tfrac{1}{4},\tfrac{3}{4}\right]$ & $\left[\tfrac{3}{8},\tfrac{5}{8}\right]$\tabularnewline
\midrule 
$\left[\tfrac{1}{2},1\right]$ & $\left(\tfrac{5}{8},\tfrac{7}{8}\right)$\tabularnewline
\midrule 
$\left[\tfrac{1}{4},1\right]$ & $\left[\tfrac{7}{8},1\right)$\tabularnewline
\midrule 
$\necess$ & $\necess$\tabularnewline
\bottomrule
\end{tabular}
\end{table}
Given two quantum probability measure~$\mu_{1}$ and $\mu_{\mathbb{1}/3}^{\mathrm{B}}$,
where $\frac{\mathbb{1}}{3}=\frac{1}{3}\left(\proj{0}+\proj{1}+\proj{2}\right)$,
it is then easy to compute $\iota_{2}\circ\mu_{1}$ and $\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
on the minimal subspace of events containing $\proj{0}$, $\proj{1}$,
and $\proj{2}$, $\events_{C}$ as in Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
...
Given any state~$\rho$ satisfying $0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$
for any $P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $,
called totally mixed state, we know not only $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
...
be bounded by $\left(12n\right)^{-1}$. Reversely, if the distance
between $\rho$ and $\rho'$ is bounded by $1/n$, then $\rho'$ is
consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$. In other
words, for the states consistent with this QIVPM, $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
will become closer and closer to $\rho'$ as $n$ goes to infinity. 

For example, since $\mu_{1}\left(\proj{2}\right)=0$, any state corresponds
to $\mu_{1}$ is not totally mixed state. On the other hand, $\mathbb{1}/3$
is a totally mixed state. \yutsung{To be continue...}
