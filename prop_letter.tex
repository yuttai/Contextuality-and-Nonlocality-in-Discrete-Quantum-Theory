...
respectively, if there is a function~$\iota$ mapping any number~$x\in\left[0,1\right]$
...
Although the idea is simple, $\iota$ needs to satisfy some conditions;
otherwise, the concatenated function~$\bar{\mu}$ may not be a QIVPM.
...
If the first three constraints, Eqs.~(\ref{eq:iota-constraints}),
fails, it is easy to see that the concatenated function~$\iota\circ\mu$
