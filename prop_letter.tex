...
measure, i.e., no state is consistent with these kind of QIVPMs, while
a quantum probability measure is always induced by an unique state
...
a rule to select the correct interval when intervals are overlapping.
...
After we have the rule to replace a real number by an interval, any
...
where $\circ$ is the function concatenation~\citep{HOSTunpublished}.
For example, when $n=2$, two quantum probability measures~$\mu_{\mathbb{1}/3}^{\mathrm{B}}$
and $\mu_{\rho_{0}}^{\mathrm{B}}$ can induce two QIVPMs~$\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
...
consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$~\citep{HOSTunpublished}.
