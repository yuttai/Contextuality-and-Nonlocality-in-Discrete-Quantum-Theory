...
\newcommand{\set}[2]{\ensuremath{\left\{ {#1}\mathrel{\vphantom{|}}\middle|\mathrel{\vphantom{|}}{#2}\right\} }}
...
and get a QIVPM~$\bar{\mu}:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$.
...
must subject to the following constraints:\begin{subequations}\label{eq:iota-constraints}
\begin{eqnarray}
 &  & \iota(0)=\imposs\,,\\
 &  & \iota(1)=\necess\,,\\
...
If $\iota$ fails to satisfy the first three constraints, Eqs.~(\ref{eq:iota-constraints}),
it is easy to see that the concatenated function~$\iota\circ\mu$
may not be a QIVPM. Suppose $\iota$ fails to satisfy Eq.~(\ref{eq:iota-Inclusion}),
i.e., there exists three numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
...
Consider the state
\begin{equation}
\begin{aligned}\rho=\vphantom{x_{0}} & x_{0}\proj{0}+x_{1}\proj{1}+x_{2}\proj{2}\\
 & +\left(1-x_{0}-x_{1}-x_{2}\right)\proj{3}\textrm{ .}
\end{aligned}
\end{equation}
Let $P_{0}=\proj{0}+\proj{2}$, $P_{1}=\proj{1}+\proj{2}$. Then,
the function~$\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}:\events\rightarrow\mathscr{I}$
...
\nonumber 
\end{eqnarray}
and cannot be a QIVPM. Therefore, $\iota$ must satisfy Eqs.~(\ref{eq:iota-constraints})
and Eq.~(\ref{eq:iota-Inclusion}) to guarantee $\iota\circ\mu$
is always a QIVPM.
