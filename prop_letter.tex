...
% \end{enumerate}
% }
\yutsung{Still not comfortable about the current statement... To
clarify what we did as requested by Andy, here we review the assumption
in the original Kochen-Specker theorem, and what we used in our variant
version.}

The original Kochen-Specker theorem assumes there is a hidden variable
assignment~$v$ for every observables~$\mathbf{O}$ such that $v\left(\mathbf{O}\right)\in\mathbb{R}$
and satisfying the following two assumptions.

\begin{assumption}\label{assu:linear}If $\mathbf{O}_{0}$ and $\mathbf{O}_{1}$
are commuting, then $v$ is linear, i.e., $v\left(x_{0}\mathbf{O}_{0}+x_{1}\mathbf{O}_{1}\right)=x_{0}v\left(\mathbf{O}_{0}\right)+x_{1}v\left(\mathbf{O}_{1}\right)$
for $x_{0}$ and $x_{1}\in\mathbb{C}$.\end{assumption}

\begin{assumption}\label{assu:times}If $\mathbf{O}_{0}$ and $\mathbf{O}_{1}$
are commuting, then $v\left(\mathbf{O}_{0}\mathbf{O}_{1}\right)=v\left(\mathbf{O}_{0}\right)v\left(\mathbf{O}_{1}\right)$.\end{assumption}

\noindent These two assumptions implies the following conditions:

\begin{condition}\label{cond:eigenvalue}$v\left(\mathbf{O}\right)$
must be an eigenvalue of $\mathbf{O}$.\end{condition}

\noindent In particular, Condition~\ref{cond:eigenvalue} implies
$v(P)=0$ or $1$ for any projector~$P$. Together with $v\left(\mathbb{1}\right)=1$.
We have:

\begin{condition}\label{cond:sum-one}If a set of commuting projectors~$P_{i}$
satisfies $\sum_{i}P_{i}=1$, then $\sum_{i}v(P_{i})=1$.\end{condition}

\noindent Or equivalently.

\begin{condition}\label{cond:one-one}If a set of commuting projectors~$P_{i}$
satisfies $\sum_{i}P_{i}=1$, then there exists exactly one $P_{i_{0}}$
such that $v\left(P_{i_{0}}\right)=1$ and $v\left(P_{i}\right)=0$
for $i\ne i_{0}$.\end{condition}

\noindent Kochen-Specker~\citep{kochenspecker1967} and Peres~\citep{peres1995quantum}
then got the contradiction based on Condition~\ref{cond:sum-one}
or \ref{cond:one-one}. Instead of Condition~\ref{cond:sum-one},
Mermin~\citep{Mermin1990Simple} got the contradiction based on Assumption~\ref{assu:times}
and Condition~\ref{cond:eigenvalue} which makes his proof particularly
short and easy to understand.

\yutsung{Let's move on to what we did for 0-Deterministic Variant
of Kochen-Specker theorem~\ref{thm:Kochen-Specker}.}

The natural KS assumption in 0-deterministic QIVPM is the following
one correspondence of Condition~\ref{cond:sum-one}.

\begin{assumption}\label{assu:0-deterministicQIVPM}There exists
a 0-deterministic QIVPM.\end{assumption}

\noindent This assumption itself can deduce the same contradiction
exactly the same as Kochen-Specker~\citep{kochenspecker1967} and
Peres~\citep{peres1995quantum} did, but the proof will be as long
and tedious as theirs. In order to follow the clearer Mermin's proof,
we deduce the following condition based on our assumption:

\noindent \begin{condition}\label{cond:one-one-0-deterministicQIVPM}For
a $0$-deterministic QIVPMs~$\bar{\mu}$, if a set of commuting projectors~$P_{i}$
satisfies $\sum_{i}P_{i}=1$, then there exists exactly one $P_{i_{0}}$
such that $\bar{\mu}\left(P_{i_{0}}\right)=\necess$ and $\bar{\mu}\left(P_{i}\right)=\imposs$
for $i\ne i_{0}$.\end{condition}

\noindent The preceding condition is the key step to deduce the following
two conditions used in Mermin's proof.

\begin{condition}\label{cond:eigenvalue-0-deterministicQIVPM}For
a $0$-deterministic QIVPMs~$\bar{\mu}$, $\expval{\mathbf{O}}_{\bar{\mu}}$
must be an eigenvalue of $\mathbf{O}$.\end{condition}

\begin{condition}\label{assu:times-0-deterministicQIVPM}For a $0$-deterministic
QIVPMs~$\bar{\mu}$, if $\mathbf{O}_{0}$ and $\mathbf{O}_{1}$ are
commuting, then $\expval{\mathbf{O}_{0}\mathbf{O}_{1}}_{\bar{\mu}}=\expval{\mathbf{O}_{0}}_{\bar{\mu}}\expval{\mathbf{O}_{1}}_{\bar{\mu}}$.\end{condition}

\noindent Notice that the expectation value relative to $0$-deterministic
$\bar{\mu}$ plays the same role as the hidden variable assignment~$v$
in the original Kochen-Specker theorem. After we proved these two
conditions, we can easily follow Mermin to build a contradiction.

\yutsung{Finally, we can say about Finite-precision Extension of
Kochen-Specker theorem~\ref{cor:Kochen-Specker-IVPM}.}

The natural KS assumption for 0-deterministic QIVPM is just ``it
exists.'' We might image the natural KS assumption for $\delta$-deterministic
QIVPM is the same, and start from the following assumption.

\begin{assumption}\label{assu:delta-deterministicQIVPM}There exists
a $\delta$-deterministic QIVPM.\end{assumption}

\noindent This assumption draw the same contradiction when $\delta<\frac{1}{3}$,
and there is no contradiction when $\delta\ge\frac{1}{3}$. Therefore,
we want to understand what happens when $\delta$ passing $\frac{1}{3}$.
Notice that Assumption~\ref{assu:delta-deterministicQIVPM} looks
similar, but is not exactly equivalent to Condition~\ref{cond:sum-one}
and Assumption~\ref{assu:0-deterministicQIVPM}. Therefore, in order
to understand better the physical meaning, and in order to know whether
we get a non-contextual hidden variable theory, a contextual hidden
variable theory, or not a hidden variable theory at all, we might
need to prove the corresponding states of Condition~\ref{cond:one-one},
Condition~\ref{cond:eigenvalue}, Assumption~\ref{assu:linear},
and Assumption~\ref{assu:times}. 

\noindent I personally believe they are approximately hold for small
$\delta$, but void when $\delta$ is large enough. 

I think these two properties might be weakened when $0<\delta<\frac{1}{3}$,
and completely break down when $\delta\ge\frac{1}{3}$. (Need a proof!)
If this is the case, the correct statement is that ``the assumption
of non-contextuality hidden variable theory could be considered hold
when $\delta<\frac{1}{3}$, and is completely void when $\delta\ge\frac{1}{3}$.''
So we should not even ask ``whether the existence of QIVPMs when
$\delta\ge\frac{1}{3}$ implies non-contextuality hidden variable
theory or not.'' A little bit different analog could be: we should
not ask ``where is the momentum of a particle when we measure its
position...''

\yutsung{End of discussion!}
...
While the finite-precision extension of Kochen-Specker theorem asserts
the non-existence of certain kind of QIVPMs, we also want to understand
what a QIVPM could be when it exists. In Sec.~\ref{sec:Interval-Uncertainty},
the only non-trivial QIVPM~ $\bar{\mu}_{3}$ is induced by the state~$\proj{\ps}$.
The way we extending a state to a QIVPM will first be generalized
in this section, and introduce a finite-precision extension of the
Born rule~\citep{Born1983,peres1995quantum,544199,Jaeger2007}. The
Born rule is essential to compute the conventional quantum probability
because any quantum probability measure can be induced by the Born
rule according to Gleason's theorem~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum}.
We then provide a finite precision variant of Gleason's theorem for
the Born QIVPMs, but there are also some general QIVPMs may not be
induced by any quantum probability measure, i.e., no state is consistent
with these kind of QIVPMs.

When we constructed $\bar{\mu}_{3}$, we replaced the real-valued
probability~$0$ and $\frac{1}{2}$ by $\left[0,0\right]$ and $\left[\frac{1}{2},\frac{1}{2}\right]$,
respectively. Replacing numbers by intervals containing them can be
generalized if we carefully pick a set of intervals and a rule to
select the correct interval when intervals are overlapping. For example,
...
with length~$\left(2n\right)^{-1}$ at each end. Since these intervals~$[\ell,r]$
are overlapping, we define the non-overlapping interior part of a
interval, $\textrm{Int}\left([\ell,r]\right)$, as in Table~\ref{tab:I2}.
\begin{table}
\caption{\label{tab:I2}Intervals in $\mathscr{I}_{n}$ and their interior
with an concrete example for $\mathscr{I}_{2}$. Although intervals~$[\ell,r]$
are overlapping, their interior~$\textrm{Int}\left([\ell,r]\right)$
are non-overlapping.}

\begin{tabular}{cccc}
\toprule 
\addlinespace
\multicolumn{2}{c}{$\mathscr{I}_{n}$} & \multicolumn{2}{c}{$\mathscr{I}_{2}$}\tabularnewline\addlinespace
\midrule 
\addlinespace
$[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$ & $[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
$\imposs$ & $\imposs$ & $\imposs$ & $\imposs$\tabularnewline\addlinespace
\midrule 
\addlinespace
$\left[0,\tfrac{1}{2n}\right]$ & $\left(0,\tfrac{0.5}{2n}\right]$ & $\left[0,\tfrac{1}{4}\right]$ & $\left(0,\tfrac{1}{8}\right]$\tabularnewline\addlinespace
\midrule 
\addlinespace
$\begin{gathered}\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\\
\textrm{for odd }k
\end{gathered}
$ & $\left(\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right)$ & $\left[0,{\textstyle \tfrac{1}{2}}\right]$ & $\left(\tfrac{1}{8},\tfrac{3}{8}\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
\multirow{2}{*}{$\begin{gathered}\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\\
\textrm{for even }k\\
0<k<2n
\end{gathered}
$} & \multirow{2}{*}{$\left[\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right]$} & $\left[\tfrac{1}{4},\tfrac{3}{4}\right]$ & $\left[\tfrac{3}{8},\tfrac{5}{8}\right]$\tabularnewline\addlinespace
\cmidrule{3-4} 
\addlinespace
 &  & $\left[\tfrac{1}{2},1\right]$ & $\left(\tfrac{5}{8},\tfrac{7}{8}\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
$\left[\tfrac{2n-1}{2n},1\right]$ & $\left[\tfrac{2n-0.5}{2n},1\right)$ & $\left[\tfrac{1}{4},1\right]$ & $\left[\tfrac{7}{8},1\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
$\necess$ & $\necess$ & $\necess$ & $\necess$\tabularnewline\addlinespace
\bottomrule
...
\yutsung{Not sure what is the best way embedding the condition for
$k$ in the table...}
\end{table}
Hence, given a real-valued probability~$x$, we could easily replace
it with an interval~$\iota_{n}\left(x\right)=[\ell,r]$ if $x\in\textrm{Int}\left([\ell,r]\right)$.
...
\begin{thm}[Finite-precision Extension of the Born rule~\citep{HOSTunpublished}]For
any state $\rho$ on a three-dimensional Hilbert space, $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
is a QIVPM, where $\circ$ is the function concatenation.\end{thm}\yutsung{Do
we need to summarize the proof?}

\noindent For example, when $n=2$, two states~$\mathbb{1}/3$ and
$\rho_{0}$ can induce two QIVPMs~$\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
and $\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$, where $\mathbb{1}/3=\left(\proj{0}+\proj{1}+\proj{2}\right)/3$
and $\rho_{0}=\frac{7}{24}\cdot\proj{0}+\frac{1}{3}\cdot\proj{1}+\frac{3}{8}\cdot\proj{2}$.
The interval-valued probability of the induced QIVPMs, $\iota_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$
and $\iota_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$,
for any projector $P$ in the minimal subspace of events containing
$\proj{0}$, $\proj{1}$, and $\proj{2}$, $\events_{C}$, are computed
straightforwardly in Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
and $\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$ based on $\mu_{\mathbb{1}/3}^{\mathrm{B}}$
and $\mu_{\rho_{0}}^{\mathrm{B}}$ on $\events_{C}$.}

\begin{tabular}{ccccc}
\toprule 
\addlinespace
$P$ & $\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)$ & $\iota_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$ & $\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)$ & $\iota_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
...
with its corresponding $\iota_{\infty}\left(x\right)=[x,x]$, we can
formulate Gleason's theorem in our framework.
...
the states consistent with it on every project, $\coreBorn\left(\bar{\mu},\events\right)$
has a unique element. \end{thm}

Our framework allows us to generalize the above theorem to the Born
QIVPMs~$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$: The collection of
states consistent $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$ get closer
and closer to $\rho$ as the intervals get narrower and narrower,
eventually collapsing to $\rho$ when the intervals become infinitely
precise real numbers. To state this result, we introduce a few technical
notions. We call a state~$\rho$ a \emph{totally mixed state} if
$0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$ for any $P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $.
We also define a notion of distance between two states~$\rho$ and
$\rho'$ as $\left\Vert \rho-\rho'\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{\rho-\rho'}{\psi}\right|$~\citep{544199},
...
Born QIVPMs~\citep{HOSTunpublished}]In a Hilbert space $\Hilb$
of dimension $3$, given any totally mixed state~$\rho$, 
\begin{equation}
...
\end{equation}
\end{thm}\yutsung{Do we need to summarize the proof?}

\noindent In another word, if a state~$\rho'$ is consistent with
$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$, its distance with $\rho$,
$\left\Vert \rho-\rho'\right\Vert $, must be bounded by $1/n$. Reversely,
if the distance between $\rho$ and $\rho'$ is bounded by $\frac{1}{12n}$,
then $\rho'$ is consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$.
For example, $\mathbb{1}/3$ and $\rho_{0}$ are both totally mixed
states. Since $\left\Vert \frac{\mathbb{1}}{3}-\rho_{0}\right\Vert \le\frac{1}{24}$,
we should have $\rho_{0}\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}},\events\right)$
and $\mathbb{1}/3\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$,
and this is indeed the case for $P\in\events_{C}$ according to Table~\ref{tab:Compute-the-QIVPM}.
In general, as $n$ goes to infinity, only the states very close to
$\rho$ is guarantied belong to $\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}},\events\right)$,
...
there are some QIVPMs which cannot be consistent with any state.
...
\noindent Given a three-dimensional Hilbert space $\Hilb$ with an
orthonormal basis $\left\{ \ket{0},\ket{1},\ket{2}\right\} $, let
$\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} $, $\ket{\ps}=\left(\ket{0}+\ket{1}\right)/\sqrt{2}$,
$\ket{\ps'}=\left(\ket{0}+\ket{2}\right)/\sqrt{2}$, and \begin{subequations}
\begin{eqnarray}
...
is a QIVPM. Then we will prove $\coreBorn\left(\bar{\mu},\events\right)$
is an empty set by contradiction. Suppose there is a state $\rho=\sum_{j=1}^{N}q_{j}\proj{\phi_{j}}\in\coreBorn\left(\bar{\mu},\events\right)$,
...
for all $j$. However, the last equation never holds. Thus, there
is no state consistent with the QIVPM on all projectors~$P\in\events$,
so Gleason's theorem cannot be extended to general QIVPMs because
of this example.

Despite the non-extensible of Gleason's theorem to general QIVPMs,
when we restrict $\bar{\mu}$ on any commuting subspace of events~$\events_{C}$,
$\bar{\mu}$ becomes classical, and Thm.~\ref{thm:Shapley} guarantees
$\coreBorn\left(\bar{\mu},\events_{C}\right)$ is nonempty. In this
sense, Thm.~\ref{thm:Non-extensible-of-Gleason's} is a completely
new phenomena when we generalize both classical IVPMs and quantum
probability measures to QIVPMs.
