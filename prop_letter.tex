...
\emph{convexity}~\cite{Shapley1971,GilboaSchmeidler1994,NgMoYeh1997,Marinacci1999,Marinacci_2004,Grabisch2016},
...
theorem~\cite{Shapley1971,GilboaSchmeidler1994,NgMoYeh1997,Marinacci_2004,Grabisch2016},
...
\emph{Choquet integral}~\citep{Choquet1954,GilboaSchmeidler1994,Marinacci_2004,Grabisch2016}
...
implies
$\bar{\mu}^{\textrm{D}}\left(P_{0}+P_{1}\right)=\imposs$~\citep{HOSTunpublished}. Indeed when
$\bar{\mu}^{\textrm{D}}\left(P_{0}\right)=\bar{\mu}^{\textrm{D}}\left(P_{1}\right)=\imposs$,
...
To prove the theorem, we just need to construct a QIVPM, and verify
that there are no states that are consistent with it on all
...
clustering functions $f:\left[0,1\right]\rightarrow\mathscr{I}$ that
...
state~$\ket{\psi}$ is defined as usual
...
Then, a version of Gleason's theorem can be stated as follows. 
...
\begin{thm}[Finite-Precision Extension of Gleason's
  theorem]\label{thm:Finite-precision-Gleason}Let $f$ be a clustering
...
one-dimensional projector $\proj{\psi}$. Since the maximum length of
...
measure $\mu$, it is the case that $f \circ \mu$ is a QIVPM, then $f$
...
proof is to show that if $\ultramodular$ is not ultramodular, then for
...
\end{aligned}
\end{equation}
The induced map using the Born rule and clustered by $\ultramodular$,
...
clustering functions can be gleaned from the properties of
...
\end{eqnarray}
i.e., $\ultramodularL$ is midpoint-convex or
Jensen-convex~\citep{Klee_1956,Kenyon_1956,RobertsVarberg1973,PecaricTong1992}.
...
$\left(0,1\right)$~\citep{Jensen_1906,RobertsVarberg1973}.  Therefore,
...
uncountable range $[0,1]$. A QIVPM constructed by clustering such a
conventional quantum probabily measure, must also have an uncountable
...
% QIVPM~\citep{HOSTunpublished}.  For example, consider an orthonormal
...
% also have
% $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\in\ultramodular_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$
% for any two-dimensional projector~$P$. Hence,
% $\rho_{1}\in\coreBorn\left(\ultramodular_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$,
