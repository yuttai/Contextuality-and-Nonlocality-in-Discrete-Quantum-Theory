...
To this end, we develop a mathematical framework that includes the
uncertainties of finite precision and imperfections in the quantum
measurement process. In particular, we quantify the concept of
...
foundations of quantum mechanics.  Our first observation is that
...
outcome~\citep{Born1983,peres1995quantum,544199}. The Gleason theorem
establishes that there is no alternative to the Born rule by
...
Our first result is that a QIVPM is consistent with the inability to
avoid uncertainty found in conventional infinite precise quantum
mechanics~\cite{PeresRon1988,peres1995quantum}.

KS is formulated under the assumption that all observables for a QM
system have definite values at all times. (cite) 

Our QIVPM framework allows us to reformulate KS by relaxing this
assumption. 

This quantifies Mermin's argument:
...
The technical question is what happens if we relax the perfect
measurement assumption that the intervals are precisely $[0,0]$ and
$[1,1]$. We need to examine what happens if measurement imperfections
occasionally cause an ideally impossible event to be detected or an
ideally certain event not to be detected. 
...
behavior. We now prove that for small enough $\delta$, the Kochen-Specker result is
...
%%%%%
\subsection{Expectation Values of Fuzzy Observables} 
...
%%%%%
\subsection{Finite Precision KS}

Statement of reformulated KS
...
Proof of reformulated KS. We will proceed by assuming a 0-deterministic QIVPM $\bar{\mu}$ and
...
\paragraph*{Stronger KS: Main Result.}
