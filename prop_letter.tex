...
if $\iota\circ\mu$ is always a IVPM, then $\iota$ must be a THOS
rule. In other words, $\iota$ is a THOS rule is a necessary and sufficient
...
\end{aligned}
\end{equation}
When $P_{0}=\proj{0}+\proj{2}$ and $P_{1}=\proj{1}+\proj{2}$, we
have $P_{0}P_{1}=\proj{2}$ and $P_{0}+P_{1}-P_{0}P_{1}=\proj{0}+\proj{1}+\proj{2}$.
By applying $\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$ on these
projectors, Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion})
becomes Eq.~(\ref{eq:iota-Inclusion}) which cannot be satisfied
by how we pick $x_{0}$, $x_{1}$, and $x_{2}$. Therefore, $\iota$
is a THOS rule if and only if $\iota\circ\mu$ is always a QIVPM.
Similarly, Statement~\ref{enu:iota-subject-to} and \ref{enu:iota-mu-CIVPM}
