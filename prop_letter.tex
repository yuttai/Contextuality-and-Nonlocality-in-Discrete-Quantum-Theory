...
they can be diagonalized by a common orthonormal basis~$\Omega$.
By restricting to the subspace of commuting events generated by the
set of projectors associated with $\Omega$, the quantum expectation
values of the observables~$\expval{\mathbf{O}_{j}}_{\bar{\mu}}$
...
to $\bar{\mu}$~\citep{HOSTunpublished}. The Choquet integral relative
to 0-deterministic $\bar{\mu}'$ is a singleton set which have an
intuitive the product rule $\left\{ x_{0}\right\} \cdot\left\{ x_{1}\right\} =\left\{ x_{0}x_{1}\right\} $.
Because the product of the Choquet integrals of individual random
variables relative to $\bar{\mu}'$ is the Choquet integral of the
product of the family relative to $\bar{\mu}'$, we can then prove
that the expectation values of the observables has the corresponding
property: 
\begin{equation}
