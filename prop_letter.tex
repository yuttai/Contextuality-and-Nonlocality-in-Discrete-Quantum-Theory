...
QIVPMs become to quantum probability measures~\citep{HOSTunpublished},
and Gleason's theorem claims that there is always a unique state consistent
with $\bar{\mu}:\events\rightarrow\mathscr{I}_{\infty}$~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum}.
...
results we get might contradict to each other, and exclude all possible
states consistent with the measurement results. There are many ways
to avoid these kind of too imprecise intervals~\citep{HOSTunpublished}.
Following, we consider a concrete family of intervals and QIVPMs~$\bar{\mu}$
such that if we monotonically shrink the length of intervals, the
states consistent with $\bar{\mu}$ will be closer and closer to $\rho$
which inducing $\bar{\mu}$.
\begin{equation}
...
in Table~\ref{tab:I_n}, we select the correct interval from these
overlapping intervals~$[\ell,r]$, and define $\textrm{Int}\left([\ell,r]\right)$,
the non-overlapping interior part of an interval.
\begin{table}
...
\noindent Since $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$ clearly
satisfies Eqs.~(\ref{eq:QIVPM-constraints}), the condition that
needs to be checked is Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}).
...
can be deduced easily. Therefore, $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$
is indeed a QIVPM.
...
The collection of states consistent $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$
...
by a Born QIVPM~$\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$, we obtain
the following theorem that estimates the states consistent with $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$.
...
\noindent If a state~$\rho'$ is consistent with $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$,
...
that $\rho_{0}$ is consistent with $\left[\mu_{\rho_{1}}^{\mathrm{B}}\right]_{2}$
