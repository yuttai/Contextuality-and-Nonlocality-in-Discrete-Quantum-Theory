...
a QIVPM can be when it exists. In Sec.~\ref{sec:Interval-Uncertainty},
the only non-trivial QIVPM~ $\bar{\mu}_{3}$ is induced by the quantum
probability measure~$\mu_{1}$ defined in Sec.~\ref{sec:fuzzy}.
In this section, we first generalize this result, and then show general
...
which are essentially the same as the replaced real numbers. In general,
if we carefully pick a set of overlapping intervals, we can replace
the numbers by the interval containing it. For example, consider the
