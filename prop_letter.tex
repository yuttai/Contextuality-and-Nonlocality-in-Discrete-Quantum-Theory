...
which can then be concatenated with a quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
and easily produce an interval-valued function~$\iota\circ\mu:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$,
where $\circ$ is the function concatenation. However, the concatenated
function~$\iota\circ\mu$ may or may not be a QIVPM. For example,
in a four-dimensional Hilbert space, consider the function 
\begin{equation}
\iota'\left(x\right)=\begin{cases}
\imposs\,, & \textrm{ if }x=0\,;\\
...
\end{equation}
While the function~$\iota'\circ\mu_{\rho_{0}}^{\mathrm{\mathrm{B}}}$
induced by $\rho_{0}=\frac{1}{4}\cdot\proj{0}+\frac{1}{4}\cdot\proj{1}+\frac{1}{4}\cdot\proj{2}+\frac{1}{4}\cdot\proj{3}$
is a QIVPM, the function~$\iota'\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
induced by $\rho_{1}=\frac{1}{8}\cdot\proj{0}+\frac{1}{8}\cdot\proj{1}+\frac{1}{8}\cdot\proj{2}+\frac{5}{8}\cdot\proj{3}$
is \emph{not} a QIVPM. Although we could verify whether $\iota\circ\mu$
is a QIVPM or not for any pair of $\iota$ and $\mu$ individually,
the verification process is long and tedious. Therefore, it is desirable
to have some easy criteria to decide when $\iota\circ\mu$ is a QIVPM.
...
Since $\iota'$ defined in Eq.~(\ref{eq:iota}) satisfies Eqs.~(\ref{eq:iota-constraints})
and Eq.~(\ref{eq:iota-add}), every function~$\iota'\circ\mu$ on
a three-dimensional Hilbert space is a QIVPM. However, on four-dimensional
Hilbert space, $\iota'\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
is not a QIVPM. This highlight the necessity of the conditions to
guarantee $\iota\circ\mu$ is always a QIVPM.

\begin{thm}If $\iota\circ\mu:\events\rightarrow\mathscr{I}$ is always
a QIVPM for any quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
in any Hilbert space $\Hilb$, then $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
must subject to Eqs.~(\ref{eq:iota-constraints}) and satisfying
...
The proof is by contradiction: Suppose $\iota$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}),
we can find three particular numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
...
\end{eqnarray*}
and cannot be a QIVPM. Therefore, to make sure $\iota\circ\mu$ is
always a QIVPM, $\iota:\left[0,1\right]\rightarrow\mathscr{I}$ should
satisfy Eqs.~(\ref{eq:iota-constraints}) and Eq.~(\ref{eq:iota-Inclusion}).

Since $\iota'\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$ is not a
QIVPM, we know $\iota'$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}).
...
A Wright-convex function is midpoint-convex or Jensen-convex~\citep{RobertsVarberg1973,PecaricTong1992},
i.e., we have
\begin{equation}
