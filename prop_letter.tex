...
clarify what we did, here we review the assumption in the original
Kochen-Specker theorem, and what we used in our variant version.}

The original Kochen-Specker theorem assumes the following there is
a hidden variable assignment~$v$ for every observables~$\mathbf{O}$
such that

\begin{assumption}If $\mathbf{O}_{0}$ and $\mathbf{O}_{1}$ are
commuting, then $v$ is linear, i.e., $v\left(x_{0}\mathbf{O}_{0}+x_{1}\mathbf{O}_{1}\right)=x_{0}v\left(\mathbf{O}_{0}\right)+x_{1}v\left(\mathbf{O}_{1}\right)$
...
then proved the contradiction based on Condition~\ref{cond:sum-one}.
The proof given by Mermin~\citep{Mermin1990Simple} is particular
short and easy to understand, because he proved proved the contradiction
based on Assumption~\ref{assu:times} and Condition~\ref{cond:eigenvalue}.
...
Notice that the key properties of the non-contextual hidden variable
theory used in our Mermin-Peres square proof is
\begin{itemize}
\item The expectation value of an observable is one of the eigenvalue of
that observable.
\item For $0$-deterministic QIVPMs, we have
\begin{equation}
