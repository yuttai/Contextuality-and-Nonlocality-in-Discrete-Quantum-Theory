...
a QIVPM. Notice that the biggest difference between a quantum probability
measure~$\mu:\events\rightarrow\left[0,1\right]$ and a QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$
is their ranges. If there is a function~$\iota$ mapping a number~$x\in\left[0,1\right]$
to an interval~$\iota\left(x\right)\in\mathscr{I}$, then we can
concatenate it with a quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
and easily get a $\mathscr{I}$-valued function~$\iota\circ\mu:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$,
where $\circ$ is the function concatenation. This $\iota\circ\mu$
may or may not be a QIVPM as shown in the following example. In the
four-dimensional Hilbert space with 
\begin{equation}
...
\end{equation}
the function~$\iota'\circ\mu_{\rho_{0}}^{\mathrm{\mathrm{B}}}$ induced
by $\rho_{0}=\frac{1}{4}\cdot\proj{0}+\frac{1}{4}\cdot\proj{1}+\frac{1}{4}\cdot\proj{2}+\frac{1}{4}\cdot\proj{3}$
is a QIVPM, but the function~$\iota'\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
...
is a QIVPM or not directly, it is a long and tedious process. Therefore,
it is desirable to find some criteria when $\iota\circ\mu$ is a QIVPM.
