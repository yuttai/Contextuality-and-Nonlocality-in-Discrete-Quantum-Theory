...
\lineskiplimit=1pt
\newcommand{\ultramodular}{\ensuremath{f}}
...
is called a ultramodular function if $\ultramodular$ is subject to
the following constraints:\begin{subequations}\label{eq:iota-constraints}
\begin{eqnarray}
...
is the direct counterpart of the convexity conditions, Eq.~(\ref{eq:classicalconvex})
...
\begin{thm}[Equivalence among Ultramodular Functions and IVPMs]\label{thm:iota-statements}The
following three statements are equivalent:
\begin{enumerate}
\item \label{enu:iota-subject-to}A function~$\ultramodular:\left[0,1\right]\rightarrow\mathscr{I}$
is ultramodular.
\item \label{enu:iota-mu-CIVPM}The composited function~$\ultramodular\circ\mu:\events_{\mathrm{C}}\rightarrow\mathscr{I}$
is always a classical IVPM for any classical probability measure~$\mu:\events_{\mathrm{C}}\rightarrow\left[0,1\right]$.
\item \label{enu:iota-mu-QIVPM}The composited function~$\ultramodular\circ\mu:\events\rightarrow\mathscr{I}$
...
\ref{enu:iota-mu-CIVPM} is a well-known classical result~\citep{Choquet1954,NgMoYeh1997,Marinacci_2004}.
For quantum case, Statement~\ref{enu:iota-subject-to} implies \ref{enu:iota-mu-QIVPM}
as we have discussed. The rest of the proof is to show that if $\ultramodular$
is not ultramodular, then $\ultramodular\circ\mu$ might not be a
...
but they don't satisfy Eq.~(\ref{eq:iota-Inclusion}). Consider the
state 
\begin{equation}
...
\end{aligned}
\end{equation}
Its inducing function $\ultramodular\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$
fails to satisfy Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion})
when $P_{0}=\proj{0}+\proj{2}$ and $P_{1}=\proj{1}+\proj{2}$ so
that $\ultramodular\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$ cannot
be a QIVPM. Therefore, $\ultramodular$ is ultramodular if and only
if $\ultramodular\circ\mu$ is always a QIVPM.

In contrast with the QIVPM defined in Table~\ref{tab:non-Born-QIVPM},
there is always a state consistent with any composited QIVPM $\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$.
Moreover, the collection of states consistent with $\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$
gets closer and closer to $\rho$ as the intervals get narrower and
narrower. Eventually, the collection of consisting states collapses
to the singleton set~$\left\{ \rho\right\} $ as the intervals become
the infinitely precise intervals~$\mathscr{I}_{\infty}$. This fact
is a weak version of Gleason's theorem because we only consider composited
QIVPMs $\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$ instead of general
QIVPMs~$\bar{\mu}$. To precisely state this result, we need to introduce
the norm of pure states and operators. The norm of a pure state~$\ket{\psi}$
is defined as usual $\left\Vert \psi\right\Vert =\sqrt{\ip{\psi}{\psi}}$.
Similarly, given any Hermitian operator~$A$, we consider the usual
...
Then, a weak version of Gleason's theorem can be stated as follow.

\begin{thm}[Finite-Precision Extension of Weak Gleason's theorem]\label{thm:Finite-precision-Gleason}Consider
an interval-valued function $\ultramodular:\left[0,1\right]\rightarrow\mathscr{I}$
with the maximum length of intervals~$m$. Given any state~$\rho$,
if a state $\rho'\in\coreBorn\left(\ultramodular\circ\mu_{\rho}^{\mathrm{B}},\events\right)$,
then the norm of their difference $\left\Vert \rho-\rho'\right\Vert \le m$.\end{thm}
