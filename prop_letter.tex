...
Kochen-Specker
theorem~\cite{kochenspecker1967,Redhead1987-REDINA,Mermin1990Simple,peres1995quantum,Jaeger2007,Held2016}.
...
long version of this paper~\cite{TaiThesis2018}. The idea of the
proof is as follows. Given a subspace of mutually commuting
events~$\eventsC$, we can construct a partial orthonormal
basis by diagonalization and complete it to a full orthonormal basis
$\overline{\eventsC}$. We then build a bijection between
...
correspondence~\cite{TaiThesis2018} and Shapley's
...
that is consistent with every event in $\eventsC$. In contrast, as we
...
theorem~\cite{kochenspecker1967,Redhead1987-REDINA,Mermin1990Simple,peres1995quantum,Jaeger2007,Held2016}. Our
...
Eq.~(\ref{eq:hbar}) that $\muB_{\rho}$ must also map
...
$\muB_{\rho}$ is the eigenvalue $\lambda$ whose projector
...
theorem below, an arbitrary assignment of intervals to projectors
might be globally inconsistent.
...
\item \label{enu:iota-mu-CIVPM}The composite
  ...
  measure~$\mu:\eventsC\rightarrow\left[0,1\right]$.
