...
Therefore, either $\iota$ maps every number in the same interval,
or the number of intervals that $\iota$ maps to must be uncountable.

\begin{thm}\label{thm:convex-uncountable}Let $\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} $.
For any function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
described in Thm.~\ref{thm:iota-statements}, either $\mathscr{I}$
is $\mathscr{I}_{0}$ or $\mathscr{I}$ contains uncountably many
intervals.\end{thm}
...
where $\mathscr{I}_{0}$ is defined in Thm.~\ref{thm:convex-uncountable}.
\begin{table}
...
and a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ considered
...
First, the norm of a pure state is defined as usual $||\psi||=\sqrt{\ip{\psi}{\psi}}$.
Then, the norm of the difference of two states~$\rho$ and $\rho'$
...
\end{equation}
which is also commonly known as $L^{2}$ norm or spectral norm~\citep{GolubVanLoan1996,Foucart2012}.
...
Since $\rho-\rho'$ is Hermitian, $\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{\rho-\rho'}{\psi}\right|$is
the maximum absolute value of the eigenvalues of $\rho-\rho'$~\citep{544199},
