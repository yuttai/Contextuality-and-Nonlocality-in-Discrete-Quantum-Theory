...
$\rho$ consistent with the statistical predictions computed from the Born
...
the intervals. We then demonstrate that all
...
\yutsung{``volume of states" are not defined in the Gleason's section.}
...
not only of resolving the Meyer-Mermin debate, but also of revealing a
...
\end{equation}
which is sensible because for any event $E$ with $\mu(E)=[\ell,r]$, we always
...
events $E_1$ and $E_2$, necessarily disjoint, we have:
\begin{equation}
...
\end{definition}
\yutsung{Clarify the domain of this.}
...
$\unknown\subseteq\left[1-\frac{1}{3},1\right]$ are false.  Finally, when
...
\yutsung{contextuality is never defined.}
...
Let $P$ be an operator projecting onto an $n$-dimensional subspace of
...
\yutsung{To Amr, the ``beyond'' is changed from ``to''. Andy wants to make sure
you are OK with it.}
...
\yutsung{``quantum measurement'' should be replaced by the }
In addition, we showed how
