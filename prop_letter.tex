...
is closer to the center of $[\ell_{i},r_{i}]$. When there is a tie,
we set $\widehat{\iota}_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)=\left[\frac{k-1}{2n},\frac{k+1}{2n}\right]\cap\left[0,1\right]$
...
is a QIVPM. It will be denoted by $\bar{\mu}_{4}$ whose values on
the minimal subspace of events containing $\proj{0}$, $\proj{1}$,
and $\proj{2}$, $\events_{C}$, is computed by Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\bar{\mu}_{4}$ based on $\mu_{1}$
on $\events_{C}$.}
\begin{tabular}{ccc}
\toprule 
$P$ & $\mu_{1}\left(P\right)$ & $\bar{\mu}_{4}\left(P\right)$\tabularnewline
\midrule
\midrule 
$\mathbb{0}$ & $0$ & $\left[0,0\right]$\tabularnewline
\midrule 
$\proj{0}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$\tabularnewline
\midrule 
$\proj{1}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$\tabularnewline
\midrule 
$\proj{2}$ & $0$ & $\left[0,0\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{0}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{1}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{2}$ & $1$ & $\left[1,1\right]$\tabularnewline
\midrule 
$\mathbb{1}$ & $1$ & $\left[1,1\right]$\tabularnewline
\bottomrule
\end{tabular}
\end{table}

The QIVPM~$\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$ is well-behavior
in a sense that the collection of states consistent it get closer
...
precise real numbers. To state this result, we need some more technical
notions. We first define a notion of distance based on the following
norm of a matrix~$A$ as $\left\Vert A\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{A}{\psi}\right|$~\citep{544199}.
Then, for any state~$\rho$ satisfying $0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$
for any $P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $,
any other state~$\rho'$ consistent with $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$
must close to $\rho$ in a sense that $\left\Vert \rho-\rho'\right\Vert \le1/\left(12n\right)$.
Reversely, if a state~$\rho'$ is close to $\rho$ in a sense that
$\left\Vert \rho-\rho'\right\Vert \le1/n$, then $\rho'$ is consistent
