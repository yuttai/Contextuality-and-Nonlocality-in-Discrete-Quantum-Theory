...
state from being consistent with the measurement results. Although
we will not dig into the general framework to avoid this problem here~\citep{HOSTunpublished},
we will address this problem with a concrete family of intervals and
QIVPMs. Consider the following intervals 
\begin{equation}
...
for any 1-dimensioal projector~$\proj{\psi}$. Since the maximum
length of the intervals in $\mathscr{I}_{n}$ is $1/n$, it is the
upper bound of the difference between the Born quantum probabilities
$\left|\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right|=\left|\melem{\psi}{\rho-\rho'}{\psi}\right|$.
