...
at length~$\left(2n\right)^{-1}$ at each end. Then, any Born rule
probability~$\mu_{\rho}^{\mathrm{B}}$ on a three-dimensional Hilbert
space can induce a QIVPM over the intervals above as follows. We first
define a map $\widehat{\iota}_{n}$ from $[0,1]$ to $\widehat{\mathscr{I}}_{n}$
as: 
\begin{itemize}
