...
However, the composited function~$\iota\circ\mu$ may or may not
be a QIVPM. For example, in a four-dimensional Hilbert space, consider
the function 
\begin{equation}
...
\begin{thm}\label{thm:QRVPM2QIVPM-3}In  for any quantum probability
measure~$\mu:\events\rightarrow\left[0,1\right]$.\end{thm}
...
is not a QIVPM on four-dimensional Hilbert space. Hence, to guarantee
$\iota\circ\mu$ is always a QIVPM, we need to strengthen Eq.~(\ref{eq:iota-add})
and consider the following conditions.

\yutsung{Another idea might be fixing a quantum probability measure~$\mu$,
whether there is a length of intervals, such that if $\iota$ maps
to intervals tighter than the given length, then $\iota\circ\mu$
is always a QIVPM... I don't know the answer of this question, and
it might not be very easy...}

\yutsung{The other idea: We never proved a non-convex QIVPM always
have a non-empty core or find an counterexample that a non-convex
QIVPM has empty core. Moreover, we have no idea the relation between
its expectation value and the Choquet integral... Depending the on
situation, the convex condition may not be necessary, but this might
be hard to prove...}
...
state from being consistent with the measurement results. To address
this problem, we are going to consider a particular family of QIVPMs
induced by quantum probability measures. When we generalized quantum
probability measures to QIVPMs, we changed the range from the unit
interval~$\left[0,1\right]$ to a set of intervals~$\mathscr{I}$.
This replacement suggests us to find a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
which has the same domain and range as a QIVPM. Although there are
infinitely many different choice of $\iota$, the function~$\iota$
satisfies the following homomorphism-like properties are of particular
interest, because it guarantee $\iota\circ\mu$ is always a QIVPM.

\begin{definition}Given a collection of intervals $\mathscr{I}$
including $\imposs$ and $\necess$, a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
is called a \emph{morphism}\yutsung{I may need to know the precise
conditions when $\iota$ is a morphism in the sense of category theory,
or we need to find a better name for this function.}if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
is subject to the following constraints:\begin{subequations}\label{eq:iota-constraints}
\begin{eqnarray}
 &  & \iota(0)=\imposs\,,\\
 &  & \iota(1)=\necess\,,\\
...
\noindent Then, $\iota$ is a morphism precisely characterizes that
$\iota\circ\mu$ is always a IVPM no matter in classical or quantum
cases.

\begin{thm}\label{thm:iota-statements}The following three statements
are equivalent:
\begin{enumerate}
\item \label{enu:iota-subject-to}The function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
is a morphism.
\item \label{enu:iota-mu-CIVPM}The composited function~$\iota\circ\mu:\events\rightarrow\mathscr{I}$
...
Because the similarity between classical and quantum cases, we will
only prove the equivalence between Statement~\ref{enu:iota-subject-to}
and Statement~\ref{enu:iota-mu-QIVPM}. It is clear that $\iota$
is a morphism implies $\iota\circ\mu$ is always a QIVPM, and the
rest of the proof is to prove that if $\iota$ is not a morphism,
then $\iota\circ\mu$ might not be a QIVPM. If $\iota$ doesn't satisfy
Eq.~(\ref{eq:iota-Inclusion}), we can find three particular numbers~$x_{0}$,
$x_{1}$, and $x_{2}\in\left[0,1\right]$ such that $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$,
...
\end{eqnarray*}
and cannot be a QIVPM. Therefore, $\iota$ is a morphism if and only
if $\iota\circ\mu$ is always a QIVPM.

Although the composited function~$\iota\circ\mu$ may not guarantee
to be a QIVPM when $\iota$ is not a morphism, it may still be a QIVPM
under so suitable conditions. For example, given a Hilbert space of
dimension~$d\le3$, we can guarantee $\iota\circ\mu$ is a QIVPM
when $\iota$ satisfies Eqs.~(\ref{eq:iota-constraints}) and a weaker
condition: for any numbers~$x_{0}$ and $x_{1}\in\left[0,1\right]$,
their sum~$x_{0}+x_{1}\in\left[0,1\right]$ implies
\begin{equation}
...
if a QIVPM~$\iota\circ\mu_{\rho}^{\mathrm{B}}$ is induced by a conventional
quantum probability measure~$\mu_{\rho}^{\mathrm{B}}$ and a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
, there is always a state consistent with it because $\rho\in\coreBorn\left(\iota\circ\mu_{\rho}^{\mathrm{B}},\events\right)$.
...
narrower. To state this result, we introduce a few technical notions.
First, the norm of a pure state~$\ket{\psi}$ is defined as usual
$||\psi||=\sqrt{\ip{\psi}{\psi}}$. Then, the norm of the difference
of two states~$\rho$ and $\rho'$ is just the usual operator norm
\begin{equation}
\left\Vert \rho-\rho'\right\Vert =\max_{\left\Vert \psi\right\Vert =1}\left\Vert \left(\rho-\rho'\right)\ket{\psi}\right\Vert \,,
\end{equation}
which is also commonly known as $2$-norm or spectral norm~\citep{RobertsVarberg1973,peres1995quantum,GolubVanLoan1996,Foucart2012}.

\begin{thm}[A finite-precision extension of weak Gleason's theorem]\label{thm:Finite-precision-Gleason}Consider
a three-dimensional Hilbert space, and an interval-valued function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
then the norm of their difference is $\left\Vert \rho-\rho'\right\Vert \le\delta$.\end{thm}
...
\begin{thm}\label{thm:convex-uncountable}Let 
\begin{equation}
\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} \,.
\end{equation}
For any function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
described in Thm.~\ref{thm:iota-statements}, either $\mathscr{I}=\mathscr{I}_{0}$
...
is Wright-convex, i.e., $\iotal\left(x_{0}+x_{1}+x_{2}\right)+\iotal\left(x_{2}\right)\ge\iotal\left(x_{0}+x_{2}\right)+\iotal\left(x_{1}+x_{2}\right)$~\citep{Wright1954,RobertsVarberg1973,PecaricTong1992}.
...
Therefore, either $\iota$ maps every number to the same interval,
or the number of intervals that $\iota$ maps to must be uncountable.
