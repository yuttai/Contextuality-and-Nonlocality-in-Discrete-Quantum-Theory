...
be closer and closer to $\rho$, and vise visa. In another word, although
we can prove a finite precision version of Gleason's theorem, there
might be more rich phenomenon regarding to quantum probability when
considering finite precision measurement. Our result also highlight
the necessity to develop a mathematical theory of quantum mechanics
that intrinsically consistent with the resources available for realistic
measurement.
...
are non-overlapping. See Table~\ref{tab:I_2} for a concrete example
when $n=2$.}
...
Eqs.(\ref{eq:QIVPM-constraints}), the condition really need to be
...
$n=2$, two states~$\rho_{0}=\left(\proj{0}+\proj{1}+\proj{2}\right)/3$
...
is computed straightforwardly in Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
...
\noindent For example, $\rho_{0}$ and $\rho_{1}$ are both totally
mixed states. Since $\left\Vert \rho_{0}-\rho_{1}\right\Vert \le\frac{1}{24}$,
