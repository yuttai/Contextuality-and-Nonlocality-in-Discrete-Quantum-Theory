...
$|\alpha|=1$. \yutsung{Notice that we also have
$\mu_{1}=\mu_{\rho}^{\mathrm{B}}$ for a mixed state~$\rho=0.5\proj{0}+0.5\proj{1}$.
Maybe we should add ``Or other mixed states.''}
The expectation values of the observable $\mathbf{O}$,
...
the Born rule probability induced by every state in
...
Eq.~(\ref{eq:quantum-expectation}).
For example, we can extend one of the fragments of valid probability
measures~$\mu_{1}$ defined in Sec.~\ref{sec:fuzzy} to a QIVPM~$\bar{\mu}_{3}$:
$\bar{\mu}_{3}(\proj{0})=\left[0.5,0.5\right]$, $\bar{\mu}_{3}(\proj{1})=\left[0.5,0.5\right]$,
and $\bar{\mu}_{3}(\proj{2})=\left[0,0\right]$. For the same observable~$\mathbf{O}=\proj{0}+2\,\proj{1}+3\,\proj{2}$,
the Born rule probability~$\mu_{\rho}^{\mathrm{B}}$ of any state~$\rho$
consistent with $\bar{\mu}_{3}$ on $\proj{0}$, $\proj{1}$, and
$\proj{2}$ must be $\mu_{1}$. Therefore, the expectation value~$\expval{\mathbf{O}}_{\bar{\mu}_{3}}=\left[\expval{\mathbf{O}}_{\mu_{1}},\expval{\mathbf{O}}_{\mu_{1}}\right]=\left[1.5,1.5\right]$
...
Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}), \yutsung{Check whether
we can explain how to apply
Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}) in this situation
better.} we must have
...
the non-existence of certain QIVPMs, we also want to understand what
a QIVPM could be when it exists. In Sec.~\ref{sec:Interval-Uncertainty},
the only non-trivial QIVPM~ $\bar{\mu}_{3}$ is induced by the quantum
probability measure~$\mu_{1}$ defined in Sec.~\ref{sec:fuzzy}.
This result will first be generalized, and then we will show some
QIVPMs may not be induced by any classical IVPM, i.e., no state is
consistent with these kind of QIVPMs.

The quantum probability measure~$\mu_{1}$ maps some projectors to
$0$ or $0.5$, which are replaced by $\left[0,0\right]$ or $\left[0.5,0.5\right]$
to get $\bar{\mu}_{3}$. The way we replace two numbers by the smallest
intervals containing them can be generalized if we carefully pick
a set of intervals and the rule to select the correct interval when
intervals are overlapping. For example, consider the following collection
of intervals: 
\begin{eqnarray}
\mathscr{I}_{n} & = & \set{\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\cap\left[0,1\right]}{k=0,\ldots,2n}\nonumber \\
 &  & \cup\{\imposs,\necess\}
\end{eqnarray}
...
which also contains an concrete example for $\mathscr{I}_{2}$.
\begin{table}
...
\begin{tabular}{cccc}
\toprule 
\multicolumn{2}{c}{$\mathscr{I}_{n}$} & \multicolumn{2}{c}{$\mathscr{I}_{2}$}\tabularnewline
\midrule 
$[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$ & $[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$\tabularnewline
\midrule
\midrule 
$\imposs$ & $\imposs$ & $\imposs$ & $\imposs$\tabularnewline
\midrule 
$\left[0,\tfrac{1}{2n}\right]$ & $\left(0,\tfrac{0.5}{2n}\right]$ & $\left[0,\tfrac{1}{4}\right]$ & $\left(0,\tfrac{1}{8}\right]$\tabularnewline
\midrule 
$\begin{gathered}\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\\
\textrm{for odd }k
\end{gathered}
$ & $\left(\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right)$ & $\left[0,{\textstyle \tfrac{1}{2}}\right]$ & $\left(\tfrac{1}{8},\tfrac{3}{8}\right)$\tabularnewline
\midrule 
\multirow{2}{*}{$\begin{gathered}\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\\
\textrm{for even }k\\
0<k<2n
\end{gathered}
$} & \multirow{2}{*}{$\left[\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right]$} & $\left[\tfrac{1}{4},\tfrac{3}{4}\right]$ & $\left[\tfrac{3}{8},\tfrac{5}{8}\right]$\tabularnewline
\cmidrule{3-4} 
 &  & $\left[\tfrac{1}{2},1\right]$ & $\left(\tfrac{5}{8},\tfrac{7}{8}\right)$\tabularnewline
\midrule 
$\left[\tfrac{2n-1}{2n},1\right]$ & $\left[\tfrac{2n-0.5}{2n},1\right)$ & $\left[\tfrac{1}{4},1\right]$ & $\left[\tfrac{7}{8},1\right)$\tabularnewline
\midrule 
$\necess$ & $\necess$ & $\necess$ & $\necess$\tabularnewline
\bottomrule
\end{tabular}
...
After we have a rule to replace a real number by an interval, any
Born rule probability~$\mu_{\rho}^{\mathrm{B}}$ on a three-dimensional
Hilbert space can be extended to a QIVPM~$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$,
where $\circ$ is the function concatenation. For example, when $n=2$,
two quantum probability measure~$\mu_{1}$ and $\mu_{\mathbb{1}/3}^{\mathrm{B}}$
can induce two QIVPMs, where $\mathbb{1}/3=\left(\proj{0}+\proj{1}+\proj{2}\right)/3$.
The interval-valued probability of the induced QIVPMs, $\iota_{2}\left(\mu_{1}\left(P\right)\right)$
and $\iota_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$,
for projector $P$ in the minimal subspace of events containing $\proj{0}$,
$\proj{1}$, and $\proj{2}$, $\events_{C}$, are computed straightforwardly
in Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\iota_{2}\circ\mu_{1}$ and
$\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$ based on $\mu_{1}$
and $\mu_{\mathbb{1}/3}^{\mathrm{B}}$ on $\events_{C}$.}
\begin{tabular}{ccccc}
\toprule 
$P$ & $\mu_{1}\left(P\right)$ & $\iota_{2}\left(\mu_{1}\left(P\right)\right)$ & $\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)$ & $\iota_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$\tabularnewline
\midrule
\midrule 
$\mathbb{0}$ & $0$ & $\imposs$ & $0$ & $\imposs$\tabularnewline
\midrule 
$\proj{0}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$ & $\frac{1}{3}$ & $\left[0,\frac{1}{2}\right]$\tabularnewline
\midrule 
$\proj{1}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$ & $\frac{1}{3}$ & $\left[0,\frac{1}{2}\right]$\tabularnewline
\midrule 
$\proj{2}$ & $0$ & $\imposs$ & $\frac{1}{3}$ & $\left[0,\frac{1}{2}\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{0}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$ & $\frac{2}{3}$ & $\left[\frac{1}{2},1\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{1}$ & $0.5$ & $\left[\frac{1}{4},\frac{3}{4}\right]$ & $\frac{2}{3}$ & $\left[\frac{1}{2},1\right]$\tabularnewline
\midrule 
$\mathbb{1}-\proj{2}$ & $1$ & $\necess$ & $\frac{2}{3}$ & $\left[\frac{1}{2},1\right]$\tabularnewline
\midrule 
$\mathbb{1}$ & $1$ & $\necess$ & $1$ & $\necess$\tabularnewline
\bottomrule
\end{tabular}
\end{table}
Verifying $\iota_{2}\circ\mu_{1}$ and $\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
are QIVPMs are also straightforward.

A state~$\rho$ is called totally mixed state if it satisfies $0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$
for any $P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $.
Given any totally mixed state~$\rho$, we know not only $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
is a QIVPM, but also the collection of states consistent it get closer
...
precise real numbers. To state this result, we need to define a notion
of distance based on the following norm of a matrix~$A$ as $\left\Vert A\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{A}{\psi}\right|$~\citep{544199}.
If a state~$\rho'$ is consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$,
its distance with $\rho$, $\left\Vert \rho-\rho'\right\Vert $, must
be bounded by $1/n$. Reversely, if the distance between $\rho$ and
$\rho'$ is bounded by $\left(12n\right)^{-1}$, then $\rho'$ is
consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$. For example,
since $\mu_{1}\left(\proj{2}\right)=0$, any state corresponds to
$\mu_{1}$ is not totally mixed state. On the other hand, $\mathbb{1}/3$
is a totally mixed state. Consider $\rho'=\left(\frac{1}{3}+\frac{1}{24}\right)\proj{0}+\left(\frac{1}{3}-\frac{1}{24}\right)\proj{1}+\frac{1}{3}\cdot\proj{2}$.
Since $\left\Vert \rho-\rho'\right\Vert \le\frac{1}{12\cdot2}$, we
should have $\rho'\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}},\events\right)$.
Although verifying the general case is non-trivial, it is easy to
see $\mu_{\rho'}^{\mathrm{B}}\left(P\right)\in\iota_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$
for $P\in\events_{C}$. As $n$ goes to infinity, only the states
very close to $\rho$ is guarantied belong to $\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}},\events\right)$,
...
$\set{\left[x,x\right]}{x\in\left[0,1\right]}$, even if a QIVPM is
not \emph{a priori} known to be induced by a state, it can be recast
as a real-valued one and will have a unique state consistent with
it on every projector. This fact is called Gleason's theorem.

\begin{thm}[Gleason~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum}]\label{cor:Gleason's}In
a Hilbert space $\Hilb$ of dimension $d\geq3$, given a quantum probability
measure~$\mu:\events\rightarrow[0,1]$, there exists a unique state~$\rho$
such that $\mu=\mu_{\rho}^{\mathrm{B}}$. \end{thm}

\noindent However, if the intervals are only with finite-precision,
there are some QIVPMs which cannot be consistent with any state. Given
a three-dimensional Hilbert space $\Hilb$ with an orthonormal basis
$\left\{ \ket{0},\ket{1},\ket{2}\right\} $, let $\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} $,
