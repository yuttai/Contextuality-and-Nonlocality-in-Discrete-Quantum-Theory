...
On the one hand, $\iota_{1}$ defined in Eq.~(\ref{eq:iota}) satisfies
Eqs.~(\ref{eq:iota-constraints}) and Eq.~(\ref{eq:iota-add})~\citep{HOSTunpublished}
so that every function~$\iota_{1}\circ\mu$ on a three-dimensional
Hilbert space is a QIVPM. On the other hand, $\iota_{1}\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
is not a QIVPM on four-dimensional Hilbert space. Therefore, we need
to strengthen Eqs.~(\ref{eq:iota-constraints}) and Eq.~(\ref{eq:iota-add})
to guarantee $\iota\circ\mu$ is always a QIVPM.
...
and the right-end function~$\iotar:\left[0,1\right]\rightarrow\left[0,1\right]$
...
\begin{thm}For any function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
described in Thm.~\ref{thm:iota-statements}, either $\mathscr{I}=\left\{ \necess,\imposs,\unknown\right\} $
