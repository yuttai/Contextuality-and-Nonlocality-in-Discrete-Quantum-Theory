...
Let $S_{1}=\left\{ \proj{0},\proj{\ps}\right\} $. Because $\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps})=\imposs$,
we have $\mu_{\rho}^{\mathrm{B}}(\proj{0})=\mu_{\rho}^{\mathrm{B}}(\proj{\ps})=0$
for any mixed state~$\rho\in\coreBorn\left(\bar{\mu},S_{1}\right)$.
Hence, $\rho$ is actually $\ket{2}$, the only pure state orthogonal
to both $\ket{0}$ and $\ket{\ps}$. It is easy to verify $\proj{2}$
is actually in $\coreBorn\left(\bar{\mu},S_{1}\right)$.
