...
theorem\nb\cite{kochenspecker1967,peres1995quantum}. In particular,
the Kochen-Specker theorem is based on the two sphere\nb$S^{2}$
cannot be colored by red and green such that any orthogonal basis
has exactly one red vector. On the one hand, Meyer argued that the
rational dense subset of $S^{2}$, $S^{2}\cap\mathbb{Q}^{3}$, is
colorable, and we cannot distinguish $S^{2}$ and $S^{2}\cap\mathbb{Q}^{3}$
since finite precision measurement\nb\citep{PhysRevLett.83.3751,Szangolies2015}.
On the another hand, Mermin argued any physical outcome must be a
continuous function on the experimental configuration so that small
experimental imperfection should not ``nullify'' an idealized theorem\nb\citep{Mermin1999,Szangolies2015}.
Discussion
...
close to $\rho$ when we shirk the length of intervals under some
good conditions, and recover original Gleason's theorem in the limiting
case. Since Gleason's theorem cannot apply on QIVPMs in general, it
thus remains possible that a refinement of the basic definition of
a quantum state or the computation of probability from a state may
be required when we rigorously consider the implications of finite
