...
of event~$\events'$. In particular, if $\events'$ commutes, the
classical Thm.~\ref{thm:Shapley} claims that there is always a state~$\rho$
consistent with $\bar{\mu}$. In this section, we want to shift our
attention to consider state consistent with $\bar{\mu}$ on every
event, $\coreBorn\left(\bar{\mu},\events\right)$. While the intervals
are infinitely precise, i.e., $\mathscr{I}_{\infty}=\set{\left[x,x\right]}{x\in\left[0,1\right]}$,
Gleason's theorem claimed that there is always a unique consistent
...
intervals, there are some QIVPMs that cannot be consistent with any
state.
...
In other words, if the measurement intervals are too unprecise, the
results we get might contradict to each other, and exclude all possible
states consistent with the results.
