...
precise measurement should be close to idealized measurement, and
cannot nullify the Kochen-Specker theorem. This idea can be modeled
precisely by considering the following kind of QIVPMs.

\begin{definition}\label{def:delta-deterministic}A QIVPM\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
is called $\delta$-deterministic if either $\bar{\mu}\left(P\right)\subseteq\left[0,\delta\right]$
or $\bar{\mu}\left(P\right)\subseteq\left[1-\delta,1\right]$ for
any $P\in\events$.\end{definition}

For example, any cryptodeterministic QIVPM is $0$-deterministic.
As another example, a $\frac{1}{3}$-deterministic QIVPM can be given
by $\bar{\mu}\left(P\right)=\left[x/3,x/3\right]$, where $P$ projects
onto a $x$-dimensional subspace in a three-dimensional Hilbert space.
By utilizing $\delta$-deterministic QIVPMs, we can have a Kochen-Specker
theorem with finite precision measurement.

\begin{thm}\label{cor:Kochen-Specker-IVPM}Given a Hilbert space
of dimension\nb$d\ge3$, there is no $\delta$-deterministic QIVPM
for $\delta<\frac{1}{3}$.\end{thm}

\begin{proof}Suppose there is a $\delta$-deterministic QIVPM\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$,
we can define a cryptodeterministic QIVPM\nb$\bar{\mu}^{\textrm{D}}:\events\rightarrow\left\{ \imposs,\necess\right\} $
by 
\begin{equation}
...
\end{cases}
\end{equation}
However, there is no cryptodeterministic QIVPM as we discussed according
to the Mermin-Peres square, so that there is no $\delta$-deterministic
QIVPM.

The essential part to verify $\bar{\mu}^{\textrm{D}}$ is a cryptodeterministic
QIVPM is to verify a special case of Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}),\begin{subequations}
\begin{eqnarray}
...
By discussing the remaining cases, we can easily verify that $\bar{\mu}^{\textrm{D}}$
is a cryptodeterministic QIVPM.\end{proof}
...
Although Gleason's theorem doesn't hold for every quantum IVPM, if
a quantum IVPM\nb$\bar{\mu}$ is induced by a totally mixed state\nb$\rho$
nicely, then whether another density matrix\nb$\rho'\in\coreBorn\left(\bar{\mu},\events'\right)$
can be estimated by $\left\Vert \rho-\rho'\right\Vert $, where a
state\nb$\rho$ is called totally mixed if $0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$
for any $P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $,
and the the norm of a matrix\nb$A$ is defined by $\left\Vert A\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{A}{\psi}\right|$\nb\citep{544199}.
For example, consider the intervals\nb$\widehat{\mathscr{I}}_{n}=\set{\left[\frac{k-1}{2n},\frac{k+1}{2n}\right]\cap\left[0,1\right]}{k=0,\ldots,2n}\cup\left\{ \left[0,0\right],\left[1,1\right]\right\} $
...
\end{itemize}
In summary, the states consistent with $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$
will be closer and closer to $\rho$ as $n$ goes to infinity. In
a limit case when the intervals are infinitely precise, i.e., $\set{\left[x,x\right]}{x\in\left[0,1\right]}$,
