...
$|\alpha|=1$. \yutsung{Notice that we also have \mu_{1}=\mu_{\rho}^{\mathrm{B}}
for a mixed state \rho=0.5\proj{0}+0.5\proj{1}. Maybe we should add ``Or other mixed states.''} 
...
be replaced by the interval containing them if we carefully pick a
set of overlapping intervals. For example, consider the following
...
at length~$\left(2n\right)^{-1}$ at each end. Then, we can define
a QIVPM mapping to $\widehat{\mathscr{I}}_{n}$ based on any Born
rule probability~$\mu_{\rho}^{\mathrm{B}}$ on a three-dimensional
Hilbert space by replacing the probability~$\mu_{\rho}^{\mathrm{B}}\left(P\right)$
with the interval containing it, $\widehat{\iota}_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$.
Since intervals in $\widehat{\mathscr{I}}_{n}$ are overlapping, if
$\mu_{\rho}^{\mathrm{B}}\left(P\right)$ are contained in both intervals~$[\ell_{1},r_{1}]$
and $[\ell_{2},r_{2}]$, $\widehat{\iota}_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$
will be $[\ell_{i},r_{i}]$ if $\widehat{\iota}_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$
is closer to the center of $[\ell_{i},r_{i}]$\yutsung{{Verify...}}.
When there is a tie, we set $\widehat{\iota}_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)=\left[\frac{k-1}{2n},\frac{k+1}{2n}\right]\cap\left[0,1\right]$
for even $k$. For example, when $n=2$, 
\begin{eqnarray}
\widehat{\mathscr{I}}_{2} & = & \set{\left[\frac{k-1}{4},\frac{k+1}{4}\right]\cap\left[0,1\right]}{k=0,\ldots,4}\nonumber \\
 &  & \cup\{\imposs,\necess\}\nonumber \\
 & = & \left\{ \imposs,\left[0,\frac{1}{4}\right],\left[0,\frac{1}{2}\right],\left[\frac{1}{4},\frac{3}{4}\right],\right.\nonumber \\
 &  & \left.\left[\frac{1}{2},1\right],\left[\frac{1}{4},1\right],\necess\right\} \,.
\end{eqnarray}
It is straightforward to verify $\widehat{\iota}_{2}\circ\mu_{1}$
and $\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$ are
QIVPMs whose values on the minimal subspace of events containing $\proj{0}$,
$\proj{1}$, and $\proj{2}$, $\events_{C}$, is computed by Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\widehat{\iota}_{2}\circ\mu_{1}$
and $\widehat{\iota}_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$ based
on $\mu_{1}$ and $\mu_{\mathbb{1}/3}^{\mathrm{B}}$ on $\events_{C}$.}
\begin{tabular}{ccccc}
\toprule 
$P$ & $\mu_{1}\left(P\right)$ & $\widehat{\iota}_{2}\left(\mu_{1}\left(P\right)\right)$ & $\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)$ & $\widehat{\iota}_{2}\left(\mu_{\mathbb{1}/3}^{\mathrm{B}}\left(P\right)\right)$\tabularnewline
\midrule
\midrule 
...
called totally mixed state, we know not only $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$
...
If a state~$\rho'$ is consistent with $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$,
...
with $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$. In other
words, for the states consistent with this QIVPM, $\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}}$
