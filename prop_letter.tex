...
some other proper sub-event spaces. Formally, given a quantum IVPM~$\bar{\mu}$,
the \emph{core} relative to a sub-event space~$\events'\subseteq\events$
as follow.\footnote{\yutsung{I changed the definition of core to embedding the Born rule.
While this change might make it easier to explain the core of quantum
IVPMs, it could increase the difficulty to explain classical IVPMs
because we might need to explain the meaning of the Born rule for
...
\end{equation}
For example, $\mathrm{core}\left(\iota_{\infty}\circ\mu_{0},\events_{0}\right)=\left\{ \proj{\ps}\right\} $
and $\mathrm{core}\left(\iota_{\infty}\circ\mu_{0},\events\right)=\emptyset$,
where $\circ$ denote function composition, and $\iota_{\infty}$
is defined in Eq.~(\ref{eq:iota_infty}).
