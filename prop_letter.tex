...
with it on some of the projectors. However, if we only focus on the
Born QIVPMs~$\bar{\mu}$, that is, QIVPM's induced by a state~$\rho$
...
be closer and closer to $\rho$, and vise visa.
...
\begin{equation}
\iota_{n}\left(x_{0}+x_{1}\right)\subseteq\iota_{n}\left(x_{0}\right)+\iota_{n}\left(x_{1}\right)\,.
\end{equation}
...
two possibilities:
\begin{itemize}
\item Their complements are orthogonal reducing to the case we discussed.
\item One of them projects onto the subspace which the other one projects
onto.
\end{itemize}
\noindent Therefore, $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$ is indeed
a QIVPM. For example, when $n=2$, two states~$\mathbb{1}/3$ and
