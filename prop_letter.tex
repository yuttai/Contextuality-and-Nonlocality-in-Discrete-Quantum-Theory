...
\yutsung{{Even if we have expectation value of }an observable, I
don't know how to reason Mermin square~(\ref{eq:MerminSquare}) directly
with quantum IVPM so far. The problem is the valuation function~$v$
is not an expectation value. Since the corresponding of Eq.~(\ref{eq:deterministic-product}),
$E\left[XY\right]=E\left[X\right]E\left[Y\right]$, doesn't hold for
expectation value except $X$ and $Y$ are uncorrelated or even independent~\cite{rohatgi2011introduction}.}
