...
to get $\bar{\mu}_{3}$. This is replacing two numbers by the smallest
intervals containing them. In general, any set of the numbers can
be replaced by an interval containing them if we carefully pick a
set of intervals and the rule to select the correct interval when
...
are overlapping, we define the interior part of these intervals

Then, we can define a QIVPM mapping to $\mathscr{I}_{n}$ based on
any Born rule probability~$\mu_{\rho}^{\mathrm{B}}$ on a three-dimensional
Hilbert space by replacing the probability~$\mu_{\rho}^{\mathrm{B}}\left(P\right)$
with the interval containing it, $\iota_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$.
Since intervals in $\mathscr{I}_{n}$ are overlapping, if $\mu_{\rho}^{\mathrm{B}}\left(P\right)$
are contained in both intervals~$[\ell_{1},r_{1}]$ and $[\ell_{2},r_{2}]$,
$\iota_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$ will
be $[\ell_{i},r_{i}]$ if $\iota_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)$
is closer to the center of $[\ell_{i},r_{i}]$\yutsung{Verify...}.
When there is a tie, we set $\iota_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(P\right)\right)=\left[\frac{k-1}{2n},\frac{k+1}{2n}\right]\cap\left[0,1\right]$
for even $k$. For example, when $n=2$, 
\begin{eqnarray}
\mathscr{I}_{2} & = & \set{\left[\tfrac{k-1}{4},\tfrac{k+1}{4}\right]\cap\left[0,1\right]}{k=0,\ldots,4}\nonumber \\
 &  & \cup\{\imposs,\necess\}\nonumber \\
 & = & \left\{ \imposs,\left[0,\tfrac{1}{4}\right],\left[0,{\textstyle \tfrac{1}{2}}\right],\left[\tfrac{1}{4},\tfrac{3}{4}\right],\right.\nonumber \\
 &  & \left.\left[\tfrac{1}{2},1\right],\left[\tfrac{1}{4},1\right],\necess\right\} \,.
\end{eqnarray}
It is straightforward to verify $\iota_{2}\circ\mu_{1}$ and $\iota_{2}\circ\mu_{\mathbb{1}/3}^{\mathrm{B}}$
are QIVPMs whose values on the minimal subspace of events containing
$\proj{0}$, $\proj{1}$, and $\proj{2}$, $\events_{C}$, is computed
by Table~\ref{tab:Compute-the-QIVPM}. 
\begin{table}
...
\midrule
\midrule 
$\mathbb{0}$ & $0$ & $\left[0,0\right]$ & $0$ & $\left[0,0\right]$\tabularnewline
\midrule 
...
\midrule 
$\proj{2}$ & $0$ & $\left[0,0\right]$ & $\frac{1}{3}$ & $\left[0,\frac{1}{2}\right]$\tabularnewline
\midrule 
...
\midrule 
$\mathbb{1}-\proj{2}$ & $1$ & $\left[1,1\right]$ & $\frac{2}{3}$ & $\left[\frac{1}{2},1\right]$\tabularnewline
\midrule 
$\mathbb{1}$ & $1$ & $\left[1,1\right]$ & $1$ & $\left[1,1\right]$\tabularnewline
\bottomrule
\end{tabular}
\end{table}
...
be bounded by $1/\left(12n\right)$. Reversely, if the distance between
$\rho$ and $\rho'$ is bounded by $1/n$, then $\rho'$ is consistent
with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$. In other words, for
the states consistent with this QIVPM, $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
