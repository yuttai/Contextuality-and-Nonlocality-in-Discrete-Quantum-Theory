...
the Gaussian width of a probability distribution, but for notational
convenience we will continue in the following to write this as a definite interval.
...
\yutsung{Notice that the use of quantum is minimized in this section.
In another word, the results of this section should apply to both
quantum and classical IVPMs.}

Recall in Sec.~\ref{sec:fuzzy}, we illustrate the non-constructive
postulates for the quantum probability measures first, and then introduce
the Born rule which induces a quantum probability measures from each
state. Since the postulates for QIVPMs in Sec.~\ref{sec:Interval-Uncertainty}
are also non-constructive, we want to find a simple rule to construct
a QIVPM as well. When we generalized quantum probability measures
to QIVPMs, we changed the range from the unit interval~$\left[0,1\right]$
to a set of intervals~$\mathscr{I}$. This replacement suggests us
to find a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
which can be used to replace a real-valued probability~$\mu\left(P\right)$
by an interval-valued one~$\iota\left(\mu\left(P\right)\right)$.
In other words, compositing $\iota$ with a quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
produces an interval-valued function~$\iota\circ\mu:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$
which has the same domain and range as a QIVPM. However, the composited
function~$\iota\circ\mu$ may or may not be a QIVPM. For example,
in a four-dimensional Hilbert space, consider the function 
\begin{equation}
\iota_{1}\left(x\right)=\begin{cases}
\imposs\,, & \textrm{ if }x=0\,;\\
\left[0,\tfrac{1}{2}\right]\,, & \textrm{ if }x\in\left(0,\tfrac{1}{4}\right]\,;\\
\unknown\,, & \textrm{ if }x\in\left(\tfrac{1}{4},\tfrac{3}{4}\right)\,;\\
\left[\tfrac{1}{2},1\right]\,, & \textrm{ if }x\in\left[\tfrac{3}{4},1\right)\,;\\
\necess\,, & \textrm{ if }x=1\,.
\end{cases}\label{eq:iota}
\end{equation}
While the function~$\iota_{1}\circ\mu_{\rho_{0}}^{\mathrm{\mathrm{B}}}$
induced by $\rho_{0}=\frac{1}{4}\cdot\proj{0}+\frac{1}{4}\cdot\proj{1}+\frac{1}{4}\cdot\proj{2}+\frac{1}{4}\cdot\proj{3}$
is a QIVPM, the function~$\iota_{1}\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
induced by $\rho_{1}=\frac{1}{8}\cdot\proj{0}+\frac{1}{8}\cdot\proj{1}+\frac{1}{8}\cdot\proj{2}+\frac{5}{8}\cdot\proj{3}$
is \emph{not} a QIVPM~\citep{HOSTunpublished}. In order not to verify
whether $\iota\circ\mu$ is a QIVPM for each individual case, it is
desirable to have some criteria to discriminate different cases. We
will start from the simpler criteria for the dimension~$d\le3$,
and then the general case.

\begin{thm}\label{thm:QRVPM2QIVPM-3}In a Hilbert space of dimension~$d\le3$,
if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$ is subject to the
following constraints:\begin{subequations}\label{eq:iota-constraints}
\begin{eqnarray}
 &  & \iota(0)=\imposs\,,\\
 &  & \iota(1)=\necess\,,\\
 &  & \iota\left(1-x\right)=\necess-\iota\left(x\right)\,,
\end{eqnarray}
\end{subequations}and satisfies that for any numbers~$x_{0}$ and
$x_{1}\in\left[0,1\right]$, $x_{0}+x_{1}\in\left[0,1\right]$ implies
\begin{equation}
\iota\left(x_{0}+x_{1}\right)\subseteq\iota\left(x_{0}\right)+\iota\left(x_{1}\right)\,.\label{eq:iota-add}
\end{equation}
Then, $\iota\circ\mu:\events\rightarrow\mathscr{I}$ is always a QIVPM
for any quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$.\end{thm}

The situation for $d=2$ is trivial. On a three-dimensional Hilbert
space, to prove that $\iota\circ\mu$ is a QIVPM, it is sufficient
to check Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}):
\begin{equation}
\begin{aligned} & \iota\left(\mu\left(P_{0}+P_{1}-P_{0}P_{1}\right)\right)+\iota\left(\mu\left(P_{0}P_{1}\right)\right)\\
\subseteq{} & \iota\left(\mu\left(P_{0}\right)\right)+\iota\left(\mu\left(P_{1}\right)\right)\,.
\end{aligned}
\label{eq:QuantumInterval-valuedProbability-Inclusion-case}
\end{equation}
 Depending on the relation between $P_{0}$ and $P_{1}$, we have
three possibilities:
\begin{itemize}
\item $P_{0}$ and $P_{1}$ are orthogonal.
\item Their complements~$\mathbb{1}-P_{0}$ and $\mathbb{1}-P_{1}$ are
orthogonal.
\item One of them projects onto a subspace of what the other one projects
onto.
\end{itemize}
\noindent All cases deduce Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion-case})
easily~\citep{HOSTunpublished} so that $\iota\circ\mu$ is indeed
a QIVPM.

Since $\iota_{1}$ defined in Eq.~(\ref{eq:iota}) satisfies Eqs.~(\ref{eq:iota-constraints})
and Eq.~(\ref{eq:iota-add})~\citep{HOSTunpublished}, every function~$\iota_{1}\circ\mu$
on a three-dimensional Hilbert space is a QIVPM. However, on four-dimensional
Hilbert space, $\iota_{1}\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
is not a QIVPM. This highlights the necessity of the conditions to
guarantee $\iota\circ\mu$ is always a QIVPM.

\begin{thm}\label{thm:iota-statements}The following two statements
are equivalent:
\begin{enumerate}
\item \label{enu:iota-subject-to}The function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
is subject to Eqs.~(\ref{eq:iota-constraints}) and satisfies that
for any three numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$,
$x_{0}+x_{1}+x_{2}\in\left[0,1\right]$ implies
\begin{equation}
\begin{aligned} & \iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)\\
\subseteq{} & \iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)\,.
\end{aligned}
\label{eq:iota-Inclusion}
\end{equation}
...
It is clear that Statement~\ref{enu:iota-subject-to} implies Statement~\ref{enu:iota-mu-QIVPM},
and the rest of the proof is to prove that if Statement~\ref{enu:iota-subject-to}
is false, then Statement~\ref{enu:iota-mu-QIVPM} is false as well.
If $\iota$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}), we can
find three particular numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
such that $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, but $\iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)\nsubseteq\iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)$.
Consider the state $\rho=x_{0}\proj{0}+x_{1}\proj{1}+x_{2}\proj{2}+\left(1-x_{0}-x_{1}-x_{2}\right)\proj{3}$.
Then, the function~$\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}:\events\rightarrow\mathscr{I}$
...
\begin{eqnarray*}
 ...
 & \nsubseteq & \iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{0}+\proj{2}\right)\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{1}+\proj{2}\right)\right)\,,
\end{eqnarray*}
and cannot be a QIVPM. Therefore, Statement~\ref{enu:iota-subject-to}
is true if and only if Statement~\ref{enu:iota-mu-QIVPM} is true.

Since $\iota_{1}\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$ is not
a QIVPM, we know $\iota_{1}$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}).
In order to find a function~$\iota$ satisfying Eq.~(\ref{eq:iota-Inclusion}),
we decompose it into two functions: its left-end and right-end~$\left[\iotal\left(x\right),\iotar\left(x\right)\right]=\iota\left(x\right)$.
By Eq.~(\ref{eq:iota-Inclusion}), the left-end~$\iotal:\left[0,1\right]\rightarrow\left[0,1\right]$
is Wright-convex, i.e., $\iotal\left(x_{0}+x_{1}+x_{2}\right)+\iotal\left(x_{2}\right)\ge\iotal\left(x_{0}+x_{2}\right)+\iotal\left(x_{1}+x_{2}\right)$,
and the right-end~$\iotar:\left[0,1\right]\rightarrow\left[0,1\right]$
is Wright-concave, i.e., $\iotar\left(x_{0}+x_{1}+x_{2}\right)+\iotar\left(x_{2}\right)\le\iotar\left(x_{0}+x_{2}\right)+\iotar\left(x_{1}+x_{2}\right)$~\citep{Wright1954,RobertsVarberg1973,PecaricTong1992,Brzdek_2014}.
It is well-known that a Wright-convex function is midpoint-convex
or Jensen-convex~\citep{Klee_1956,Kenyon_1956,RobertsVarberg1973,PecaricTong1992},
i.e., we have
\begin{equation}
...
$\iotal$ must be continuous on the unit open interval~$\left(0,1\right)$~\citep{Jensen_1906,RobertsVarberg1973}
which implies the number of intervals that $\iota$ maps to must be
uncountable.

\begin{thm}If $\iota$ satisfies the Statements in Thm.~\ref{thm:iota-statements},
then the range of $\iota$ either is $\left\{ \necess,\imposs,\unknown\right\} $
or contains uncountably many intervals.\end{thm}

\yutsung{Another idea might be fixing a quantum probability measure~$\mu$,
whether there is a length of intervals, such that if $\iota$ mapping
to a interval tighter than the given length, then $\iota\circ\mu$
is always a QIVPM... I don't know the answer of this question.}
...
consistent with $\bar{\mu}$ on \emph{every} event, $\coreBorn\left(\bar{\mu},\events\right)$,
...
quantum probability measure~\citep{HOSTunpublished}. In this situation,
Gleason's theorem claims that there is always a unique state consistent
with $\bar{\mu}$~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum},
and this state can easily induce $\bar{\mu}$ according to the Born
rule~\citep{Born1983,peres1995quantum,544199,Jaeger2007}. However,
if we consider general QIVPMs mapping to an \emph{arbitrary} set of
intervals, there is a QIVPM that no state can be consistent with it.

\begin{thm}[Empty core for general QIVPMs]\label{thm:Non-extensible-of-Gleason's}In
a three-dimensional Hilbert space, there exists a QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$
...
In other words, if the measurement intervals are too imprecise, the
results we get might contradict to each other, and exclude any possible
state from being consistent with the measurement results. We will
address this problem with a concrete family of intervals and QIVPMs,
and leave for another time the general framework~\citep{HOSTunpublished}.
...
their interiors~$\textrm{Int}\left([\ell,r]\right)$ are non-overlapping.}
...
\end{tabular}
\end{table}
Since their interior parts are non-overlapping, we can define a function~$\iota_{n}:\left[0,1\right]\rightarrow\mathscr{I}_{n}$
by $\iota_{n}\left(x\right)=[\ell,r]$ if $x\in\textrm{Int}\left([\ell,r]\right)$
to map a real-valued probability to an interval-valued one. This function
can then induce a QIVPM from a state according to the following theorem:

\begin{thm}[A finite-precision Extension of the Born rule]\label{thm:Finite-precision-Born}For
...
is a QIVPM, called a Born QIVPM.\end{thm}

To prove that $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$ is a QIVPM,
it is sufficient to check Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}).
Given $x_{0}$, $x_{1}$, and $x_{0}+x_{1}\in\left[0,1\right]$,
\begin{equation}
\iota_{n}\left(x_{0}+x_{1}\right)\subseteq\iota_{n}\left(x_{0}\right)+\iota_{n}\left(x_{1}\right)\label{eq:iota-add-1}
\end{equation}
...
\begin{itemize}
\item Their complements~$\mathbb{1}-P_{0}$ and $\mathbb{1}-P_{1}$ are
orthogonal so that we can apply Eq.~(\ref{eq:iota-add-1}) on their
complements.
\item One of them projects onto a subspace of what the other one projects
onto.
\end{itemize}
\noindent Both cases deduce Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion})
easily~\citep{HOSTunpublished} so that $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$
is indeed a QIVPM.

In contrast with the QIVPM defined in Table~\ref{tab:non-Born-QIVPM},
there is always a state consistent with a Born QIVPM $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$
because $\rho\in\coreBorn\left(\left[\mu_{\rho}^{\mathrm{B}}\right]_{n},\events\right)$.
Moreover, with respect to a notion of distance between two states~$\rho$
and $\rho'$~\citep{544199}, 
\begin{equation}
\left\Vert \rho-\rho'\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{\rho-\rho'}{\psi}\right|\,,\label{eq:norm}
\end{equation}
...
\begin{thm}[A finite-precision extension of weak Gleason's theorem
for Born QIVPMs]\label{thm:Finite-precision-Gleason}In a three-dimensional
Hilbert space, given any state~$\rho$, if a state $\rho'\in\coreBorn\left(\left[\mu_{\rho}^{\mathrm{B}}\right]_{n},\events\right)$,
then their distance $\left\Vert \rho-\rho'\right\Vert \le\frac{1}{n}$.\end{thm}

\noindent Given a state~$\rho'$ consistent with $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$,
we have $\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\in\iota_{n}\left(\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right)$
for any 1-dimensional projector~$\proj{\psi}$. Since the maximum
length of the intervals in $\mathscr{I}_{n}$ is $1/n$, it is also
the upper bound of the difference $\left|\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right|=\left|\melem{\psi}{\rho-\rho'}{\psi}\right|$.
 This implies $\left\Vert \rho-\rho'\right\Vert \le\frac{1}{n}$ by
definition.

In the limiting case when the intervals are infinitely precise real
numbers covering the real line, the original Gleason theorem is recovered,
and can be formulated in our framework as follows.
...
\yutsung{Following are the discussion and personal notes about our
norm and other notation of norm and distance.}

Nielsen and Chuang~\citep{544199} defined their norm~(\ref{eq:norm})
in Appendix~6. While their Appendix~6 followed Simon~\citep{Simon2005}
and Uhlmann~\citep{Uhlmann1977} to prove Lieb's theorem~\citep{Lieb_1973,544199,Simon2005},
Simon~\citep{Simon2005} and Uhlmann~\citep{Uhlmann1977} didn't
explicitly define the norm as in Eq.~(\ref{eq:norm}) although Simon
used the same norm thoroughly in his book~\citep{Simon2005}. There
are two possibilities:
\begin{enumerate}
\item Simon really used Eq.~(\ref{eq:norm}) to define his norm.\\
If this is true, there must be some books cited by Simon, and defined
the norm in the same way.
\item The norm Simon used is different from Eq.~(\ref{eq:norm}), and Nielsen
and Chuang defined this norm from nowhere in order to write a proof
of Lieb's theorem.\\
If this is true, Nielsen and Chuang might be the \emph{only} literature
which used this norm. We might have no other literature to check the
properties of this norm and other distances, and need to prove the
properties of this norm by ourselves.
\end{enumerate}
In order to understand this norm and compare with others, the following
are useful facts. Let $\sigma\left(\rho-\rho'\right)$ denote the
set of eigenvalues of $\rho-\rho'$. Since $\rho$ and $\rho'$ are
states, they are Hermitian~\citep{544199}, and so does $\rho-\rho'$.
In this situation~\citep{544199}, 
\begin{equation}
\left\Vert \rho-\rho'\right\Vert =\max_{\lambda\in\sigma\left(\rho-\rho'\right)}\left|\lambda\right|
\end{equation}
which is just its spectral radius or spectral norm~\citep{GolubVanLoan1996,Foucart2012}
\begin{equation}
\max_{\left\Vert \ket{\psi}\right\Vert =1}\left\Vert \left(\rho-\rho'\right)\ket{\psi}\right\Vert \,.
\end{equation}
For their trace distance~\citep{544199}
\begin{equation}
D\left(\rho,\rho'\right)=\frac{1}{2}\Tr\left[\sqrt{\left(\rho-\rho'\right)^{2}}\right]=\frac{1}{2}\sum_{\lambda\in\sigma\left(\rho-\rho'\right)}\left|\lambda\right|\,,
\end{equation}
since it can also be represented as the eigenvalue of $\rho-\rho'$.
It should not be hard to estimate their trace distance based on their
norm.
