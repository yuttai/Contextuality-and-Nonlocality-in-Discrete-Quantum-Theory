...
states consistent with the measurement results. Although we will not
dig into the general abstract framework to avoid this problem here~\citep{HOSTunpublished},
...
\end{tabular}
\end{table}
Since the interior parts are non-overlapping, we can define a function~$\iota_{n}:\left[0,1\right]\rightarrow\mathscr{I}_{n}$
...
In the limiting case when the intervals are infinitely precise, the
original Gleason theorem is recovered, and can be formulated in our
