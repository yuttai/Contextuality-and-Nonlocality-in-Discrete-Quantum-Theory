...
where $s\in\left\{ +,-\right\} $ and $f\in\left\{ l,r\right\} $.
We can combine the classical Choquet integral~\cite{Vitali1925,Choquet1954,GilboaSchmeidler1994,Grabisch2016}
with (\ref{eq:quantum-expectation}) to define the expectation value
of $\mathbf{O}$ with respect to $\bar{\mu}$ as
\begin{eqnarray}
\expval{\mathbf{O}}_{\bar{\mu}} & = & \sum_{i=1}^{N^{+}}\lambda_{i}^{+}\left[\Delta l\left(P_{i}^{+\prime}\right),\Delta r\left(P_{i}^{+\prime}\right)\right]\\
 &  & +\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\left[\Delta l\left(P_{i}^{-\prime}\right),\Delta r\left(P_{i}^{-\prime}\right)\right]\textrm{ .}\label{eq:quantum-interval-expectation}
\end{eqnarray}
