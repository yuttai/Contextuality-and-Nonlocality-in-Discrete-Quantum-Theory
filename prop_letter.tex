...
there is always a state consistent with composited QIVPM~$\iota\circ\mu_{\rho}^{\mathrm{B}}$~\citep{HOSTunpublished}.
...
$\iota\circ\mu$ could only map to finite intervals. For example,
we can weaken the convex condition, Eq.~(\ref{eq:iota-Inclusion}),
...
\end{tabular}
\end{table}
Because of $\rho_{0}\in\coreBorn\left(\iota_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}},\events\right)$
and $\rho_{1}\in\coreBorn\left(\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$~\citep{HOSTunpublished},
Thm.~\ref{thm:Finite-precision-Gleason} asserts $\left\Vert \rho_{0}-\rho_{1}\right\Vert $
