...
are also non-constructive, we also want to find a simple rule to construct
a QIVPM. When we generalize quantum probability measures to QIVPMs,
we replace the unit interval range~$\left[0,1\right]$ by a set of
intervals~$\mathscr{I}$. This replacement suggests a function~$\iota$
mapping a number~$x\in\left[0,1\right]$ to an interval~$\iota\left(x\right)\in\mathscr{I}$.
The function~$\iota$ can then be concatenated with a quantum probability
measure~$\mu:\events\rightarrow\left[0,1\right]$ and easily get
an interval-valued function~$\iota\circ\mu:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$,
...
to have some easy criteria to get a QIVPM~$\iota\circ\mu$.
...
Suppose $\iota$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}),
...
\[
\begin{aligned} & \mathrel{\hphantom{\nsubseteq}}\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{0}+\proj{1}+\proj{2}\right)\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{2}\right)\right)\\
...
In order to find $\iota$ satisfying Eq.~(\ref{eq:iota-Inclusion}),
...
Notice that a Wright-convex function is midpoint-convex or Jensen-convex,
i.e., 
\begin{equation}
...
we cannot use the Axiom of Choices~\citep{Blumberg_1919,Sierpinski1920,RobertsVarberg1973}.
