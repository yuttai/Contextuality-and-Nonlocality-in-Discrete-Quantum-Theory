...
given a quantum real value probability measure~$\mu:\events\rightarrow\left[0,1\right]$,
$\bar{\mu}(P)=\iota\left(\mu(P)\right)$ is a quantum IVPM, and vice
versa. In another word, when quantum IVPMs with $\mathscr{I}_{\infty}$
are just the same as quantum real-valued probability measures which
can fit Gleason's theorem. 

However, quantum IVPMs certainly contains richer phenomena. For example,
we will show the Kochen-Specker theorem~\cite{kochenspecker1967,peres1995quantum,Redhead1987-REDINA}
...
the measurement resource, i.e., pick finer intervals when constructing
IVPMs, it would be natural to ask whether quantum IVPMs would approach
the Born rule. In order to understand this question, it is instructive
to consider the 2-dimensional non-Born quantum probability measure~$\mu:\events\rightarrow[0,1]$
defined as follow:
\begin{equation}
\mu(P)=\begin{cases}
\mu_{\ket{\ps}}^{B}(P) & \textrm{, if }P=\proj{\ps}\textrm{ or }\proj{\ms}\textrm{ ;}\\
\mu_{\ket{0}}^{B}(P) & \textrm{, otherwise.}
\end{cases}
\end{equation}
Notice that $\mu$ is consistent with the Born rule partially, because
it is computed according to different states when applying on different
projectors. \newpage{}
