...
as follow.\footnote{\yutsung{{I changed the definition of core} to density matrix. While
this change might make it easier to explain the core of quantum IVPMs,
it could increase the difficulty to explain classical IVPMs because
we might need to explain the meaning of density matrix and the Born
rule for classical probability measures...}}
\begin{equation}
...
is easy to verify $\ket{2}$ is actually in $\mathrm{core}\left(\bar{\mu},\events_{1}\right)$.

Similarly, let $\events_{2}$ be the minimal event space containing
both $\proj{0}$ and $\proj{\ps'}$. We have $\mathrm{core}\left(\bar{\mu},\events_{2}\right)=\left\{ \ket{1}\right\} $.
Therefore, 
\begin{equation}
