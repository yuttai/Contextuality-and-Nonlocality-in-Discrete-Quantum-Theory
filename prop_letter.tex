...
formulate Gleason's theorem in our framework as follow.
...
In another word, if a state~$\rho'$ is consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$,
its distance with $\rho$, $\left\Vert \rho-\rho'\right\Vert $, must
be bounded by $1/n$. Reversely, if the distance between $\rho$ and
$\rho'$ is bounded by $\frac{1}{12n}$, then $\rho'$ is consistent
with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$. 
...
\noindent For example, $\rho_{1}$ and $\rho_{0}$ are both totally
mixed states. Since $\left\Vert \rho_{1}-\rho_{0}\right\Vert \le\frac{1}{24}$,
...
$\rho$ are guaranteed to belong to $\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}},\events\right)$,
...
is a QIVPM. Next we will prove $\coreBorn\left(\bar{\mu},\events\right)$
...
Despite the non-extensibility of Gleason's theorem to general QIVPMs,
...
new phenomenon when we generalize both classical IVPMs and quantum
