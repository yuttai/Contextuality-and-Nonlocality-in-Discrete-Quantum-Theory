...
results we get might contradict to each other, and exclude the possibility
that any state is consistent with the measurement results. Although
...
QIVPMs. The following intervals 
\begin{equation}
\begin{aligned}\mathscr{I}_{n}= & \set{\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]\cap\left[0,1\right]}{k=0,\ldots,2n}\\
 & \cup\{\imposs,\necess\}
\end{aligned}
\end{equation}
are overlapping intervals with length~$1/n$ except for the intervals
with length~$\left(2n\right)^{-1}$ at each end. As shown in Table~\ref{tab:I_n},
for each interval~$[\ell,r]$, we define $\textrm{Int}\left([\ell,r]\right)$,
the non-overlapping interior part of $[\ell,r]$.
\begin{table}
...
gets closer and closer to $\rho$ with respect to $\left\Vert \cdot\right\Vert $
as the intervals get narrower and narrower.
...
$\Hilb$ of dimension $3$, given any state~$\rho$, if a state~$\rho'\in\coreBorn\left(\left[\mu_{\rho}^{\mathrm{B}}\right]_{n},\events\right)$,
then their distance~$\left\Vert \rho-\rho'\right\Vert \le\frac{1}{n}$.\end{thm}

\noindent Given state~$\rho'$ consistent with $\left[\mu_{\rho}^{\mathrm{B}}\right]_{n}$,
the difference between their Born quantum probabilities on any 1-dimensioal
projector~$\proj{\psi}$ is $\left|\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\right|$,
and must be bounded by $1/n$ which implies $\left\Vert \rho-\rho'\right\Vert \le\frac{1}{n}$
by definition.
