...
\end{aligned}
\end{equation}
Then, the function induced by $\rho$, $\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$,
...
we only consider composited QIVPMs~$\iota\circ\mu_{\rho}^{\mathrm{B}}$
instead of general QIVPMs~$\bar{\mu}$. To state this weak version
of Gleason's theorem, we need to introduce a few technical notions.
...
and a weak version of Gleason's theorem can be stated as follow.
...
On the other hand, 
\begin{eqnarray*}
\mu_{\rho_{1}}^{\mathrm{B}}\left(\proj{\psi}\right) & = & \tfrac{7}{24}\left|\ip{\psi}{0}\right|^{2}+\tfrac{1}{3}\left|\ip{\psi}{0}\right|^{2}+\tfrac{3}{8}\left|\ip{\psi}{0}\right|^{2}
\end{eqnarray*}
...
any two dimensional projector~$P$, we also have $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\in\iota_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$.
Hence, $\rho_{1}\in\coreBorn\left(\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$,
