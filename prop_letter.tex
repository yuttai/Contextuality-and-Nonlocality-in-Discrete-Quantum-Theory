...
\begin{abstract}
It is well-known that any quantum measurement can be only performed
in finite precision. However, quantum theory is usually based on infinitely
precise real and complex numbers, and physicists handle this imperfection
by statistical methods without understanding how finite precision
measurement affect the mathematical foundation of quantum theory.
Our purpose here is to examine a quantum model cooperating the idea
of finite precision at very beginning. Within our quantum interval-valued
probability measure (QIVPM) framework, we found a QIVPM become classical
when the projectors all commute, and the expectation value of an observable
can still be computed as expected. Two fundamental quantum theorems,
the Kochen-Specker and Gleason theorem, can be reformulated by QIVPM
given a particular choice of intervals, and we can discuss their validity
within a certain bound. Our research provides a new insight to quantify
the idea of finite precision measurement, and provides a possible
...
% exploiting the fact that a density matrix approach can be
% considered as equivalent to a description of quantum states
...
Kochen-Specker theorem, is shown to have a resolution in terms of the quantum interval-valued probability measure framework that we introduce.  To proceed, we use a formulation of the Mermin square that uses the
