...
\begin{condition}\label{cond:one-one}Given an orthonormal basis~$\Omega$,
then there exists exactly one $\ket{\psi_{i_{0}}}\in\Omega$ such
that $v\left(\proj{\psi_{i_{0}}}\right)=1$ and $v\left(\proj{\psi_{i}}\right)=0$
...
The natural KS assumption in 0-deterministic QIVPM is the one corresponding
to Condition~\ref{cond:sum-one}.
...
\begin{condition}[0-deterministic Variant of Condition~\ref{cond:one-one}]\label{cond:one-one-0-deterministicQIVPM}For
a $0$-deterministic QIVPMs~$\bar{\mu}$, given an orthonormal basis~$\Omega$,
then there exists exactly one $\ket{\psi_{i_{0}}}\in\Omega$ such
that $\bar{\mu}\left(\proj{\psi_{i_{0}}}\right)=\necess$ and $\bar{\mu}\left(\proj{\psi_{i}}\right)=\imposs$
for $i\ne i_{0}$.\end{condition}

\noindent The preceding condition is the key step to deduce the following
two conditions used in Mermin's proof.

\begin{condition}[0-deterministic Variant of Condition~\ref{cond:eigenvalue}]\label{cond:eigenvalue-0-deterministicQIVPM}For
a $0$-deterministic QIVPMs~$\bar{\mu}$, $\expval{\mathbf{O}}_{\bar{\mu}}$
must be an eigenvalue of $\mathbf{O}$.\end{condition}

\begin{condition}[0-deterministic Variant of Assumption~\ref{assu:times}]\label{assu:times-0-deterministicQIVPM}For
a $0$-deterministic QIVPMs~$\bar{\mu}$, if $\mathbf{O}_{0}$ and
$\mathbf{O}_{1}$ are commuting, then $\expval{\mathbf{O}_{0}\mathbf{O}_{1}}_{\bar{\mu}}=\expval{\mathbf{O}_{0}}_{\bar{\mu}}\expval{\mathbf{O}_{1}}_{\bar{\mu}}$.\end{condition}

\noindent After we proved these two conditions, we can easily follow
Mermin's proof to build a contradiction. Notice that the expectation
value relative to $0$-deterministic $\bar{\mu}$ plays the same role
as the hidden variable assignment~$v$ in the original Kochen-Specker
theorem despite their physical meanings are not exactly the same.
...
\begin{assumption}[Finite-precision Extension of {Condition~}\ref{cond:sum-one}]\label{assu:delta-deterministicQIVPM}There
exists a $\delta$-deterministic QIVPM.\end{assumption}
