...
\begin{table*}
\caption{\label{table:quantum-interval-expectation}This table highlight the
process to compute $\expval{\mathbf{O}}_{\bar{\mu}_{1}}$. Notice
that $\mul_{1}$ and $\mur_{1}$ are defined by $\left[\mul_{1}\left(P\right),\mur_{1}\left(P\right)\right]=\bar{\mu}_{1}\left(P\right)$
...
\end{table*}
Since Eq.~(\ref{eq:quantum-interval-expectation}) is more complex
than Eq.~(\ref{eq:quantum-expectation}), it is instructive to check
an example. Given a three dimensional Hilbert space with an orthonormal
basis $\left\{ \ket{0},\ket{1},\ket{2}\right\} $. Let $J_{\mathrm{z}}=\proj{0}-\proj{2}$,
and $\mathbf{O}=2\mathbb{1}-J_{\mathrm{z}}^{2}=2\proj{1}+\proj{0}+\proj{2}$.
By following Table~\ref{table:quantum-interval-expectation}, we
have 
\begin{equation}
\expval{\mathbf{O}}_{\bar{\mu}_{1}}=\sum_{i=1}^{2}\lambda_{i}^{+}\left[\Delta\mul\left(P_{i}^{+\prime}\right),\Delta\mur\left(P_{i}^{+\prime}\right)\right]=\left[1,2\right]\textrm{ .}
\end{equation}
$\left[1,2\right]$ is an intuitively result. Since two possible outcomes
are $1$ and $2$, the expectation value is between one of them even
if we no prior knowledge about the likelihood of outcomes. In contrast,
if we naively multiply eigenvalues with intervals, their sum is $\sum_{i=1}^{2}\lambda_{i}^{+}\bar{\mu}_{1}\left(P_{i}^{+}\right)=\left[0,3\right]$
which is a worse estimation. Because even if we no prior knowledge,
we know one of them will happen, and estimating the lower-bound by$1\cdot0+2\cdot0=0$
is too pessimistic.

Formally, the expectation value of an observable with respect to an
IVPM should intuitively be the minimum and maximum expectation value
of the same observable with respect to its consistent states. We found
this is true when we require our IVPM to be convex, and on a commuting
