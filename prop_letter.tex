...
\end{aligned}
\end{equation}
whose inducing function $\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$
fails to satisfy Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion})
when $P_{0}=\proj{0}+\proj{2}$ and $P_{1}=\proj{1}+\proj{2}$. Thus,
$\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$ cannot be a QIVPM. Therefore,
$\iota$ is a THOS rule if and only if $\iota\circ\mu$ is always
a QIVPM. Similarly, Statement~\ref{enu:iota-subject-to} and \ref{enu:iota-mu-CIVPM}
are equivalent as well~\citep{HOSTunpublished}.
...
\end{eqnarray*}
which is bounded between $\frac{7}{24}$ and $\frac{3}{8}$. Therefore,
