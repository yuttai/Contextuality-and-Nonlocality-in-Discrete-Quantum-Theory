...
QIVPMs induced by the Born rule, but there are also some general QIVPMs
may not be induced by any quantum probability measure, i.e., no state
is consistent with these kind of QIVPMs.
...
Our framework allows us to generalize the above theorem to QIVPMs
induced by the finite-precision extension of the Born rule~$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$:
The collection of states consistent $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$
get closer and closer to $\rho$ as the intervals get narrower and
narrower, eventually collapsing to $\rho$ when the intervals become
infinitely precise real numbers. To state this result, we introduce
a few technical notions. We call a state~$\rho$ a \emph{totally
mixed state} if $0<\mu_{\rho}^{\mathrm{B}}\left(P\right)<1$ for any
$P\in\events\backslash\left\{ \mathbb{0},\mathbb{1}\right\} $. We
also define a notion of distance between two states~$\rho$ and $\rho'$
as $\left\Vert \rho-\rho'\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{\rho-\rho'}{\psi}\right|$~\citep{544199},
...
\begin{thm}[Finite-precision Extension of Gleason~\citep{HOSTunpublished}]In
a Hilbert space $\Hilb$ of dimension $3$, given any totally mixed
state~$\rho$, 
\begin{equation}
...
\begin{example*}Given a three-dimensional Hilbert space $\Hilb$
with an orthonormal basis $\left\{ \ket{0},\ket{1},\ket{2}\right\} $,
