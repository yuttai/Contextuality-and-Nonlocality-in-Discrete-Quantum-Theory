...
By denoting the product observables\nb$\prod_{j=1}^{M}\mathbf{O}_{j}$
...
and $\expval{\overline{\mathbf{O}}}_{\mu}$, respectively. Also, since
$\mu\left(\proj{k}\right)$ is either $0$ or $1$, there exists an
unique $\ket{k_{0}}$ such that $\mu\left(\proj{k}\right)=1$ if and
only if $k=k_{0}$. Both $\prod_{j=1}^{M}\expval{\mathbf{O}_{j}}_{\mu}$
