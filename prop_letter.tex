...
\section{Gleason's Theorem and Born rule}
...
\noindent Because Eqs.~(\ref{eq:iota-constraints}) are the direct
counterpart of Eqs.~(\ref{eq:QIVPM-constraints}) and Eq.~(\ref{eq:iota-Inclusion})
is the direct counterpart of the convexity condition, Eq.~(\ref{eq:classicalconvex})
and Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}),
$\iota\circ\mu$ should always be a IVPM no matter in classical or
quantum case. Conversely, if $\iota\circ\mu$ is always a IVPM, then
$\iota$ must be a THOS rule. In other words, $\iota$ is a THOS rule
is a necessary and sufficient condition for $\iota\circ\mu$ is always
a IVPM as we next show.
...
Then, the function~$\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$
...
of the difference between two states~$\rho$ and $\rho'$ is just
...
Although a THOS rule have many desirable properties, the set of intervals
...
Even if a THOS rule~$\iota$ maps to $\mathscr{I}_{0}$ or infinite
intervals, the composited function~$\iota\circ\mu$ mapping to a
finite set of intervals~$\mathscr{I}$ may still be a QIVPM when
$\iota$ is not a THOS rule. For example, we can weaken Eq.~(\ref{eq:iota-Inclusion})
to the following condition: for any numbers~$x_{0}$ and $x_{1}\in\left[0,1\right]$,
...
\begin{equation}
\iota_{2}\left(x\right)=\begin{cases}
\imposs\,, & \textrm{ if }x=0\,;\\
\left[0,\tfrac{1}{4}\right]\,, & \textrm{ if }x\in\left(0,\tfrac{1}{8}\right]\,;\\
\left[0,{\textstyle \tfrac{1}{2}}\right]\,, & \textrm{ if }x\in\left(\tfrac{1}{8},\tfrac{3}{8}\right)\,;\\
\left[\tfrac{1}{4},\tfrac{3}{4}\right]\,, & \textrm{ if }x\in\left[\tfrac{3}{8},\tfrac{5}{8}\right]\,;\\
\left[\tfrac{1}{2},1\right]\,, & \textrm{ if }x\in\left(\tfrac{5}{8},\tfrac{7}{8}\right)\,;\\
\left[\tfrac{1}{4},1\right]\,, & \textrm{ if }x\in\left[\tfrac{7}{8},1\right)\,;\\
\necess\,, & \textrm{ if }x=1\,.
...
and $\iota_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$ are a QIVPMs. By
Thm.~\ref{thm:Finite-precision-Gleason}, $\rho_{0}\in\coreBorn\left(\iota_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}},\events\right)$
