...
In other words, if the measurement intervals are too imprecise and
the QIVPM are defined so wildly, the results we get might contradict
to each other, and exclude any possible state from being consistent
with the measurement results. To address this problem, we are going
to consider a simple rule which can induce well-behaved QIVPMs by
quantum probability measures systematically. Recall when we generalized
quantum probability measures to QIVPMs, we changed the range from
the unit interval~$\left[0,1\right]$ to a set of intervals~$\mathscr{I}$.
...
is called a \emph{morphism}\yutsung{Haven't found a better name so
far, but haven't proved that it is a morphism in the sense of category
theory as well...}if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
Since the similarity between classical and quantum cases, we will
only prove the equivalence between Statement~\ref{enu:iota-subject-to}
and \ref{enu:iota-mu-QIVPM}. It is clear that $\iota$ is a morphism
implies $\iota\circ\mu$ is always a QIVPM, and the rest of the proof
is to prove that if $\iota$ is not a morphism, then $\iota\circ\mu$
might not be a QIVPM. If $\iota$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}),
