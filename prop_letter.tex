...
state~\citep{Born1983,peres1995quantum,544199,Jaeger2007}. Since
the postulates for QIVPMs in Sec.~\ref{sec:Interval-Uncertainty}
...
\yutsung{The other idea: We never proved or find an counterexample
that a non-convex QIVPM has empty core, and its expectation value
is not the same as defined from core... If it is true, the convex
condition may not be necessary, but this might be hard to prove...}
...
\noindent However, if we consider general QIVPMs mapping to an \emph{arbitrary}
...
state from being consistent with the measurement results. In contrast
with the QIVPM defined in Table~\ref{tab:non-Born-QIVPM}, if a QIVPM~$\iota\circ\mu_{\rho}^{\mathrm{B}}$
is induced by a conventional quantum probability measure~$\mu_{\rho}^{\mathrm{B}}$
and a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ defined
in Sec.~\ref{sec:Born}, there is always a state consistent with
it because $\rho\in\coreBorn\left(\iota\circ\mu_{\rho}^{\mathrm{B}},\events\right)$.
...
\end{equation}
the collection of states consistent with $\iota\circ\mu_{\rho}^{\mathrm{B}}$
...
for Born QIVPMs]\label{thm:Finite-precision-Gleason}Consider a three-dimensional
Hilbert space, and an interval-valued function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
with the maximum length of intervals in $\mathscr{I}$ is $\delta$.
Given any state~$\rho$, if a state $\rho'\in\coreBorn\left(\iota\circ\mu_{\rho}^{\mathrm{B}},\events\right)$,
then their distance $\left\Vert \rho-\rho'\right\Vert \le\delta$.\end{thm}

\noindent Given a state~$\rho'$ consistent with $\iota\circ\mu_{\rho}^{\mathrm{B}}$,
...
This implies $\left\Vert \rho-\rho'\right\Vert \le\delta$ by definition.
