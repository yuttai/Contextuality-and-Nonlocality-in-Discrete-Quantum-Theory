...
we will solve this problem with a concrete family of intervals and
...
In contrast with the QIVPM defined in Thm.~\ref{thm:Non-extensible-of-Gleason's},
...
on any 1-dimensioal projector~$\proj{\psi}$ by Thm.~\ref{thm:Finite-precision-Born}.
Since the maximum length of the intervals in $\mathscr{I}_{n}$ is
$1/n$, the difference between the Born quantum probabilities~$\left|\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right|$
is bounded by $1/n$.  This implies $\left\Vert \rho-\rho'\right\Vert \le\frac{1}{n}$
by definition.
