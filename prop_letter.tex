...
which can be reformulated in QIVPM using infinitely precise intervals~$\mathscr{I}_{\infty}=\set{\left[x,x\right]}{x\in\left[0,1\right]}$:
...
\noindent When generalizing to finite precision QIVPMs, we also want
to seek a simple rule to construct them, and understand which states
are consistent with them. However, if the measurement intervals are
too imprecise and the QIVPM are defined so wildly, the results we
get might contradict to each other, and exclude any possible state
from being consistent with the measurement results.
...
To address this problem, we are going to consider a simple rule which
...
matter in classical or quantum case. Conversely, if $\iota\circ\mu$
...
QIVPM. If $\iota$ doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}),
...
Although THOS rules have many desirable properties, the intervals
it mapping to either is trivial or contains infinity intervals.

\begin{thm}[The range of THOS rules]\label{thm:convex-uncountable}For
