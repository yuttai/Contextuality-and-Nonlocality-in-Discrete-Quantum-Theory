...
to understand what a QIVPM could be when it exists. In contrast with
the conventional Born rule~\citep{Born1983,peres1995quantum,544199,Jaeger2007}
and Gleason's theorem~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum}:

\begin{thm}[$\mathscr{I}_{\infty}$ Variant of Gleason]\label{cor:Gleason's}In
a Hilbert space $\Hilb$ of dimension $d\geq3$, given a QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}_{\infty}=\set{\left[x,x\right]}{x\in\left[0,1\right]}$,
the state consistent with it on every projector, $\coreBorn\left(\bar{\mu},\events\right)$,
is unique. \end{thm}

\noindent We haven't discussed how to construct a QIVPM according
to a simple rule. Even if we have a QIVPM, there might not have any
states consistent with it.
...
a THOS rule if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$ is
...
\noindent Because the similarity between Def.~\ref{def:THOS} and
Def.~\ref{def:QIVPM}, $\iota\circ\mu$ should always be a IVPM no
matter in classical or quantum case. Surprisingly, if $\iota\circ\mu$
is always a IVPM, then $\iota$ must be a THOS rule.

\begin{thm}[The equivalence among THOS rules and IVPMs]\label{thm:iota-statements}The
...
QIVPM for any quantum probability measure~$\mu$. If $\iota$ doesn't
satisfy Eq.~(\ref{eq:iota-Inclusion}), we can find three particular
numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$ such
that $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, but $\iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)\nsubseteq\iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)$.
...
\end{eqnarray*}
and cannot be a QIVPM. Therefore, $\iota$ is a THOS rule if and only
...
Even if $\iota$ is not a THOS rule, the composited function~$\iota\circ\mu$
...
Since THOS rules have many desirable properties, the intervals it
mapping to either is trivial or contains infinity intervals.

\begin{thm}[{The range of THOS rule}s]\label{thm:convex-uncountable}For
any function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ described
in Thm.~\ref{thm:iota-statements}, either $\mathscr{I}=\mathscr{I}_{0}$
