...
Since the Born rule is the rule to compute the probability providing
infinity measurement resource, it would be nature to ask whether quantum
IVPM would approach the Born rule if we increase the measurement resource.
In order to understand this question, it is instructive to consider
the non-Born real-valued probability measure when $d=2$.

\begin{example}[One-qubit quantum probability measure]Consider a
...
\end{cases}
\]
Notice that $\mu$ is consistent with the Born rule, but computed
according to different states when applying on different projectors.\qed\end{example}
