...
``unsupportable''~\citep{Mermin1999}.  The debate on whether
...
measurement outcome~\citep{Born1983,peres1995quantum,544199}.  Here,
...
mutually commuting events, i.e., to compatible events that can be
measured simultaneously. In Sec.~\ref{sec:Gleason} we will discuss the
...
proof is as follows. Given a subspace of commuting events~$\events_{\mathrm{C}}$,
we can construct a partial orthonormal basis by diagonalization and
complete it to a full orthonormal basis $\overline{\events_{\mathrm{C}}}$. We
...
\end{equation}
If we identify the states ~$\proj{0}$ and $\proj{1}$ with the head and
the tail of a coin, respectively, then $\bar{\mu}_{2}$ represents the
situation in which an experimenter knows nothing about a coin before
she tosses it. Moreover, for each $p\in\left[0,1\right]$, the
...
We also note that, when restricted to commuting projectors, the
definition above is consistent with the classical notion of the
\emph{Choquet integral}~\citep{GilboaSchmeidler1994,Grabisch2016}
...
\begin{table}
  \noindent \centering{}\caption{\label{tab:classical-IVPMs}Example 
    IVPMs~$\bar{\mu}_{4}$, $\bar{\mu}_{5}$ and $\bar{\mu}_{6}$.}
\begin{tabular}{cccc}
\toprule 
\addlinespace
...
\noindent The definition puts no restrictions on the set of intervals
...
for more indeterminate behavior. For example, the IVPM~$\bar{\mu}_{4}$
defined in Table~\ref{tab:classical-IVPMs} is $0$-deterministic
because $\imposs\subseteq\left[0,0\right]$ and
$\necess\subseteq\left[1-0,1\right]$.  By interpreting the states
~$\proj{0}$ and $\proj{1}$ as the head and the tail of a coin,
$\bar{\mu}_{4}$ represents tossing a double-headed coin. When $\delta$
is increased to $\frac{1}{3}$, both the IVPMs~$\bar{\mu}_{5}$ and
$\bar{\mu}_{6}$ become $\frac{1}{3}$-deterministic. In contrast, the
IVPM~$\bar{\mu}_{2}$ defined in
...
start with a remark that the original Kochen-Specker theorem is a
statement regarding the non-existence of $0$-deterministic QIVPM and
...
from a state according to the Born
rule~\citep{Born1983,peres1995quantum,544199,Jaeger2007}.  According
to Gleason's
theorem~\citep{gleason1957,Redhead1987-REDINA,peres1995quantum}, this
state is also the unique state consistent with any possible
...
\noindent For general QIVPMs mapping to finitely precise intervals, we
also want to seek a simple rule to construct them, and to understand
which states are consistent with them. However, if measurement
intervals are too imprecise, the state consistent with the results of
measurements on some projectors might contradict the state induced by
some other projectors. Indeed we can formally prove that, in general,
there may be \emph{no} state consistent with some arbitrary QIVPM.
...
verify that there are no states that are consistent with it on all
projectors. Given an orthonormal basis
$\left\{ \ket{0},\ket{1},\ket{2},\ldots,\ket{d-1}\right\} $, let
...
\end{equation}
The function~$\bar{\mu}:\events\rightarrow\mathscr{I}_{0}$ defined in
Table~\ref{tab:non-Born-QIVPM} can be verified as a
...
To that end, we construct a family of QIVPMs parameterized by a
functions $\ultramodular:\left[0,1\right]\rightarrow\mathscr{I}$ that
formalize how the real-valued probabilities are mapped to
intervals. These functions, are called \emph{ultramodular}, when they
satisfy certain properties defined next.

\begin{definition}[Ultramodular Functions]\label{def:THOS}Given
a collection of intervals $\mathscr{I}$ including $\imposs$ and
$\necess$, a function $\ultramodular:\left[0,1\right]\rightarrow\mathscr{I}$
is called a ultramodular function if:
...
$x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, we have:
\begin{equation}
\ultramodular\left(x_{0}+x_{1}+x_{2}\right)+\ultramodular\left(x_{2}\right)\subseteq\ultramodular\left(x_{0}+x_{2}\right)+\ultramodular\left(x_{1}+x_{2}\right)\,.\label{eq:iota-Inclusion}
\end{equation}
\end{definition}
...
(\ref{eq:QuantumInterval-valuedProbability-Inclusion}). Therefore,
these conditions guarantee that, for any conventional quantum
probability measure $\mu$, the composition $\ultramodular \circ \mu$,
defines a valid QIVPM. Conversely, it is the case that if
$f \circ \mu$ is a QIVPM for some function $f$ then $f$ is an
ultramodular function. Formally, we have the following result. 
...
\item \label{enu:iota-mu-CIVPM}The composite
  ...
  is a classical IVPM for any classical probability
...
\item \label{enu:iota-mu-QIVPM}The composite
  ...
  QIVPM for any quantum probability
  measure~$\mu:\events\rightarrow\left[0,1\right]$ on any Hilbert
  space~$\Hilb$.
\end{enumerate}
\end{thm}

The equivalence between Statement~\ref{enu:iota-subject-to} and
\ref{enu:iota-mu-CIVPM} is a well-known classical
result~\citep{Choquet1954,NgMoYeh1997,Marinacci_2004}.  For the
quantum case, Statement~\ref{enu:iota-subject-to} implies
\ref{enu:iota-mu-QIVPM} as we have outlined above. The rest of the
proof is to show that if $\ultramodular$ is not ultramodular, then
$\ultramodular\circ\mu$ might not be a QIVPM. Suppose there are three
particular numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
such that $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, but they don't
...
\end{aligned}
\end{equation}
The induced map using the Born rule and approximated by
$\ultramodular$, $\ultramodular\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$
fails to satisfy
Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}) when
$P_{0}=\proj{0}+\proj{2}$ and $P_{1}=\proj{1}+\proj{2}$. In other
words, this induced map fails to be a QIVPM. 
...
$\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$.  Moreover, the collection
of states consistent with $\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$
...
\begin{thm}[Finite-Precision Extension of Gleason's
  theorem]\label{thm:Finite-precision-Gleason}Consider
  an ultramodular function
  $\ultramodular:\left[0,1\right]\rightarrow\mathscr{I}$ and let $m$
  be the maximum length of intervals in its image. Given any
  state~$\rho$, if a state
  $\rho'\in\coreBorn\left(\ultramodular\circ\mu_{\rho}^{\mathrm{B}},\events\right)$,
  then the norm of their difference is bounded by $m$, i.e.,
  $\left\Vert \rho-\rho'\right\Vert \le m$.\end{thm}

Given a state~$\rho'$ consistent with $\ultramodular\circ\mu_{\rho}^{\mathrm{B}}$,
we have $\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\in\ultramodular\left(\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right)$
for any one-dimensional projector $\proj{\psi}$. Since the maximum
length of the intervals in $\mathscr{I}$ is $m$, it is also the
upper bound of the difference:
\[
\left|\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right|=\left|\melem{\psi}{\rho-\rho'}{\psi}\right|\le m\,.
\]
...
Hence, $\left\Vert \rho-\rho'\right\Vert \le m$.

The previous theorem generalizes Gleason's theorem in the sense that
it accounts for a larger class of probability measures which includes
the conventional one as a limit. The theorem is however ``weak'' in
the sense that it only applies to the particular class of QIVPMs
constructed by composing an ultramodular function with a conventional
quantum probability measure.

\amr{***WAVEFRONT***}

Although a ultramodular function has many desirable properties, the
set of intervals it mapping to either is trivial or contains infinite
intervals.
...
either $\mathscr{I}=\mathscr{I}_{0}$ or $\mathscr{I}$ contains uncountably
many intervals.\end{thm}
...
i.e., 
\begin{equation}
\begin{aligned} & \ultramodularL\left(x_{0}+x_{1}+x_{2}\right)+\ultramodularL\left(x_{2}\right)\\
\ge{} & \ultramodularL\left(x_{0}+x_{2}\right)+\ultramodularL\left(x_{1}+x_{2}\right)\,.
\end{aligned}
\end{equation}
...
\begin{eqnarray}
\frac{\ultramodularL\left(x_{0}'\right)+\ultramodularL\left(x_{1}'\right)}{2} & \ge & \ultramodularL\left(\frac{x_{0}'+x_{1}'}{2}\right)\,,
\end{eqnarray}
i.e., $\ultramodularL$ is midpoint-convex or Jensen-convex~\citep{Klee_1956,Kenyon_1956,RobertsVarberg1973,PecaricTong1992}.
Together with the fact that $\ultramodularL$ maps to a bounded interval
$\left[0,1\right]$, the left-end function~$\ultramodularL$ must
be continuous on the unit open interval $\left(0,1\right)$~\citep{Jensen_1906,RobertsVarberg1973}.
Therefore, either $\ultramodular$ maps every number in $\left(0,1\right)$
to the same interval, or the number of intervals that $\ultramodular$
maps to must be uncountable.

Even if $\ultramodular$ has to be ultramodular to guarantee $\ultramodular\circ\mu$
a QIVPM, the composited function $\ultramodular\circ\mu$ may still
be a QIVPM when $\ultramodular$ is not ultramodular. In this case,
the composited function $\ultramodular\circ\mu$ could map to non-trivial
finite number of intervals. For example, we can weaken the convex
condition, Eq.~(\ref{eq:iota-Inclusion}), to the following additive
...
\end{equation}
Then, given a three-dimensional Hilbert space, when $\ultramodular$
satisfies Eqs.~(\ref{eq:iota-constraints}) and Eq.~(\ref{eq:iota-add}),
$\ultramodular\circ\mu$ still must be a QIVPM~\citep{HOSTunpublished}.
More concretely, consider an orthonormal basis $\{\ket{0},\ket{1},\ket{2}\}$,
two states $\rho_{0}=\frac{1}{3}\proj{0}+\frac{1}{3}\proj{1}+\frac{1}{3}\proj{2}$
and $\rho_{1}=\frac{7}{24}\proj{0}+\frac{1}{3}\proj{1}+\frac{3}{8}\proj{2}$,
...
and Eq.~(\ref{eq:iota-add})~\citep{HOSTunpublished}, $\ultramodular_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$
and $\ultramodular_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$ are both
QIVPMs.
\begin{table}
...
\end{minipage}\hfill{}
\end{table}
Given any one-dimensional projector $\proj{\psi}$, we have $\mu_{\rho_{0}}^{\mathrm{B}}\left(\proj{\psi}\right)=\frac{1}{3}$
so that $\ultramodular_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(\proj{\psi}\right)\right)=\left[0,{\textstyle \tfrac{1}{2}}\right]$.
On the other hand, 
\begin{eqnarray*}
\mu_{\rho_{1}}^{\mathrm{B}}\left(\proj{\psi}\right) & = & \tfrac{7}{24}\left|\ip{\psi}{0}\right|^{2}+\tfrac{1}{3}\left|\ip{\psi}{1}\right|^{2}+\tfrac{3}{8}\left|\ip{\psi}{2}\right|^{2}
\end{eqnarray*}
is bounded between $\frac{7}{24}$ and $\frac{3}{8}$. Therefore,
$\mu_{\rho_{1}}^{\mathrm{B}}\left(\proj{\psi}\right)\in\ultramodular_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(\proj{\psi}\right)\right)$.
According to the complement condition, Eq.~(\ref{eq:complement}),
we also have $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\in\ultramodular_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$
for any two-dimensional projector~$P$. Hence, $\rho_{1}\in\coreBorn\left(\ultramodular_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$,
and Thm.~\ref{thm:Finite-precision-Gleason} asserts $\left\Vert \rho_{0}-\rho_{1}\right\Vert $
less than or equal to the maximum length of intervals $\frac{1}{2}$.
