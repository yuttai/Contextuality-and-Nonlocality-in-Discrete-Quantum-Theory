...
probability measure\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
is called $\left(1/n,\delta\right)$-deterministic if for every $\left[\ell,r\right]\in\mathscr{I}$,
there is a $k\in\mathbb{Z}$ such that $\left[\ell,r\right]\subseteq\left[k/n-\delta,k/n+\delta\right]$.\end{definition}

For example, any cryptodeterministic QIVPM is $\left(1,0\right)$-deterministic.
...
where $\iota_{\infty}\left(x\right)=\left[x,x\right]$. However, we
will prove there is generally no $\left(1/n,\delta\right)$-deterministic
...
is $\left(1/n,0\right)$-deterministic.\end{lemma}

\begin{proof} The essential part to verify $\mu$ is a quantum probability
measure is to verify Eq.~(\ref{eq:QuantumProbability-Addition}). Given a pair of orthogonal
operators\nb$P_{0}$ and $P_{1}$, let $\pmeas\left(P_{0}\right)=k_{0}/n$ and $\pmeas\left(P_{1}\right)=k_{1}/n$.
Then, 
\begin{equation}
\bar{\mu}\left(P_{0}+P_{1}\right)\subseteq\left[\frac{k_{0}+k_{1}}{n}-2\delta,\frac{k_{0}+k_{1}}{n}+2\delta\right]\textrm{ .}
\end{equation}
Since $\delta<1/\left(3n\right)$, we have
\begin{subequations}
\begin{eqnarray}
\frac{k_{0}+k_{1}-1}{n}+\delta&<&\frac{k_{0}+k_{1}}{n}-2\delta\\
\frac{k_{0}+k_{1}}{n}+2\delta&<&\frac{k_{0}+k_{1}+1}{n}-\delta\textrm{ .}
\end{eqnarray}
\end{subequations}
The previous inequalities imply that if $\bar{\mu}\left(P_{0}+P_{1}\right)$
