...
\yutsung{Notice that the use of quantum is minumalized in this section.
...
postulates of the quantum probability measures first, and then introduce
the Born rule which induces a quantum probability measures from each
state. When we shift our attention to QIVPMs, we also want to construct
QIVPMs according to a simple rule. Since the ranges of quantum probability
measures and QIVPMs are $\left[0,1\right]$ and $\mathscr{I}$, respectively,
if there is a function~$\iota$ mapping a number~$x\in\left[0,1\right]$
...
and get a $\mathscr{I}$-valued function~$\bar{\mu}:\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$.
Although $\bar{\mu}$ and QIVPMs have the same domain and range, $\bar{\mu}$
is not necessary a QIVPM. For example, in the four-dimensional Hilbert
space with 
\begin{equation}
...
is \emph{not} a QIVPM, where $\circ$ is the function concatenation.
In particular, verifying whether an \emph{arbitrary} function is a
QIVPM is a long and tedious process~\citep{HOSTunpublished}, so
that it would be good if $\iota\circ\mu$ is automatically a QIVPM
...
\end{eqnarray}
\end{subequations}and satisfying for any three numbers~$x_{0}$
and $x_{1}\in\left[0,1\right]$, $x_{0}+x_{1}\in\left[0,1\right]$
implies
\begin{equation}
...
The situation for $d=2$ is trivial. When $d=3$, to prove that $\iota\circ\mu$
is a QIVPM, it is sufficient to check Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}).
When $P_{0}$ and $P_{1}$ are orthogonal, Eq.~(\ref{eq:iota-add})
implies Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}).
When $P_{0}$ and $P_{1}$ are not orthogonal on a three-dimensional
Hilbert space, we have two possibilities:
\begin{itemize}
...
Since $\iota'$ defined in Eq.~(\ref{eq:iota}) satisfies Eq.~(\ref{eq:iota-constraints})
and Eq.~(\ref{eq:iota-add}), every function~$\iota'\circ\mu$ on
a three-dimensional Hilbert space is a QIVPM according to Thm.~\ref{thm:QRVPM2QIVPM-3}.
However, we knows that $\iota'\circ\mu_{\rho_{1}}^{\mathrm{\mathrm{B}}}$
...
there must exist three numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
...
In order to find a function~$\iota$ satisfies Eq.~(\ref{eq:iota-Inclusion}),
we break down into two functions: its left-end and right-end~$\left[\iotal\left(x\right),\iotar\left(x\right)\right]=\iota\left(x\right)$.
