...
regarding to the expectation values of a family of commuting observables
relative to $\bar{\mu}$. A family of $M$ commuting observables\nb$\left\{ \mathbf{O}_{j}\right\} _{j=1}^{M}$
can be diagonalized by a common orthonormal basis~$\Omega=\left\{ \ket{k}\right\} _{k=0}^{d-1}$.
Since $\bar{\mu}$ is 0-deterministic, a unique $\ket{k_{0}}$ exists
such that $\bar{\mu}\left(\proj{k}\right)=\necess$ if and only if
$k=k_{0}$\nb\citep{HOSTunpublished}. Also, a unique quantum probability
...
Especially, $\mu\left(\proj{k}\right)=1$ if and only if $k=k_{0}$.
