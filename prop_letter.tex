...
each event $P_{i}$ is mapped to a probability using a probability measure
$\mu : P \rightarrow [0,1]$ to a probability $\mu(P_{i})$. We impose
...
$\rho\in\coreBorn\left(\bar{\mu},S\right)$ means~$\rho$ is
...
For example, assuming the three intervals $\imposs=\left[0,0\right]$,
...
is a commuting sub-event space. Restricting $\bar{\mu}_{1}$ to
...
projectors\nb$\events$, so Gleason's theorem does not hold for at least this
example.
\end{example}
