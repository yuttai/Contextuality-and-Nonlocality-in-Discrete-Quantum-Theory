...
if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$ subject to the
...
\end{eqnarray}
\end{subequations}and satisfying for any numbers~$x_{0}$ and $x_{1}\in\left[0,1\right]$,
$x_{0}+x_{1}\in\left[0,1\right]$ implies
\begin{equation}
...
subject to Eqs.~(\ref{eq:iota-constraints}) and satisfying for any
three numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$,
...
They both map from a unit interval to another unit interval. Eq.~(\ref{eq:iota-Inclusion})
then implies that $\iotal$ is Wright-convex, i.e., $\iotal\left(x_{0}+x_{1}+x_{2}\right)+\iotal\left(x_{2}\right)\ge\iotal\left(x_{0}+x_{2}\right)+\iotal\left(x_{1}+x_{2}\right)$,
and $\iotar$ is Wright-concave, i.e., $\iotar\left(x_{0}+x_{1}+x_{2}\right)+\iotar\left(x_{2}\right)\le\iotar\left(x_{0}+x_{2}\right)+\iotar\left(x_{1}+x_{2}\right)$~\citep{Wright1954,RobertsVarberg1973,PecaricTong1992,Brzdek_2014}.
...
\end{equation}
Furthermore, a midpoint-convex function is either continuous on $\left(0,1\right)$
or Lebesgue non-measurable~\citep{Blumberg_1919,Sierpinski1920,RobertsVarberg1973}.
When $\iotal$ is Lebesgue non-measurable, 

When $\iotal$ is continuous, the range of $\iota$ either is $\left\{ \necess,\imposs,\unknown\right\} $
or contains uncountably many intervals. 
