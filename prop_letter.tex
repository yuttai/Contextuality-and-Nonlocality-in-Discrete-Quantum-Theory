...
This idea can be modeled precisely by considering the following kind
of QIVPMs.

\begin{definition}\label{def:delta-deterministic}An QIVPM\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
...
For another example, given a three-dimensional Hilbert space, a $\frac{1}{3}$-deterministic
QIVPM can be given by $\bar{\mu}\left(P\right)=\left[x,x\right]$,
where $P$ projects onto a $x$-dimensional subspace. By utilizing
$\delta$-deterministic QIVPMs, we can have a Kochen-Specker theorem
...
then we can define a cryptodeterministic QIVPM\nb$\bar{\mu}^{\textrm{D}}:\events\rightarrow\left\{ \imposs,\necess\right\} $
by 
\begin{equation}
...
QIVPM is to verify\begin{subequations}
\begin{eqnarray}
...
which is a special case of Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}).

When $\bar{\mu}^{\textrm{D}}\left(P_{0}\right)=\bar{\mu}^{\textrm{D}}\left(P_{1}\right)=\imposs$,
we have $\bar{\mu}\left(P_{0}\right)\subseteq\left[0,\delta\right]$
and $\bar{\mu}\left(P_{1}\right)\subseteq\left[0,\delta\right]$ which
implies $\bar{\mu}\left(P_{0}+P_{1}\right)\subseteq\left[0,2\delta\right]$.
...
The remain part of the proof are routine verification.\end{proof}
