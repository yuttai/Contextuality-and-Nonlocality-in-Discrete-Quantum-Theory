...
According to Mermin's comment\nb\cite{Mermin1999},
\begin{quote}
...although the outcomes deduced from such imperfect measurements
...
\end{quote}
if the imperfection of the measurement is small enough, the finitely
precise measurement should not nullify the Kochen-Specker theorem.
This idea can be modeled precisely by QIVPMs.

\begin{definition}\label{def:delta-deterministic}An interval-valued
probability measure\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
is called $\delta$-deterministic if either $\left[\ell,r\right]\subseteq\left[0,\delta\right]$
or $\left[\ell,r\right]\subseteq\left[1-\delta,1\right]$ for every
$\left[\ell,r\right]\in\mathscr{I}$.\end{definition}

For example, any cryptodeterministic QIVPM is $0$-deterministic.
...
\end{equation}
is a $\frac{1}{3}$-deterministic, where $\iota_{\infty}\left(x\right)=\left[x,x\right]$.
However, we will prove there is no $\delta$-deterministic probability
measures for small $\delta$ due to the Kochen-Specker theorem.

\begin{lemma}\label{lemma:QIVPM2QCM}Given a Hilbert space of dimension\nb$d$,
and a quantum $\delta$-deterministic probability measure\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
with $\delta<\left(1+d\right)^{-1}$, then $\mu:\events\rightarrow\left[0,1\right]$
defined by
\begin{equation}
\mu(P)=\begin{cases}
0 & \textrm{if }\bar{\mu}(P)\subseteq\left[0,\delta\right]\textrm{ ;}\\
1 & \textrm{if }\bar{\mu}(P)\subseteq\left[1-\delta,1\right]
\end{cases}
\end{equation}
 is a cryptodeterministic quantum probability measure.\end{lemma}

\begin{proof}~
\begin{itemize}
\item Since $\bar{\mu}(\mathbb{0})=\imposs$, we have $\mu(\mathbb{0})=0$.
\item Since $\bar{\mu}(\mathbb{1})=\necess$, we have $\mu(\mathbb{1})=1$.
\item For every event $P$, we have\begin{subequations}
\begin{eqnarray}
 &  & \pmeas\left(P\right)=0\Rightarrow\bar{\mu}(P)\subseteq\left[0,\delta\right]\\
 & \Rightarrow & \bar{\mu}(\mathbb{1}-P)=\left[1,1\right]-\bar{\mu}(P)\subseteq\left[1-\delta,1\right]\\
 & \Rightarrow & \pmeas\left(\mathbb{1}-P\right)=1=1-\pmeas\left(P\right)\textrm{ .}
\end{eqnarray}
\end{subequations}Similarly, when $\pmeas\left(P\right)=1$, we also
have $\pmeas\left(\mathbb{1}-P\right)=1-\pmeas\left(P\right)$.
\item Given a collection $\left\{ P_{i}\right\} _{i=1}^{N}$ of pairwise
orthogonal events, we must have $N\le d$. 
\begin{itemize}
\item If $\pmeas(P_{i})=0$ for all $i$, then\begin{subequations}
\begin{eqnarray}
 &  & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\sum_{i=1}^{N}\bar{\mu}(P_{i})\subseteq\left[0,N\delta\right]\\
 & \overset{d\delta<1-\delta}{\Rightarrow} & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\cap\left[1-\delta,1\right]=\textrm{Ø}\\
 & \Rightarrow & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\left[0,\delta\right]\\
 & \Rightarrow & \pmeas\left(\bigcup_{i=1}^{N}P_{i}\right)=0=\sum_{i=1}^{N}\pmeas(P_{i})\textrm{ .}
\end{eqnarray}
\end{subequations}
\item If $\pmeas\left(P_{i_{0}}\right)=1$ for a unique $i_{0}$, then\begin{subequations}
\begin{eqnarray}
 &  & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\sum_{i=1}^{N}\bar{\mu}(P_{i})\subseteq\left[1-\delta,1\right]\\
 & \Rightarrow & \pmeas\left(\bigcup_{i=1}^{N}P_{i}\right)=1=\sum_{i=1}^{N}\pmeas(P_{i})\textrm{ .}
\end{eqnarray}
\end{subequations}
\item If $\pmeas\left(P_{i_{0}}\right)=\pmeas\left(P_{i_{1}}\right)=1$
for $i_{0}\ne i_{1}$, then
\begin{equation}
\bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\sum_{i=1}^{N}\bar{\mu}(P_{i})\subseteq\left[2-2\delta,N\right]
\end{equation}
which contradicts to definition~\ref{def:delta-deterministic} so
...
of dimension\nb$d\ge3$, there is no quantum $\delta$-deterministic
probability measure for $\delta<\left(1+d\right)^{-1}$. \end{cor}
