...
If $\bar{\mu}$ is a conventional (Born) probability measure induced by
a state $\rho$, then $\coreBorn\left(\bar{\mu},\events'\right)$ will
contain the singleton state $\rho$ and both endpoints of the interval
will agree, thus reducing the definition to that of
...
to strengthen one of the fundamental theorems of quantum information:
the Kochen-Specker theorem~\cite{PeresRon1988,peres1995quantum}. Our
stronger version of the theorem will suggest a resolution to the
debate initiated by Meyer and Mermin on the relevance of the
...
our stronger statement and theorem quantifies Mermin's intuition:
\begin{quote}
...
If a QIVPM is 0-deterministic, then for a family of commuting
observables~$\left\{ \mathbf{O}_{j}\right\} _{j=1}^{M}$, we can prove
that the product of expectation values of the individual observables
is the expectation value of the product of the family:
\begin{equation}
\prod_{j=1}^{M}\expval{\mathbf{O}_{j}}_{\bar{\mu}}=\expval{\prod_{j=1}^{M}\mathbf{O}_{j}}_{\bar{\mu}}\label{eq:Kochen-Specker-Rules}
\end{equation}
\amr{Talk with Yu-Tsung about this} Intuitively the expectation value
of each observable lies between two possible outcomes, which
themselves lie between the minimum and maximum bounds of the given
probability intervals.  If the QIVPM is 0-deterministic, this interval
collapses to a singleton $\left\{ \lambda\right\} $, where $\lambda$
is an eigenvalue of~$\mathbf{O}$.
...
We can in fact generalize the above theorem to state than for small
enough $\delta$, it is impossible to have just the two intervals
...
\emph{a fundamental part of quantum theory}. There are in fact two
sources of uncertainty: one due to the finite measurement resources
and one due to complementarity, which requires every quantum framework
to be truly probabilistic.
...
For the proof, suppose there is a $\delta$-deterministic
...
It remains to verify that $\bar{\mu}^{\textrm{D}}$ is indeed a 0-deterministic
QIVPM by checking the following property:
\begin{subequations}
\begin{eqnarray}
...
changes. Assuming a Hilbert space $\Hilb$ of dimension $\ge 3$, then
