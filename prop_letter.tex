...
\begin{itemize}
\item $\left\{ \mathbb{0},\mathbb{1}\right\} \cup\left\{ P_{i}^{-}\right\} _{i=1}^{N^{-}}\cup\left\{ P_{i}^{+}\right\} _{i=1}^{N^{+}}\subseteq\events'$.
...
it, and two classical states~$\proj{0}$ and $\proj{1}$ are both
consistent with $\bar{\mu}_{2}$ on $\events_{0}$.
...
system, there is always at least one system that is consistent with
the measurements.

\begin{thm}[Shapley~\cite{Shapley1971,Grabisch2016}\label{thm:Shapley}]
For every convex IVPM~$\bar{\mu}:\events'\rightarrow\mathscr{I}$,
