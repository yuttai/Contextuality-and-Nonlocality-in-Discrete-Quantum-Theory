...
and Eq.~\ref{eq:QuantumInterval-valuedProbability-Inclusion}, maybe
it's better to write just two terms 
\begin{equation}
...
$\mu_2$ that can associated with this space are:
...
framework. In the classical setting, there are several proposals for
``imprecise probabilities.'' Although these proposals differ in some
details, they all share the fact that the probability $\mu(E)$ of an
event $E$ is generalized from a single real number to an interval of
certainty $[\ell,r]$ where~$\ell$ intuitively corresponds to the
strength of evidence for the event and $r$ corresponds to the strength
of evidence against the event. The collection of intervals used in the
