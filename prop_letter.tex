...
which can be used to replace the probability on a projector, $\mu\left(P\right)$,
by an interval~$\iota\left(\mu\left(P\right)\right)$. In other words,
concatenating $\iota$ with a quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
...
which has the same domain and range as a QIVPM. However, the concatenated
...
is \emph{not} a QIVPM~\citep{HOSTunpublished}. Although we could
verify whether $\iota\circ\mu$ is a QIVPM or not for any pair of
$\iota$ and $\mu$ individually, the verification process is long
and tedious. Therefore, it is desirable to have some easy criteria
to decide when $\iota\circ\mu$ is a QIVPM.
...
orthogonal so that we can apply Eq.~(\ref{eq:iota-add}) on their
complements.
...
\begin{enumerate}
\item \label{enu:iota-subject-to}$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
\end{equation}
\item \label{enu:iota-mu-QIVPM}$\iota\circ\mu:\events\rightarrow\mathscr{I}$
is always a QIVPM for any quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
in any Hilbert space~$\Hilb$.
\end{enumerate}
\end{thm}

It is clear that Statement~\ref{enu:iota-subject-to} implies Statement~\ref{enu:iota-mu-QIVPM},
and the rest of the proof is to prove the negation of Statement~\ref{enu:iota-subject-to}
implies the negation of Statement~\ref{enu:iota-mu-QIVPM}. If $\iota$
doesn't satisfy Eq.~(\ref{eq:iota-Inclusion}), we can find three
particular numbers~$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$
such that $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, but 
\[
...
They are both usual functions from a unit interval to another unit
interval. Eq.~(\ref{eq:iota-Inclusion}) then implies that $\iotal$
...
A Wright-convex function is midpoint-convex or Jensen-convex~\citep{Jensen_1906,Klee_1956,Kenyon_1956,RobertsVarberg1973,PecaricTong1992},
i.e., we have
\begin{equation}
\frac{\iotal\left(x_{0}\right)+\iotal\left(x_{1}\right)}{2}\ge\iotal\left(\frac{x_{0}+x_{1}}{2}\right)\,.
\end{equation}
A midpoint-convex function is continuous on an open interval if we
cannot use the Axiom of Choices~\citep{Blumberg_1919,Sierpinski1920,RobertsVarberg1973}.
Therefore, the range of $\iota$ either is $\left\{ \necess,\imposs,\unknown\right\} $
or contains infinitely many intervals.
