...
If a QIVPM is 0-deterministic, then for a family of $M$ commuting
...
relative to $\bar{\mu}$ is the expectation value of the product of the
family relative to $\bar{\mu}$:
\begin{equation}
\prod_{j=1}^{M}\expval{\mathbf{O}_{j}}_{\bar{\mu}}=\expval{\prod_{j=1}^{M}\mathbf{O}_{j}}_{\bar{\mu}}\label{eq:Kochen-Specker-Rules}
\end{equation}
If the QIVPM $\bar{\mu}$ is 0-deterministic, the expectation value of
each observable $\mathbf{O}_j$ given by
Eq.~(\ref{eq:quantum-interval-expectation-core}) collapses to a single
point

this interval collapses
to a singleton $\left\{ \lambda\right\} $, where $\lambda$ is an
eigenvalue of~$\mathbf{O}$.
...
changes. Assuming a Hilbert space $\Hilb$ of dimension $d \ge 3$, then
