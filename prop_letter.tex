...
In particular, there is a QIVPM such that no state can be consistent
with it on every projector. However, if we only focus on the Born
QIVPMs~$\bar{\mu}$, that is, QIVPM's induced by a state~$\rho$
according to the Born rule, we find that if we monotonically shrink
the length of intervals, the states consistent with $\bar{\mu}$ will
be closer and closer to $\rho$, and vise visa. As the result, although
a finite precision version of Gleason's theorem is stated, there might
be more rich phenomenon regarding to quantum probability when considering
finite precision measurement, and highlight the necessity to develop
a mathematical theory of quantum mechanics that intrinsically consistent
with the resources available for realistic measurement.
...
intervals with length~$\left(2n\right)^{-1}$ at each end. To select
the correct interval from these overlapping intervals~$[\ell,r]$,
we define the non-overlapping interior part of a interval, $\textrm{Int}\left([\ell,r]\right)$,
as in Table~\ref{tab:I_n}.
\begin{table}
\caption{\label{tab:I_n}Intervals in $\mathscr{I}_{n}$ and their interior.
Although intervals~$[\ell,r]$ are overlapping, their interior~$\textrm{Int}\left([\ell,r]\right)$
are non-overlapping.}

\begin{tabular}{ccc}
\toprule 
\addlinespace
$k$ & $[\ell,r]$ & $\textrm{Int}\left([\ell,r]\right)$\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
 & $\imposs$ & $\imposs$\tabularnewline\addlinespace
\midrule 
\addlinespace
$0$ & $\left[0,\tfrac{1}{2n}\right]$ & $\left(0,\tfrac{0.5}{2n}\right]$\tabularnewline\addlinespace
\midrule 
\addlinespace
Any odd number & $\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]$ & $\left(\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
\begin{tabular}{c}
Any even number\tabularnewline
with $0<k<2n$\tabularnewline
\end{tabular} & \multirow{1}{*}{$\left[\tfrac{k-1}{2n},\tfrac{k+1}{2n}\right]$} & \multirow{1}{*}{$\left[\tfrac{k-0.5}{2n},\tfrac{k+0.5}{2n}\right]$}\tabularnewline\addlinespace
\midrule 
\addlinespace
$2n$ & $\left[\tfrac{2n-1}{2n},1\right]$ & $\left[\tfrac{2n-0.5}{2n},1\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
 & $\necess$ & $\necess$\tabularnewline\addlinespace
\bottomrule
\end{tabular}
\end{table}
 To understand these intervals and their interior part more clear,
a special case $\mathscr{I}_{2}$ is also listed in Table~\ref{tab:I_2}.
\begin{table}
\caption{\label{tab:I_2}Intervals in $\mathscr{I}_{2}$ and their interior.
Although intervals~$[\ell,r]$ are overlapping, their interior~$\textrm{Int}\left([\ell,r]\right)$
are non-overlapping.}
...
\midrule 
\addlinespace
$0$ & $\left[0,\tfrac{1}{4}\right]$ & $\left(0,\tfrac{1}{8}\right]$\tabularnewline\addlinespace
\midrule 
\addlinespace
$1$ & $\left[0,{\textstyle \tfrac{1}{2}}\right]$ & $\left(\tfrac{1}{8},\tfrac{3}{8}\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
$2$ & $\left[\tfrac{1}{4},\tfrac{3}{4}\right]$ & $\left[\tfrac{3}{8},\tfrac{5}{8}\right]$\tabularnewline\addlinespace
\midrule 
\addlinespace
$3$ & $\left[\tfrac{1}{2},1\right]$ & $\left(\tfrac{5}{8},\tfrac{7}{8}\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
$4$ & $\left[\tfrac{1}{4},1\right]$ & $\left[\tfrac{7}{8},1\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
 & $\necess$ & $\necess$\tabularnewline\addlinespace
\bottomrule
\end{tabular}
\end{table}
 Then, given a real-valued probability~$x$, we will select an interval~$\iota_{n}\left(x\right)=[\ell,r]$
...
are not orthogonal on a three-dimensional Hilbert space, then we have
two possibilities:
\begin{itemize}
\item Their complements are orthogonal so that we can apply Eq.~(\ref{eq:iota-add})
on their complements.
\item One of them projects onto the subspace which the other one projects
onto.
\end{itemize}
...
is indeed a QIVPM. As a concrete example of Thm.~\ref{thm:Finite-precision-Born},
when $n=2$, two states~$\rho_{1}$ and $\rho_{0}$ can induce two
QIVPMs~$\iota_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$ and $\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$,
where $\rho_{1}=\left(\proj{0}+\proj{1}+\proj{2}\right)/3$ and $\rho_{0}=\frac{7}{24}\cdot\proj{0}+\frac{1}{3}\cdot\proj{1}+\frac{3}{8}\cdot\proj{2}$.
The interval-valued probability of the induced QIVPMs, $\iota_{2}\left(\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\right)$
...
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\iota_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$
and $\iota_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$ based on $\mu_{\rho_{1}}^{\mathrm{B}}$
and $\mu_{\rho_{0}}^{\mathrm{B}}$ on $\events_{C}$.}

\begin{tabular}{ccccc}
\toprule 
\addlinespace
$P$ & $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)$ & $\iota_{2}\left(\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\right)$ & $\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)$ & $\iota_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
...
For example, $\rho_{1}$ and $\rho_{0}$ are both totally mixed states.
Since $\left\Vert \rho_{1}-\rho_{0}\right\Vert \le\frac{1}{24}$,
we should have $\rho_{0}\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}},\events\right)$
and $\rho_{1}\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$,
...
$\rho$ is guarantied belong to $\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}},\events\right)$,
