...
states consistent with the measurement results. To avoid this problem,
instead of proposing an abstract framework~\citep{HOSTunpublished},
we start from a concrete family of intervals as follow:
\begin{equation}
...
\end{tabular}
\end{table}
Since the interior part of intervals are non-overlapping, we can define
a function~$\iota_{n}:\left[0,1\right]\rightarrow\mathscr{I}_{n}$
by selecting the interval~$\iota_{n}\left(x\right)=[\ell,r]$ if
$x\in\textrm{Int}\left([\ell,r]\right)$.
