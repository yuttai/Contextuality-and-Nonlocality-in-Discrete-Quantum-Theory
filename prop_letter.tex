...
\sum_{j=1}^{N}
q_{j}\mu_{\phi_{j}}^{\mathrm{B}}\left(P\right)$~\citep{Born1983bibTeX,Mermin2007,Jaeger2007}.
...
follows. Consider a non-contextual hidden variable model (e.g., as
proposed by Bell or
Kochen-Specker~\cite{BELL_1966,kochenspecker1967}). Such a model
assigns definite values to all observables at all times, and hence
assigns a \emph{determinate} probability (0 or 1) to each event. Such
a probability measure directly induces a 0-deterministic QIVPM by
changing 0 to $\imposs$ and 1 to $\necess$.
...
We have quantified one aspect of uncertainty in quantum
...
property of a system without disturbing it somewhat. There are
fundamental limits to what one can measure and these limits are
typically attributed to complementarity. Our imprecision is an
\emph{additional} indeterminacy to the inherent probabilistic nature
