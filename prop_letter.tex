...
for more indeterminate behavior. % For example, when
% $\delta=\frac{1}{3}$, two possible collections of
% intervals\nb$\mathscr{I}$ could be
% $\left\{
%   \imposs,\left[0,\frac{1}{3}\right],\left[\frac{2}{3},1\right],
%   \necess\right\}$ and
% $\left\{ \imposs,\left[\frac{1}{6},\frac{1}{6}\right],
%   \left[\frac{1}{3},\frac{1}{3}\right],\left[\frac{2}{3},\frac{2}{3}\right],
%   \left[\frac{5}{6},\frac{5}{6}\right],\necess\right\}$.
Our technical result will be that for ``small enough'' $\delta$, a
variant of the Kochen-Specker result is still valid. To state and
prove this generalization we will first reformulate the original
Kochen-Specker theorem as a statement regarding the non-existence of
$0$-deterministic QIVPM. We will then strengthen this result to show
the non-existence of any $\delta$-deterministic QIVPM when
$\delta < 1/3$ and the Hilbert space dimension is greater than 2.
...
mathbb{1} is identity projector/observable/matrix
expectation value relative to mubar
explain why expectation value of 0-determinstic qivpm is eigen value
of observable
...
nine observables $\mathbf{O}_{ij}$ with both $i$ and $j$ in the range
$\{1,2,3\}$ used in the ``Mermin-Peres'' proof of the Kochen-Specker
theorem~\cite{Mermin1990Simple,peres1995quantum}. These nine
observables are given below: {\renewcommand{\arraystretch}{2}%
\begin{center}
...
\noindent where $\left\{ \mathbb{1},\sigma_{x},\sigma_{y},\sigma_{z}\right\} $
are the Pauli matrices acting on a pair of spin-$\frac{1}{2}$ states.
These matrices satisfy $\left(\sigma_{z}\otimes\sigma_{z}\right)\left(\sigma_{x}\otimes\sigma_{x}\right)=-\sigma_{y}\otimes\sigma_{y}$.
Furthermore we have that the observables in the square satisfy the
following properties: each observable has expectation value~$1$
or~$-1$; 
...
\noindent in each row and column, the observables commute; and in
each row and column except the third column, each operator is the
product of the other two. To find a QIVPM~$\bar{\mu}$ \ldots{}
