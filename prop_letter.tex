...
the Born rule which induces the quantum probability measures for each
state. When we shift our attention to QIVPMs, we also want to construct
a QIVPMs according to a simple rule. Since the ranges of the quantum
...
to $\iota\left(x\right)\in\mathscr{I}$, then we can concatenate it
with a quantum probability measure~$\mu:\events\rightarrow\left[0,1\right]$
...
\begin{thm}If there exists three numbers~$x_{0}$, $x_{1}$, and
...
\begin{eqnarray}
 &  & \begin{aligned} & \mathrel{\hphantom{=}}\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{0}+\proj{1}+\proj{2}\right)\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{2}\right)\right)\end{aligned}
\nonumber \\
...
\end{aligned}
\\
 &  & \begin{aligned} & =\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{0}+\proj{2}\right)\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(\proj{1}+\proj{2}\right)\right)\textrm{ ,}\end{aligned}
\nonumber 
\end{eqnarray}
...
individually. For example, consider the following easy example. In
the four-dimensional Hilbert space, 
\begin{equation}
...
\end{cases}
\end{equation}
and $\rho=\left(\proj{0}+\proj{1}+\proj{2}+\proj{3}\right)/4$ induce
a QIVPM~$\iota\circ\mu_{\rho}^{\mathrm{\mathrm{B}}}$ which is a
non-trivial fact~\citep{HOSTunpublished}. In order to verify whether
$\iota\circ\mu$ is a QIVPM easier, we want to impose some conditions
on $\iota$. Notice that our counterexamples are in a four-dimensional
Hilbert space, the conditions for $\iota$ for a Hilbert space of
dimension~$d\le3$ is not as strict as the general conditions.
