...
I need to check whether it qualities as a classical probabilitic hidden variable
theory\ldots(?)
\end{enumerate}
}
...
$\expval{\mathbf{O}}_{\mu_{1,2}}$, are 1.5 in the first case is 1.5
and 2 in the second. 
...
probability of an event $E$, the other refers to the probability of
the event's complement $\overline{E}$.  For example, if we discover as
a result of an experiment that $\mu(E) = [0.2,0.3]$ for some
event~$E$, we may conclude that $\mu(\overline{E}) = [0.7,0.8]$ for
the complement event $\overline{E}$. In addition to these simple
...
In conventional quantum mechanics the expectation value of an
observable is defined in Eq.~\eqref{eq:quantum-expectation} as a precise
real number. The generalization of quantum measures to QIVPMs implies
that the real-valued expectation value will be generalized to an
interval.
...
\begin{definition}[Expectation Value of Observables over QIVPMs] Let
  ...
  subspace of events containing all the projectors $P_i$, then we define:
\begin{equation}
...
If $\bar{\mu}$ is a conventional (Born) probability measure defined by
state $\rho$, then $\coreBorn\left(\bar{\mu},\events'\right)$ will
...
to strengthen one of the fundamental theorems in of quantum
...
interval-valued probability framework will allow us to state, and prove, a
stronger version of the theorem that holds even if the observables
have values that are only definite up to some specific parameter that
quantifies experimental uncertainty. Put differently, our stronger
statement and theorem quantifies Mermin's intuition:
\begin{quote}
...
observables~$\left\{ \mathbf{O}_{j}\right\} _{j=1}^{M}$, we have:
\begin{equation}
\prod_{j=1}^{M}\expval{\mathbf{O}_{j}}_{\bar{\mu}}=\expval{\prod_{j=1}^{M}\mathbf{O}_{j}}_{\bar{\mu}}\label{eq:Kochen-Specker-Rules}
\end{equation}
Intuitively the expectation value of an observable lies between two
possible outcomes, which themselves lie between the minimum and
maximum bounds of the given probability intervals.  If the QIVPM is
...
\begin{thm}
[0-Deterministic Variant of Kochen-Specker] \label{thm:Kochen-Specker} 
...
expectation value over the QIVPM for our Mermin-Peres square.

Since the expectation value~$\expval{\mathbf{O}_{ij}}_{\bar{\mu}}$ satisfies
...
%\begin{proof} 
Suppose there is a $\delta$-deterministic
...
$\bar{\mu}\left(P\right)=\left[n/3,n/3\right]$, where $P$ projects
onto an $n$-dimensional subspace of a three-dimensional Hilbert space.
