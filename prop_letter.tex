...
theorem~\cite{kochenspecker1967,Redhead1987-REDINA,peres1995quantum,Jaeger2007,Held2016}. Our finite-precision
...
interval $[0,0]$ or $[1,1]$, i.e., every event is completely
determined with certainty. As $\delta$ gets larger, the QIVPM allows for
more indeterminate behavior. For example, when $\delta=\frac{1}{3}$,
two possible collections of intervals\nb$\mathscr{I}$ could be 
$\left\{ \imposs,\left[0,\frac{1}{3}\right],\left[\frac{2}{3},1\right],
\necess\right\}$ and $\left\{ \imposs,\left[\frac{1}{6},\frac{1}{6}\right],
\left[\frac{1}{3},\frac{1}{3}\right],\left[\frac{2}{3},\frac{2}{3}\right],
\left[\frac{5}{6},\frac{5}{6}\right],\necess\right\}$. Our technical result will be
that for ``small enough'' $\delta$, a variant of the Kochen-Specker
result is still valid. To state and prove this generalization we will first
reformulate the original Kochen-Specker theorem as a statement regarding the
...
A 0-deterministic QIVPM\nb$\bar{\mu}$ has an important property
regarding to the expectation values of a family of $M$ commuting
observables\nb$\left\{ \mathbf{O}_{j}\right\} _{j=1}^{M}$ relative
to $\bar{\mu}$. Since $\left\{ \mathbf{O}_{j}\right\} _{j=1}^{M}$
commutes, they can be diagonalized by a common orthonormal basis~$\Omega=\left\{ \ket{k}\right\} _{k=0}^{d-1}$.
Since $\bar{\mu}$ is 0-deterministic, a unique $\ket{k_{0}}$ and
a unique quantum probability measure\nb$\mu$ exists such that $\bar{\mu}\left(P\right)=\left\{ \mu\left(P\right)\right\} $
and $\mu\left(P\right)\in\left\{ 0,1\right\} $ for all $P\in\events$;
$\mu\left(\proj{k}\right)=1$ if and only if $k=k_{0}$\nb\citep{HOSTunpublished}.
If we denote the product observables\nb$\prod_{j=1}^{M}\mathbf{O}_{j}$
as $\overline{\mathbf{O}}$, the expectation values of the observables\nb$\expval{\mathbf{O}_{j}}_{\bar{\mu}}$
and $\expval{\overline{\mathbf{O}}}_{\bar{\mu}}$ are equal to the
singleton sets whose only elements are $\expval{\mathbf{O}_{j}}_{\mu}$
and $\expval{\overline{\mathbf{O}}}_{\mu}$, respectively. Both $\prod_{j=1}^{M}\expval{\mathbf{O}_{j}}_{\mu}$
and $\expval{\overline{\mathbf{O}}}_{\mu}$ are equal to the product
of the eigenvalues corresponding to the pure state\nb$\ket{k_{0}}$
for each observable\nb$\mathbf{O}_{j}$. By adopting an intuitive
