...
there is always a state consistent with composited QIVPM $\iota\circ\mu_{\rho}^{\mathrm{B}}$.
...
To state this result, we need to introduce the norm of pure states
and operstors. The norm of a pure state~$\ket{\psi}$ is defined
as usual $\left\Vert \psi\right\Vert =\sqrt{\ip{\psi}{\psi}}$, and
the norm of the difference between two states~$\rho$ and $\rho'$
