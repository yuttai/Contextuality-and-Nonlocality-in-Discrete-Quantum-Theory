...
from a state according to Born rule~\citep{Born1983,peres1995quantum,544199,Jaeger2007}.
...
\noindent Since QIVPMs are just finite precision generalization of
conventional quantum probability measures, we also want to seek a
simple rule to construct them, and understand which states are consistent
with the constructed QIVPM. However, there exists a QIVPM such that
no state can be consistent with it. Since no state is consistent with
it, there is no natural way to construct it from any state.
...
This replacement suggests to find a function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
