...
By expending the definitions, we have $\rho'\in B_{\epsilon}\left(\rho\right)$
if and only if $\left|\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\right|\le\epsilon$
for any pure state~$\ket{\psi}$. This fact will be used to prove
the following theorem that is obtained by replacing the QIVPM~$\bar{\mu}$
in Thm.~\ref{cor:Gleason's} by a Born QIVPM~$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$,
and estimate the states consistent with it.
...
\noindent On the one hand, if a state~$\rho'$ is consistent with
$\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$, the difference between the
Born quantum probabilities on any 1-dimensioal projector~$\proj{\psi}$,
$\left|\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\right|$,
must be bounded by $1/n$ which is the maximal length of the intervals
in $\mathscr{I}_{n}$, i.e., $\rho'\in B_{1/n}\left(\rho\right)$.
On the other hand, if $\rho'\in B_{\frac{1}{12n}}\left(\rho\right)$,
we have $\left|\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)\right|\le\frac{1}{12n}$
for any pure state~$\ket{\psi}$. Since the dimension of the Hilbert
space is $3$, the same difference on any projector~$P$, $\left|\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)-\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\right|$,
...
and $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)$ must both be in the
same intervals, and $\rho'$ is consistent with $\iota_{n}\circ\mu_{\rho}^{\mathrm{B}}$.

As a concrete example of Thm.~\ref{thm:Finite-precision-Gleason},
since $\left\Vert \rho_{0}-\rho_{1}\right\Vert \le\frac{1}{24}$~\citep{HOSTunpublished},
we have $\rho_{0}\in B_{1/24}\left(\rho_{1}\right)$ and $\rho_{1}\in B_{1/24}\left(\rho_{0}\right)$
that should imply $\rho_{1}\in\coreBorn\left(\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}},\events\right)$
