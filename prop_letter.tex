...
$\iota\circ\mu$ is always a QIVPM, we consider the following homomorphism-like
properties for $\iota$.

\begin{definition}[{morphism}]{\label{def:morphism}}Given a collection
of intervals $\mathscr{I}$ including $\imposs$ and $\necess$, a
function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$ is called
a \emph{morphism}\yutsung{We can give it a better name. If sticking
with this name, we may need to prove that it is a morphism in the
sense of category theory or something...}if $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
\noindent Because the similarity between Def.~\ref{def:morphism}
and Def.~\ref{def:QIVPM}, $\iota\circ\mu$ should always be a IVPM
no matter in classical or quantum case. Surprisingly, if $\iota\circ\mu$
is always a IVPM, then $\iota$ must be a morphism.
...
\ref{enu:iota-mu-QIVPM}. The rest of the proof is to prove that if
$\iota$ is not a morphism, then $\iota\circ\mu$ might not be a QIVPM
for any quantum probability measure~$\mu$. If $\iota$ doesn't satisfy
...
Even if $\iota$ is not a morphism, the composited function~$\iota\circ\mu$
may still be a QIVPM under so suitable conditions. For example, Eq.~(\ref{eq:iota-Inclusion})
can be weaken as follow: for any numbers~$x_{0}$ and $x_{1}\in\left[0,1\right]$,
...
satisfies Eqs.~(\ref{eq:iota-constraints}) and Eq.~(\ref{eq:iota-add}),$\iota\circ\mu$
still must be a QIVPM~\citep{HOSTunpublished}. 
...
$\left\Vert \psi\right\Vert =\sqrt{\ip{\psi}{\psi}}$. Then, the norm
of the difference of two states~$\rho$ and $\rho'$ is just the
usual operator norm
\begin{equation}
...
Since morphisms have many desirable properties, the intervals it mapping
to either is trivial or contains infinity intervals.

\begin{thm}\label{thm:convex-uncountable}For any function~$\iota:\left[0,1\right]\rightarrow\mathscr{I}$
described in Thm.~\ref{thm:iota-statements}, either $\mathscr{I}=\mathscr{I}_{0}$
defined in Eq.(\ref{eq:3-value-intervals}) or $\mathscr{I}$ contains
uncountably many intervals.\end{thm}
