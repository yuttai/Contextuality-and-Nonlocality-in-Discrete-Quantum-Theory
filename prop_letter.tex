...
if $k$ even. For example, consider $n=2$, 
\begin{eqnarray}
\widehat{\mathscr{I}}_{1} & = & \set{\left[\frac{k-1}{4},\frac{k+1}{4}\right]\cap\left[0,1\right]}{k=0,\ldots,4}\nonumber \\
 &  & \cup\{\imposs,\necess\}\nonumber \\
...
\end{eqnarray}
$\widehat{\iota}_{2}\circ\mu_{1}$ can be computed by Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute the QIVPM~$\widehat{\iota}_{2}\circ\mu_{1}$
from $\mu_{1}$ on the minimal subspace of events containing $\proj{0}$,
$\proj{1}$, and $\proj{2}$.}
\begin{tabular}{ccc}
\toprule 
...
\end{tabular}
\end{table}
Let $\bar{\mu}_{4}$ be $\widehat{\iota}_{2}\circ\mu_{1}$. It is
clear that $\bar{\mu}_{4}$ satisfies Eqs.~(\ref{eq:QIVPM-constraints}).
For Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion}),
given a pair of commuting projectors~$P_{0}$ and $P_{1}$ on a three-dimensional
Hilbert space, one of the following must hold:
\begin{enumerate}
\item When the pair of $P_{0}$ and $P_{1}$ is the same as the pair $P_{0}P_{1}$
and $P_{0}+P_{1}-P_{0}P_{1}$, we have $\bar{\mu}_{4}\left(P_{0}+P_{1}-P_{0}P_{1}\right)+\bar{\mu}_{4}\left(P_{0}P_{1}\right)=\bar{\mu}_{4}\left(P_{0}\right)+\bar{\mu}_{4}\left(P_{1}\right)$,
so that Eq.~(\ref{eq:QuantumInterval-valuedProbability-Inclusion})
clearly holds.
\item When either $P_{0}P_{1}=\mathbb{0}$ or $P_{0}+P_{1}-P_{0}P_{1}=\mathbb{1}$,
we must have 
\begin{eqnarray}
 &  & \bar{\mu}_{4}\left(P_{0}+P_{1}-P_{0}P_{1}\right)+\bar{\mu}_{4}\left(P_{0}P_{1}\right)\nonumber \\
 & \subseteq & \left[\mu_{1}\left(P_{0}+P_{1}-P_{0}P_{1}\right)+\mu_{1}\left(P_{0}P_{1}\right)-\frac{1}{4},\right.\nonumber \\
 &  & \left.\mu_{1}\left(P_{0}+P_{1}-P_{0}P_{1}\right)+\mu_{1}\left(P_{0}P_{1}\right)+\frac{1}{4}\right]\nonumber \\
 & = & \left[\mu_{1}\left(P_{0}\right)+\mu_{1}\left(P_{1}\right)-\frac{1}{4},\mu_{1}\left(P_{0}\right)+\mu_{1}\left(P_{1}\right)+\frac{1}{4}\right]\nonumber \\
 & \subseteq & \bar{\mu}_{4}\left(P_{0}\right)+\bar{\mu}_{4}\left(P_{1}\right)\,.
\end{eqnarray}
\end{enumerate}
Therefore, $\bar{\mu}_{4}$ is a QIVPM.

\yutsung{Clean up until here.}

In the classical setting, measurements done with limited resources
and poor precision may not uniquely identify the true state of a system.
However, there is always at least one state that is consistent with
the imprecise measurements.
...
Gleason's theorem does not hold, at least for this example.
...
\end{eqnarray}
which are overlapping intervals with length~$1/n$ except the intervals
at length~$\left(2n\right)^{-1}$ at each end.
