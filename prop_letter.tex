...
and contextuality~\cite{BarrettKent2004,Appleby_2005}.  For example,
...
$P$. These include the empty projection~$\mathbb{0}$, the identity
...
there are several proposals for ``imprecise probabilities~\cite{Dempster1967,Artstein1972,Shafer1976,GilboaSchmeidler1994,Marinacci1999,Weichselberger2000,JamisonLodwick2004,Grabisch2016}.''
...
$\mu(E) = [0.2,0.3]$ for some event~$E$, we may conclude thatv
...
or $E_2$ occurs might be $\mu(E_1+E_2) = [\ell_1+\ell_2,r_1+r_2]$. In
...
of combining the intervals would give $\mu(E+\overline{E})=[0.9,1.1]$
which is not a valid probability interval. Moreover the event
$E+\overline{E}$ is the identity event covering the entire space; its
...
correct constraint regarding the probability of $E_1+E_2$ when $E_1$
...
understanding that the union of classical sets $E_1+E_2$ is replaced
...
intervals, and we say a state~$\rho$ is consistent with $\bar{\mu}$ on
a projector~$P$ if
...
is the probability that the event~$P$ happens determined by the
Born rule~\cite{Born1983,peres1995quantum,544199,Jaeger2007}.
...
\item For each pair of orthogonal projections~$P_0 \in \events'$ and
...
crypto-deterministic in the sense of Peres~\cite{PeresRon1988,peres1995quantum}. We
...
\begin{definition}Given a Hilbert space of $d\ge3$, for every QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$,
any observable~$\mathbf{O}$ with spectral decomposition $\mathbf{O}=\sum_{i=1}^{N}\lambda_{i}P_{i}$,
...
theorem~\cite{Rosenmuller1971,GilboaSchmeidler1994,Grabisch2016}.
If the QIVPM is crypto-deterministic, this
...
have the the expectation value~$\left\{ 1\right\} $ or $\left\{ -1\right\} $.
...
\begin{thm}\label{thm:Shapley}
  ...
  sub-event space~$\events'\subseteq\events$ commutes, then
...
a quantum IVPM on all projectors~$\events$.
...
for any mixed
state~$\rho\in\coreBorn\left(\bar{\mu}_{3},S_{1}\right)$.  Hence,
