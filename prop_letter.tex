...
based on infinitely precised numbers is proved not experimental verifiable~\cite{PhysRevLett.83.3751,Kent1999}.
...
a quantum state as a probability measure which are proved to be the
same as a density matrix by Gleason's theorem~\cite{Varadarajan2008}.
If we follow Mackey's idea of probability measures together with the
classical IVPM~\cite{JamisonLodwick2004} which could be used to
formulate finite precision measurement in classical situation, we
can define quantum IVPMs. Surprisingly, there is a quantum IVPM which
can not be induced by a density matrix according to the Born rule.
Similar to the tension between finite precision and contextuality
are still inspiring experimental testable definition of contextuality,
the very basic definition of a quantum state or how to compute the
probability from a state might need to be refined when we consider
the finite precision measurement. \newpage{}
