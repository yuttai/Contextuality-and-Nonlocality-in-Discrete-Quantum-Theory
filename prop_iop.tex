...
\begin{definition}\label{def:norm}\cite{544199} Given a operator
or matrix\nb$A$, the norm of $A$ is defined by 
\begin{equation}
...
norms as shown in the following example.

\begin{example}Consider $A=\left(\begin{array}{cc}
1 & 0\\
1 & 1
\end{array}\right)$. It is easy to verify $\left\Vert A\right\Vert =3/2$. On the other
hand, its Frobenius norm or Hilbert-Schmidt norm
\begin{equation}
\left\Vert A\right\Vert _{\mathrm{F}}=\Tr\left(A^{\dagger}A\right)=\left(\sum_{i,j}\left|a_{ij}\right|^{2}\right)^{1/2}
\end{equation}
is $\sqrt{3}$\nb\cite{GolubVanLoan1996,Jaeger2007}, where $a_{ij}$
are the $\left(i,j\right)$-th entry of $A$; its operator $p$-norms
\begin{equation}
\left\Vert A\right\Vert _{p}=\sup_{\left\Vert \ket{\psi}\right\Vert _{p}=1}\left\Vert A\ket{\psi}\right\Vert _{p}=\sup_{\ket{\psi}\ne0}\left\Vert A\ket{\psi}\right\Vert _{p}/\left\Vert \ket{\psi}\right\Vert _{p}
\end{equation}
...
is different from them.\end{example}
