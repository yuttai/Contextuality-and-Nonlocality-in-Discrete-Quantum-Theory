...
To simplify the discussion later, we will only consider a random variable~$X:\Omega\rightarrow\left[0,1\right]$
...
\begin{eqnarray}
\int X\rmd\pmeas & = & \sum_{i=1}^{N}x_{i}\pmeas\left(E_{i}\right)\textrm{ .}
\end{eqnarray}
...
After we discussed the conventional expectation value, we can start
to discuss the expectation value of IVPM. First, the only source use
the exact same definition as ours is written by Jamison and Lodwick~\cite{JamisonLodwick2004}
so it's reasonable to start from their definition of integration\footnote{Although they said they would explore their integration in more details,
they change their definition of IVPM later. Hence, I don't know if
they have any further development of their integration.}. Their integral of $X$ is defined by 
\begin{eqnarray*}
\int_{E}^{\textrm{JL}}X\rmd\bar{\mu} & = & \left[\inf\set{\sum_{i=1}^{N}x_{i}e_{i}}{e_{i}\in\bar{\mu}\left(E_{i}\right)\textrm{ and }\sum_{i=1}^{N}e_{i}=\min\bar{\mu}\left(E\right)},\right.\\
 &  & \left.\sup\set{\sum_{i=1}^{N}x_{i}e_{i}}{e_{i}\in\bar{\mu}\left(E_{i}\right)\textrm{ and }\sum_{i=1}^{N}e_{i}=\max\bar{\mu}\left(E\right)}\right]\textrm{ .}
\end{eqnarray*}
In this definition, we still work on the weighted average. Since we
don't know the exact probability, we want to average on all possible
allowed probability, which is represented by the requirement on $e_{i}\in\bar{\mu}\left(E_{i}\right)$.
However, I don't have a clear intuition on why they want $\sum_{i=1}^{N}e_{i}=\min\bar{\mu}\left(E\right)$
and $\sum_{i=1}^{N}e_{i}=\max\bar{\mu}\left(E\right)$, and we will
have a more intuitive definition following this line when we move
on to Choquet integral~\cite{Choquet1954,Grabisch2016}. After we
have the definition, we can consider the IVPM~$\bar{\mu}$ in example~\ref{ex3}.
Because $\bar{\mu}\left(\Omega\right)=\left[1,1\right]$, we have
\begin{eqnarray*}
\int_{\Omega}^{\textrm{JL}}X\rmd\bar{\mu} & = & \left[\inf\set{\sum_{i=1}^{N}x_{i}e_{i}}{e_{i}\in\bar{\mu}\left(E_{i}\right)\textrm{ and }\sum_{i=1}^{N}e_{i}=1},\right.\\
 &  & \left.\sup\set{\sum_{i=1}^{N}x_{i}e_{i}}{e_{i}\in\bar{\mu}\left(E_{i}\right)\textrm{ and }\sum_{i=1}^{N}e_{i}=1}\right]\\
 & = & \left[\inf\set{e_{1}+2e_{2}}{e_{0}\in\bar{\mu}\left(\{HH\}\right)\textrm{ , }e_{1}\in\bar{\mu}\left(\{HT,TH\}\right)\textrm{ , }e_{2}\in\bar{\mu}\left(\{TT\}\right)\textrm{ , and }e_{0}+e_{1}+e_{2}=1},\right.\\
 &  & \left.\sup\set{e_{1}+2e_{2}}{e_{0}\in\bar{\mu}\left(\{HH\}\right)\textrm{ , }e_{1}\in\bar{\mu}\left(\{HT,TH\}\right)\textrm{ , }e_{2}\in\bar{\mu}\left(\{TT\}\right)\textrm{ , and }e_{0}+e_{1}+e_{2}=1}\right]\\
 & = & \left[\inf\set{e_{1}+2e_{2}}{0\le e_{0}\le\frac{1}{2}\textrm{ , }\frac{1}{2}\le e_{1}\le1\textrm{ , }e_{2}=0\textrm{ , and }e_{0}+e_{1}+e_{2}=1},\right.\\
 &  & \left.\sup\set{e_{1}+2e_{2}}{0\le e_{0}\le\frac{1}{2}\textrm{ , }\frac{1}{2}\le e_{1}\le1\textrm{ , }e_{2}=0\textrm{ , and }e_{0}+e_{1}+e_{2}=1}\right]\\
 & = & \left[\inf\set{e_{1}}{0\le e_{0}\le\frac{1}{2}\textrm{ , }\frac{1}{2}\le e_{1}\le1\textrm{ , and }e_{0}+e_{1}=1},\right.\\
 &  & \left.\sup\set{e_{1}}{0\le e_{0}\le\frac{1}{2}\textrm{ , }\frac{1}{2}\le e_{1}\le1\textrm{ , and }e_{0}+e_{1}=1}\right]\\
 & = & \left[\frac{1}{2},1\right]\textrm{ .}
\end{eqnarray*}

\yutsung{{Put the definition of }Choquet integral, other meta-discourse
here...}

\begin{thm}\cite{Grabisch2016}For every classical interval-valued
probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$ and
any random variable~$X:\Omega\rightarrow\mathbb{R}$, let $l:\events\rightarrow\left[0,1\right]$
and $r:\events\rightarrow\left[0,1\right]$ be the left-end and the
right-end of $\bar{\mu}$, respectively, i.e., $\bar{\mu}\left(E\right)=\left[l\left(E\right),r\left(E\right)\right]$.
The following statements are equivalent.
\begin{itemize}
\item $\bar{\mu}$ is convex.
\item The Choquet integral on $l$ satisfies $\int X\rmd l=\min_{\mu\in\mathrm{core}\left(\bar{\mu}\right)}\int X\rmd\mu$.
\item The Choquet integral on $r$ satisfies $\int X\rmd r=\max_{\mu\in\mathrm{core}\left(\bar{\mu}\right)}\int X\rmd\mu$.
\end{itemize}
\end{thm}

\yutsung{{Put an example}, and other integral here...}
