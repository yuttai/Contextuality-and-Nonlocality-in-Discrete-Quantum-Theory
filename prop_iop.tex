...
\subsection{From Real-valued to Interval-valued}
...
\begin{definition}An interval-valued probability measure\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$
is called $\delta$-deterministic if either $\left[l,r\right]\subseteq\left[0,\delta\right]$
or $\left[l,r\right]\subseteq\left[1-\delta,1\right]$ for every $\left[l,r\right]\in\mathscr{I}$.\end{definition}

For example, recall $\mathscr{I}_{\mathrm{D}}=\left\{ \imposs,\necess\right\} $,
then a $\mathscr{I}_{\mathrm{D}}$-valued QIVPM is $0$-deterministic.
For another example, given a three-dimensional Hilbert space, $\iota_{\frac{1}{3}}\circ\mu_{\frac{\mathbb{1}}{3}}^{\mathrm{B}}:\events\rightarrow\left\{ \imposs,\left[\frac{1}{3},\frac{1}{3}\right],\left[\frac{2}{3},\frac{2}{3}\right],\necess\right\} $
is a $\frac{1}{3}$-deterministic. However, we will prove there is
no $\delta$-deterministic probability measures for smaller $\delta$
due to the Kochen-Specker theorem.
...
\begin{equation} 
\mu(P)=\cases{ 
0 & if $\bar{\mu}(P)\subseteq\left[0,\delta\right]$ ;\\ 
1 & if $\bar{\mu}(P)\subseteq\left[1-\delta,1\right]$
}\end{equation}
...
\Rightarrow & \bar{\mu}(P)\subseteq\left[0,\delta\right]\\
...
Similarly, when $\pmeas\left(P\right)=1$, we also have $\pmeas\left(\mathbb{1}-P\right)=1-\pmeas\left(P\right)$.
\item Given a collection $\left\{ P_{i}\right\} _{i=1}^{N}$ of pairwise
orthogonal events, we must have $N\le d$. Also, $\delta<\left(1+d\right)^{-1}\Rightarrow\delta\le d\delta<1-\delta$.
\begin{itemize}
\item If $\pmeas(P_{i})=0$ for all $i$, then
\begin{equation}\eqalign{ 
& \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\sum_{i=1}^{N}\bar{\mu}(P_{i})\subseteq\left[0,N\delta\right]\subseteq\left[0,d\delta\right]\\
\Rightarrow & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\cap\left[1-\delta,1\right]=\textrm{Ø}\Rightarrow\bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\left[0,\delta\right]\\
\Rightarrow & \pmeas\left(\bigcup_{i=1}^{N}P_{i}\right)=0=\sum_{i=1}^{N}\pmeas(P_{i})\textrm{ .}
}\end{equation}
\item If $\pmeas\left(P_{i_{0}}\right)=1$ for a unique $i_{0}$, then
\begin{equation}\eqalign{ 
& \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\sum_{i=1}^{N}\bar{\mu}(P_{i})\subseteq\left[1-\delta,1+(N-1)\delta\right]\\
\Rightarrow & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\cap\left[0,\delta\right]=\textrm{Ø}\Rightarrow\bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\subseteq\left[1-\delta,1\right]\\
...
\Rightarrow & \bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\cap\left[0,\delta\right]=\bar{\mu}\left(\bigcup_{i=1}^{N}P_{i}\right)\cap\left[1-\delta,1\right]\textrm{Ø}\textrm{ .}
}\end{equation}
A contradiction! Therefore, this case is impossible, and need no further
discussion.
\end{itemize}
\end{itemize}
\end{proof}
...
probability measure for $\delta<\left(1+d\right)^{-1}$. When $\delta\ge\left(1+d\right)^{-1}$,
there exists a quantum $\delta$-deterministic probability measure
$\bar{\mu}:\events\rightarrow\mathscr{I}$ for some $\mathscr{I}$.\end{cor}
