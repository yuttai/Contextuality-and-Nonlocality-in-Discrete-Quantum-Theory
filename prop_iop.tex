...
\begin{itemize}
\item Let $X=\sum_{i=N^{-}}^{N^{+}}x_{i}\mathbf{1}_{E_{i}}$ be a positive
random variable with 
\[
x_{N^{-}}\le x_{N^{-}+1}\le\ldots\le x_{-2}\le x_{-1}\le x_{0}=0\le x_{1}\le x_{2}\le\ldots\le x_{N^{+}-1}\le x_{N^{+}}
\]
 and $E_{i}\cap E_{j}=\emptyset$.
\item Let $E_{i}'=\bigcup_{j\ge i}E_{j}=\set{\omega\in\Omega}{X\left(\omega\right)\ge x_{i}}\in\events$
for $i>0$, and $E_{i}'=\bigcup_{j\le i}E_{j}=\set{\omega\in\Omega}{X\left(\omega\right)\le x_{i}}\in\events$
for $i<0$.
...
\end{equation}
\item We also need to define the sign function as 
\begin{equation} 
\mathrm{sign}=\cases{ 
1 & for $x\ge0$ ;\\
0 & for $x=0$ ;\\
...
\begin{equation}
\int X\rmd\bar{\mu}=\sum_{i=N^{-}}^{N^{+}}x_{i}\left[l\left(E_{i}'\right)-l\left(E_{i+\mathrm{sign}\left(i\right)}'\right),r\left(E_{i}'\right)-r\left(E_{i+\mathrm{sign}\left(i\right)}'\right)\right]\textrm{ .}\label{eq:interval-expectation}
\end{equation}
\end{definition}
...
\[
\left[l\left(E_{i}'\right)-l\left(E_{i+1}'\right),r\left(E_{i}'\right)-r\left(E_{i+1}'\right)\right]\subseteq\left[l\left(E_{i}\right),r\left(E_{i}\right)\right]=\bar{\mu}\left(E_{i}\right)
\]
...
Given the same observable with an extra condition~
\[
\lambda_{N^{-}}\le\lambda_{N^{-}+1}\le\ldots\le\lambda_{-2}\le\lambda_{-1}\le\lambda_{0}=0\le\lambda_{1}\le\lambda_{2}\le\ldots\le\lambda_{N^{+}-1}\le\lambda_{N^{+}}
\]
and let $P_{i}'=\sum_{j\ge i}P_{j}\in\events$ for $i>0$, and $P_{i}'=\sum_{j\le i}P_{j}\in\events$
for $i<0$, we can combine equation~(\ref{eq:interval-expectation})
and (\ref{eq:quantum-expectation}) to define the expectation value
of $\mathbf{O}$ with respect to $\bar{\mu}$ as
\begin{equation}
\expval{\mathbf{O}}_{\bar{\mu}}=\sum_{i=1}^{N}\lambda_{i}\left[l\left(P_{i}'\right)-l\left(P_{i+\mathrm{sign}\left(i\right)}'\right),r\left(P_{i}'\right)-r\left(P_{i+\mathrm{sign}\left(i\right)}'\right)\right]\textrm{ .}\label{eq:quantum-interval-expectation}
\end{equation}
