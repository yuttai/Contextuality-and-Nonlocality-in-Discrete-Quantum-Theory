...
non-commuting observables is considered as one of the reason why quantum
theory is different from the classical one. In another word, the uncertainty
principle would not be observed if we could only access commuting
observables. This is also true for a quantum interval-valued probability
measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$. If we restrict
our event space~$\events$ to a mutually commuting subspace~$\events'$,
as proved in theorem~\ref{thm:sub-event-space}, the restricted $\bar{\mu}|_{\events'}$
is essentially a classical interval-valued probability measure. Furthermore,
if $\bar{\mu}$ is convex, then $\bar{\mu}|_{\events'}$ will be convex,
and have a non-empty core classically. 
...
and $\ket{\ps}$, respectively. We define 
\begin{equation}\eqalign{ 
\bar{\mu}\left(\proj{\psi}\right)=\likely\textrm{ ,}\\ 
\bar{\mu}\left(\mathbb{1}-\proj{\psi}\right)=\unlikely\textrm{ ,} 
}\end{equation}
