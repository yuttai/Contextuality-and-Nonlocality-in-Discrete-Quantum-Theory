...
eigenvalue~$\lambda_{i}$. This spectral decomposition might be different
from the one people used to~.

Since $\lambda_{i}$ are distinct, $N$ may be less than $d$. Although
we requires the eigenvalues in equation~(\ref{eq:spectrum-decomposition-distinct})
to be distinct, we will consider the other type of spectral decomposition,
especially $\mathbf{O}=\sum_{k=1}^{d}\lambda_{k}\proj{k}$ in lemma~\ref{lemma:observable2random-variable}
later. Then, the Born rule of CQT tells us that the expectation value
of the observable~$\mathbf{O}$ is given by~\cite{544199,Jaeger2007}
\begin{equation}
