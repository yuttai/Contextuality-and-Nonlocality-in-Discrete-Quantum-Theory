...
Let $X=\sum_{i=1}^{N}x_{i}\mathbf{1}_{E_{i}}$ be a positive random
variable with $0\le x_{1}\le x_{2}\le\ldots\le x_{N-1}\le x_{N}$
and $\bar{\mu}:\events\rightarrow\mathscr{I}$ a classical IVPM such
that $l:\events\rightarrow\left[0,1\right]$ and $r:\events\rightarrow\left[0,1\right]$
be the left-end and the right-end of $\bar{\mu}$, respectively, i.e.,
$\bar{\mu}\left(E\right)=\left[l\left(E\right),r\left(E\right)\right]$,
the Choquet integral of $X$ with respect to $\bar{\mu}$ is 
\begin{eqnarray}
\int X\rmd\bar{\mu} & = & \sum_{i=1}^{N}\left(x_{i}-x_{i-1}\right)\bar{\mu}\left(\bigcup_{j\ge i}E_{j}\right)\textrm{ ,}\\
 & = & \sum_{i=1}^{N}x_{i}\left[l\left(\bigcup_{j\ge i}E_{j}\right)-l\left(\bigcup_{j\ge i+1}E_{j}\right),r\left(\bigcup_{j\ge i}E_{j}\right)-r\left(\bigcup_{j\ge i+1}E_{j}\right)\right]\label{eq:interval-expectation}
\end{eqnarray}
where $x_{0}=0$, and $x[l,r]=[xl,xr]$ for $x\ge0$. Similar to equation~(\ref{eq:expectation}),
equation~(\ref{eq:interval-expectation}) can be considered as weighted
average of the possible outcomes of $X$ as well, where the ``weight''
\[
\left[l\left(\bigcup_{j\ge i}E_{j}\right)-l\left(\bigcup_{j\ge i+1}E_{j}\right),r\left(\bigcup_{j\ge i}E_{j}\right)-r\left(\bigcup_{j\ge i+1}E_{j}\right)\right]\subseteq\left[l\left(E_{i}\right),r\left(E_{i}\right)\right]=\bar{\mu}\left(E_{i}\right)\textrm{ .}
\]
is tighter than the natural weight. This is reasonable because only
one event can happen at once so we don't be so pessimistic to accumulate
all possible fuzziness together, and use $\sum_{i=1}^{N}x_{i}\bar{\mu}\left(E_{i}\right)$.
