...
\begin{equation}
\mathbf{O}=\sum_{i=1}^{N}\lambda_{i}P_{i}\,,\label{eq:spectrum-decomposition}
\end{equation}
where $P_{i}$ is the projector onto the eigenspace of each distinct
eigenvalue~$\lambda_{i}$. Since $\lambda_{i}$ are distinct, $N$
may be less than $d$. Although we requires the eigenvalues in a spectral
decomposition to be distinct, we will relax this requirement in the
next section. Then, the Born rule of CQT tells us that the expectation
value of the observable~$\mathbf{O}$ is given by~\cite{544199,Jaeger2007}
\begin{equation}
