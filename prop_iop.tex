...
\begin{definition}\cite{544199} Given a operator or matrix\nb$A$,
...
The previously defined matrix norm is not one of the common matrix
norms as in the following example.

\begin{example}Consider $A=\left(\begin{array}{cc}
1 & 0\\
1 & 1
\end{array}\right)$. Then, it is easy to verify that $\left\Vert A\right\Vert =3/2$.
On the other hand, its Frobenius norm or Hilbert-Schmidt norm\nb$\left\Vert A\right\Vert _{\mathrm{F}}$
is $\sqrt{3}$\nb\cite{Jaeger2007}; its operator $p$-norms\nb$\left\Vert A\right\Vert _{p}$
are $\left\Vert A\right\Vert _{1}=\left\Vert A\right\Vert _{\infty}=2$
and $\left\Vert A\right\Vert _{2}=\left[\left(3+\sqrt{5}\right)/2\right]^{\frac{1}{2}}$\nb\cite{RobertsVarberg1973,peres1995quantum}.\end{example}
...
\begin{itemize}
\item $\mu_{\rho}^{\mathrm{B}}\left(P\right)=0$ if and only if $P=\mathbb{0}$.
\item $\mu_{\rho}^{\mathrm{B}}\left(P\right)=1$ if and only if $P=\mathbb{1}$.
\end{itemize}
\end{definition}
...
\begin{proof}Since
\begin{equation}
\frac{\delta}{d}\ge\left\Vert \rho'-\rho\right\Vert =\max_{\ip{\psi}{\psi}=1}\left|\melem{\psi}{\rho'-\rho}{\psi}\right|\,,
\end{equation}
we have
\begin{equation}
\frac{\delta}{d}\ge\left|\melem{\psi}{\rho'-\rho}{\psi}\right|=\left|\mu_{\rho'}^{\mathrm{B}}\left(\proj{\psi}\right)-\mu_{\rho}^{\mathrm{B}}\left(\proj{\psi}\right)\right|\label{eq:Born_rule_difference}
\end{equation}
