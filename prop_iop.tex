...
Consider the random variable~$X_{0}$ representing the number of
tails. For the probability measure~$\pmeas$ in example~\ref{ex1},
the expectation value of $X_{0}$ is
\begin{eqnarray*}
...
Now consider the IVPM~$\bar{\mu}$ in example~\ref{ex3}. First,
the only source use the exact same definition as ours is written by
Jamison and Lodwick~\cite{JamisonLodwick2004}. They introduced a
definition of integration as follow\footnote{\yutsung{Although they said they would explore their integration
in more details, they change their definition of IVPM later. Hence,
I don't know if they have any further development of their integration.}}. If we denote the indicator function by 
\begin{equation} 
...
\end{equation}
then given a random variable~$X=\sum_{i=1}^{N}x_{i}\mathbf{1}_{A_{i}}$,
where $x_{i}$ are all different, the integral of $X$ is defined
by 
\begin{eqnarray*}
...
\end{eqnarray*}
For example, $X_{0}=0\cdot\mathbf{1}_{\{HH\}}+1\cdot\mathbf{1}_{\{HT,TH\}}+2\cdot\mathbf{1}_{\{TT\}}$,
and $\bar{\mu}\left(\Omega\right)=\left[1,1\right]$. Therefore, 
\begin{eqnarray*}
