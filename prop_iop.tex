...
a counterexample to this statement in the quantum case, and have the
following theorem.
...
has an empty core departs from the classical cases. In order to understand
its meaning, we will first study how to recover the non-empty core
by adding conditions. Then, we will consider having an empty core
...
weak to guarantee an convex quantum interval-valued probability measure
has an non-empty core. However, the product of non-commuting projectors
is not a projector so that equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
cannot be meaningful when $P_{0}$ and $P_{1}$ are non-commuting.

Classically, equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
...
be extended to the quantum case, and consider a quantum interval-valued
...
However, example~\ref{ex:three-dimensional-three-value} satisfies
equation~(\ref{eq:Dempster-Shafer-game-quantum}) as well. In another
word, this kind of conditions cannot eliminate the situation of empty
core.
