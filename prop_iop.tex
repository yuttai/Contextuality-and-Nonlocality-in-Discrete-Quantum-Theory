...
to verify whether $\bar{\mu}$ has a non-empty core. Usually, the
easiest way to prove the existence is to construct an example. As
in example~\ref{ex:three-dimensional-5-value}, constructing a QIVPM
directly is long and tedious. In contrast, a quantum probability measure\nb$\mu:\events\rightarrow\left[0,1\right]$
can be easily constructed by the Born rule. Therefore, it would be
desirable if we can construct a QIVPM\nb$\bar{\mu}$ from $\mu$,
and expect $\mu$ is in the core of $\bar{\mu}$.

Notice that the range of $\mu$ and $\bar{\mu}$ are different. If
there is a good function\nb$\iota:\left[0,1\right]\rightarrow\mathscr{I}$,
then a QIVPM can be easily defined by function composition\nb$\events\overset{\mu}{\rightarrow}\left[0,1\right]\overset{\iota}{\rightarrow}\mathscr{I}$.
This gives the following definition.

\begin{definition}We say a pair\nb$\left(\mathscr{I},\iota\right)$
is additive\footnote{\yutsung{This whole definition might be rewritten using category
theory... I would investigate it if it is necessary...}} if $\mathscr{I}$ is a collection of intervals and $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
\end{itemize}
Moreover, $\left(\mathscr{I},\iota\right)$ is called convex if for
any three numbers\nb$x_{0}$, $x_{1}$, and $x_{2}\in\left[0,1\right]$,
if $x_{0}+x_{1}+x_{2}\in\left[0,1\right]$, then $\iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)\subseteq\iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)$.\end{definition}

Although a convex pair\nb$\left(\mathscr{I},\iota\right)$ may look
exotic, the left end of $\iota$, defined by $\iotal\left(x\right)=\min\iota\left(x\right)$,
is the well-studied Wright-convex satisfying $\iotal\left(x_{0}+x_{1}+x_{2}\right)+\iotal\left(x_{2}\right)\ge\iotal\left(x_{0}+x_{2}\right)+\iotal\left(x_{1}+x_{2}\right)$\nb\cite{Wright1954,RobertsVarberg1973,Pales2013}.
After we define\nb$\left(\mathscr{I},\iota\right)$, we are ready
to construct a QIVPM from a quantum probability measure.

\begin{lemma}\label{lemma:real2interval}Consider an additive pair\nb$\left(\mathscr{I},\iota\right)$.
Then, given a quantum probability measure\nb$\mu:\events\rightarrow\left[0,1\right]$,
a function\nb$\bar{\mu}:\events\rightarrow\mathscr{I}$ defined by
$\bar{\mu}(P)=\iota\left(\mu(P)\right)$ is a quantum IVPM. Moreover,
if $\left(\mathscr{I},\iota\right)$ is convex, then $\bar{\mu}(P)$
is a convex.\end{lemma}
...
and $P_{0}P_{1}$. Hence, 
\begin{equation}\eqalign{ 
 & \bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)=\iota\left(\mu\left(P_{0}\vee P_{1}\right)\right)+\iota\left(\mu\left(P_{0}P_{1}\right)\right)\\
...
= & \iota\left(\mu\left(P_{0}\right)\right)+\iota\left(\mu\left(P_{1}\right)\right)=\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right) 
}\end{equation}
\end{itemize}
\end{proof}

\begin{lemma}Consider an additive pair\nb$\left(\mathscr{I},\iota\right)$.
...
Obviously, if $\left(\mathscr{I},\iota\right)$ is convex, so does
$\bar{\mu}$.\end{lemma}
...
there is an additive pair\nb$\left(\mathscr{I}_{n},\iota_{n}\right)$
defined by 
\begin{equation}\eqalign{ 
...
Although a QIVPM can be constructed by an additive pair easily, a
convex pair may not give a convex QIVPM with non-trivial finite number
of intervals. Notice a Wright-convex function is midpoint-convex or
Jensen-convex, i.e., 
\begin{equation}
...
Therefore, for any convex QIVPM constructed by a convex pair\nb$\left(\mathscr{I},\iota\right)$,
$\mathscr{I}$ must either be $\left\{ \necess,\imposs,\unknown\right\} $
or contain infinitely many intervals.
