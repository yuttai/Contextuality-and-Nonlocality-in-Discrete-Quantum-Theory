...
has the same range as all expectation value in the core.
...
\subsection{Between Expectation Values for Classical and Quantum IVPMs}
...
Notice that a sub-event space induced by any observable commutes so
that we can find a set~$\Omega$, a function~$\tau:\events'\rightarrow2^{\Omega}$
to pull quantum real-valued or interval-valued probability measures
to classical ones by theorem~\ref{thm:sub-event-space}. Although
it is a powerful tool, it would be more powerful if we can pull an
observable to a random variable as well.
...
\begin{lemma}Adopting the definitions in this subsection. Given a
density matrix~$\rho$, there is a classical real-valued probability
measure~$\mu':2^{\Omega}\rightarrow\left[0,1\right]$ such that $\mu_{\rho}^{\mathrm{B}}=\mu'\circ\tau$.
Then, we have $\expval{\mathbf{O}}_{\rho}=\int X\rmd\mu'$.\end{lemma}
...
\expval{\mathbf{O}}_{\rho} & \stackrel{\textrm{Equation }(\ref{eq:quantum-expectation})}{=} & \sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{s}\right)\\
 ...
 & \stackrel{\textrm{Equation }(\ref{eq:expectation})}{=} & \int X\rmd\left(\mu'\right)
\end{eqnarray*}
\end{proof}

\begin{lemma}Adopting the definitions in this subsection. Given a
quantum IVPM~$\bar{\mu}$, there is a classical real-valued probability
measure~$\bar{\mu}':2^{\Omega}\rightarrow\mathscr{I}$ such that
$\bar{\mu}=\bar{\mu}'\circ\tau$. Then, we have $\expval{\mathbf{O}}_{\bar{\mu}}=\int X\rmd\bar{\mu}'$.\end{lemma}
...
\begin{proof}Since $\events'$ is induced by $\mathbf{O}$, it is
commuting. By theorem~\ref{thm:sub-event-space}, there is set~$\Omega$,
a function~$\tau:\events'\rightarrow2^{\Omega}$, and a convex classical
interval-valued probability measure~$\bar{\mu}':2^{\Omega}\rightarrow\mathscr{I}$
such that 
\begin{equation}
\bar{\mu}=\bar{\mu}'\circ\tau\textrm{ .}\label{eq:sub-event-space}
\end{equation}
Let $\mul[\prime]:\events'\rightarrow\left[0,1\right]$ and $\mur[\prime]:\events'\rightarrow\left[0,1\right]$
are the left-end and the right-end of $\bar{\mu}'$, respectively,
i.e., $\bar{\mu}'\left(E\right)=\left[\mul[\prime]\left(E\right),\mur[\prime]\left(E\right)\right]$
for all $E\in2^{\Omega}$. Consider the random variable~$X:\Omega\rightarrow\mathbb{R}$
defined by $X=\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mathbf{1}_{\tau\left(P_{i}^{s}\right)}$.
Then, 
\begin{eqnarray*}
 &  & \expval{\mathbf{O}}_{\bar{\mu}}\\
 & \stackrel{\textrm{Equation }(\ref{eq:quantum-interval-expectation})}{=} & \sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\left[\Delta\mul\left(P_{i}^{s\prime}\right),\Delta\mur\left(P_{i}^{s\prime}\right)\right]\\
 & \stackrel{\textrm{Equation }(\ref{eq:sub-event-space})}{=} & \sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\left[\Delta\mul[\prime]\left(\tau\left(P_{i}^{s\prime}\right)\right),\Delta\mur[\prime]\left(\tau\left(P_{i}^{s\prime}\right)\right)\right]\\
 & \stackrel{\textrm{Definition }\ref{def:Choquet}}{=} & \int X\rmd\bar{\mu}'\\
 ...
 & \stackrel{\textrm{Definition }\ref{def:Choquet}}{=} & \left[\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right),\max_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right)\right]\\
 & \stackrel{\textrm{Claim }\ref{claim:min-max-core}}{=} & \left[\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right),\max_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)\right]\\
 & \stackrel{\textrm{Definition }\ref{def:consistent-state}}{=} & \left[\min_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{s}\right),\max_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{s}\right)\right]\\
 & \stackrel{\textrm{Equation }(\ref{eq:quantum-interval-expectation})}{=} & \left[\min_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\expval{\mathbf{O}}_{\rho},\max_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\expval{\mathbf{O}}_{\rho}\right]\textrm{ ,}
\end{eqnarray*}
where claim~\ref{claim:min-max-core} is going to be proved in the
next paragraph.

\begin{claim}\label{claim:min-max-core}
\begin{eqnarray}
\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right) & = & \min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)\label{eq:min-core}\\
\max_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right) & = & \max_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)\label{eq:max-core}
\end{eqnarray}

\end{claim}

\begin{proof}Since equation~(\ref{eq:min-core}) and (\ref{eq:max-core})
are essentially the same, we are only going to prove equation~(\ref{eq:min-core}).

We have
\begin{equation}
\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right)\le\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)
\end{equation}
because the same $\tau$ in theorem~\ref{thm:sub-event-space} applies
to both real-valued and interval-valued probability measure, i.e.,
for any $\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)$, there
exists a $\mu'\in\mathrm{core}\left(\bar{\mu}'\right)$ such that
$\mu=\mu'\circ\tau$. 

On the other hand, 
\begin{equation}
\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu'\left(\tau\left(P_{i}^{s}\right)\right)\ge\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)
\end{equation}
because for any $\mu'\in\mathrm{core}\left(\bar{\mu}'\right)$, we
have 
\begin{eqnarray}
...
Then, we have 
\begin{eqnarray}
\mu_{\rho}^{\mathrm{B}}\left(P_{j}^{-}\right) & = & \sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\frac{\mu'\left(\tau\left(P_{i}^{s}\right)\right)}{\Tr\left(P_{i}^{s}\right)}\Tr\left(P_{i}^{s}P_{j}^{-}\right)=\mu'\left(\tau\left(P_{j}^{-}\right)\right)\\
\mu_{\rho}^{\mathrm{B}}\left(P_{j}^{+}\right) & = & \sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\frac{\mu'\left(\tau\left(P_{i}^{s}\right)\right)}{\Tr\left(P_{i}^{s}\right)}\Tr\left(P_{i}^{s}P_{j}^{+}\right)=\mu'\left(\tau\left(P_{j}^{+}\right)\right)
\end{eqnarray}
Therefore, $\mu_{\rho}^{\mathrm{B}}\in\mathrm{core}\left(\bar{\mu},\events'\right)$.\end{proof}\end{proof}

Our next lemma discusses the quantum version of lemma~\ref{lemma:Choquet-Lebesgue}.

\begin{lemma}Given a Hilbert space of $d\ge3$, when $\mul:\events\rightarrow\left[0,1\right]$
...
\begin{proof}The first part of the lemma is the consequence of Gleason's
theorem directly. For the second part of the lemma \end{proof}
...
Kochen-Specker theorem directly.

\begin{thm}\label{cor:Kochen-Specker-expectation}Given a Hilbert
...
\begin{proof}Assume there is a quantum IVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}_{\mathrm{D}}$,
