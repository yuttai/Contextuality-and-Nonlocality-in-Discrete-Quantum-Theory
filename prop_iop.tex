...
We can apply the idea of restricting the event space in example~\ref{ex:three-dimensional-three-value},
but instead of commuting sub-event space, we will try to find some
subset~$\events'\subseteq\events$ such that $\bar{\mu}|_{\events'}$
has an non-empty core. The following are three of all possibilities:
\begin{itemize}
\item When $\events'=\events\backslash\left\{ \proj{\ps'}\right\} $, because
$\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps})=\imposs$, we have $\mathrm{core}\left(\bar{\mu}|_{\events'}\right)=\left\{ \mu_{\ket{2}}^{B}\right\} $.
\item When $\events'=\events\backslash\left\{ \proj{\ps}\right\} $, because
$\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps'})=\imposs$, we have $\mathrm{core}\left(\bar{\mu}|_{\events'}\right)=\left\{ \mu_{\ket{1}}^{B}\right\} $.
\item When $\events'=\events\backslash\left\{ \proj{0},\proj{\ps},\proj{\ps'}\right\} $,
because $\bar{\mu}(\proj{\phi})\ne\necess$ for all $\ket{\phi}$,
we have $\mu_{\frac{\mathbb{1}}{3}}^{B}\in\mathrm{core}\left(\bar{\mu}|_{\events'}\right)$
as well as $\mu_{\ket{2}}^{B}$ and $\mu_{\ket{1}}^{B}\in\mathrm{core}\left(\bar{\mu}|_{\events'}\right)$.
\end{itemize}
Each of these possibilities is compatible with some but not all of
the observations. The first two possibilities are particular interesting
since each two 1-dimensional projectors~$\proj{\psi_{0}}$ and $\proj{\psi_{1}}$
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.38]{figure3} 
\par\end{centering}
