...
is called a sub-event if it satisfies the follow conditions.
\begin{itemize}
...
sub-event~$\events'\subseteq\events$, there is set~$\Omega$ and
a function~$\tau:\events'\rightarrow2^{\Omega}$ such that:
\begin{itemize}
...
for all $P\in\events'$.
\item If $\mu:\events'\rightarrow\mathscr{I}$ is a quantum interval-valued
probability measure, then there is a classical interval-valued probability
measure~$\mu':\Omega\rightarrow\mathscr{I}$ such that $\mu\left(P\right)=\mu'\left(\tau\left(P\right)\right)$
for all $P\in\events'$
\end{itemize}
\end{thm}
...
By definition~\ref{definition:quantum-classical}, $\mu\left(P\right)=\mu^{\Omega}\left(\tau\left(P\right)\right)$,
and $\mu^{\Omega}$ is a quantum interval-valued probability measure
by lemma~\ref{lemma:classical-probability-measures}.\end{proof}
