...
a Hilbert space of dimension~$d$, there exists a convex QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$
...
let $\ket{\ps}=\left(\ket{0}+\ket{1}\right)/\sqrt{2}$ and $\ket{\ps'}=\left(\ket{0}+\ket{2}\right)/\sqrt{2}$.
Consider the function~$\bar{\mu}:\events\rightarrow\mathscr{I}_{0}$
defined in Table~\ref{tab:non-Born-QIVPM}, where $\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} $.
\begin{table}
