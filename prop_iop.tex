...
\section{Expectation Values for IVPMs (Version 1)}
...
famous one is called the Choquet integral~\cite{Choquet1954,GilboaSchmeidler1994,Grabisch2016}.

\begin{definition}Consider the following.
\begin{itemize}
\item $X=\sum_{i=1}^{N}x_{i}\mathbf{1}_{E_{i}}$ be a random variable with
$x_{1}\le x_{2}\le\ldots\le x_{N-1}\le x_{N}$ and $E_{i}\cap E_{j}=\emptyset$.
\item $\bar{\mu}:\events\rightarrow\mathscr{I}$ a classical IVPM such that
$l:\events\rightarrow\left[0,1\right]$ and $r:\events\rightarrow\left[0,1\right]$
are the left-end and the right-end of $\bar{\mu}$, respectively,
i.e., $\bar{\mu}\left(E\right)=\left[l\left(E\right),r\left(E\right)\right]$.
\item Multiplying a negative number switching the left and right end of
an interval 
\begin{equation} 
x[l,r]=\cases{ 
[xl,xr] & for $x\ge0$ ;\\
...
\begin{equation}
\int X\rmd\bar{\mu}=\sum_{i=1}^{N}x_{i}\left[l\left(\bigcup_{j\ge i}E_{j}\right)-l\left(\bigcup_{j\ge i+1}E_{j}\right),r\left(\bigcup_{j\ge i}E_{j}\right)-r\left(\bigcup_{j\ge i+1}E_{j}\right)\right]\textrm{ .}\label{eq:interval-expectation}
\end{equation}
\end{definition}
...
\[
\left[l\left(\bigcup_{j\ge i}E_{j}\right)-l\left(\bigcup_{j\ge i+1}E_{j}\right),r\left(\bigcup_{j\ge i}E_{j}\right)-r\left(\bigcup_{j\ge i+1}E_{j}\right)\right]\subseteq\left[l\left(E_{i}\right),r\left(E_{i}\right)\right]=\bar{\mu}\left(E_{i}\right)
\]
...
expect $\sum_{i=1}^{N}x_{i}\bar{\mu}\left(E_{i}\right)$ to happen.

The Choquet integral has many desirable properties. For example, it
has the same range as all expectation value in the core.

\begin{thm}\cite{GilboaSchmeidler1994,Grabisch2016}For every classical
IVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$ and any random variable~$X:\Omega\rightarrow\mathbb{R}$,
$\bar{\mu}$ is convex if and only if 
\begin{equation}
\int X\rmd\bar{\mu}=\left[\min_{\mu\in\mathrm{core}\left(\bar{\mu}\right)}\int X\rmd\mu,\max_{\mu\in\mathrm{core}\left(\bar{\mu}\right)}\int X\rmd\mu\right]\textrm{ .}
\end{equation}
\end{thm}
...
Consider a quantum IVPM~$\bar{\mu}$ such that $l:\events\rightarrow\left[0,1\right]$
and $r:\events\rightarrow\left[0,1\right]$ are the left-end and the
right-end of $\bar{\mu}$, respectively, i.e., $\bar{\mu}\left(E\right)=\left[l\left(E\right),r\left(E\right)\right]$.
Given the same observable with an extra condition~$x_{1}\le x_{2}\le\ldots\le x_{N-1}\le x_{N}$,
we can combine equation~(\ref{eq:interval-expectation}) and (\ref{eq:quantum-expectation})
to define the expectation value of $\mathbf{O}$ with respect to $\bar{\mu}$
as
\begin{equation}
\expval{\mathbf{O}}_{\bar{\mu}}=\sum_{i=1}^{N}\lambda_{i}\left[l\left(\sum_{j\ge i}P_{j}\right)-l\left(\sum_{j\ge i+1}P_{j}\right),r\left(\sum_{j\ge i}P_{j}\right)-r\left(\sum_{j\ge i+1}P_{j}\right)\right]\textrm{ .}\label{eq:quantum-interval-expectation}
\end{equation}
Let $\events\left(\mathbf{O}\right)$ denote the minimal sub-event
space containing $\left\{ P_{i}\right\} _{i=1}^{N}$. Then, we have
...
\begin{equation}
\expval{\mathbf{O}}_{\bar{\mu}}=\left[\min_{\rho\in\mathrm{core}\left(\bar{\mu},\events\left(\mathbf{O}\right)\right)}\expval{\mathbf{O}}_{\rho},\max_{\rho\in\mathrm{core}\left(\bar{\mu},\events\left(\mathbf{O}\right)\right)}\expval{\mathbf{O}}_{\rho}\right]\textrm{ .}\label{eq:quantum-interval-expectation-core}
\end{equation}
Conversely, if equation~(\ref{eq:quantum-interval-expectation-core})
holds for every observable, then $\bar{\mu}$ is convex.\end{thm}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\section{Expectation Values for IVPMs (Version 2)}
...
and see how to compute the expectation value of different random variables.
To simplify the discussion later, we will only consider a random variable~$X:\Omega\rightarrow\mathbb{R}$
which is a step function, i.e., $X=\sum_{i=N^{-}}^{N^{+}}x_{i}\mathbf{1}_{E_{i}}$,
where 
\begin{itemize}
\item $x_{i}\ge0$ for $i\ge0$ and $x_{i}\le0$ for $i\le0$.
\item $E_{N^{+}}\subseteq E_{N^{+}-1}\subseteq\ldots\subseteq E_{2}\subseteq E_{1}\subseteq\Omega\backslash E_{-1}$,
$E_{N^{-}}\subseteq E_{N^{-}+1}\subseteq\ldots\subseteq E_{-2}\subseteq E_{-1}\subseteq\Omega\backslash E_{1}$,
and $\Omega=E_{0}$.
\item $\mathbf{1}_{E}$ is the indicator function defined by 
\begin{equation} 
\mathbf{1}_{A}(x)=\cases{ 
1 & if $x\in A$ ;\\ 
0 & if $x\notin A$. 
...
the expectation value of $X$ is 
\begin{eqnarray}
\int X\rmd\pmeas & = & \sum_{i=N^{-}}^{N^{+}}x_{i}\pmeas\left(E_{i}\right)\textrm{ .}\label{eq:expectation-1}
\end{eqnarray}

There are various kind of integrals defined for IVPMs, and the most
famous one is called the Choquet integral~\cite{Choquet1954,GilboaSchmeidler1994,Grabisch2016}.

\begin{definition}Given the random variable~$X$ as previous paragraph,
and a classical IVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$.
Then, the Choquet integral of $X$ with respect to $\bar{\mu}$ is
\begin{equation}
\int X\rmd\bar{\mu}=\sum_{i=N^{-}}^{N^{+}}x_{i}\bar{\mu}\left(E_{i}\right)\textrm{ ,}\label{eq:interval-expectation-1}
\end{equation}
where multiplying a negative number switching the left and right end
of an interval 
\begin{equation} 
x[l,r]=\cases{ 
[xl,xr] & for $x\ge0$ ;\\
