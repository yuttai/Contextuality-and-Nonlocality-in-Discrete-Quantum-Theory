...
is additive if $\mathscr{I}$ is a collection of intervals and $\iota:\left[0,1\right]\rightarrow\mathscr{I}$
...
$x_{0}+x_{1}+x_{2}\in\left[0,1\right]$ implies $\iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)\subseteq\iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)$.\end{definition}
...
\item For any projection $P$, $\bar{\mu}\left(\mathbb{1}-P\right)=\iota\left(1-\mu(P)\right)=\left[1,1\right]-\bar{\mu}\left(P\right)$.
...
Therefore, for any convex pair\nb$\left(\mathscr{I},\iota\right)$,
$\mathscr{I}$ must either be $\left\{ \necess,\imposs,\unknown\right\} $
or contain infinitely many intervals.

Although a pair\nb$\left(\mathscr{I},\iota\right)$ is not convex,
it can still induce a convex QIVPM. In particular, every QIVPM on
a three-dimensional Hilbert space is convex by theorem~\ref{thm:convex-3}.
The next natural question is whether a non-convex pair always induces
some non-convex QIVPM, and the answer is ``yes'' by the following
lemma.

\begin{lemma}Given an additive but non-convex pair\nb$\left(\mathscr{I},\iota\right)$,
...
\end{equation}
Let $P_{0}=\proj{0}+\proj{2}$, $P_{1}=\proj{1}+\proj{2}$, and $\bar{\mu}(P)=\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}(P)\right)$.
Then,
\begin{equation}\eqalign{ 
 & \bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)
=\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{0}+\proj{1}+\proj{2})\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{2})\right)\\ 
= & \iota\left(x_{0}+x_{1}+x_{2}\right)+\iota\left(x_{2}\right)
\nsubseteq\iota\left(x_{0}+x_{2}\right)+\iota\left(x_{1}+x_{2}\right)\\ 
= & \iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{0}+\proj{2})\right)+\iota\left(\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{1}+\proj{2})\right)
