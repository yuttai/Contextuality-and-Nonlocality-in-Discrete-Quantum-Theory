...
case. Since $\events'$ is a set of mutually commuting projections,
...
basis~$\Omega=\left\{ \ket{j}\right\} _{j=0}^{d-1}$, a classical
...
is call classical with respect to $\Omega$.\end{definition}
...
if $\events'$ commutes, then $\coreBorn\left(\bar{\mu},\events'\right)\ne\emptyset$.\end{cor}

\begin{proof}Since $\events'$ is a set of mutually commuting projections,
they can be diagonalized by a common orthonormal basis~$\Omega=\left\{ \ket{j}\right\} _{j=0}^{d-1}$,
and we can find a function~$\tau:\events'\rightarrow2^{\Omega}$
such that the condition of theorem~\ref{thm:sub-event-space} holds.
This means that we can find a convex classical IVPM~$\bar{\mu}':2^{\Omega}\rightarrow\mathscr{I}$
such that $\bar{\mu}\left(P\right)=\bar{\mu}'\left(\tau\left(P\right)\right)$
for all $P\in\events'$. By theorem~\ref{thm:Shapley}, there is
a. classical probability measure~$\pmeas:2^{\Omega}\rightarrow\left[0,1\right]$
such that $\pmeas\in\mathrm{core}\left(\bar{\mu}'\right)$. Then,
it is easy to verify that the density matrix~$\sum_{j=0}^{d-1}\pmeas\left(\left\{ j\right\} \right)\proj{j}\in\coreBorn\left(\bar{\mu},\events'\right)$.\end{proof}
