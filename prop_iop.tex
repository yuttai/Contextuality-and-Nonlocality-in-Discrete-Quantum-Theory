...
\begin{example}[Three-dimensional quantum four-interval-valued 
  ...
  $\ket{\ps}=\left(\ket{0}+\ket{1}\right) / \sqrt{2}$, and
  $\ket{\minus}=\left(\ket{0}-\ket{1}\right) / \sqrt{2}$. The definition of
...
In this section, we are going to discuss the relation between quantum
and classical. We first prove that any quantum phenomenon would not
be observed if we could only access commuting projectors since we
can pick a common basis to measure mutually commuting observables
simultaneously~\cite{Mermin_1993}. Based on this fact, we show a
quantum IVPM~$\bar{\mu}$ is convex if its restriction on any orthonormal
basis~$\Omega$, $\bar{\mu}^{\Omega}$ is a convex classical IVPM.
Together with the fact that every classical IVPM~$\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$
is convex when the number of elements in $\Omega$ is less than or
equal to 3, every quantum IVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$
must be convex on a three-dimensional Hilbert space. 

Before we dive into the lemmas and proofs, we start from two definitions.
...
\begin{definition}Given a Hilbert space $\Hilb$ with an orthonormal
basis~$\Omega=\left\{ \ket{j}\right\} _{j=0}^{d-1}$.
\begin{itemize}
...
\item For any quantum IVPM~$\bar{\mu}:\events\rightarrow\left[0,1\right]$,
we define $\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$ by
$\bar{\mu}^{\Omega}\left(E\right)=\bar{\mu}\left(\sum_{\ket{j}\in E}\proj{j}\right)$.
\end{itemize}
\end{definition}

\begin{lemma}$\mu^{\Omega}$ and $\bar{\mu}^{\Omega}$ in the previous
definition are classical interval-valued and real-valued probability
measures, respectively.\end{lemma}
...
\begin{equation}\eqalign{ 
\bar{\mu}^{\Omega}\left(\Omega\backslash E\right)&=\bar{\mu}\left(\sum_{\ket{j}\in\Omega\backslash E}\proj{j}\right)=\bar{\mu}\left(\mathbb{1}-\sum_{\ket{j}\in E}\proj{j}\right)\\
&=\bar{\mu}\left(\mathbb{1}\right)-\bar{\mu}\left(\sum_{\ket{j}\in E}\proj{j}\right)=\left[1,1\right]-\bar{\mu}^{\Omega}\left(E\right)\textrm{ .} 
}\end{equation}commuting
\item For a collection $\left\{ E_{i}\right\} _{i=1}^{N}$ of pairwise disjoint
events, we have
\begin{equation}\eqalign{ 
\bar{\mu}^{\Omega}\left(\bigcup_{i=1}^{N}E_{i}\right)&=\bar{\mu}\left(\sum_{\ket{j}\in\bigcup_{i=1}^{N}E_{i}}\proj{j}\right)=\bar{\mu}\left(\sum_{i=1}^{N}\sum_{\ket{j}\in E_{i}}\proj{j}\right)\\
&\subseteq\sum_{i=1}^{N}\bar{\mu}\left(\sum_{\ket{j}\in E_{i}}\proj{j}\right)=\sum_{i=1}^{N}\bar{\mu}\left(E_{i}\right)\textrm{ .} 
}\end{equation}
\end{itemize}
\end{proof}
...
a non-empty core. However, the core elements of each $\bar{\mu}|_{\events^{\Omega}}$
