...
we have
\begin{equation}
\int X\rmd\bar{\mu}\stackrel{\textrm{Lemma }\ref{lemma:Choquet-Lebesgue}}{=}\left\{ \int X\rmd\mu\right\} \stackrel{\textrm{Equation }(\ref{eq:expectation})}{=}\left\{ X\left(\pmeas^{-1}\left(1\right)\right)\cdot1\right\} =\left\{ X\left(\bar{\mu}^{-1}\left(\necess\right)\right)\right\} \,.\label{eq:expectation-value-eigenvalue-classical}
\end{equation}
\end{proof}
...
\begin{proof} 
\begin{equation}
\prod_{j}\int X_{j}\rmd\bar{\mu}\stackrel{\textrm{Lemma }\ref{lemma:deterministic-CIVPM-expectation}}{=}\prod_{j}\left\{ X_{j}\left(\bar{\mu}^{-1}\left(\necess\right)\right)\right\} =\left\{ \prod_{j}X_{j}\left(\bar{\mu}^{-1}\left(\necess\right)\right)\right\} \stackrel{\textrm{Lemma }\ref{lemma:deterministic-CIVPM-expectation}}{=}\int\prod_{j}X_{j}\rmd\bar{\mu}\,.\label{eq:expectation-value-eigenvalue-classical-1}
\end{equation}
\end{proof}
