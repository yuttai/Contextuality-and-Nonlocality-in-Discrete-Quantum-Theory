...
real-valued probability measures called $\mu:\events\rightarrow\left\{ 1,0\right\} $.
By lemma~\ref{lemma:expectation-singleton}, $\expval{\mathbf{O}}_{\bar{\mu}}=\left\{ \expval{\mathbf{O}}_{\rho}\right\} $.
Since $\mu$ is a probability measure, there is only one pair of $\left(s_{0},i_{0}\right)$
such that $\mu\left(P_{i_{0}}^{s_{0}}\right)=1$. Therefore, 
\[
\expval{\mathbf{O}}_{\rho}\stackrel{\textrm{Equation }(\ref{eq:quantum-expectation})}{=}\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu\left(P_{i}^{s}\right)=\lambda_{i_{0}}^{s_{0}}\textrm{ .}
\]
\end{proof}
