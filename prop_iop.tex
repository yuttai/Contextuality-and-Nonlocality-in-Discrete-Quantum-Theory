...
knowledge about the particular quantum system in question.\footnote{\yutsung{Actually, $\iota\left(\mu_{\ket{0}}^{B}(P)\right)$ is quite
informative. Since $\iota\left(\mu_{\ket{0}}^{B}(P)\right)=\necess$
if and only if $P=\proj{0}$, we know the core of $\iota\left(\mu_{\ket{0}}^{B}(P)\right)$
is exactly $\left\{ \mu_{\ket{0}}^{B}\right\} $. The quantum interval-valued
probability measure for an experimenter with no prior knowledge should
be $\bar{\mu}(P)=\unknown$ for every projection~$P$.}} Formally, we can ask: what can we deduce about the state of a quantum
system given a quantum interval-valued probability measure, i.e.,
given observations done with finite resources. In the case the intervals
are infinitely precise the question reduces to Gleason's theorem which
states that the state of the quantum system is uniquely determined
by the probability measure. But surely the less resources are available,
the less precise the intervals, and the less we expect to know about
the state of the system. To formally state and answer this question
we begin with defining the \emph{core} and \emph{convexity} of a probability
measure as follows. 
\begin{figure}
\begin{centering}
...
Since the discover of Heisenberg's uncertainty principle~\cite{Heisenberg1983,peres1995quantum,544199,Griffiths2003,Jaeger2007},
the existence of non-commuting observables is one of the reason why
quantum theory is different from the classical theory. In another
word, the uncertainty principle would not be observed if we could
only access commuting observables. This is also true for a quantum
interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$.
If we restrict our event space~$\events$ to a mutually commuting
subspace~$\events'$, as proved in theorem~\ref{thm:sub-event-space},
...
When $\bar{\mu}$ is restricted to different $\events'$, the core
elements of each $\bar{\mu}|_{\events'}$ may not be glued together,
and gives a core element of $\bar{\mu}$. The inability of gluing
core elements among commuting $\events'$ to non-commuting $\events$
is consistent with the proof of the Kochen-Specker theorem, where
each triple of commuting observables could be colored well, but we
cannot color all observables which are not all commuting. This is
also consistent with the assertion that mutually commuting observables
...
variable theory in this setting.
...
quantum interval-valued probability measure, and $\bar{\mu}$ is convex
by theorem~\ref{thm:convex-3}. The following argument establishes
...
always holds because the properties of $\bar{f}$.\end{proof}
...
Hilbert space with an orthonormal basis $\left\{ \ket{0},\ket{1},\ket{2}\right\} $.
Let\emph{ }$\ket{\ps}=\left(\ket{0}+\ket{1}\right)/\sqrt{2}$, $\ket{\minus}=\left(\ket{0}-\ket{1}\right)/\sqrt{2}$,
and $\mathscr{I}=\left\{ \imposs,\unlikely,\likely,\necess,\midd\right\} $,
...
\left[1,1\right]-\bar{f}\left(\ket{\psi_2}\right)\subseteq\bar{f}\left(\ket{\psi_0}\right)+\bar{f}\left(\ket{\psi_1}\right)\textrm{ ,}
...
in figure~\ref{fig:three-dimensional-5-value}.
\begin{enumerate}
...
\left[1,1\right]-\bar{f}\left(\ket{\psi_1}\right)=\midd\subseteq\midd+\imposs=\bar{f}\left(\ket{\psi_2}\right)+\bar{f}\left(\ket{0}\right)\textrm{ ,}
}\end{equation}
Similarly, when $\ket{\psi_{0}}$ is $\ket{\ps}$, equation~(\ref{eq:non-additive-vectors})
holds. 
\item When $\ket{0}\notin\mathcal{B}$, $\ket{\ps}\notin\mathcal{B}$, $\ip{\psi_{0}}{0}=0$,
and $\ip{\psi_{1}}{\ps}=0$, we have $\ip{\psi_{2}}{0}\ne0$ and $\ip{\psi_{2}}{\ps}\ne0$.
Then, we have 
\begin{equation}\eqalign{ 
...
\left[1,1\right]-\bar{f}\left(\ket{\psi_1}\right)=\likely\nsubseteq\unlikely+\midd=\bar{f}\left(\ket{\psi_2}\right)+\bar{f}\left(\ket{\psi_0}\right)\textrm{ .}
}\end{equation}
\footnote{\yutsung{{My previous footnote was wrong. }Need to recompute. Sigh...}}
\item The ``otherwise'' case. Then, equation~(\ref{eq:non-additive-vectors})
...
\end{equation}
\end{enumerate}
$\bar{\mu}$ is convex by theorem~\ref{thm:convex-3}. The following
argument establishes that is has an empty core. Assume there is a
real-valued probability measure satisfying $\mu_{\rho}^{B}(P)\in\bar{\mu}(P)$
