...
forward from conventional quantum theory. Assume that a physical system
is represented by a density matrix~$\rho$, and that we wish to measure
a physical quantity~${\cal O}$ of this system, with corresponding
observable~$\mathbf{O}$, whose spectral decomposition is given by~\cite{Griffiths1994,FraleighBeauregard1995,544199,FriedbergInselSpence2002,Jaeger2007}
\begin{equation}
...
\end{equation}
and $P_{i}$ is the projector onto the eigenspace of each distinct
eigenvalue~$\lambda_{i}$. Compare to equation~(\ref{eq:spectrum-decomposition-distinct}),
the spectral decomposition of $\mathbf{O}$ may be represented in
term of outer product as $\mathbf{O}=\sum_{k=1}^{d}\lambda_{k}\proj{k}$
which would be used in lemma~\ref{lemma:observable2random-variable}
later. Disregard the representation of the spectral decomposition,
it can be used to defined the expectation value of the observable~$\mathbf{O}$
...
Consider a QIVPM~$\bar{\mu}$ such that $\mul:\events\rightarrow\left[0,1\right]$
and $\mur:\events\rightarrow\left[0,1\right]$ are the left-end and
the right-end of $\bar{\mu}$. Similar to the classical Choquet integral
in definition~\ref{def:Choquet}, we can let $P_{i}^{s\prime}=\sum_{j\ge i}P_{j}^{s}$
and $\Delta\mu^{*}\left(P_{i}^{s\prime}\right)=\mu^{*}\left(P_{i}^{s\prime}\right)-\mu^{*}\left(P_{i+1}^{s\prime}\right)$,
where $s\in\left\{ +,-\right\} $ and $\mu^{*}\in\left\{ \mul,\mur\right\} $,
and define the expectation value of $\mathbf{O}$ with respect to
$\bar{\mu}$ as
\begin{equation}
...
we would prove some lemmas\footnote{\yutsung{functor? Galois connection?}}
which can help us move the classical results to quantum.
