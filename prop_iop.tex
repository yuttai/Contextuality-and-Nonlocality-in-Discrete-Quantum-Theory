...
For example, any pure state is not totally mixed, and $\mathbb{1}/3$
...
\begin{itemize}
\item If $\left\Vert \rho-\rho'\right\Vert \le\left(4dn\right)^{-1}$, then
$\rho'\in\coreBorn\left(\iota\circ\mu_{\rho}^{\mathrm{B}},\events\right)$.
\item If $\rho'\in\coreBorn\left(\iota\circ\mu_{\rho}^{\mathrm{B}},\events\right)$,
