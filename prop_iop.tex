...
a quantum IVPM. Moreover, if $\left(\mathscr{I},\iota\right)$ is
convex, then $\bar{\mu}$ is convex.\end{lemma}
...
$\left(\mathscr{I}_{\infty},\iota_{\infty}\right)$ is an additive
pair.\end{example}
...
Although a convex QIVPM can be constructed by a convex pair easily,
any convex pair either is $\left\{ \necess,\imposs,\unknown\right\} $
...
Even if an additive pair\nb$\left(\mathscr{I},\iota\right)$ is not
convex, it may still induce a convex QIVPM. In particular, every QIVPM
on a three-dimensional Hilbert space is convex by theorem~\ref{thm:convex-3}.
...
In conclusion, we may still use an additive but non-convex pair to
construct QIVPMs, but we need to verify the convexity of the constructed
QIVPM one-by-one which might be long and tedious.
