...
In~\cite{PeresRon1988}, Peres and Ron defined
\begin{quote}
...
\end{quote}
Unfortunately, Google Book cannot preview all pages, and the InterLibrary
Loan hasn't finished my request, yet...

In~\cite{Atmanspacher2014}, Atmanspacher defined 
\begin{quote}
...one might paraphrase the deterministic yet non-determinable behavior
of such systems as ``cryptodeterminism''...
\end{quote}
where ``deterministic'' and ``determinable'' is defined in the
same section as 
\begin{quote}
The recent creation of the term ``deterministic chaos'' expresses
the tension between ontic (hidden) lawfulness and epistemic (apparent)
irregularity in chaotic system. Our description of the underlying
laws of nature, e.g. by differential equations governing the dynamics
of such systems, is no doubt deterministic, but their observable behavior
is everything but determinable (in the sense of measurable, computable,
or predictable) with arbitrary precision....
\end{quote}
Is Atmanspacher's usage consistent with Peres' and ours here?}}Although a real or interval-valued probability measure could be used
...
event has happened can be model by the following example.
\begin{equation}
\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
\bar{\mu}(\emptyset) & = & 0\\
...
than probabilistic inspired Einstein, Podolsky, and Rosen to ask whether
quantum mechanics have any chance to be improved and predict in the
same way~\cite{EPR1935,peres1995quantum}. This question can be formulated
by whether the quantum mechanics could be modeled by quantum cryptodeterministic
measures.
...
\noindent The answer is ``no'' due to the Kochen-Specker theorem.
