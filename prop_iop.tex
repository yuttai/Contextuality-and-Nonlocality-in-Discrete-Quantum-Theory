...
\author{Andrew J Hanson$^{1}$, Gerardo Ortiz$^{2}$, Amr Sabry$^{1}$ and
Yu-Tsung Tai$^{3,1}$}
...
\noindent{\it Keywords\/}: {\noindent Finite precision measurement, the Born rule, Gleason's
...
described by the wave function $\ket{\psi}=\frac{\sqrt{\pi}}{2}\fket{\ps\frac{1}{2}}+\frac{\sqrt{4-\pi}}{2}\fket{\minus\frac{1}{2}}$
and given the observable $S_{z}=\frac{\hbar}{2}(\fproj{\ps\frac{1}{2}}-\fproj{\minus\frac{1}{2}})$,
then the probability of observing the eigenvalue $\ps\frac{\hbar}{2}$
(with post-measurement state $\fket{\ps\frac{1}{2}}$) is $\frac{\pi}{4}$
and the probability of observing the eigenvalue $\minus\frac{\hbar}{2}$
(with post-measurement state $\fket{\minus\frac{1}{2}}$) is $1-\frac{\pi}{4}$.
According to the frequency interpretation of probability, the above
assertion implies that when performing a very large number of experiments
on quantum systems described by wave function $\ket{\psi}$, a fraction
$\frac{\pi}{4}$ of the experiments will result in the observation
$\ps\frac{\hbar}{2}$ and the remaining fraction $1-\frac{\pi}{4}$
will result in the observation $\minus\frac{\hbar}{2}$.
...
(ii) we cannot build a measurement apparatus corresponding \emph{exactly}
to observable $S_{z}$; and (iii) we cannot prepare \emph{exact} replicas
of any state. In other words, we can only perform a limited number
of experiments on quantum systems closely related to wave function
$\ket{\psi}$ and using an apparatus that closely approximates observable
$S_{z}$. Given any fixed resources, these experiments will \emph{not}
result in the \emph{exact} fraction $\frac{\pi}{4}$ of observations
$\ps\frac{\hbar}{2}$ and $1-\frac{\pi}{4}$ of observations $\minus\frac{\hbar}{2}$.
In fact, these experiments can be viewed as a method (algorithm) to
calculate the value $\pi$ and it is known that the state of the art
algorithms for computing the $n$th binary digit of~$\pi$ require
on the order of $O\left(n\log^{O\left(1\right)}\left(n\right)\right)$
...
worlds. In the classical world, these observations are believed to
be well-understood approximations to an idealized independent reality
that can continuously approached with more and more resources. The
question in the quantum world is more subtle; it is not evident at
all that this resource-aware perspective does not alter the very foundations
of quantum mechanics. Indeed taking such resource bounds into consideration
is what founded computer science as a discipline and has become crucial
for understanding the very nature of computation. Following Feynman~\cite{Feynman1982Simulating},
Landauer~\cite{Landauer1996188}, and others, one might argue that
this resource-bounded perspective is also crucial for understanding
the very nature of physical processes.

In previous work, we analyzed the wave function description of quantum
systems from a resource-bounded perspective. Technically, instead
of postulating that quantum states are rays in a Hilbert space, i.e.,
we instead used special finite fields whose size reflected the available
resources. \ldots{}

\yutsung{If the core is non-empty, could the core characterize $\bar{\mu}$
comprehensively?}
...
and uncomputable real interval~$[0,1]$~\cite{Turing_1937,Ziegler2007,weihrauch2012computable}.
...
includes numbers like $\pi/4$ which can only be computed with increasingly
large resources as the precision increases. Therefore, in a resource-aware
computational or experimental setting, it is more appropriate to consider
probability measures that map events to a set of elements computable
with a fixed set of resources. We expand on this observation in the
next section and then recall its formalization using interval-valued
probability measures~\cite{Weichselberger2000,JamisonLodwick2004}.
...
by the relative frequency~$\sum_{i=1}^{M}x_{i}/M$ with the probability
converging to one as $M$ goes to infinity, i.e., 
\begin{equation}
...
of the needle crossing a line is $2\ell/\left(\pi h\right)$~\cite{Buffon1777,DeMorgan1872,Hall1873,Uspensky1937}.
...
a line to be $M_{c}/M$. In an actual experiment with $500$ needles
and the ratio $\ell/h=0.75$~\cite{Hall1873}, it was found that
$236$ crossed a line so the relative frequency is $0.472$ whereas
the idealized mathematical probability is $0.4774\ldots$. In a larger
experiment with $5000$ needles and the ratio $\ell/h=0.8$~\cite{Uspensky1937},
...
the observed probability approaches $2\ell/\left(\pi h\right)$ but
only if \emph{larger and larger resources} are expended. These resource
...
100 digits in the expansion of $2\ell/\left(\pi h\right)$.
...
with $3204$ needles and the ratio $\ell/h=0.6$~\cite{DeMorgan1872},
...
(e.g., the probability of the union of two disjoint events is the
sum of their probabilities). But given limited resources, the calculation
of a probability measure can only be approximate and hence the probabilities
cannot be absolutely guaranteed to be coherent in the ideal sense.
...
probability measure of example~\ref{ex1} by mapping each numeric
value to the smallest sub-interval containing it to get the following:
\begin{equation}
\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
...
For example, the event $\proj{0}$ indicates that the post-measurement
state will be $\ket{0}$; the probability of such an event depends
on the current state; the event $\proj{1}$ indicates that the post-measurement
state will be $\ket{1}$; the event $\proj{\ps}$ where $\ket{\ps}=\left(\ket{0}+\ket{1}\right)/\sqrt{2}$
...
$\mu\left(\proj{0}\right)=1$, $\mu\left(\proj{1}\right)=0$, $\mu\left(\proj{\ps}\right)=\frac{1}{2}$,
...
the Born rule induces a probability measure $\mu_{\phi}^{\mathrm{B}}$
defined as $\mu_{\phi}^{\mathrm{\mathrm{B}}}(P)=\ip{\phi}{P\phi}$.
The situation generalizes to mixed states $\rho=\sum_{j=1}^{N}q_{j}\proj{\phi_{j}}$,
where $\sum_{j=1}^{N}q_{j}=1$ in which case the generalized Born
rule induces a probability measure $\mu_{\rho}^{\mathrm{\mathrm{B}}}$
defined as $\mu_{\rho}^{\mathrm{\mathrm{B}}}\left(P\right)=\Tr\left(\rho P\right)=\sum_{j=1}^{N}q_{j}\mu_{\phi_{j}}^{\mathrm{\mathrm{B}}}\left(P\right)$~\cite{peres1995quantum,544199,Jaeger2007}.
...
state~$\rho$ such that $\mu=\mu_{\rho}^{\mathrm{\mathrm{B}}}$.
\end{thm}
...
1-dimensional projectors mapped by $\bar{\mu}_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)=\iota\left(\mu_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)\right)$
plotted in $\mathbb{R}^{3}$. The red dotted vector is $\ket{0}$,
and $\bar{\mu}_{\ket{0}}^{\mathrm{\mathrm{B}}}\left(\proj{0}\right)=\necess$.
$\ket{0}$ is the normal vector of the plane so that we have $\bar{\mu}_{\ket{0}}^{\mathrm{\mathrm{B}}}\left(\proj{\psi}\right)=\imposs$
for any vector~$\ket{\psi}$ in the plane. For any other vector~$\ket{\psi}$,
we have $\bar{\mu}_{\ket{0}}^{\mathrm{\mathrm{B}}}\left(\proj{\psi}\right)=\unknown$.}
\end{figure}
For example, figure~\ref{fig:three-dimensional-3-value} plots the
1-dimensional projectors mapped by $\iota\left(\mu_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)\right)$.
This measure represents the beliefs of an experimenter with no prior
knowledge about the particular quantum system in question.\footnote{\yutsung{Actually, $\iota\left(\mu_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)\right)$
is quite informative. Since $\iota\left(\mu_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)\right)=\necess$
if and only if $P=\proj{0}$, we know the core of $\iota\left(\mu_{\ket{0}}^{\mathrm{\mathrm{B}}}(P)\right)$
is exactly $\left\{ \mu_{\ket{0}}^{\mathrm{\mathrm{B}}}\right\} $.
The quantum interval-valued probability measure for an experimenter
with no prior knowledge should be $\bar{\mu}(P)=\unknown$ for every
projection~$P$.}} 
...
a Hilbert space of dimension~$d\ge3$, there exists a convex QIVPM~$\bar{\mu}:\events\rightarrow\mathscr{I}$
...
on the Hilbert space, and verify that there are no states that are
consistent with it on all possible events. Assume a Hilbert space
of dimension $d\ge3$ with orthonormal basis $\left\{ \ket{0},\ket{1},\ldots,\ket{d-1}\right\} $
and $\mathscr{I}_{0}=\left\{ \necess,\imposs,\unknown\right\} $.
For $i=1,\ldots,d-1$, let $\ket{i'}=\left(\ket{0}+\ket{i}\right)/\sqrt{2}$.
Given two distinguish states $\ket{i'}\ne\ket{j'}$, since the inner
products $\ip{i'}{j'}$ and $\ip{i'}{0}$ are both non-zero, all projectors
in $\mathcal{S}=\left\{ \proj{0}\right\} \cup\set{\proj{i'}}{i=1,\ldots,d-1}$
are not commuting with each other. Consider the function~$\bar{\mu}:\events\rightarrow\mathscr{I}_{0}$
defined by 
\begin{equation} 
\bar{\mu}\left(P\right)=\cases{ 
\imposs\,, & if $P=\mathbb{0}$ or $P\in\mathcal{S}$ ;\\ 
\necess\,, & if $P=\mathbb{1}$ or $\mathbb{1}-P\in\mathcal{S}$ ;\\ 
\unknown\,, & otherwise. 
}\label{eq:non-Born-QIVPM} \end{equation}
The situation when $d=3$ is illustrated in figure~\ref{fig:three-dimensional-3-value-2}.
\begin{figure}
\begin{centering}
\includegraphics[scale=0.38]{figure2_1} 
\par\end{centering}
\caption{\label{fig:three-dimensional-3-value-2}This figure illustrates theorem~\ref{thm:Non-extensible-of-Gleason's}
plotted in $\mathbb{R}^{3}$. The red, green, and blue vectors are
$\ket{0}$, $\ket{1'}$, and $\ket{2'}$ respectively. }
\end{figure}
To verify $\bar{\mu}$ be a convex QIVPM, it is sufficient to verify
the convexity condition, equation~(\ref{eq:QuantumInterval-valuedProbability-Convex}).
There are several cases:
\begin{itemize}
\item When $P_{0}=\mathbb{0}$, equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
holds by lemma~\ref{lemma:quantum-convex-condition-lemma}. This
lemma also implies equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
when $P_{0}=\mathbb{1}$.
\item When $P_{0}=\proj{\psi}\in\mathcal{S}$, if $P_{1}=\proj{\psi}+P_{2}$,
then equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
holds by lemma~\ref{lemma:quantum-convex-condition-lemma}. Otherwise,
we have $P_{0}P_{1}=\mathbb{0}$, and $\bar{\mu}\left(P_{0}P_{1}\right)=\bar{\mu}\left(P_{0}\right)=\imposs$.
There are two cases:
\begin{itemize}
\item When $P_{1}=\mathbb{1}-\proj{\psi}$, we have
\begin{equation}\eqalign{ 
 & \bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)=\bar{\mu}\left(\mathbb{1}\right)+\bar{\mu}\left(\mathbb{0}\right)\\
={} & \necess+\imposs=\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right)\,.
}\end{equation}
\item When $P_{1}\ne\mathbb{1}-\proj{\psi}$, neither $P_{1}\in\mathcal{S}$
nor $\mathbb{1}-P_{1}\in\mathcal{S}$ because all projectors in $\mathcal{S}$
are not commuting. Hence,$\bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)\subseteq\unknown+\imposs=\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right)$.
\end{itemize}
Therefore, equation~(\ref{eq:QuantumInterval-valuedProbability-Convex})
holds when $P_{0}=\proj{\psi}\in\mathcal{S}$.
\item When $P_{0}=\mathbb{1}-\proj{\psi}$ for $\proj{\psi}\in\mathcal{S}$,
equation~(\ref{eq:QuantumInterval-valuedProbability-Convex}) follows
from the previous case with lemma~\ref{lemma:quantum-convex-condition-lemma}.
\item If neither the above cases holds, we have $\bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)\subseteq\unknown+\unknown=\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right)$.
\end{itemize}
...
for any~$P$, the state~$\rho$ must satisfy $\mu_{\rho}^{\mathrm{B}}(P)=0$
for all $P\in\mathcal{S}$. This implies 
\begin{equation}
\ip{\phi_{j}}{0}=\ip{\phi_{j}}{i'}=0\label{eq:probability-zero-on-states}
\end{equation}
for all $i=1,\ldots d-1$ and for all $j=1,\ldots N$. Eq.~(\ref{eq:probability-zero-on-states})
never holds proving the theorem.\end{proof}
...
However, the quantum interval-valued probability measure in theorem~\ref{thm:Non-extensible-of-Gleason's}
...
to theorem~\ref{thm:Non-extensible-of-Gleason's}, and the following
...
$\mathrm{core}\left(\bar{\mu}|_{\events_{1}}\right)=\left\{ \mu_{\ket{2}}^{\mathrm{\mathrm{B}}}\right\} $.
\item When $\events_{2}=\events\backslash\left\{ \proj{\ps}\right\} $,
because $\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps'})=\imposs$, we
have $\mathrm{core}\left(\bar{\mu}|_{\events_{2}}\right)=\left\{ \mu_{\ket{1}}^{\mathrm{\mathrm{B}}}\right\} $.
\item When $\events_{3}=\events\backslash\left\{ \proj{0},\proj{\ps},\proj{\ps'}\right\} $,
because $\bar{\mu}(\proj{\phi})\ne\necess$ for all $\ket{\phi}$,
we have $\mu_{\frac{\mathbb{1}}{3}}^{\mathrm{\mathrm{B}}}\in\mathrm{core}\left(\bar{\mu}|_{\events_{3}}\right)$
as well as $\mu_{\ket{2}}^{\mathrm{\mathrm{B}}}$ and $\mu_{\ket{1}}^{\mathrm{\mathrm{B}}}\in\mathrm{core}\left(\bar{\mu}|_{\events_{3}}\right)$.
\end{itemize}
Each of these possibilities is compatible with some but not all of
the observations. In another word, 
\begin{equation}
\mathrm{core}\left(\bar{\mu}\right)=\bigcap_{\events'\subseteq\events}\mathrm{core}\left(\bar{\mu}|_{\events'}\right)\subseteq\mathrm{core}\left(\bar{\mu}|_{\events_{1}}\right)\cap\mathrm{core}\left(\bar{\mu}|_{\events_{2}}\right)=\left\{ \mu_{\ket{2}}^{\mathrm{\mathrm{B}}}\right\} \cap\left\{ \mu_{\ket{1}}^{\mathrm{\mathrm{B}}}\right\} =\emptyset\,.
\end{equation}
...
such that $\mathrm{core}\left(\bar{\mu}|_{\events'}\right)=\left\{ \mu_{\ket{\psi_{0}}\times\ket{\psi_{1}}}^{\mathrm{\mathrm{B}}}\right\} $
...
and this area intersects a plane~$\events'$, then $\mathrm{core}\left(\bar{\mu}|_{\events'}\right)=\left\{ \mu_{\ket{\psi}}^{\mathrm{\mathrm{B}}}\right\} $,
...
mapping to $\imposs$ between $\arcsin\left(1/\sqrt{3}\right)$ and
$\pi/4$, we have $\unknown\in\mathscr{I}$, i.e., $\underset{\left[l,r\right]\in\mathscr{I}}{\max}r-l=1$
...
laboratory. E, P, and R measure the spin-1 system described in theorem~\ref{thm:Non-extensible-of-Gleason's}.
...
measure in theorem~\ref{thm:Non-extensible-of-Gleason's} which
has an empty core.
...
\begin{table}
\caption{\label{table_long}Relation between measurement resources and interval-valued
...
Although quantum IVPMs in theorem~\ref{thm:Non-extensible-of-Gleason's}
and \ref{ex:three-dimensional-5-value} have empty cores, we might
still think that the probability measure~$\bar{\mu}$ is induced
by some states. In theorem~\ref{thm:Non-extensible-of-Gleason's},
...
quantum probability measure~$\mu_{\phi}^{\mathrm{\mathrm{B}}}$ induced
by the Born rule, its frame function is denoted by $f_{\phi}^{\mathrm{\mathrm{B}}}$.\end{definition}
...
\begin{proof} To prove that $\ket{\varphi}\parallel\left(\ket{\psi}\times\ket{\phi}\right)\Rightarrow\ip{\psi}{\varphi}=\ip{\phi}{\varphi}=0$,
we can just verify 
\begin{equation}\eqalign{ 
...
defined by $T\left(\ket{\varphi}\right)=\left(\begin{array}{c}
\ip{\psi}{\varphi}\\
\ip{\phi}{\varphi}
...
\end{proof} Since interval-valued probability measures~$\bar{\mu}$
and interval-valued frame functions~$\bar{f}$ are one-to-one correspondence,
studying 1-dimensional projectors mapping to impossible by $\bar{\mu}$
is the same as studying the points on $\mathbb{C}S^{2}$ mapping to
impossible by $\bar{f}$. These points, denoted by $\bar{f}^{-1}\left(\imposs\right)=\set{\ket{\psi}\in\mathbb{C}S^{2}}{\bar{f}(\ket{\psi})=\imposs}$,
play an important role to understand $\bar{f}$. There are 4 different
cases when analyzing $\bar{f}^{-1}\left(\imposs\right)$: 
\begin{enumerate}
\item $\bar{f}^{-1}\left(\imposs\right)=\emptyset$. If $f_{\phi}^{\mathrm{\mathrm{B}}}\in\mathrm{core}\left(\bar{f}\right)$
for some pure state~$\ket{\phi}$, we could say nothing about $\ket{\phi}$. 
\item $\bar{f}^{-1}\left(\imposs\right)=\left\{ \ket{\psi_{0}}\right\} $.
If $f_{\phi}^{\mathrm{\mathrm{B}}}\in\mathrm{core}\left(\bar{f}\right)$
for some pure state~$\ket{\phi}$, $\ket{\phi}$ could be any state
orthogonal to $\ket{\psi_{0}}$. 
...
which might induce $\bar{f}$. In another word, if $f_{\phi}^{\mathrm{\mathrm{B}}}\in\mathrm{core}\left(\bar{f}\right)$
...
by $\bar{f}\left(\ket{\psi}\right)=\iota\left(f_{\phi}^{\mathrm{\mathrm{B}}}\left(\ket{\psi}\right)\right)$
is also in this case when the core is the singleton set~$\left\{ f_{\phi}^{\mathrm{\mathrm{B}}}\right\} $,
...
might induce $\bar{f}$. However, neither $f_{\psi_{0}\times\psi_{1}}^{\mathrm{\mathrm{B}}}$
nor $f_{\psi_{0}'\times\psi_{1}'}^{\mathrm{\mathrm{B}}}$ is in the
core of $\bar{f}$ because $f_{\psi_{0}\times\psi_{1}}^{\mathrm{\mathrm{B}}}$
is inconsistent with $\bar{f}\left(\ket{\psi_{0}'}\right)=\bar{f}\left(\ket{\psi_{1}'}\right)=\imposs$
and $f_{\psi_{0}'\times\psi_{1}'}^{\mathrm{\mathrm{B}}}$ is inconsistent
with $\bar{f}\left(\ket{\psi_{0}}\right)=\bar{f}\left(\ket{\psi_{1}}\right)=\imposs$.
Actually, $\bar{f}$ has an empty core, and this is the case for theorem~\ref{thm:Non-extensible-of-Gleason's}. 
\end{enumerate}
...
\begin{equation}
\left[1-r_{0},1-l_{0}\right]=\left[1,1\right]-\left[l_{0},r_{0}\right]=\left[l_{0},r_{0}\right]\Rightarrow l_{0}+r_{0}=1\,.
\end{equation}
Also, we have 
\begin{equation}
1\in\bar{f}\left(\ket{0'}\right)+\bar{f}\left(\ket{1}\right)+\bar{f}\left(\ket{2'}\right)=\left[2l_{0},2r_{0}\right]\Rightarrow l_{0}\le\frac{1}{2}\le r_{0}\,.
\end{equation}
Thus the lemma is proved. \end{proof} \begin{thm}\label{thm:pi-div-4}Given
$\theta_{0}\in\left[0,\pi/2\right]$ and an interval-valued frame
function~$\bar{f}:S^{2}\rightarrow\mathscr{I}$, if $\bar{f}\left(\ket{\psi}\right)=\imposs$
for every $\ket{\psi}\in\angle^{-1}\left(\left[0,\theta_{0}\right]\right)$,
then $\theta_{0}<\pi/4$.\end{thm} \begin{proof} We are going to
prove by contradiction. Suppose $\theta_{0}\ge\pi/4$, we have $\left[0,\theta_{0}\right]\cap\left[\pi/2-\theta_{0},\pi/2\right]=\left[\pi/2-\theta_{0},\theta_{0}\right]\ne\emptyset$.
...
where $l_{0}\in\left[0,\pi/2\right]$. A contradiction! \end{proof}
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.38]{figureb2} 
\par\end{centering}
...
\begin{proof} We could illustrate the situation in figure~\ref{fig:three-dimensional-real-frame-2},
...
whose latitude is $\pi/2-\arcsin\left(1/\sqrt{3}\right)$ so that
they are below the green circle, and these three states form an equilateral
triangle whose center is $H$. Since the latitude of this triangle
is $\pi/2-\arcsin\left(1/\sqrt{3}\right)$, we know $\overline{OH}=1/\sqrt{3}$,
where $O$ is the center of the sphere. By the Pythagorean theorem,
$\overline{HA}=\overline{HB}=\sqrt{\frac{2}{3}}$. Notice that $\triangle HAB$
is an isosceles triangle with $\angle AHB=2\pi/3$, we have $\overline{AB}=\sqrt{2}$.
Since $\overline{OA}=\overline{OB}=1$ and $\overline{AB}=\sqrt{2}$,
$\triangle OAB$ is an isosceles right triangle with $\angle AHB=\pi/2$,
i.e., $\ket{A}$ and $\ket{B}$ are orthogonal. Applying the same
reason on all $\ket{A}$, $\ket{B}$, and $\ket{C}$, $\left\{ \ket{A},\ket{B},\ket{C}\right\} $
...
\begin{thm}\label{lemma:classical-convex-condition-lemma}Given a
classical IVPM~$\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$,
the following statements are true:
\begin{enumerate}
\item If $E_{0}\subseteq E_{1}$, then $\bar{\mu}^{\Omega}\left(E_{0}\cup E_{1}\right)+\bar{\mu}^{\Omega}\left(E_{0}\cap E_{1}\right)\subseteq\bar{\mu}^{\Omega}\left(E_{0}\right)+\bar{\mu}^{\Omega}\left(E_{1}\right)$.
\item If $\bar{\mu}^{\Omega}\left(E_{0}\cup E_{1}\right)+\bar{\mu}^{\Omega}\left(E_{0}\cap E_{1}\right)\subseteq\bar{\mu}^{\Omega}\left(E_{0}\right)+\bar{\mu}^{\Omega}\left(E_{1}\right)$,
then $\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cup\overline{E_{1}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cap\overline{E_{1}}\right)\subseteq\bar{\mu}^{\Omega}\left(\overline{E_{0}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{1}}\right)$,
where $\overline{E_{0}}=\Omega\backslash E_{0}$ and $\overline{E_{1}}=\Omega\backslash E_{1}$.
\end{enumerate}
\end{thm}

\begin{proof}
\begin{enumerate}
\item $\bar{\mu}^{\Omega}\left(E_{0}\cup E_{1}\right)+\bar{\mu}^{\Omega}\left(E_{0}\cap E_{1}\right)=\bar{\mu}^{\Omega}\left(E_{0}\right)+\bar{\mu}^{\Omega}\left(E_{1}\right)$.
\item 
\begin{eqnarray*}
 &  & \bar{\mu}^{\Omega}\left(E_{0}\cup E_{1}\right)+\bar{\mu}^{\Omega}\left(E_{0}\cap E_{1}\right)\subseteq\bar{\mu}^{\Omega}\left(E_{0}\right)+\bar{\mu}^{\Omega}\left(E_{1}\right)\\
 & \Rightarrow & \bar{\mu}^{\Omega}\left(\Omega\backslash\left(\overline{E_{0}}\cap\overline{E_{1}}\right)\right)+\bar{\mu}^{\Omega}\left(\Omega\backslash\left(\overline{E_{0}}\cup\overline{E_{1}}\right)\right)\subseteq\bar{\mu}^{\Omega}\left(\Omega\backslash\left(\overline{E_{0}}\right)\right)+\bar{\mu}^{\Omega}\left(\Omega\backslash\left(\overline{E_{1}}\right)\right)\\
 & \Rightarrow & 2-\left[\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cap\overline{E_{1}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cup\overline{E_{1}}\right)\right]\subseteq2-\left[\bar{\mu}^{\Omega}\left(\overline{E_{0}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{1}}\right)\right]\\
 & \Rightarrow & \bar{\mu}^{\Omega}\left(\overline{E_{0}}\cap\overline{E_{1}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cup\overline{E_{1}}\right)\subseteq\bar{\mu}^{\Omega}\left(\overline{E_{0}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{1}}\right)\,.
\end{eqnarray*}
\end{enumerate}
\end{proof}

\begin{lemma}\label{lemma:classical-3-elements}When the number of
elements in $\Omega$ is less than or equal to 3, every classical
IVPM~$\bar{\mu}^{\Omega}:2^{\Omega}\rightarrow\mathscr{I}$ is convex.\end{lemma}
...
must happen
\begin{enumerate}
\item When $E_{0}\cap E_{1}=\emptyset$, equation~(\ref{eq:classical-convex})
is a special case of equation~(\ref{eq:classical-include}). 
\item When $E_{0}\cup E_{1}=\Omega$, we have $\overline{E_{0}}\cap\overline{E_{1}}=\emptyset$,
where $\overline{E_{0}}=\Omega\backslash E_{0}$ and $\overline{E_{1}}=\Omega\backslash E_{1}$.
By equation~(\ref{eq:classical-include}), we have $\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cup\overline{E_{1}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{0}}\cap\overline{E_{1}}\right)\subseteq\bar{\mu}^{\Omega}\left(\overline{E_{0}}\right)+\bar{\mu}^{\Omega}\left(\overline{E_{1}}\right)$.
Then, theorem~\ref{lemma:classical-convex-condition-lemma} proves
equation~(\ref{eq:classical-convex}).
\item When $E_{0}\subseteq E_{1}$ or $E_{1}\subseteq E_{0}$, equation~(\ref{eq:classical-convex})
is also followed by theorem~\ref{lemma:classical-convex-condition-lemma}.
\end{enumerate}
so that $\bar{\mu}^{\Omega}$ is convex.\end{proof}

\begin{lemma}\label{lemma:quantum-convex-condition-lemma}Given a
QIVPM $\bar{\mu}:\events\rightarrow\mathscr{I}$, the following statements
are true:
\begin{enumerate}
\item If $P_{1}=P_{0}+P_{2}$, then $\bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)\subseteq\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right)$.
\item Given commuting $P_{0}$ and $P_{1}\in\events$, if $\bar{\mu}\left(P_{0}\vee P_{1}\right)+\bar{\mu}\left(P_{0}P_{1}\right)\subseteq\bar{\mu}\left(P_{0}\right)+\bar{\mu}\left(P_{1}\right)$,
then $\bar{\mu}\left(\overline{P_{0}}\vee\overline{P_{1}}\right)+\bar{\mu}\left(\overline{P_{0}}\overline{P_{1}}\right)\subseteq\bar{\mu}\left(\overline{P_{0}}\right)+\bar{\mu}\left(\overline{P_{1}}\right)$,
where $\overline{P_{0}}=\Omega\backslash P_{0}$ and $\overline{P_{1}}=\Omega\backslash P_{1}$.
\end{enumerate}
\end{lemma}

\begin{proof}The direct consequence of theorem~\ref{lemma:classical-convex-condition-lemma}
and theorem~\ref{thm:sub-event-space}.\end{proof}
...
\begin{proof}The direct consequence of lemma~\ref{lemma:classical-3-elements}
and theorem~\ref{thm:sub-event-space}. \end{proof}
...
\left[\frac{k-\delta}{n},\frac{k+\delta}{n}\right]\,, &if $x=\frac{k}{n}\in\left(0,1\right)$ ;\\ 
...
is a convex interval-valued function.\end{example}
...
\begin{example}\label{ex:equilateral}Consider the following intervals
\begin{equation}\eqalign{ 
...
intervals with length~$\left(2n\right)^{-1}$ at each end. As shown
in Table~\ref{tab:I_n}, for each interval~$[\ell,r]$, we define
$\textrm{Int}\left([\ell,r]\right)$, the non-overlapping interior
part of $[\ell,r]$.
\begin{table}
\caption{\label{tab:I_n}Intervals in $\widehat{\mathscr{I}}_{n}$, their lengths,
and their interiors, where $2\mid k$ means $k$ is even, and $2\nmid k$
...
\end{tabular}
\end{table}
Since their interior parts are non-overlapping, we can define a function~$\widehat{\iota}_{n}:\left[0,1\right]\rightarrow\widehat{\mathscr{I}}_{n}$
by $\widehat{\iota}_{n}\left(x\right)=[\ell,r]$ if $x\in\textrm{Int}\left([\ell,r]\right)$
to map a real-valued probability to an interval-valued one. Table~\ref{tab:I_2}
lists a special case $\widehat{\mathscr{I}}_{2}$ to illustrate these
intervals and their interior parts more clearly.
\begin{table}
\caption{\label{tab:I_2}Intervals in $\widehat{\mathscr{I}}_{2}$, , their
lengths, and their interiors. Although intervals~$[\ell,r]$ are
overlapping, their interior~$\textrm{Int}\left([\ell,r]\right)$
are non-overlapping.}
\centering{}%
\begin{tabular}{cccc}
\toprule 
...
\midrule 
\addlinespace
$4$ & $\left[\tfrac{3}{4},1\right]$ & $\tfrac{1}{4}$ & $\left[\tfrac{7}{8},1\right)$\tabularnewline\addlinespace
\midrule 
\addlinespace
 & $\necess$ & $0$ & $\necess$\tabularnewline\addlinespace
\bottomrule
...
\Rightarrow & \left[x_{0}+x_{1}+x_{2}+c\left(x_{0}+x_{1}+x_{2}\right)\right]+\left[x_{2}+c\left(x_{2}\right)\right]\\
 & \le\left[x_{0}+x_{2}+c\left(x_{0}+x_{2}\right)\right]+\left[x_{1}+x_{2}+c\left(x_{1}+x_{2}\right)\right]\,.
}\end{equation}
Also, 
\begin{equation}\eqalign{ 
 & c\left(x_{0}+x_{1}+x_{2}\right)+c\left(x_{2}\right)\le c\left(x_{0}+x_{2}\right)+c\left(x_{1}+x_{2}\right)\\
...
\begin{lemma}\label{thm:Finite-precision-Gleason}Given a Hilbert
space with the set of all projectors\nb$\events$, and an additive
...
By applying the previous lemma on example~\ref{ex:equilateral},
...
$1/n$. Therefore, for any totally mixed state\nb$\rho$ on a $d$-dimensional
Hilbert space, if $\rho'\in\coreBorn\left(\widehat{\iota}_{n}\circ\mu_{\rho}^{\mathrm{B}},\events\right)$,
then $\left\Vert \rho-\rho'\right\Vert \le1/n$.

For example, when $n=2$, two states~$\rho_{0}=\frac{1}{3}\cdot\proj{0}+\frac{1}{3}\cdot\proj{1}+\frac{1}{3}\cdot\proj{2}$
and $\rho_{1}=\frac{7}{24}\cdot\proj{0}+\frac{1}{3}\cdot\proj{1}+\frac{3}{8}\cdot\proj{2}$
can induce two QIVPMs~$\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$
and $\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$. Given
any projector $P$ in the minimal subspace of events containing $\proj{0}$,
$\proj{1}$, and $\proj{2}$, $\events_{C}$, the interval-valued
probability of the induced QIVPMs, $\widehat{\iota}_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$
and $\widehat{\iota}_{2}\left(\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\right)$,
can be computed straightforwardly in Table~\ref{tab:Compute-the-QIVPM}.
\begin{table}
\caption{\label{tab:Compute-the-QIVPM}Compute $\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$
and $\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$ based on
$\mu_{\rho_{0}}^{\mathrm{B}}$ and $\mu_{\rho_{1}}^{\mathrm{B}}$
on $\events_{C}$.}
\centering{}%
\begin{tabular}{ccccc}
\toprule 
\addlinespace
$P$ & $\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)$ & $\widehat{\iota}_{2}\left(\mu_{\rho_{0}}^{\mathrm{B}}\left(P\right)\right)$ & $\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)$ & $\widehat{\iota}_{2}\left(\mu_{\rho_{1}}^{\mathrm{B}}\left(P\right)\right)$\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
...
\end{tabular}
\end{table}
Table~\ref{tab:Compute-the-QIVPM} shows that $\rho_{0}$ is consistent
with $\widehat{\iota}_{2}\circ\mu_{\rho_{1}}^{\mathrm{B}}$ and $\rho_{1}$
is consistent with $\widehat{\iota}_{2}\circ\mu_{\rho_{0}}^{\mathrm{B}}$
...
by lemma~\ref{thm:Finite-precision-Gleason}. This is true because
the exact value of $\left\Vert \rho_{0}-\rho_{1}\right\Vert $ is
$\frac{1}{24}$~\cite{544199}.\end{example}

Previously, we proved that if $\rho'$ is consistent with $\iota\circ\mu_{\rho}^{\mathrm{B}}$,
then $\rho'$ is close to $\rho$. However, if $\rho'$ is close to
$\rho$, there is no way to guarantee $\rho'$ is consistent with
$\iota\circ\mu_{\rho}^{\mathrm{B}}$ because no matter how close between
two real numbers~$x$ and $y$, we can never guarantee $y\in\iota\left(x\right)$
as in the following example.

\begin{example}Given an additive interval-valued function\nb$\iota:\left[0,1\right]\rightarrow\mathscr{I}$,
consider let $x_{0}=\sup\set{x\in\left[0,1\right]}{\iota\left(x\right)=\left[0,0\right]}$.
Since $\iota\left(1\right)=\left[1,1\right]$, $x_{0}<1$. For any
$\varepsilon>0$, $\left|\left(x+\frac{\varepsilon}{2}\right)-x\right|=\frac{\varepsilon}{2}<\varepsilon$,
and $x+\frac{\varepsilon}{2}\ge\frac{\varepsilon}{2}>0$ implies $x+\frac{\varepsilon}{2}\notin\left[0,0\right]=\iota\left(x\right)$.

Notice that even if we explicitly exclude $0$ and $1$, the previous
argument applies on any boundary points among intervals. Therefore,
there is no way to avoid this problem if we have more than three intervals.\end{example}

\subsection{Category Theory}

It looks like $\iota$ might be a morphism in a category. Let's try
to check if it is true here.

\begin{definition}Given $\mathscr{I}$ is a collection of intervals,
we say a interval-valued function\nb$\iota:\mathscr{I}'\rightarrow\mathscr{I}''$
is additive if $\iota$ satisfies the following properties. 
\begin{itemize}
\item $\iota\left(\imposs\right)=\imposs$. 
\item $\iota\left(\necess\right)=\necess$. 
\item For any $\left[\ell,r\right]\in\mathscr{I}'$, $\iota\left(\necess-\left[\ell,r\right]\right)=\necess-\iota\left(\left[\ell,r\right]\right)$. 
\item For a pair\nb$\left[\ell_{0},r_{0}\right]$ and $\left[\ell_{1},r_{1}\right]\in\mathscr{I}'$,
if $\left[\ell_{0},r_{0}\right]\subseteq\left[\ell_{1},r_{1}\right]$,
then $\iota\left(\left[\ell_{0},r_{0}\right]\right)\subseteq\iota\left(\left[\ell_{1},r_{1}\right]\right)$. 
\item For a pair\nb$\left[\ell_{0},r_{0}\right]$ and $\left[\ell_{1},r_{1}\right]\in\mathscr{I}'$,
if $\left[\ell_{0},r_{0}\right]+\left[\ell_{1},r_{1}\right]\cap\left[0,1\right]\neq\emptyset$,
then $\left[\ell_{0},r_{0}\right]+\left[\ell_{1},r_{1}\right]\in\mathscr{I}'$
and $\iota\left(\left[\ell_{0},r_{0}\right]+\left[\ell_{1},r_{1}\right]\right)\subseteq\iota\left(\left[\ell_{0},r_{0}\right]\right)+\iota\left(\left[\ell_{1},r_{1}\right]\right)$. 
\end{itemize}
Moreover, $\iota$ is called convex if for any three intervals\nb$\left[\ell_{0},r_{0}\right]$,
$\left[\ell_{1},r_{1}\right]$, and $\left[\ell_{2},r_{2}\right]\in\mathscr{I}'$,
their sum $\left[\ell_{0},r_{0}\right]+\left[\ell_{1},r_{1}\right]+\left[\ell_{2},r_{2}\right]\in\mathscr{I}'$
implies $\iota\left(\left[\ell_{0},r_{0}\right]+\left[\ell_{1},r_{1}\right]+\left[\ell_{2},r_{2}\right]\right)+\iota\left(\left[\ell_{2},r_{2}\right]\right)\subseteq\iota\left(\left[\ell_{0},r_{0}\right]+\left[\ell_{2},r_{2}\right]\right)+\iota\left(\left[\ell_{1},r_{1}\right]+\left[\ell_{2},r_{2}\right]\right)$.\end{definition}

\begin{lemma}\label{lemma:interval2interval}Consider an additive
interval-valued function\nb$\iota:\mathscr{I}'\rightarrow\mathscr{I}''$.
Then, given a QIVPM\nb$\bar{\mu}':\events\rightarrow\mathscr{I}'$,
a function\nb$\bar{\mu}'':\events\rightarrow\mathscr{I}''$ defined
by $\bar{\mu}''\left(P\right)=\iota\left(\bar{\mu}'\left(P\right)\right)$
is a QIVPM.\end{lemma}

\begin{proof}~
\begin{itemize}
\item $\bar{\mu}''\left(\mathbb{0}\right)=\iota\left(\bar{\mu}'\left(\mathbb{0}\right)\right)=\iota\left(\imposs\right)=\imposs$.
\item $\bar{\mu}''\left(\mathbb{1}\right)=\iota\left(\bar{\mu}'\left(\mathbb{1}\right)\right)=\iota\left(\necess\right)=\necess$.
\item For any projection $P$, $\bar{\mu}''\left(\mathbb{1}-P\right)=\iota\left(\necess-\bar{\mu}'\left(P\right)\right)=\necess-\bar{\mu}''\left(P\right)$.
\item For a pair of mutually orthogonal projections\nb$P_{0}$ and $P_{1}\in\events$,
since $\emptyset\neq\bar{\mu}'\left(P_{0}+P_{1}\right)\cap\left[0,1\right]\subseteq\bar{\mu}'\left(P_{0}\right)+\bar{\mu}'\left(P_{1}\right)\cap\left[0,1\right]$,
we have $\bar{\mu}'\left(P_{0}\right)+\bar{\mu}'\left(P_{1}\right)\in\mathscr{I}'$
and
\[
\bar{\mu}''\left(P_{0}+P_{1}\right)=\iota\left(\bar{\mu}'\left(P_{0}+P_{1}\right)\right)\subseteq\iota\left(\bar{\mu}'\left(P_{0}\right)+\bar{\mu}'\left(P_{1}\right)\right)\subseteq\iota\left(\bar{\mu}'\left(P_{0}\right)\right)+\iota\left(\bar{\mu}'\left(P_{1}\right)\right)=\bar{\mu}''\left(P_{0}\right)+\bar{\mu}''\left(P_{1}\right)\,.
\]
\end{itemize}
\end{proof}
...
probability measures. Moreover, since $\iota_{\infty}:\left[0,1\right]\rightarrow\mathscr{I}_{\infty}$
is convex, we have the following corollary.

\begin{cor}If every interval in $\mathscr{I}$ is a singleton set,
then every QIVPM\nb$\bar{\mu}:\events'\rightarrow\mathscr{I}$ is
convex.\end{cor}
...
\par\end{centering}
\caption{\label{fig:three-dimensional-5-value}This figure illustrates example~\ref{ex:three-dimensional-5-value}
plotted in $\mathbb{R}^{3}$. The red and green dotted vectors are
$\ket{0}$ and $\ket{\ps}$ respectively. All possible real vectors
of the subspaces orthogonal to $\ket{0}$ or $\ket{\ps}$ are drawn
in the red and green circles, respectively.}
\end{figure}
...
$\mu_{\rho}^{\mathrm{\mathrm{B}}}(P)\in\bar{\mu}(P)$ for all $P\in\events$.
Because $\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{0})\in\bar{\mu}(\proj{0})=\imposs$
and $\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{\ps})\in\bar{\mu}(\proj{\ps})=\imposs$,
we must have $\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{0})=\mu_{\rho}^{\mathrm{\mathrm{B}}}(\proj{\ps})=0$
so that $\mu_{\rho}^{\mathrm{\mathrm{B}}}=\mu_{\ket{2}}^{\mathrm{\mathrm{B}}}$.
However, 
\begin{equation}
\mu_{\ket{2}}^{\mathrm{\mathrm{B}}}(\proj{2})=1\notin\unlikely=\bar{\mu}(\proj{2})\,.
\end{equation}
\end{example}
...
we separate the positive and negative terms and write 
\begin{equation}
X=\sum_{i=1}^{N^{-}}x_{i}^{-}\mathbf{1}_{E_{i}^{-}}+\sum_{i=1}^{N^{+}}x_{i}^{+}\mathbf{1}_{E_{i}^{+}}\,,\label{eq:classical-decomposition}
\end{equation}
where 
\begin{equation}
x_{N^{-}}^{-}<\ldots<x_{1}^{-}<0\le x_{1}^{+}<\ldots<x_{N^{+}}^{+}
\end{equation}
...
\yutsung{{Example for when }$\bar{\mu}$ is not convex, Eq.~(\ref{eq:Rosenmuller})
doesn't hold. This can justify why we need convex...}
...
\subsection{Expectation Values for QIVPMs}

Before discussing QIVPMs, we first review various notations brought
forward from the conventional quantum theory. Assume that we wish
to measure a physical quantity~${\cal O}$ of this system, with corresponding
observable~$\mathbf{O}$. Its spectral decomposition can either be
represented in term of outer product or projector. Since we want to
define the expectation value of~$\mathbf{O}$ with respect to a QIVPM
as the classical Choquet integral in definition~\ref{def:Choquet},
we are going to order the eigenvalues of~$\mathbf{O}$ as we ordered
the step functions for classical random variables, and to represent
the spectral decomposition of~$\mathbf{O}$ by~\cite{Griffiths1994,FraleighBeauregard1995,544199,FriedbergInselSpence2002,Jaeger2007}
\begin{equation}
\mathbf{O}=\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}P_{i}^{s}\,,\label{eq:spectrum-decomposition-distinct}
\end{equation}
where the positive and negative eigenvalues are ordered from the smallest
to the largest 
\begin{equation}
\lambda_{N^{-}}^{-}<\ldots<\lambda_{1}^{-}<0\le\lambda_{1}^{+}<\ldots<\lambda_{N^{+}}^{+}\,,
\end{equation}
and $P_{i}^{s}$ is the projector onto the eigenspace of each distinct
eigenvalue~$\lambda_{i}^{s}$.

For the conventional quantum theory, a physical system can be represented
by a density matrix~$\rho$, the expectation value of the observable~$\mathbf{O}$
is defined as~\cite{544199,Jaeger2007}
\begin{equation}
\expval{\mathbf{O}}_{\mu_{\rho}^{\mathrm{B}}}=\Tr\left(\rho\mathbf{O}\right)=\sum_{s\in\left\{ +,-\right\} }\sum_{i=1}^{N^{s}}\lambda_{i}^{s}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{s}\right)\ .\label{eq:quantum-expectation}
\end{equation}
Equation~(\ref{eq:quantum-expectation}) can be combined with definition~\ref{def:Choquet}
to define the expectation value of $\mathbf{O}$ with respect to a
QIVPM~$\bar{\mu}$. Similar to definition~\ref{def:Choquet}, we
first separate the left-end and the right-end of $\bar{\mu}$ as $\mul:\events\rightarrow\left[0,1\right]$
and $\mur:\events\rightarrow\left[0,1\right]$. Then, let $P_{i}^{s\prime}=\sum_{j\ge i}P_{j}^{s}$
and $\Delta\mu^{*}\left(P_{i}^{s\prime}\right)=\mu^{*}\left(P_{i}^{s\prime}\right)-\mu^{*}\left(P_{i+1}^{s\prime}\right)$,
where $s\in\left\{ +,-\right\} $ and $\mu^{*}\in\left\{ \mul,\mur\right\} $.
Finally, the expectation value of $\mathbf{O}$ with respect to a
QIVPM~$\bar{\mu}$ is defined to be
\begin{equation}
...
Notice that the similarity between classical Choquet integral and
quantum expectation values as in Table~\ref{tab:compare-CIVPM-QRVPM}.
\begin{table}
\caption{\label{tab:compare-CIVPM-QRVPM}Compare the similarity between the
classical and quantum}
\centering{}%
\begin{tabular}{cc}
\toprule 
\addlinespace
Classical & Quantum\tabularnewline\addlinespace
\midrule
\midrule 
\addlinespace
CIVPM~$\bar{\mu}$ & QIVPM~$\bar{\mu}$\tabularnewline\addlinespace
\midrule 
\addlinespace
Random variable~$X$ & Observable~$\mathbf{O}$\tabularnewline\addlinespace
\midrule 
\addlinespace
Classical step function decomposition, equation~(\ref{eq:classical-decomposition}) & Spectral decomposition, equation~(\ref{eq:spectrum-decomposition-distinct})\tabularnewline\addlinespace
\midrule 
\addlinespace
Distinct value~$x_{i}^{s}$ & Distinct eigenvalue~$\lambda_{i}^{s}$\tabularnewline\addlinespace
\midrule 
\addlinespace
Non-overlapping sets~$E_{i}^{s}$ & Orthogonal projector~$E_{i}^{s}$\tabularnewline\addlinespace
\midrule 
\addlinespace
...
\end{tabular}
\end{table}
\footnote{\yutsung{functor? Galois connection?}}, 

we would prove some lemmas which can help us move the classical results
to quantum.

\begin{definition}Given an observable~$\mathbf{O}$ with any its
spectral decomposition~(\ref{eq:spectrum-decomposition}) whose eigenvalues
are not necessary distinct. A subspace of events~$\events'\subseteq\events$
is called consistent with $\mathbf{O}$ if it is the minimal subspace
of events satisfying $\left\{ P_{i}\right\} _{i=1}^{N}\subseteq\events'$.
Obviously, $\events'$ is commuting and convex.\end{definition}
...
observable~$\mathbf{O}$ with its spectral decomposition~(\ref{eq:spectrum-decomposition-distinct})
