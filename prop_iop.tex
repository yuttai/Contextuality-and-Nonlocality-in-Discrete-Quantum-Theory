...
\section{Cryptodeterministic Measures}

Although a real or interval-valued probability measure could be used
...
whether an event happens or not for sure~\cite{DiaconisHolmesMontgomery2007}.
...
One obvious way to discretize probability is to only consider a finite
set of probability values. For example, instead of $\left[0,1\right]$,
we only consider the probability $\check{\mathscr{L}}$ to be two
values $\left\{ 0,1\right\} $, three values $\left\{ 0,\frac{1}{2},1\right\} $,
or any finite set $\check{\mathscr{L}}\subset\left[0,1\right]$. Then,
a finite value classical probability distribution is a function $\check{\mathcal{Q}}:\Omega\rightarrow\check{\mathscr{L}}$
with 
\begin{equation}
\sum_{i=0}^{d-1}\check{\mathcal{Q}}\left(\omega_{i}\right)=1\textrm{ .}
\end{equation}
For example, a classical probability distribution for a fair coin
is $\check{\mathcal{Q}}\left(H\right)=\check{\mathcal{Q}}\left(T\right)=\frac{1}{2}$.
Because $\check{\mathcal{Q}}$ only takes value $\frac{1}{2}$, we
can pick $\check{\mathscr{L}}=\left\{ \frac{1}{2}\right\} $. 

A finite value quantum probability distribution $\check{\mathcal{P}}:\C^{d*}\rightarrow\check{\mathscr{L}}$
can be defined similarly with 
\begin{equation}\eqalign{ 
{\displaystyle \sum_{i=0}^{d-1}\check{\mathcal{P}}\left(\ket{i}\right)=1}\textrm{ for any orthonormal basis }\left\{ \ket{i}\right\} _{i=0}^{d-1}\textrm{ ;}\label{eq:total-probability-CQT-1}\\
\check{\mathcal{P}}\left(\ket{\Phi}\right)=\check{\mathcal{P}}\left(\lambda\ket{\Phi}\right)\textrm{ for }\lambda\in\C^{*}=\C\backslash\left\{ 0\right\} \textrm{ .}\label{eq:parallel-CQT-1} 
}\end{equation} 
Although it seems that the definition is almost the same as the one
in $\left[0,1\right]$-valued, actually this lead to the well-known
Kochen-Specker theorem.

\begin{thm}[Kochen-Specker~\cite{kochenspecker1967,peres1995quantum,Redhead1987-REDINA}]\label{thm:Kochen-Specker}For
$d\ge3$, there is no $\left\{ 0,1\right\} $-valued quantum probability
distribution.\end{thm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
