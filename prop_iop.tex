...
such that $\mu_{\rho}^{\mathrm{B}}|_{\events'}=\mu'\circ\tau$. Then,
we have $\expval{\mathbf{O}}_{\rho}=\int X\rmd\mu'$.\end{lemma}

\begin{proof}Since $\mu_{\rho}^{\mathrm{B}}:\events\rightarrow\left[0,1\right]$
is a QRVPM, $\mu_{\rho}^{\mathrm{B}}|_{\events'}$ is a QRVPM on the
commuting sub-event space\nb$\events'$. By case\nb\ref{enu:sub-event-space-real-quantum2classical}
in theorem~\ref{thm:sub-event-space}, there is a CRVPM such that
$\mu_{\rho}^{\mathrm{B}}|_{\events'}=\mu'\circ\tau$. 
\begin{eqnarray*}
