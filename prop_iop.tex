...
be $\left\{ \imposs,\necess\right\} $. Given a quantum interval-valued
probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}_{\mathrm{D}}$,
$\mu:\events\rightarrow\left[0,1\right]$ defined by 
\begin{equation} 
\mu(P)=\cases{ 
...
is called a sub-event space induced by $\mathbf{O}$ if $\left\{ P_{i}^{-}\right\} _{i=1}^{N^{-}}\cup\left\{ P_{i}^{+}\right\} _{i=1}^{N^{+}}\subseteq\events'$.
...
for all $E\in2^{\Omega}$. Consider the random variable~$X:\Omega\rightarrow\mathscr{I}$
...
Our next lemma discusses the Choquet integral on the real-valued probability
measure reduced to the conventional expectation value.
