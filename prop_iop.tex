...
Consider the random variable~$X$ representing the number of tails.
For the probability measure~$\pmeas$ in example~\ref{ex1}, the
expectation value of $X$ is
\begin{eqnarray*}
\int X\rmd\pmeas & = & 0\cdot\pmeas(\{HH\})+1\cdot\pmeas(\{HT,TH\})+2\cdot\pmeas(\{TT\})\\
 & = & 0\cdot\frac{1}{3}+1\cdot\frac{2}{3}+2\cdot0=\frac{2}{3}\textrm{ .}
\end{eqnarray*}
Now consider the IVPM~$\bar{\mu}$ in example~\ref{ex3}. First,
since the left-end and the right-end satisfies equation~(\ref{eq:classical-complement})
