...
\begin{equation}
\int X\rmd\bar{\mu}=\sum_{i=1}^{N^{+}}x_{i}^{+}\left[\Delta\mul\left(E_{i}^{+\prime}\right),\Delta\mur\left(E_{i}^{+\prime}\right)\right]+\sum_{i=1}^{N^{-}}x_{i}^{-}\left[\Delta\mul\left(E_{i}^{-\prime}\right),\Delta\mur\left(E_{i}^{-\prime}\right)\right]\textrm{ .}\label{eq:interval-expectation}
\end{equation}
\end{definition}

The following lemma shows our definition is exactly the same as in
the literature.
...
Similar to equation~(\ref{eq:expectation}), equation~(\ref{eq:interval-expectation})
...
\begin{thm}\cite{Rosenmuller1971,GilboaSchmeidler1994,Grabisch2016}For
...
\begin{equation}
\expval{\mathbf{O}}_{\bar{\mu}}=\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\left[\Delta\mul\left(P_{i}^{+\prime}\right),\Delta\mur\left(P_{i}^{+\prime}\right)\right]+\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\left[\Delta\mul\left(P_{i}^{-\prime}\right),\Delta\mur\left(P_{i}^{-\prime}\right)\right]\textrm{ .}\label{eq:quantum-interval-expectation}
\end{equation}
...
\begin{definition}A state~$\rho$ is consistent with $\bar{\mu}$
on a projector~$P$ if $\mu_{\rho}^{\mathrm{B}}\left(P\right)\in\bar{\mu}\left(P\right)$,
and denoted by $\rho\in\coreBorn\left(\bar{\mu},\left\{ P\right\} \right)$.\end{definition}
...
such that $\bar{\mu}=\bar{\mu}'\circ\tau$. Let $\mul[\prime]:\events'\rightarrow\left[0,1\right]$
...
defined by $X=\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mathbf{1}_{\tau\left(P_{i}^{-}\right)}+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mathbf{1}_{\tau\left(P_{i}^{+}\right)}$.
Then, 
\begin{eqnarray*}
 &  & \expval{\mathbf{O}}_{\bar{\mu}}\\
 & = & \sum_{i=1}^{N^{+}}\lambda_{i}^{+}\left[\Delta\mul\left(P_{i}^{+\prime}\right),\Delta\mur\left(P_{i}^{+\prime}\right)\right]+\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\left[\Delta\mul\left(P_{i}^{-\prime}\right),\Delta\mur\left(P_{i}^{-\prime}\right)\right]\\
 & = & \sum_{i=1}^{N^{+}}\lambda_{i}^{+}\left[\Delta\mul[\prime]\left(\tau\left(P_{i}^{+\prime}\right)\right),\Delta\mur[\prime]\left(\tau\left(P_{i}^{+\prime}\right)\right)\right]+\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\left[\Delta\mul[\prime]\left(\tau\left(P_{i}^{-\prime}\right)\right),\Delta\mur[\prime]\left(\tau\left(P_{i}^{-\prime}\right)\right)\right]\\
 & = & \int X\rmd\bar{\mu}'\\
 & = & \left[\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\int X\rmd\mu',\max_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\int X\rmd\mu'\right]\\
 & = & \left[\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu'\left(\tau\left(P_{i}^{-}\right)\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu'\left(\tau\left(P_{i}^{+}\right)\right),\max_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu'\left(\tau\left(P_{i}^{-}\right)\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu'\left(\tau\left(P_{i}^{+}\right)\right)\right]\\
 & = & \left[\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu\left(P_{i}^{+}\right),\max_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu\left(P_{i}^{+}\right)\right]\\
 & = & \left[\min_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{+}\right),\max_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu_{\rho}^{\mathrm{B}}\left(P_{i}^{+}\right)\right]\\
 & = & \left[\min_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\expval{\mathbf{O}}_{\rho},\max_{\rho\in\coreBorn\left(\bar{\mu},\events'\right)}\expval{\mathbf{O}}_{\rho}\right]\textrm{ ,}
\end{eqnarray*}
where 
\begin{equation}
\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu'\left(\tau\left(P_{i}^{-}\right)\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu'\left(\tau\left(P_{i}^{+}\right)\right)\le\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu\left(P_{i}^{+}\right)
\end{equation}
because $\tau$ in theorem~\ref{thm:sub-event-space} applies to
both real-valued and interval-valued probability measure, for any
$\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)$, there exists
a $\mu'\in\mathrm{core}\left(\bar{\mu}'\right)$ such that $\mu=\mu'\circ\tau$.
On the other hand, 
\begin{equation}
\min_{\mu'\in\mathrm{core}\left(\bar{\mu}'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu'\left(\tau\left(P_{i}^{-}\right)\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu'\left(\tau\left(P_{i}^{+}\right)\right)\ge\min_{\mu\in\mathrm{core}\left(\bar{\mu},\events'\right)}\sum_{i=1}^{N^{-}}\lambda_{i}^{-}\mu\left(P_{i}^{-}\right)+\sum_{i=1}^{N^{+}}\lambda_{i}^{+}\mu\left(P_{i}^{+}\right)
\end{equation}
...
\end{eqnarray*}
Let $\rho=\sum_{i=1}^{N^{-}}\frac{\mu'\left(\tau\left(P_{i}^{-}\right)\right)}{\Tr\left(P_{i}^{-}\right)}P_{i}^{-}+\sum_{i=1}^{N^{+}}\frac{\mu'\left(\tau\left(P_{i}^{+}\right)\right)}{\Tr\left(P_{i}^{+}\right)}P_{i}^{+}$.
Then, we have 
\begin{eqnarray*}
\mu_{\rho}^{\mathrm{B}}\left(P_{j}^{-}\right) & = & \sum_{i=1}^{N^{-}}\frac{\mu'\left(\tau\left(P_{i}^{-}\right)\right)}{\Tr\left(P_{i}^{-}\right)}\Tr\left(P_{i}^{-}P_{j}^{-}\right)+\sum_{i=1}^{N^{+}}\frac{\mu'\left(\tau\left(P_{i}^{+}\right)\right)}{\Tr\left(P_{i}^{+}\right)}\Tr\left(P_{i}^{+}P_{j}^{-}\right)=\mu'\left(\tau\left(P_{j}^{-}\right)\right)\\
\mu_{\rho}^{\mathrm{B}}\left(P_{j}^{+}\right) & = & \sum_{i=1}^{N^{-}}\frac{\mu'\left(\tau\left(P_{i}^{-}\right)\right)}{\Tr\left(P_{i}^{-}\right)}\Tr\left(P_{i}^{-}P_{j}^{+}\right)+\sum_{i=1}^{N^{+}}\frac{\mu'\left(\tau\left(P_{i}^{+}\right)\right)}{\Tr\left(P_{i}^{+}\right)}\Tr\left(P_{i}^{+}P_{j}^{+}\right)=\mu'\left(\tau\left(P_{j}^{+}\right)\right)
\end{eqnarray*}
Therefore, $\mu_{\rho}^{\mathrm{B}}\in\mathrm{core}\left(\bar{\mu},\events'\right)$.\end{proof}
