...
systems described by wavefunction $\ket{\psi}$, a fraction
...
related to wavefunction $\ket{\psi}$ and using an apparatus that
...
In previous work, we analyzed the wavefunction description of quantum
...
\footnote{\yutsung{The word ''cryptodeterminism'' is used by Peres~\cite{peres1995quantum}
without a clear definition. I tried to check its meaning from other
sources. In~\cite{Atmanspacher2014}, Atmanspacher defined `one
might paraphrase the deterministic yet non-determinable behavior of
such systems as ``cryptodeterminism,''' where ``deterministic''
and ``determinable'' is defined in the same section as `The recent
creation of the term ``deterministic chaos'' expresses the tension
between ontic (hidden) lawfulness and epistemic (apparent) irregularity
in chaotic system. Our description of the underlying laws of nature,
e.g. by differential equations governing the dynamics of such systems,
is no doubt deterministic, but their observable behavior is everything
but determinable (in the sense of measurable, computable, or predictable)
with arbitrary precision.' Is Atmanspacher's usage consistent with
Peres' and ours here?}}Although a real or interval-valued probability measure could be used
to model whether we believe an event will happen or not, the classical
world could be model in an simpler way. If the complete initial condition
...
In another word, Newtonian physics could be modeled by a cryptodeterministic
measure defined as follow.

\begin{definition}[Cryptodeterministic Measure]\label{def:CryptodeterministicMeasures}Given
the set of events~$\events$, a \emph{cryptodeterministic measure}
is a function~$\pmeas:\events\rightarrow\left\{ 0,1\right\} $ such
that: 
\begin{itemize}
\item $\pmeas(\emptyset)=0$, 
\item $\pmeas(\Omega)=1$, 
...
of the prefect prediction, a cryptodeterministic measure can model
the belief of events which has already happened. For example, if we
toss the coin in example~\ref{ex1}, and see $HH$, our belief of
which event has happened can be model by the following example.
\begin{equation}
\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
\bar{\mu}(\emptyset) & = & 0\\
...
Raised by Einstein, Podolsky, and Rosen~\cite{EPR1935,peres1995quantum},
it was a famous question whether quantum mechanics have any change
to be improved and predict unambiguously, rather than probabilistic.
One of the ``no'' answer was due to the Kochen-Specker theorem.
...
with the event really happened are the only events which are askable.
...
by the Kochen-Specker theorem. The Kochen-Specker theorem provides
an example that quantum interval-valued probability measures behave
differently from the classical one, and the convexity and core in
the next section provides another example that quantum interval-valued
probability measures behave unexpectedly.
