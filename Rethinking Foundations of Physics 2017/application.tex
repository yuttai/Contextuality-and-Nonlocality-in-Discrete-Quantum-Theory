...
to be considered infinite precise and objective. If we changed the
quantum probability to a subjective probability model, which is bound
to be approximations represented by intervals of confidence, then 

quantum states themselves can only be discussed within intervals of
confidence. This raises an important philosophical question with immense
practical relevance in the context of quantum computing. The question
is whether there is a ``real'' infinitely-precise quantum state
that is being approximated and that can be more and more accurately
...
One possibility is to trace back the original definition of a quantum
state. A quantum state could be formulated as a density matrix in
von Neumann's formalism~\cite{vonNeumann1955,Varadarajan2008}. Later,
Mackey defined a quantum state as additive probability measure as
we did in definition~\ref{def:QuantumProbabilitySpace}~\cite{inun.606849520040101,Varadarajan2008}.
Although Gleason proved Mackey states are von Neumann states in dimension
$\ge3$, it might not have to be when we generalized Mackey states
to quantum IVPMs, and maybe we should use quantum IVPMs as the most
primitive definition of states.

The other possibility focus on the meaning of the probability. Since
a quantum IVPM cannot be defined without consider a particular measurement
apparatus, and the information extracted from this apparatus, the
interval-valued probability itself could be considered subjective,
