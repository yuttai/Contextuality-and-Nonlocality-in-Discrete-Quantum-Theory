...
It is widely believed that the Turing machine is equivalent to any
reasonable computational models. Yet, for centuries, the mathematical
formalization of physical processes has been founded on the infinitely
precise real or complex numbers~\cite{Ziegler2007,weihrauch2012computable,blum2012complexity}
which might provide such processes more computational power. For example,
almost every description of quantum mechanics, quantum computation,
or quantum experiments refers to entities such as $\rme$, $\pi$,
$\sqrt{2}$, etc (see, e.g., \cite{Redhead1987-REDINA,544199,Mermin2007}).
Whether these models based on real numbers faithfully describe the
nature or just an approximation is still an open question. If they
are faithful, how the nature store and compute these real numbers
to produce our experimental results would be the next natural question.
