...
\yutsung{I think there are three possible approaches to complete
the story. 
...
The other approach might be keep going section~\ref{sec:Quantum-Probability-Spaces},
but try to prove there is a unique interval-valued Born rule with
extra-conditions. I sketched a possible set of conditions, but haven't
verified whether it really works or not...}
...
defined as follow. When the event is a 1-dimensional projector~$\proj{\psi}$,
it is also illustrated in figure~\ref{fig:three-dimensional-4-value}.
\begin{enumerate}
\item $\bar{\mu}(\mathbb{0})=\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps})=\imposs$
and $\bar{\mu}(\mathbb{1})=\bar{\mu}(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{0})=\bar{\mu}(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\ps})=\necess$,
...
where $0\le\theta\le\frac{\pi}{2}$ and $0\le\gamma<2\pi$. $\ket{0_{\theta,\gamma}^{\perp}}$
and $\ket{\ps_{\theta,\gamma}^{\perp}}$ are points in the red and
green circles, and perpendicular to $\ket{0}$ and $\ket{\ps}$, respectively.
The dotted half of those states need special treatment, i.e., when
