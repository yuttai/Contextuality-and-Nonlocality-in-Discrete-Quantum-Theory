...
measure has a non-empty core when $\left|\Omega\right|\le5$ so far.\footnote{\yutsung{The program for $\left|\Omega\right|=6$ is running now.
It seems that it will not blow up the memory, but I still have no
idea when it will finish, and whether the result will be in our side
or not...?}}
