...
measures later, here is an equivalent definition of quantum probability
measures.

\begin{lemma}Given a Hilbert space $\Hilb$, a function~$\mu:\events\rightarrow[0,1]$
is a probability measure if and only if $\mu$ satisfies the following
conditions:
\begin{itemize}
\item $\mu(\mathbb{0})=0$, 
\item $\mu(\mathbb{1})=1$, 
