...
can work in some sense? This story might go to section~\ref{sec:DQT}.
The problem of this approach is that Because we don't really use $\mathbb{C}$
in example~\ref{ex:three-dimensional-three-value}, I could expect
example~\ref{ex:three-dimensional-three-value} is likely to serve
as an counterexample for discrete interval-valued Gleason's theorem...
...
probability measure directly. This approach might go to section~\ref{sec:QC}.
...
measure directly. More precisely, we might need to verify tensor product
...
Discrete interval-valued Gleason's theorem should be here, but we
don't know whether it works or not... Might need to put some extra-conditions
to by-pass the counterexample corresponding to example~\ref{ex:three-dimensional-three-value}.

Following are possible extra-conditions:
\begin{enumerate}
\item \emph{$\mathscr{I}=\left\{ \necess,\imposs,\unknown\right\} $} is
too small...
\item $\mu\otimes\mu$ is not a quantum real-valued probability measure,
...
Might be hard, because $\mu\otimes\mu$ is not a quantum real-valued
