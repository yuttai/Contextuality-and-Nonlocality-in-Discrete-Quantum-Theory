...
\emph{sample spaces}, \emph{events}, and
\emph{measures}~\cite{GrahamKnuthPatashnik1994,inun.425605319950101,rohatgi2011introduction}.
...
belong to $E$ for each equation.'' We may also write like this:
\[
\begin{array}{rcl}
...
\end{array}
\]
In contrast with the classical example previously, the event of different
electrons are not independent or correlated. More precisely, consider
the event for each electron separately:
\begin{eqnarray*}
F_{1,L} & = & \left\{ LLL,LLR,LRL,LRR\right\} \\
F_{2,L} & = & \left\{ LLL,LLR,RLL,RLR\right\} \\
F_{3,L} & = & \left\{ LLL,LRL,RLL,RRL\right\} 
\end{eqnarray*}
They are correlated means 
\[
...
Classical events may also be correlated even they come from different
sources. A typical example is events defined by the temperature is
highly likely correlated to the event related to how Coca Cola is
sold. Another example are the same events we discussed in coin-tossing.
However, this time the coins are tossed behind a veil, and somebody
tosses the three coins for you. Because she tosses the coins behind
a veil, she might actually roll an tetrahedron die with $\{HHH,HTT,THT,TTH\}$
in its four faces. If $HTT$ were face-down, she places three coins
with $H$, $T$, and $T$ face-up, respectively. Then, she uncovers
the veil, and claims she has tossed the coins. Then, the result of
coin-tossing is correlated, and we will never see $TTT$ if the coins
are tossed in this way. 

Because we do not know how nature decides the spin of an electron,
Einstein, Podolsky, and Rosen (EPR)~\cite{EPR1935} suggested the
nature might give us the probability measure~$\mathbb{P}_{xxx}$
because she rolled an tetrahedron die or performed other classical
and deterministic process behind the veil. This claim is convincing
if $\mathbb{P}_{xxx}$ is the only probability measure we have, but
will lead to a contradiction if we consider other probability measures
all together. Notice that after the coins are placed before uncovering
the veil, which face of a coin is up is decided although we do not
know. This would be also true for the quantum probability measure.
Because the three electrons can be specially separated, and each electron
can be measured along the $x$ axis separately, if the nature rolled
the tetrahedron die, this die should be rolled before the electrons
are separated and measured, and she should know the result of measurement.
Let the result of the $j$-th electron measured along the $x$ axis
be $w\left(\sigma_{x}^{j}\right)$ so that $w\left(\sigma_{x}^{1}\right)w\left(\sigma_{x}^{2}\right)w\left(\sigma_{x}^{3}\right)\in\{LLL,LRR,RLR,RRL\}$,
...
number of electrons along the $y$ axis, and the probability measure
of non-singleton set should be computed by $\mathbb{P}_{ijk}(E)=\sum_{\omega\in E}\mathbb{P}_{ijk}(\left\{ \omega\right\} )$.
\begin{eqnarray*}
\begin{array}{rcl}
...
and $w\left(\sigma_{y}^{j}\right)$ for these probabilities. Furthermore,
because she do not know along which axis we are going to measure,
she should predetermine the same $w\left(\sigma_{x}^{j}\right)$ and
$w\left(\sigma_{y}^{j}\right)$ for all different probabilities. Notice
that the number of $L$ or $D$ in $\left\{ w\left(\sigma_{x}^{1}\right),w\left(\sigma_{y}^{2}\right),w\left(\sigma_{y}^{3}\right)\right\} $,
...
Therefore, EPR's assumption is wrong, and it is not always true that
the nature can predetermine the measurement result before we perform
the measurement. \end{example}
