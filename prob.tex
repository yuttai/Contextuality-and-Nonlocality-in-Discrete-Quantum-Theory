...
For the general interval-valued probability measures, the following
theorem gives an algorithmic way to decide whether it has a non-empty
core. 
...
$\bar{\mu}$ has an non-empty core if and only if for any minimal
balanced set~$\left\{ E_{i}\right\} _{i=1}^{N}$, we have $\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}\gamma_{i}l\left(E_{i}\right)\le1$,
where:
\begin{enumerate}
\item $\left\{ E_{i}\right\} _{i=1}^{N}$ is called balanced set if they
are distinct, $\emptyset\ne E_{i}\subsetneq\Omega$, and there exist
positive coefficient~$\left\{ \gamma_{i}\right\} _{i=1}^{N}$ such
that $\tensor*[_{\mathbb{R}}]{\sum}{_{i=1\textrm{ and }\omega\in E_{i}}^{N}}\gamma_{i}=1$
for all $\omega\in\Omega$.
\item A minimal balanced set is one that includes on other balanced set.
\end{enumerate}
\qed\end{thm}

The Bondareva-Shapley theorem can be used to algorithmically decide
non-empty core because the minimal balanced set can be generated by
an algorithm~\cite{Peleg1965}. 
