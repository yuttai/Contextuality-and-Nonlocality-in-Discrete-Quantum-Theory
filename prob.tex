...
measure of the space cannot be expressed as the product of the probability
measure of each electron, i.e., 
\[
\mathbb{P}_{xxx}(\{abc\})\ne\mathbb{P}(\{a\})\mathbb{P}(\{b\})\mathbb{P}(\{c\})\textrm{ .}
\]
...
results may be interpreted classically. For example, the nature might
roll an tetrahedron die with $\{LLL,LRR,RLR,RRL\}$ in its four faces.
If $LRR$ were face-down, the measurement result for three electrons
would be $L$, $R$, and $R$, respectively. Furthermore, this tetrahedron
die might not be accessed by human for some reasons so that we think
we are handling a system constituting three electrons. 
