...
\begin{equation}
\bar{\mu}\left(\bigcup_{i}E_{i}\right)\subseteq\tensor*[_{\mathscr{I}}]{\sum}{_{i}}\bar{\mu}\left(E_{i}\right)\textrm{ ,}\label{eq:subsetInterval-valuedProbabilityMeasure}
\end{equation}
...
We will explain why equation (\ref{eq:subsetInterval-valuedProbabilityMeasure})
is expressed using $\subseteq$ by a small example.
...
in example~\ref{ex1} can be used to construct the interval-valued
probability measure in example~\ref{ex3}. Conversely, the interval-valued
probability measure in example~\ref{ex3} could be understood by
the real-valued probability measure and its Radon-Nikodym derivative
in example~\ref{ex1}. The relation between example~\ref{ex1} and
\ref{ex3} can be abstracted into the definition of core. Besides
of core, we also define another property which will be used later.
...
is the set~\cite{GilboaSchmeidler1994,Marinacci1999,Teran2014}
\[
...
\]
\item An interval-valued probability measure is called \emph{convex} or
\emph{2-monotone} if $\bar{\mu}\left(E_{1}\cup E_{2}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(E_{1}\cap E_{2}\right)\subseteq\bar{\mu}\left(E_{1}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(E_{2}\right)$
...
and the Dempster-Shafer plausibility as the right-end, which plays
an important role in the theory of non-additive belief~\cite{Dempster1967,Shafer1976,GilboaSchmeidler1994,Marinacci1999,Goodman2013,Teran2014}.
Further more, convex interval-valued probability measures have good
properties because they can always be associated with real-valued
probability measures.

\begin{thm}[Shapley~\cite{Shapley1965,GilboaSchmeidler1994}]
...
``convex'' in the theorem. However, the literature contradicts to
each other so at least I need to trace an example to see which one
is correct, or prove the theorem we want independently...

First, \cite{JamisonLodwick2004} shows a kind of equivalence between
interval-valued probability measures and clouds. 

On one hand, a combination of the following two theorems show every
cloud has a core.

\begin{thm}[Neumaier~\cite{Neumaier2003}] For every cloud $\mathbf{x}$
over $\mathbb{M}$ there exists a random variable $x$ with values
in $\mathbb{M}$ contained in $\mathbf{x}$.\qed\end{thm}

\begin{example}[A cloud defines an $R$-probability field~\cite{Lodwick2008}]
...\qed\end{example}

On the other hand, \cite{Destercke2008a} proved some clouds has no
core.

Verifying the example to see who is right... Also need to confirm
whether interval-valued probability measures and clouds are really
the same.}
...
prepares a state, suppose the probability of preparing state~$\ket{\phi_{j}}$
...
theorem: in quantum~\cite{HollandJr1970,Redhead1987-REDINA,Jaeger2007}:
...
measures have no easy Gleason's theorem. We can further define core
and convex for quantum interval-valued probability measures. Surprisingly,
a convex quantum interval-valued probability measure may have a empty
core, i.e., it cannot correspond to a quantum real-valued probability
measure or a mixed state.

\begin{definition}~
\begin{itemize}
\item A \emph{core} of a quantum interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
is the set 
\[
\mathrm{core}\left(\bar{\mu}\right)=\left\{ \mu:\events\rightarrow[0,1]\textrm{ quantum probability measure}\middle|\mu\left(P\right)\in\bar{\mu}\left(P\right)\forall P\in\events\right\} \textrm{ .}
\]
\item $\bar{\mu}$ is called convex if $\bar{\mu}\left(P_{1}\tensor*[_{\mathscr{O}}]{\vee}{}P_{2}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(P_{1}P_{2}\right)\ge\bar{\mu}\left(P_{1}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(P_{2}\right)$
...
first notice that 
\[
\bar{\mu}\left(\proj{P_{1}}\tensor*[_{\mathscr{O}}]{+}{}\proj{P_{2}}\right)\subseteq\bar{\mu}\left(\proj{P_{1}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{P_{2}}\right)
\]
...
implies 
\[
\bar{\mu}\left(\mathbb{1}\right)\subseteq\bar{\mu}\left(\proj{P}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P\right)\textrm{ .}
\]
...
verified by enumerating all possible cases\footnote{This is the internal note for the details of the proof, and can be
...
probability measure]\label{ex:three-dimensional-4-value} \footnote{\yutsung{Need to verify this example is convex!}}Given
a three dimensional Hilbert space with an orthonormal basis $\left\{ \ket{0},\ket{1},\ket{2}\right\} $.
