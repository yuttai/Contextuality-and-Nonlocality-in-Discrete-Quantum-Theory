...
latitude of $\ket{\psi}$.
\item $N_{\ket{0}}$ denotes the northern hemisphere.
...
\begin{conjecture}\cite{Hamhalter2013}~Let $\ket{\psi}$ and $\ket{\psi'}$
be points in $N_{\ket{0}}$ with the latitude~$\theta\left(\ket{\psi}\right)=\theta\left(\ket{\psi'}\right)$.
Then there exist a finite number~$n$ steps along east-west great
circles to go from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$ due
south of $\ket{\psi}$.

The problem is whether number~$n$ could be picked uniformly? Or
there is a number~$n$ satisfying the given conditions for all latitude?

Notice that the proof in \cite{RichmanBridges1999} doesn't require
the number~$n$ to be picked uniformly because we could always pick
$f\left(p\right)=0$ in Lemma~20 in \cite{RichmanBridges1999}. However,
in the corresponding lemma~\ref{lemma:analogLemma20RichmanBridges1999},
we need to deal with the case where $\eta$ doesn't approach $0$
because the maximum and minimum value may not be orthogonal in theorem~\ref{thm:analogTheorem23RichmanBridges1999}.\qed\end{conjecture}
...
and $\eta=f\left(\ket{0}\right)$, then the oscillation on the latitude~$\theta$
of the symmetrization of $f$ with respect to $\ket{0}$ is at most
$\min_{n\ge\max\left\{ 2\pi,\frac{\pi^{2}}{\theta}\right\} }2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
...
case later.}}. Let $\ket{\psi}$ and $\ket{\psi'}$ be points in $N_{\ket{0}}$
with $\theta\left(\ket{\psi}\right)=\theta\left(\ket{\psi'}\right)$.
Let $a$ be the difference in longitudes of $\ket{\psi}$ and $\ket{\psi'}$.
Take $n\ge\frac{a^{2}}{\theta\left(\ket{\psi}\right)}$ steps along
East-West great circles to go from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$
...
and 
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\min_{n\ge2\pi}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
