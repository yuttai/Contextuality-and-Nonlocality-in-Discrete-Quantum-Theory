...
measure, and each measure must correspond to some pair of coins.  In
this example, the first coin is twice as likely to be tails than heads
...
probability measures~\cite{JamisonLodwick2004,Weichselberger2000}.

\paragraph*{Set-valued Probability Measures.} The simplest such set,
...
\end{array}
\end{array}\]
Despite the fact that we lost all numeric information, the probability
measure still reveals that the second coin is double-headed. We have
however lost the information regarding the bias in the first
coin. This information can be recovered with a more refined
probability measure as we show next. 
\qed\end{example}
...
of the section. To illustrate the main ideas, we first consider a
simple example.

\begin{example}[Two-coin probability space with four intervals]
We split the interval in the following four closed sub-intervals:
\begin{itemize}
...
double-headed but it also reveals that the first coin is more likely
to be tails than heads, despite the absence of numeric information.
\qed\end{example}

In interval-valued probability measures, if $\mu\left(E\right)=\left[a,b\right]$,
the left-endpoint~$a$ called belief denotes the strength of the
evidence that $E$ happens, and the right-endpoint~$b$ called plausibility
denotes the strength of the evidence that contradicts $E$. If we
define $1-\left[a,b\right]=\left[1-b,1-a\right]$, then we intuitively
have 
\begin{eqnarray*}
...
\yutsung{Although this is clearer than the previous one, I keep some
intervals instead write everything in \imposs, \unlikely, \likely,
and \necess. The biggest point is the addition rule~$\sum_{i}\left[a_{i},b_{i}\right]=\left[\sum_{i}a_{i},\sum_{i}b_{i}\right]$:
\begin{itemize}
\item This rule without \overflow will be push complexity to ``we may abort
computing probability when....''
\item It seems more concisely than a table among \imposs, \unlikely, \likely,
and \necess.
\end{itemize}
However, if you prefer as less interval as possible, I may try to
replace them by \imposs, \unlikely, \likely, and \necess...}
\amr{I like this. We just might need to rewrite a little but overall
  it's good.}
