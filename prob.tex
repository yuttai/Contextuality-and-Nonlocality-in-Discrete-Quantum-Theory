...
if 
\[
W_{l}\le\sideset{_{\mathbb{R}}}{_{i=0}^{d-1}}\sum f\left(\ket{\psi_{i}}\right)\le W_{r}
\]
...
we shall call it East-West great circle through $\ket{\psi}$, and
denoted by $C\left(\ket{\psi}\right)$.
\end{itemize}
\qed\end{definition}

\yutsung{\begin{lemma}\label{lemma:geometricInterludeRichmanBridges1999}\cite{RichmanBridges1999}~Let
$\ket{\psi}$ and $\ket{\psi'}$ be points in $N_{\ket{0}}$ with
the latitude~$\theta\left(\ket{\psi}\right)=\theta\left(\ket{\psi'}\right)$.
Then there exist a finite number~$n\ge\frac{a^{2}}{\theta\left(\ket{\psi}\right)}$
steps along east-west great circles to go from $\ket{\psi'}$ to a
point $\ket{\psi_{s}}$ due south of $\ket{\psi}$.\footnote{Verify the details more precisely...}\qed\end{lemma}

Lemma~\ref{lemma:geometricInterludeRichmanBridges1999} is used to
prove lemma~20 in \cite{RichmanBridges1999}. However, its too week
to prove lemma~\ref{lemma:analogLemma20RichmanBridges1999} which
is the analog to lemma~20 in \cite{RichmanBridges1999}. This difference
is because we could always pick $f\left(p\right)=0$ in lemma~20
in \cite{RichmanBridges1999}, while we need to deal with the case
where $\eta$ doesn't approach $0$ in lemma~\ref{lemma:analogLemma20RichmanBridges1999}
since the maximum and minimum value of a frame function may not occur
on orthogonal vectors in theorem~\ref{thm:analogTheorem23RichmanBridges1999}.
In order to prove lemma~\ref{lemma:analogLemma20RichmanBridges1999},
we need the following geometric conjecture. 

\begin{conjecture}\label{conj:analogGeometricInterludeRichmanBridges1999}There
exist a nonempty set~$\mathcal{N}\subseteq\mathbb{N}\cap\left[2\pi,\infty\right)$
such that for all $n\in\mathcal{N}$, if $\ket{\psi}$ and $\ket{\psi'}$
are points in the northern hemisphere with the latitude~$\theta\left(\ket{\psi}\right)=\theta\left(\ket{\psi'}\right)$,
then we could takes $n$ steps along east-west great circles to go
from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$ due south of $\ket{\psi}$.\footnote{Verify the details more precisely...}\qed\end{conjecture}

I don't know whether conjecture~\ref{conj:analogGeometricInterludeRichmanBridges1999}
is valid or not. However, if it is not valid, we need to require that
the maximum and minimum value of a frame function occur on orthogonal
vectors in theorem~\ref{thm:analogTheorem23RichmanBridges1999} twice.
}
...
$\min_{n\in\mathcal{N}}2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
...
By conjecture~\ref{conj:analogGeometricInterludeRichmanBridges1999},
we can take $n\in\mathcal{N}$ steps along East-West great circles
to go from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$ due south of
$\ket{\psi}$\footnote{\yutsung{Double check how many step at least from $\ket{\psi'}$
to a point $\ket{\psi_{s}}$, and the details in the original proof...}}. The difference in latitude between $\ket{\psi}$ and $\ket{\psi_{s}}$
...
and 
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\min_{n\in\mathcal{N}}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{r}-W_{l},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$.\qed\end{thm}
\begin{proof}
...
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\min_{n\in\mathcal{N}}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{r}-W_{l},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$.\qed\end{cor}
\begin{proof}
...
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\min_{n\in\mathcal{N}}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-f\left(\ket{\psi}\right)\right|\le27\delta\left(W_{r}-W_{l},W_{r}-W_{l},\omega_{f}\right)+27\delta\left(W_{r}-W_{l},W_{r}-W_{l},\omega_{f'}\right)+18\left(W_{r}-W_{l}\right)$.\qed\end{thm}
\begin{proof}
