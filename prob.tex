...
collects necessary conditions for reasoning coherently about
...
events $\events$, the \emph{probability measure} is a function
...
\begin{example}[Two-coin probability space]
  ...
  $\Omega = \{ HH, HT, TH, TT \}$. The event that the first coin is
  ``heads'' is $\{ HH, HT \}$; the event that the two coins land on
  opposite sides is $\{ HT, TH \}$; the event that at least one coin
  is tails is $\{ HT, TH, TT\}$. Depending on the assumptions
  regarding the coins, we can define several probability
...
$\{ HT \} \cup \{ TH \}$) is the sum of the individual probabilities.
\end{example}
...
which can be computed but with increasingly large resources as the
precision increases. Therefore it is more appropriate in such
situations to consider probability measures mapping events to a finite
set of elements computable with a fixed set of
resources~\cite{Artstein1972,PuriRalescu1983}. The
simplest such set, and the one we will consider exclusively in this
paper, is the set
$\mathscr{L}_{2}=\left\{
  \textrm{impossible},\textrm{possible}\right\}$
together with the operation $\vee$ where
$x \vee y=\textrm{impossible}$ if and only if
$x=y=\textrm{impossible}$. In relation to the first definition, one
can interpret impossible as the closed interval $[0,0]$, possible as the
open interval $(0,1]$, and $\vee$ as the addition of intervals. The
definition of a probability measure in this case is modifed as being a
function $\pmeas : \events \rightarrow \mathscr{L}_{2}$ such that:
\begin{itemize}
\item $\pmeas(\Omega) = \textrm{possible}$, and 
...
\[\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
\pmeas(\emptyset) &=& \textrm{impossible} \\
\pmeas(\{ HH \}) &=& \textrm{possible} \\
\pmeas(\{ HT \}) &=& \textrm{impossible} \\
\pmeas(\{ TH \}) &=& \textrm{possible} \\
\pmeas(\{ TT \}) &=& \textrm{impossible} \\
\pmeas(\{  HH, HT \}) &=& \textrm{possible} \\
\pmeas(\{  HH, TH \}) &=& \textrm{possible} \\
\pmeas(\{  HH , TT \}) &=& \textrm{possible} \\
\end{array} & \begin{array}{rcl}
\pmeas(\{  HT, TH \}) &=& \textrm{possible} \\
\pmeas(\{  HT , TT \}) &=& \textrm{impossible} \\
\pmeas(\{  TH , TT \}) &=& \textrm{possible} \\
\pmeas(\{  HH, HT, TH \}) &=& \textrm{possible} \\
\pmeas(\{  HH, HT, TT \}) &=& \textrm{possible} \\
\pmeas(\{  HH, TH, TT \}) &=& \textrm{possible} \\
\pmeas(\{  HT, TH, TT \}) &=& \textrm{possible} \\
\pmeas(\{  HH, HT, TH, TT \}) &=& \textrm{possible} 
\end{array}
\end{array}\]
\end{example}
...
Consider the one-qubit Hilbert space with computational basis
...
\]
Each event is interpreted as a possible post-measurement state of
...
In classical probability space, two particular coins can define a
probability measure on every event. Similarly, a quantum system can
define a quantum probability measure on its event. The difference
is that only orthogonal events can test together. For example, asking
whether event~$\mathbb{1}$ occurs is the same as asking whether
event~$\proj{0}$ or event~$\proj{1}$ occurs simultaniously. However,
we can never ask whether event~$\proj{0}$ or event~$\proj{-}$
occurs simultaniously. Similarly to the classical case, a \emph{quantum
probability space}~\cite{BirkhoffVonNeumann1936,gleason1957,Redhead1987-REDINA,DBLP:journals/corr/abs-0910-2393,Maassen2010}
can be summarized into the following two rules\footnote{Recently, people extend the domain of $\mu$ to all operators~$\mathcal{A}$
on $\Hilb$ and consider $\mu:\mathcal{A}\rightarrow\C$~\cite{Maassen2010,Swart2013}.
...
\item for a collection of pairwise orthogonal $E_{i}$, we have $\mu\left(\sum_{i}E_{i}\right)=\sum_{i}\mu\left(E_{i}\right)$. 
\end{itemize}
\begin{example}{[}One-qubit Hilbert space{]} Consider an experiment
that with a qubit. The events are discribed previously. Depending on
the systems, we can define different quantum probability measures.
Here is a possible one: 
\[
\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
\mu_{0}\left(\mathbb{0}\right) & = & 0\\
\mu_{0}\left(\proj{0}\right) & = & 1\\
\mu_{0}\left(\proj{1}\right) & = & 0\\
 & \vdots
\end{array} & \begin{array}{rcl}
\mu_{0}\left(\mathbb{1}\right) & = & 1\\
\mu_{0}\left(\proj{\ps}\right) & = & 1/2\\
\mu_{0}\left(\proj{\ms}\right) & = & 1/2\\
 & \vdots
\end{array}\end{array}
\]
The assignment satisfies the two constraints for quantum probability
measures: the probability of $\mathbb{1}$ is 1, and the probability
of every summation of orthogonal events (e.g., $\proj{0}+\proj{1}$)
is the sum of the individual probabilities. \end{example}
...
we want to focus only on probability. }: 
\begin{equation}
\mu_{\psi}(E)= \ip{\psi}{E\psi} \label{eq:Born}
\end{equation}
which is called the Born rule~\cite{Born1984,Mermin2007}.\\
\yutsung{One small thing is whether we should say
\begin{quote}
for each normalized state~$\ket{\psi}$...$\mu_{\psi}(E)=\ip{\psi}{E\psi}$
\end{quote}
as in Mermin~\cite{Mermin2007} and Maassen~\cite{Maassen2010}
or
\begin{quote}
for each state~$\ket{\psi}\in\Hilb\backslash\left\{ 0\right\} $...$\mu_{\psi}(E)=\frac{\ip{\psi}{E\psi}}{\ip{\psi}{\psi}}$
\end{quote}
as in Abramsky~\cite{DBLP:journals/corr/abs-0910-2393}. Personally,
I prefer Abramsky's approach, because normalizing a state in DQT is
messier than in CQT, and we have already considered states whose norm
is not one in our previous paper~\cite{DQT2014}.}\\
These two conditions define quantum probability measures. Recall that
the Born rule (\ref{eq:Born}) induces a probability measure from
a state. Naturally, we may ask: is every probability measure induced
from a state by the Born rule? The answer is yes by Gleason's theorem
when the dimension~$\ge3$ and extend the domain of the Born rule
to mixed state~\cite{gleason1957,peres1995quantum,Redhead1987-REDINA}.

\yutsung{There are three ways to charactize a quantum probability: 
\begin{enumerate}
\item The above two conditions as Gleason did. 
\item As a linear functional on a {*} algebra. 
\item As induced by a state vector by the Born rule (\ref{eq:Born}). 
\end{enumerate}
According to equation (37) in section 4 Quantum Probability in \cite{Maassen2010},
the equalivance between 1. and 2. only hold for $d\ge3$ because of
Gleason' theorem. Therefore, it seems hard to get insight for how
to handle Gleason's theorem by {*}-algebra formulism...

The equalivance between 2. and 3. can be found in Proposition 4.1.1
in \cite{Swart2013}. Here, the Born rule~$\mu_{\psi}(E)$ is replaced
by a faithful pseudotrace $\tau$ and $\mu_{\psi}(E)=\tau\left(\proj{\psi}E\right)$.
The faithful pseudotrace is defined in section 5.2 Decomposition of
algebra with a list of conditions. The problem is:
\begin{itemize}
\item Lemma 5.2.1 proved there exists a faith pseudotrace, but said nothing
about uniqueness. Therefore, these conditions may not uniquely determine
the Born rule?
\item We don't formulate thing in algrebra... So although we may have an
analog or something, we cannot quite sure our conditions will be exactly
the same as theirs. 
\end{itemize}
If we admit the above, then the condition (a) to (c) and $\tau\left(AB\right)=\tau\left(BA\right)$
implies 
\begin{itemize}
\item Our map $\tilde{\mu}:\Hilb\rightarrow\qevents\rightarrow[0,1]$ should
satisfy $\tilde{\mu}_{\psi}$ is a probability measure for all $\ket{\psi}\in\Hilb\backslash\left\{ 0\right\} $.
\end{itemize}
The condition (a) and (d) implies
\begin{itemize}
\item Our map $\tilde{\mu}$ satisfy $\ip{\psi}{\phi}=0\Leftrightarrow\tilde{\mu}_{\psi}\left(\proj{\phi}\right)=\tilde{0}$,
...
\end{itemize}
$\tau\left(AB\right)=\tau\left(BA\right)$ also implies
\begin{itemize}
...
\end{itemize}
Besides, the condition (a) also gives the probability of mixed states
form pure states... OK... it seems that I know what I am going to
type for DQT, but I think I need to sort them out before type them...}
...
\item Instead of real-valued probability measures producing results in
  ...
  finite set-valued probablitity measures~\cite{Artstein1972,PuriRalescu1983}.
\end{itemize}
...
% Obveriously, we don't want arbirarily assign a state~$\ket{\psi}\in\Hilb\backslash\left\{ 0\right\} $
% with a proabability measure $\tilde{\mu}_{\psi}$, for example, assigning
...
variations. Speficially, it is impossible to maintain the Hllbert
...
\textrm{impossible} & \textrm{, if }E=\proj{+};\\
\bar{\mu}_{0}(E) & \textrm{, otherwise.}
\end{cases}
\]
