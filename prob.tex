...
interval-valued probability measures~\cite{JamisonLodwick2004} in
detail.\footnote{There is another possible approach that can be used to split the real
...
A \emph{core} of an interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
is the set 
\[
...
\yutsung{The following theorem proved that a wide variety of interval-valued
probability measures can be associated with real-valued probability
measures.

\begin{thm}[Shapley~\cite{Shapley1965,GilboaSchmeidler1994}]
Every convex interval-valued probability measure has a nonempty core,
where an interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
is convex if $\bar{\mu}\left(E\right)=\left[l\left(E\right),r\left(E\right)\right]$
and $l\left(E_{1}\cup E_{2}\right)+l\left(E_{1}\cap E_{2}\right)\ge l\left(E_{1}\right)+l\left(E_{2}\right)$
for all $E_{1},E_{2}\in\events$.\qed\end{thm}

In particular, Dempster-Shafer probability measures are all convex
and have nonempty cores. Need to verify whether general interval-valued
probability measures must have a core or find a counterexample.}
...
measurement} if it is a partition of the identity, i.e., $\tensor*[_{\mathscr{O}}]{\sum}{_{i}}P_{i}=\mathbb{1}$~\cite{Swart2013}. 
...
intersection between events~\cite{peres1995quantum}. 
\end{itemize}
\qed\end{definition}
...
There is a quantum interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$
