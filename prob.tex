...
Our previous examples can be verified in terms of $\mu_{\psi}$. Because
$\proj{0}$ and $\proj{-}$ are non-commutative, their total probabilities
are not a probability of an event
\begin{eqnarray*}
\mu_{0}(\proj{0})+\mu_{0}(\proj{-}) & = & \frac{3}{2}>1\textrm{ .}
\end{eqnarray*}
When events are orthogonal, i.e., commutative, they can be measured
together and their probability is additive: 
\[
\mu_{0}(\proj{0})+\mu_{0}(\proj{1})=\mu_{0}(\proj{+})+\mu_{0}(\proj{-})=1=\mu_{0}(\mathbb{1})\textrm{ .}
\]
Similarly to the classical case, the total probability is one and
the orthogonal additivity can be summarized into the following two
rules:
\begin{itemize}
\item $\mu(\mathbb{1})=1$, and 
\item for a collection of pairwise orthogonal $E_{i}$, we have $\mu(\sum_{i}E_{i})=\sum_{i}\mu(E_{i})$.
\end{itemize}
