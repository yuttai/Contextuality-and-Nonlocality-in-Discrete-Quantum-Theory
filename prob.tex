...
empty state. \\
\yutsung{How about replacing the above sentence by something like: 
\begin{quote}
Each event is interpreted as a possible post-measurement state of
a quantum system if that event occurs as follows~\cite{Abramsky2013}:
...... and the event $\mathbb{0}$ does not to state the post-measurement
state because it never occurs. 
\end{quote}
}\\
Clearly irrespective of the current state $\ket{\psi}$
...
\]
Similarly to the classical case, this probability measure satisfies: 
\begin{itemize}
...
\[
\xymatrix{\textrm{projections (\textrm{events})}\ar[d]^{\mu_{\psi}\textrm{(probability)}} & \subsetneq & \textrm{Hermitian operators (observables)}\ar[d]^{\mu_{\psi}\textrm{(expectation value)}} & \subsetneq & \textrm{all operators in a }*\textrm{-algebra(?)}\ar[d]^{\mu_{\psi}\textrm{(?)}}\\
\left[0,1\right] & \subsetneq & \mathbb{R} & \subsetneq & \mathbb{C}
}
\]
Although the above paragraph claimed $\qevents$ is a set of events,
``all linear combinations of \emph{projections}'' and $\left\{ \sum_{j=1}^{k}\lambda_{j}P_{j}~\middle|~\lambda_{1},\ldots,\lambda_{k}\in\mathbb{C}\right\} $
gives all operators in a $*$-algebra. Therefore, I got confused you
want $\qevents$ be ``the set of all events'' or ``the set of all
operators in a $*$-algebra''...}
