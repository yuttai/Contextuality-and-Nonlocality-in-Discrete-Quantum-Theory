...
\end{array}\]
\yutsung{It seems more clear to write like this: 
\[
\begin{array}{rcl}
...
\[
\begin{array}{rcl}
\mathbb{P}(\{LLL\}) & = & \frac{1}{4}\\
\mathbb{P}(\{LLR\}) & = & 0\\
\mathbb{P}(\{LRL\}) & = & 0\\
\mathbb{P}(\{LRR\}) & = & \frac{1}{4}\\
\mathbb{P}(\{RLL\}) & = & 0\\
\mathbb{P}(\{RLR\}) & = & \frac{1}{4}\\
\mathbb{P}(\{RRL\}) & = & \frac{1}{4}\\
\mathbb{P}(\{RRR\}) & = & 0\\
\mathbb{P}(E) & = & \sum_{\omega\in E}\mathbb{P}(\left\{ \omega\right\} )
\end{array}
\]
...
that 
\[
\mathbb{P}(\{abc\})=\mathbb{P}(\{a\})\mathbb{P}(\{b\})\mathbb{P}(\{c\})\textrm{ .}
\]
...
know these two coins are correlated. Einstein, Podolsky, and Rosen~\cite{EPR1935}
suggested any correlated quantum probability results may be interpreted
classically. For example, maybe the natural actually rolls an tetrahedron
die with $\{LLL,LRR,RLR,RRL\}$ in its four faces, and this tetrahedron
die could not be accessed by human for some reasons so that we think
we are handling three electrons. 
