...
\emph{projections} on~$\Hilb$. Each quantum state $\ket{\psi}\in\Hilb$ induces
a probability measure $\mu_\psi : \qevents \rightarrow [0,1]$ on the
space of events defined for any event $A\in\qevents$ as follows:
\[
...
\item for a collection of pairwise orthogonal $A_i$, we have
  $\mu(\sum_i A_i) = \sum_i \mu(A_i)$. 

\amr{CHECK because sum of
    projections may not be a projection}

\yutsung{I am going to verify if $\proj{\psi_{1}}+\proj{\psi_{2}}$
is a projection, then they are orthogonal. Given any state $\ket{\phi}=a\ket{\psi_{1}}+b\ket{\psi_{2}}$,
if $\proj{\psi_{1}}+\proj{\psi_{2}}$ is a projection, then 
\begin{eqnarray*}
\ket{\phi} & = & \left(\proj{\psi_{1}}+\proj{\psi_{2}}\right)\ket{\phi}\\
 & = & \left(\proj{\psi_{1}}+\proj{\psi_{2}}\right)\left(a\ket{\psi_{1}}+b\ket{\psi_{2}}\right)\\
 & = & a\ket{\psi_{1}}+b\ket{\psi_{2}}+a\ket{\psi_{2}}\ip{\psi_{2}}{\psi_{1}}+b\ket{\psi_{1}}\ip{\psi_{1}}{\psi_{2}}
\end{eqnarray*}
Therefore, we have 
\begin{eqnarray*}
0 & = & a\ket{\psi_{2}}\ip{\psi_{2}}{\psi_{1}}+b\ket{\psi_{1}}\ip{\psi_{1}}{\psi_{2}}
\end{eqnarray*}
However, $a$ and $b$ can be any numbers. Therefore, we must have
$0=\ip{\psi_{2}}{\psi_{1}}=\ip{\psi_{1}}{\psi_{2}}$.}
\end{itemize}
\yutsung{The following is the diagram for quantum probability objects
and their corresponding physical meaning (in parentheses): 
\[
\xymatrix{\textrm{projections (events)}\ar[d]^{\mu_{\psi}\textrm{(probability)}} & \subsetneq & \textrm{Hermitian operators (observables)}\ar[d]^{\mu_{\psi}\textrm{(expectation value)}} & \subsetneq & \textrm{all operators in a }*\textrm{-algebra(?)}\ar[d]^{\mu_{\psi}\textrm{(?)}}\\
\left[0,1\right] & \subsetneq & \mathbb{R} & \subsetneq & \mathbb{C}
}
\]
}
