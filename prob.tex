...
$\sigma_{\ket{\psi},\theta}$ is rotation by $\theta$ about $\ket{\psi}$.\footnote{\yutsung{Write its matrix representation?}}\qed\end{definition}
...
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$.\qed\end{thm}
\begin{proof}
...
the oscillation of $g_{f,\ket{0}}$ on latitudes is at most $\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)$.
...
so that 
\[
\left|F\left(a_{i}\right)-\left(W-g_{f,\ket{0}}\left(\ket{\psi_{i}}\right)\right)\right|=\left|g_{f,\ket{0}}\left(\ket{\psi_{i}}\right)-g_{f,\ket{0}}\left(\ket{\psi_{i}'}\right)\right|\stackrel{\textrm{Lemma }\ref{lemma:analogLemma20RichmanBridges1999}}{\le}\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)\textrm{ .}
\]
for all $i$. Whence, 
\begin{eqnarray*}
 &  & \left|F\left(a_{0}\right)+F\left(a_{1}\right)+F\left(a_{2}\right)-W\right|\\
 & \le & \left|F\left(a_{0}\right)+F\left(a_{1}\right)+F\left(a_{2}\right)-3W+g_{f,\ket{0}}\left(\ket{\psi_{0}}\right)+g_{f,\ket{0}}\left(\ket{\psi_{1}}\right)+g_{f,\ket{0}}\left(\ket{\psi_{2}}\right)\right|+\left|2W-\left(g_{f,\ket{0}}\left(\ket{\psi_{0}}\right)+g_{f,\ket{0}}\left(\ket{\psi_{1}}\right)+g_{f,\ket{0}}\left(\ket{\psi_{2}}\right)\right)\right|\\
...
\[
\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|=\left|W-g_{f,\ket{0}}\left(\ket{\psi}\right)-W\ell\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)\textrm{ .}
\]
\end{proof}
...
\item $\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)=\min_{n\ge2\pi}2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$.\qed\end{cor}
\begin{proof}
...
and we have $\left|Q\left(\ket{\psi}\right)-g_{f,\ket{0}}\left(\ket{\psi}\right)\right|=\left|Q'\left(\ket{\psi}\right)-g_{f',\ket{0}}\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$
...
$m=\min_{\ket{\psi}\in S^{2}}f\left(\ket{\psi}\right)=f\left(\ket{0'}\right)$, 
\item $\ket{2}=\sigma_{\ket{0}\times\ket{0'},\frac{\pi}{2}}\ket{0}$, $\ket{1}=\ket{2}\times\ket{0}$, 
\item $Q\left(\ket{\psi}\right)=M\ip{0}{\psi}^{2}+\left(W-M-m\right)\ip{1}{\psi}^{2}+m\ip{2}{\psi}^{2}$,
and 
\item $\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)=\min_{n\ge2\pi}2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-f\left(\ket{\psi}\right)\right|\le?$.\qed\end{thm}
\begin{proof}
Let $\ket{2'}=\sigma_{\ket{0}\times\ket{0'},\frac{\pi}{2}}\ket{0'}$.
Then 
\[
\left|\left(f\left(\ket{0}\right)+f\left(\ket{2}\right)\right)-\left(f\left(\ket{0'}\right)+f\left(\ket{2'}\right)\right)\right|\le W_{r}-W_{l}
\]
so
\[
f\left(\ket{2'}\right)-f\left(\ket{2}\right)\ge f\left(\ket{0}\right)-f\left(\ket{0'}\right)-\left(W_{r}-W_{l}\right)=M-m-\left(W_{r}-W_{l}\right)
\]
...
Let $f'\left(\ket{\psi}\right)=f\left(\ket{\psi}\right)-Q\left(\ket{\psi}\right)$.
Then, \footnote{\yutsung{Verify the details. Especially the left-handed or right-handed
or the coordinate system, and the rotation matrix... >''<}} 
\begin{eqnarray*}
...
we should have 
\begin{eqnarray*}
\left|f'\left(\ket{\psi}\right)+f'\left(\sigma_{\ket{0},\frac{\pi}{2}}\ket{\psi}\right)\right| & = & \left|g_{f,\ket{0}}-\left(2Mx^{2}+\left(W-M\right)\left(y^{2}+z^{2}\right)\right)\right|\le9\delta\left(0,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)\\
\left|f'\left(\ket{\psi}\right)+f'\left(\sigma_{\ket{2},\frac{\pi}{2}}\ket{\psi}\right)\right| & = & \left|g_{f,\ket{2}}-\left(2mz^{2}+\left(W-m\right)\left(x^{2}+y^{2}\right)\right)\right|\le9\delta\left(W_{r}-W_{l},W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)
\end{eqnarray*}
Follow the same procedure in the proof in \cite{RichmanBridges1999}.
We have 
\[
\left|f\left(\ket{\psi}\right)-M\left(x^{2}-z^{2}\right)-Wz^{2}\right|\le\frac{3}{2}\left(9\delta\left(W_{r}-W_{l},W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)\right)
\]
