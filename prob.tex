...
and \textquotedbl{}=\textquotedbl{} in the rule $\mu\left(\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{N}}P_{i}\right)=\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}\mu\left(P_{i}\right)$
...
where $\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}\pmeas(E_{i})$ explicitly
specifies $\pmeas(E_{i})\in\mathbb{R}$. Besides $\mathbb{R}$, we
will prepose other symbols later to specify the type of operations,
and they may be dropped when there is no ambiguity. 
\end{itemize}
...
\end{equation}
where $\rho:\Omega\rightarrow[0,1]$, $\sideset{_{\mathbb{R}}}{_{\omega\in\Omega}}\sum\rho(\omega)=1$,
and $\rho$ is called the Radon-Nikodym derivative of $\pmeas$ (with
...
to given accuracy by the relative frequency~$\frac{1}{N}\sum_{i=1}^{N}x_{i}$
...
the probability of the needle crossing a line is $\frac{2\ell}{\pi h}$~\cite{Buffon1777,DeMorgan1872,Hall1873,Uspensky1937}.
...
we have 
\[
\bar{\mu}\left(\bigcup_{i=1}^{N}E_{i}\right)\subseteq\left[\max\left\{ 1-\sideset{_{\mathbb{R}}}{_{i}}\sum r_{i}',\sideset{_{\mathbb{R}}}{_{i}}\sum l_{i}\right\} ,\min\left\{ 1-\sideset{_{\mathbb{R}}}{_{i}}\sum l_{i}',\sideset{_{\mathbb{R}}}{_{i}}\sum r_{i}\right\} \right]\textrm{ ,}
\]
...
where $\Omega\backslash E$ is the complement event, and 
\item for a collection $\left\{ E_{i}\right\} _{i=1}^{N}$ of pairwise disjoint
events, we have $\bar{\mu}\left(\bigcup_{i=1}^{N}E_{i}\right)\subseteq\tensor*[_{\mathscr{I}}]{\sum}{_{i}}\bar{\mu}\left(E_{i}\right)$,
where $\tensor*[_{\mathscr{I}}]{\sum}{_{i}}[l_{i},r_{i}]=\left[\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}l_{i},\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}r_{i}\right]$.
...
\yutsung{Because the physical meaning of convex is still not clear,
it would be better if we can find or prove a theorem which removes
``convex'' in the theorem. However, I still have not found an example
with an empty core or a real-valued probability measure in the core
of each interval-valued probability measure. What we know:
\begin{enumerate}
\item The cloud in \cite{JamisonLodwick2004} cannot induce an interval-valued
probability measure. Therefore, any results related to clouds cannot
be used directly.
\item The definition of interval-valued probability measure in \cite{JamisonLodwick2004}
is different from any later definition of interval-valued probability
measure written by Lodwick, including \cite{Lodwick2008}.
\item About linear programming, the Mathematica program solving the case
$\left|\Omega\right|=1$ or $2$ gives positive answer. The program
ceases to respond when outputting $\left|\Omega\right|=3$. Maybe
I should simplify the program?
\item Check whether the Shapley value still in the core or not in our setting...?
\end{enumerate}
}
...
probability measure $\mu:\qevents\rightarrow[0,1]$. We present an
example before giving the formal definition.

\begin{example}[One-qubit quantum probability space] Consider
a one-qubit Hilbert space with states $\ket{\phi}=\alpha\ket{0}+\beta\ket{1}$
such that $|\alpha|^{2}+|\beta|^{2}=1$, $\alpha,\beta\in\C$. The
set of events associated with this Hilbert space consists of all projection
operators. Each event is interpreted as a possible post-measurement
state of a quantum system in current state $\ket{\phi}$. For example,
the event $\proj{0}$ indicates that the post-measurement state will
be~$\ket{0}$; the event $\proj{1}$ indicates that the post-measurement
state will be~$\ket{1}$; the event $\proj{\ps}$ where $\ket{\ps}=\frac{1}{\sqrt{2}}(\ket{0}+\ket{1})$
...
will be the empty state. As in the classical case, a probability measure
is a function that maps events to $[0,1]$: here is a partial specification
of a possible probability measure: 
\[
\begin{array}{rcl}
...
if it is a partition of the identity, i.e., $\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{N}}P_{i}=\mathbb{1}$~\cite{Swart2013}.
...
\item for mutually orthogonal projections $P_{i}$, we have $\mu\left(\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{N}}P_{i}\right)=\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}\mu\left(P_{i}\right)$. 
\end{itemize}
\qed\end{definition}
...
space $\Hilb$, a function~$\mu:\events\rightarrow[0,1]$ is a quantum
probability measure if and only if $\mu$ satisfies the following
conditions: 
\begin{itemize}
\item $\mu(\mathbb{0})=0$, 
\item $\mu(\mathbb{1})=1$, 
\item for any projection $P$, $\mu\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P\right)=1\tensor*[_{\mathbb{R}}]{-}{}\mu\left(P\right)$,
and 
\item for a set of mutually orthogonal projections $P_{i}$, we have $\mu\left(\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{N}}P_{i}\right)=\tensor*[_{\mathbb{R}}]{\sum}{_{i=1}^{N}}\mu\left(P_{i}\right)$. 
\end{itemize}
\qed\end{lemma}

A set of events~$\events$ together with a quantum probability measure
is called a \emph{quantum probability space}. Comparing to classical
probability spaces, the empty set~$\emptyset$ corresponds to the
empty projection~$\mathbb{0}$, and the event of whole space~$\Omega$
corresponds to the identity projection~$\mathbb{1}$. In contrast,
the union~$\cup$ of any two events always gives an event classically,
but the operator addition~$\tensor*[_{\mathscr{O}}]{+}{}$ of two
projections may not be a projection. As the result, the classical
...
matrix~$\rho=\sum_{j}q_{j}\proj{\phi_{j}}$, where $\sum_{j}q_{j}=1$.
Then, the quantum probability measure introduced by $\rho$ is the
combination of $\mu_{\phi_{j}}^{B}$ with respect to probability $q_{j}$~\cite{peres1995quantum,544199,RiederSvozil2007}:
\begin{eqnarray}
...
In this sense, the Radon-Nikodym theorem could be generalized to Gleason's
theorem~\cite{HollandJr1970,Redhead1987-REDINA,Jaeger2007}: in Hilbert
spaces of dimension $d\geq3$, given a quantum probability measure
...
$\mu(\proj{\ps})+\mu(\proj{\ms})=1+0=1$. On the other hand, $\mu$
cannot be induced by any mixed state because $\mu(\proj{\ps})=\mu(\proj{0})=1$,
...
two spots corresponding to $\ket{0}$ and $\ket{1}$. In reality,
we need a lot of resource to keep the local field of strength directing
to the $z$-axis precisely, to keep the atoms having almost the same
velocity, and to point out the exact position each particle landed
on.

If the field of strength does not perfectly direct to the $z$-axis,
we do not really test the quantum event we want to test. This problem
will be handled later when we introduce the discrete quantum theory.
Even if the field of strength perfectly directs to the $z$-axis,
...
In another word, if somebody claims she is tossing a coin behind a
veil, and only shows the resulting heads or tails, we cannot distinguish
whether she has really tossed a coin, or she has run an Stern-Gerlach
experiment, and shows us the head, the tail, or the side of coin if
...
\begin{equation}
\bar{\mu}\left(\sideset{_{\mathscr{O}}}{_{i}}\sum P_{i}\right)\subseteq\sideset{_{\mathscr{I}}}{_{i}}\sum\bar{\mu}\left(P_{i}\right)\textrm{ .}\label{eq:QuantumInterval-valuedProbability-Inclusion}
\end{equation}
\end{itemize}
\qed\end{definition}
...
and $\iota\left(\sideset{_{\mathbb{R}}}{_{i}}\sum x_{i}\right)\subseteq\sideset{_{\mathscr{I}}}{_{i}}\sum\iota\left(x_{i}\right)$,
