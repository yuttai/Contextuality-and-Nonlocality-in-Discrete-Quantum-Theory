...
is denoted by $S^{d-1}=\left\{ \ket{\psi}\in\mathbb{R}^{d}\middle|\ip{\psi}{\psi}=1\right\} $.\qed\end{definition}

\begin{definition}A function~$f:S^{d-1}\rightarrow\mathbb{R}$ is
called a \emph{real approximation frame function} of weight~$\left(W_{l},W_{r}\right)$
if
\begin{itemize}
...
of $\mathbb{R}^{d}$.
\item For any $\lambda\in\mathbb{C}\setminus\left\{ 0\right\} $ and $\ket{\psi}\in S^{d-1}$,
$f\left(\lambda\ket{\psi}\right)=f\left(\ket{\psi}\right)$.
\end{itemize}
\qed\end{definition}
