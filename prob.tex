...
%%%%%
\subsection{Calculating Probabilities: Buffon's Needle Problem}
...
\noindent The reason the last condition is expressed using $\subseteq$
is best explained using a small example.
...
measures $\mu:\events\rightarrow[0,1]$. The Born rule~\cite{Born1984,Mermin2007},
...
\]
Moreover, the Born rule can be extended to a mixed state~$\rho$.
If the density matrix of a mixed state~$\rho$ can be expressed as
a convex combination of pure states, i.e., $\rho=\sum_{j=1}^{N}q_{j}\proj{\psi_{j}}$,
where $\sum_{j=1}^{N}q_{j}=1$, then the probability measure induced
by $\rho$ can be expressed as a convex combination as well~\cite{peres1995quantum,544199}:
\begin{eqnarray}
\mu_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\mu_{\psi_{j}}\left(E\right)\textrm{ .}\label{BornRule.mixed}
\end{eqnarray}
...
using the Born rule. The theorem is only valid in Hilbert spaces with
dimension $d\geq3$. It is instructive to study counterexamples in
$d=2$, i.e., the case of a one-qubit system. 

\begin{example}[One-qubit quantum probability measure] 
Consider a quantum probability
measure~$\mu:\events\rightarrow[0,1]$ defined as follow:
\[
\mu(E)=\begin{cases}
...
the same as $\mu_{\ket{0}}$ which is a probability measure, we only
need to check $E=\proj{\ps}$ and $E=\proj{\ms}$. $\proj{\ps}$ and
$\proj{\ms}$ are only orthogonal to each other, and we have 
\[
\mu(\proj{\ps})+\mu(\proj{\ms})=1+0=1
\]
which satisfies the conditions as a probability measure. On the other
hand, $\mu$ cannot be induced by any mixed state because 
\[
