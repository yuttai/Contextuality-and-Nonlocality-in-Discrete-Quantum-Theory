...
\paragraph*{Set-valued Probability Measures.} The simplest such set is
the set $\mathscr{L}_{2}=\left\{ \imposs, \poss\right\}$ together with
the operation $\vee$ where $x \vee y=\imposs$ if and only if
$x=y=\imposs$. In relation to the conventional definition, one can
interpret \imposs\ as the closed interval $[0,0]$, \poss\ as the
half-open interval $(0,1]$, and $\vee$ as the addition of
intervals. The definition of a probability measure in this case is
modified to a function $\pmeas : \events \rightarrow \mathscr{L}_{2}$
such that:
\begin{itemize}
...
\paragraph*{Interval-valued probability measures.} In this case, we
split the $[0,1]$ interval into a collection of overlapping closed
sub-intervals obeying certain restrictions to be described at the end
of the section. First we illustrate the main ideas using a simple
example. 

\begin{example}[Two-coin probability space with four intervals] \label{ex3}
We split the unit interval $[0,1]$ in the following four closed sub-intervals:
\begin{itemize}
\item $[0,0]$ which we call \imposs, 
\item $[0,\frac{1}{2}]$ which we call \unlikely, 
\item $[\frac{1}{2},1]$ which we call \likely, and 
\item $[1,1]$ which we call \necess.
\end{itemize}
Using these new values, the probability measure becomes:
\[
\begin{array}{c@{\qquad\qquad}c}
\begin{array}{rcl}
\pmeas(\emptyset) & = & \imposs\\
...
\end{array}\end{array}
\]
Not only does this probability measure reveal that the second coin is
double-headed but it also reveals the bias in the first coin, again despite
the absence of any numeric information.  
\qed\end{example}

The probability measure above can be understood in reference to
Ex.~\ref{ex1} by mapping each numeric value to the smallest interval
containing it. The probability measure appears quite intuitive from
that perspective. However, we also must be able to understand the
probability measure independently of Ex.~\ref{ex1} and in particular
justify how combinations such as
\[ 
...
is in the range $[1,1]$! To understand this mystery, we look at
another more general example and then give the formal mathematical
definition of interval-valued probabilities. 

\amr{See A Simple View of the Dempster-Shafer Theory of Evidence and 
  its Implication for the Rule of Combination Lotfi A. Zadeh, AAAI,
  1986}
...
\begin{example}[Dempster-Shafer Theory of Evidence] We have five
  ...
  each employee:
\[\begin{array}{rcl}
\textrm{Age}(M_{1}) & \in & [22,26]=D_{1}\\
\textrm{Age}(M_{2}) & \in & [20,22]=D_{2}\\
\textrm{Age}(M_{3}) & \in & [30,35]=D_{3}\\
\textrm{Age}(M_{4}) & \in & [20,22]=D_{4}\\
\textrm{Age}(M_{5}) & \in & [28,30]=D_{5}
\end{array}\]
What is the probability that an employee's age is in the range
$[20,25]$? Looking at the data, it is \emph{possible} for $M_1$'s age
...
$M_4$'s age is in the range. Clearly saying that an event is
\emph{necessary} is equivalent to saying that its complement is
\emph{not possible}. Aggregating the results for the five employees,
we can calculate that is necessary that 2 employees have their ages
within the range, and it is possible that 3 employees have their ages
within the range. We express this formally as saying that the
probability of the event is $[0.4,0.6]$. Now consider another event
asking whether the ages are some other disjoint range
$[26,40]$. Reasoning in a similar way we calculate that the
probability for this event is $[0.4,0.6]$. Now let's take the two events
together and ask about the possibility of the ages to be in the range
$[20,40]$. Clearly that probability must be $[1,1]$ as every
employee's age is in that range. The way to calculate this probability
from the individual probabilities of the events is as follows: 
\begin{itemize}
\item \amr{can you try to fix this... perhaps it is not consistent
    with the interval-valued formalization but it is really nice if we
    can make it work}
\item Because ages are all integers, and we only consider ages from 20 to
40. Hence, the sample space~$\Omega$ can be $\left\{ 20,21,\ldots,39,40\right\} $,
and the set of events are $2^{\Omega}$ as usual.\\
\yutsung{I have numerically verified if we set $\Omega=\left\{ 20,22,25,26,28,30,35,40\right\} $,
and the answer should be the same as $\left\{ 20,21,\ldots,39,40\right\} $
because only end-points should matter in the question... I will numerically
verify $\left\{ 20,21,\ldots,39,40\right\} $, or prove they are the
same later...} 
\item Given a event~$E\subseteq\Omega$, the probability of $E$ can be
defined by 
\[
\pmeas(E)=\left[\frac{1}{5}\left|\left\{ i\middle|D_{i}\cap E\ne\emptyset\right\} \right|,\frac{1}{5}\left|\left\{ i\middle|D_{i}\subseteq E\right\} \right|\right]\textrm{ ,}
\]
where $\left|A\right|$ is the number of elements in $A$. Then, we
can verify that $\pmeas$ in an interval-value probability measure.
\end{itemize}
\qed\end{example}
