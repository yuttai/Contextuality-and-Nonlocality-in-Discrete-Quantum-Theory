...
\begin{itemize}
\item $\mathbb{0}$ and $\mathbb{1}$ are a projection. 
...
and may be dropped when summands are clearly operators. 
\item For any pure state~$\ket{\psi}$, $\proj{\psi}$ is a projection
operator. Conversely, every projection~$P$ can be expressed as $\sum_{j}\proj{\psi_{j}}$,
...
whole sample space~$\Omega$ corresponds to the identity projection~$\mathbb{1}$.
In contrast, the union~$\cup$ of any two events always gives an
event classically, the operator addition~$\tensor*[_{\mathscr{O}}]{+}{}$
of two projections may not be a projection. As the result, the equation
$\pmeas(E_{1}\cup E_{2})=\pmeas(E_{1})\tensor*[_{\mathbb{R}}]{+}{}\pmeas(E_{2})$
...
is always true whenever the left-handed side is defined. Whether equation
is defined also effects the further properties of quantum probability
space.

\begin{definition}{[}Orthogonal Complement; Commutativity~\cite{peres1995quantum}{]} 
\begin{itemize}
\item If $P$ is a projection operator, then $\mathbb{1}\tensor*[_{O}]{-}{}P$
