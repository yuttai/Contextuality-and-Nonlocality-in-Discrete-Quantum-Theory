...
and \ref{ex:three-dimensional-4-value}, we find that example~\ref{ex:three-dimensional-three-value}
really use the property of $\unknown$ so that it cannot hold when
the intervals are more precise. Also, example~\ref{ex:three-dimensional-4-value}
is not as bad as example~\ref{ex:three-dimensional-three-value}
in a sense that the states map to $\imposs$ are all in the same plane
in example~\ref{ex:three-dimensional-4-value}. Maybe interval-valued
