...
\yutsung{In \cite{Vourdas2014,Vourdas2015}, Vourdas made a comparison
between the additivity property of Kolmogorov probabilities 
\[
\delta\left(E_{1},E_{2}\right)=\pmeas\left(E_{1}\vee E_{2}\right)-\pmeas\left(E_{1}\right)-\pmeas\left(E_{2}\right)+\pmeas\left(E_{1}\wedge E_{2}\right)=0
\]
and its quantum counterpart 
\[
\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)=\Tr\left(\rho\Pi\left(S_{1}\vee S_{2}\right)\right)-\Tr\left(\rho\Pi\left(S_{1}\right)\right)-\Tr\left(\rho\Pi\left(S_{2}\right)\right)+\Tr\left(\rho\Pi\left(S_{1}\wedge S_{2}\right)\right)\textrm{ ,}
\]
where $S_{1}$ and $S_{2}$ are arbitrary linear spaces, and $\Pi$
maps a linear space to its projection operator. While $\delta\left(E_{1},E_{2}\right)$
...
for the upper probability or plausibility. The sign of $\delta\left(E_{1},E_{2}\right)$
is then analog to the sign of $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$
given the density matrix~$\rho$ since the non-additivity operator~$\mathfrak{D}\left(S_{1},S_{2}\right)$
is non-zero when $S_{1}$ and $S_{2}$ does not commute so that $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$
is not always zero.

The Kolmogorov's $\delta\left(E_{1},E_{2}\right)=0$ is intuitive
because the relative frequencies of disjoint events should be additive
if the measurement is accurate. In the Dempster-Shafer theory, the
additivity property is modified because the imprecision of whether
an event happens or not is taken into account, but we can still decide
whether an instance belongs to different events concurrently. Applying
this idea to quantum, if we runs $N$ independent Stern-Galech 

Interestingly, this theory discussed the relation between non-commuting
subspaces while the real-valued quantum probability measures in definition~\ref{def:QuantumProbabilitySpace}
only depend on orthogonal (or commuting) subspaces. In order to understand
the intuition, let's go through the simplest example. 
