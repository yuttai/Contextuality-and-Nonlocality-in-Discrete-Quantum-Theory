...
% The following copy from `iopart.cls'
...
measure, and each measure must correspond to some pair of coins.
\qed\end{example}
...
\begin{example}\label{One-qubit-quantum-probability}[One-qubit quantum
probability space] Consider a one-qubit Hilbert space with states
...
\item If the projections $P_1$ and $P_2$ commute then $P_1P_2$ is also
  a projection; it is not sufficient in this case for $P_1$ and $P_2$
  to be orthogonal. 
\end{itemize}
\end{definition}
...
$\ket{\phi}\in\Hilb\backslash\left\{ 0\right\} $ induces a probability
measure~$\mu_{\phi}$ as follows: 
\[
\mu_{\phi}(E)=\frac{\ip{\phi}{E\phi}}{\ip{\phi}{\phi}}
\]
This Born rule can be extended to mixed quantum states~$\rho$ by
\begin{eqnarray}
\mu_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\mu_{\psi_{j}}\left(E\right)\textrm{ ,}\label{BornRule.mixed}
\end{eqnarray}
where $\rho=\sum_{j=1}^{N}q_{j}\frac{\proj{\psi_{j}}}{\ip{\psi_{j}}{\psi_{j}}}$
is a density matrix, and $\sum_{j=1}^{N}q_{j}=1$, i.e., $\Tr\left(\rho\right)=1$.
It is easy to verify that given a mixed state, the Born rule also
induces a probability measure. Conversely, Gleason's theorem states
that given a probability measure~$\mu$, there exist a mixed quantum
states~$\rho$ that induces such a measure using the Born rule. The
theorem is only valid in Hilbert spaces with dimension $d\geq3$.
It is instructive to study counterexamples in $d=2$, i.e., the case
of a one-qubit system. As in example~\ref{One-qubit-quantum-probability},
the set of events associated with it contains $\mathbb{0}$, $\mathbb{1}$,
and one-dimensional projection operators which can be expressed as
$\proj{\phi}$, where $\ket{\phi}=\left(\begin{array}{c}
\cos\theta\\
\rme^{\rmi\gamma}\sin\theta
\end{array}\right)$. Then, we can define a probability measure as follow.
\begin{eqnarray*}
\mu\left(\mathbb{0}\right) & = & 0\textrm{;}\\
\mu\left(\mathbb{1}\right) & = & 1\textrm{;}\\
\mu\left(\proj{\phi}\right) & = & \begin{cases}
0 & \textrm{, if }0\le\theta<\frac{\pi}{2}\textrm{;}\\
1 & \textrm{, if }\frac{\pi}{2}\le\theta<\pi\textrm{.}
\end{cases}
\end{eqnarray*}
$\mu$ is a probability measure, but not induced by the Born rule
because $\mu$ not continuous, but the probabilities induces by the
Born rule are always continuous. 

\yutsung{Rieder and Svozil~\cite{RiederSvozil2007} didn't provide
enough details to construct an example. It seems that the full details
in written in Svozil's book~\cite{Svozil1998}. Hence, I have recommended
the library to purchase this book and sent an interlibrary loan request
as well.}
...
the Born rule can be extended to 
\begin{eqnarray}
\mu_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\mu_{\psi_{j}}\left(E\right)\textrm{ ,}\label{BornRule.mixed-1}
\end{eqnarray}
where $\rho=\sum_{j=1}^{N}q_{j}\frac{\proj{\psi_{j}}}{\ip{\psi_{j}}{\psi_{j}}}$
is a density matrix, and $\sum_{j=1}^{N}q_{j}=1$, i.e., $\Tr\left(\rho\right)=1$.
\\
