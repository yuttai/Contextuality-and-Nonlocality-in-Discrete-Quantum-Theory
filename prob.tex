...
between events. 
\item For projection operators $P_{1}$ and $P_{2}$, we define $P_{1}\tensor*[_{\mathscr{O}}]{\ge}{}P_{2}$
if $P_{1}\tensor*[_{\mathscr{O}}]{-}{}P_{2}$ is a projection operator;
and $P_{1}\tensor*[_{\mathscr{O}}]{>}{}P_{2}$ if $P_{1}\tensor*[_{\mathscr{O}}]{\ge}{}P_{2}$
and $P_{1}\ne P_{2}$. Also, let $S_{i}$ be the linear subspace $P_{i}$
project onto, then $P_{1}\tensor*[_{\mathscr{O}}]{\ge}{}P_{2}$ if
and only if $S_{1}\supseteq S_{2}$.\yutsung{Need to verify!}
\end{itemize}
\qed\end{definition}
...
Like a classical probability measure can be easily defined by its
Radon-Nikodym derivative, a quantum probability measure can be easily
constructed by states according to the Born rule~\cite{Born1983,Mermin2007,RiederSvozil2007}.
...
The generalized Radon-Nikodym theorem is called Gleason's theorem
...
\bar{\mu}(\mathbb{1})=\bar{\mu}(1-\proj{0})=\bar{\mu}(1-\proj{\ps}) & = & \necess\textrm{ ,}\\
...
where $\sum_{j}q_{j}=1$ and $q_{j}>0$. By
equation (\ref{eq:probability-zero-on-states}),
we have $\rho=\proj{2}$. However, 
\[
\iota\left(\mu(\proj{2})\right)=\necess\ne\unknown=\bar{\mu}(P)\textrm{ .}
\]
\qed\end{example}
...
\section{Proof of Quantum Interval-valued Probability Measures}

In order to simplify the notation, consider the following definition
for interval-valued probability measures.

\begin{definition}Given a Hilbert space~$\Hilb$ with the set of
quantum events~$\events$, a collection of intervals~$\mathscr{I}$,
and a quantum $\mathscr{I}$-interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$.
Let $l:\events\rightarrow\left[0,1\right]$ and $r:\events\rightarrow\left[0,1\right]$
such that $\left[l\left(P\right),r\left(P\right)\right]=\bar{\mu}\left(P\right)$.\qed\end{definition}

Before we prove our results, we need the following lemmas.

\begin{lemma}Given a Hilbert space~$\Hilb$ with the set of quantum
events~$\events$, a collection of intervals~$\mathscr{I}$, and
a quantum $\mathscr{I}$-interval-valued probability measure~$\bar{\mu}:\events\rightarrow\mathscr{I}$.
For projection operators $P_{1}$ and $P_{2}$, if $P_{1}\tensor*[_{\mathscr{O}}]{\ge}{}P_{2}$,
then $\bar{\mu}\left(P_{1}\right)\tensor*[_{\mathscr{I}}]{\ge}{}\bar{\mu}\left(P_{2}\right)$,
where $\left[l_{1},r_{1}\right]\tensor*[_{\mathscr{I}}]{\ge}{}\left[l_{2},r_{2}\right]$
if and only if $l_{1}\tensor*[_{\mathbb{R}}]{\ge}{}l_{2}$ and $r_{1}\tensor*[_{\mathbb{R}}]{\ge}{}r_{2}$.
The preposing $\mathbb{R}$ represents its an inequality of real numbers
as usual.\qed\end{lemma}

\begin{proof}

\begin{eqnarray*}
 &  & P_{1}\tensor*[_{\mathscr{O}}]{\ge}{}P_{2}\\
 & \Rightarrow & \bar{\mu}\left(P_{1}\right)\subseteq\bar{\mu}\left(P_{1}\tensor*[_{\mathscr{O}}]{-}{}P_{2}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(P_{2}\right)\\
 & \Rightarrow & l\left(P_{1}\right)\tensor*[_{\mathbb{R}}]{\ge}{}l\left(P_{1}\tensor*[_{\mathscr{O}}]{-}{}P_{2}\right)\tensor*[_{\mathbb{R}}]{+}{}l\left(P_{2}\right)\tensor*[_{\mathbb{R}}]{\ge}{}l\left(P_{2}\right)
\end{eqnarray*}
Similarly, 
\begin{eqnarray*}
 &  & \mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P_{1}\tensor*[_{\mathscr{O}}]{\le}{}\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P_{2}\\
 & \Rightarrow & 1\tensor*[_{\mathbb{R}}]{-}{}r\left(P_{1}\right)=l\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P_{1}\right)\tensor*[_{\mathbb{R}}]{\le}{}l\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P_{2}\right)=1\tensor*[_{\mathbb{R}}]{-}{}r\left(P_{2}\right)\\
 & \Rightarrow & r\left(P_{1}\right)\tensor*[_{\mathbb{R}}]{\ge}{}r\left(P_{2}\right)
\end{eqnarray*}
