...
corresponding to $\ket{+Z}$, $\ket{0Z}$, and $\ket{-Z}$ predicted
by the semi-classical model assuming particles have definite trajectories.
In reality, all particles can from a hot oven cannot have the same
velocity, as the result, spots will be broader and more washed out
so that sometimes it may hard to decide a particle corresponding to
which state. 
