...
by an algorithm~\cite{Peleg1965}. However, the number of minimal
balanced sets grows very rapidly with $\left|\Omega\right|$, the
size of $\Omega$~\cite{Shapley1967,Zhao2008}\footnote{\yutsung{Prove the number of minimal balanced sets grow exponentially?}},
so that we have only verified every interval-valued probability measure
has a non-empty core when $\left|\Omega\right|\le5$ so far.
...
for a given Hilbert space~$\Hilb$ of dimension~$d$. If the state
of the silver atom is represented by a density matrix~$\rho$ written
in the same basis, i.e., $\rho=\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{d}}\tensor*[_{\mathscr{O}}]{\sum}{_{j=1}^{d}}r_{ij}\ket{\psi_{i}}\bra{\psi_{j}}$.
As we discussed in the last paragraph, the Born rule of the mixed
state~$\rho$induces a classical probability measure~$\pmeas':2^{\Omega}\rightarrow[0,1]$
...
have
\begin{equation}
\pmeas'\left(E\right)=\mu_{\rho}^{B}\left(P\right)\overset{\left(\ref{BornRule.mixed}\right)}{=}\Tr\left(\rho P\right)=\sideset{_{\mathbb{R}}}{_{\ket{\psi_{j}}\in E}}\sum r_{jj}\textrm{ .}\label{BornRule.mixed-1}
\end{equation}
By equation (\ref{eq:Radon-Nikodym}), the Radon-Nikodym derivative
of $\pmeas'$ $f:\Omega\rightarrow[0,1]$ is just the diagonal elements
of $\rho$:

\begin{equation}
f\left(\ket{\psi_{j}}\right)=r_{jj}\textrm{ .}\label{eq:quantum-Radon-Nikodym}
\end{equation}

In this sense, the Radon-Nikodym derivative~$f$ is a special case
of the density matrix~$\rho$, and the Radon-Nikodym theorem is a
special case of Gleason's theorem~\cite{HollandJr1970,Redhead1987-REDINA,Jaeger2007}.
...
\yutsung{I think there are two possible approaches to complete the
story. 

Our original plan was to put the Mermin v.s. Meyer debate here, and
move on to the discrete quantum probability measure. Then, discrete
real-valued quantum probability measures are expected to fail because
of John's proof, and we hope discrete interval-valued Gleason's theorem
can work in some sense? This story is scratched in section~\ref{sec:DQT}.
The problem is the discrete version of example~\ref{ex:three-dimensional-three-value}.
Because we don't really use $\mathbb{C}$ in example~\ref{ex:three-dimensional-three-value},
the discrete version of example~\ref{ex:three-dimensional-three-value}
is likely to serve as an counterexample for discrete interval-valued
Gleason's theorem... Hence, we may need to put some extra-conditions
to by-pass the discrete version of example~\ref{ex:three-dimensional-three-value}.

Another possible approach is to tell everybody: because Gleason's
theorem is an accident, the original way to formulate to a quantum
system as a state vector or density matrix is an accident. If we want
to do things correct, we should work on quantum (real/interval)-valued
probability measure directly. This approach is scratched in section~\ref{sec:QC}.
The problem of this approach is that we need an concrete example to
show it is possible to work on quantum (real/interval)-valued probability
measure directly. For example, we might need to verify tensor product
and quantum teleportation works in this model...}

\section{\label{sec:DQT}(One possible approach) Discrete Quantum Theory}

\subsection{Discrete Quantum Real-valued Probability Measures}

Killed by John... Need to be typed...
