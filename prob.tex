...
``convex'' in the theorem. However, I still have not found an interval-valued
probability measure with an empty core or proved that each interval-valued
probability measure has an nonempty core. What we know:
\begin{enumerate}
...
\item About linear programming, the answer of Mathematica program for $\left|\Omega\right|=3$
is too complex to read by human. May need to simplify the result to
...
i.e., $\rho=\tensor*[_{\mathscr{O}}]{\sum}{_{i=1}^{d}}\tensor*[_{\mathscr{O}}]{\sum}{_{j=1}^{d}}r_{ij}\ket{\psi_{i}}\bra{\psi_{j}}$.
