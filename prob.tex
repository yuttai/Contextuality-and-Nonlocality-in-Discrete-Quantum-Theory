...
\item A point~$\ket{\psi'}$ in a East-West great circle~$C\left(\ket{\psi}\right)$
is \emph{close to} $\ket{\psi}$ if 
\[
...
$\ket{\psi}$ and $\ket{\psi'}$ be points in the northern hemisphere
with the latitude~$\theta\left(\ket{\psi}\right)=\theta\left(\ket{\psi'}\right)$.
Then, we can take a finite number~$n$ close steps along East-West
great circles to go from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$
due south of $\ket{\psi}$.\qed\end{lemma}
...
then we could takes $n$ close steps along east-west great circles
to go from $\ket{\psi'}$ to a point $\ket{\psi_{s}}$ due south of
$\ket{\psi}$.\qed\end{conjecture}
