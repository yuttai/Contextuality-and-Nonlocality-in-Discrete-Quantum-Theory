...
\begin{equation}
\pmeas(E)=\tensor*[_{\mathbb{R}}]{\sum}{_{\omega\in E}}\rho(\omega)\textrm{ ,}\label{eq:Radon-Nikodym}
\end{equation}
...
This fact is a special case of the the Radon-Nikodym theorem, and
$\rho$ is called the Radon-Nikodym derivative of $\pmeas$ with respect
to the counting measure~\cite{Nikodym1930,Folland2013}. For example,
the Radon-Nikodym derivative of $\pmeas$ in example~\ref{ex1} is:
\[
\begin{array}{rcl}
...
Actually, constructing a quantum probability measures from a density
matrix~$\rho$ is similar to defining a classical probability measure
by its Radon-Nikodym derivative. This can be seen more easily if we
pick an orthonormal basis~$\left\{ \ket{\psi_{j}}\right\} _{j=1}^{d}$
such that $P=\tensor*[_{\mathscr{O}}]{\sum}{_{\psi_{j}\in S}}\proj{\psi_{j}}$,
where $S$ is the linear subspace $P$ projects onto. Then, a density
matrix~$\rho$ can also be represented in the same basis, i.e., $\rho=\sum_{i=1}^{d}\sum_{j=1}^{d}r_{ij}\proj{\psi_{j}}$,
and the Born rule becomes
\begin{eqnarray}
\mu_{\rho}^{B}\left(P\right) & = & \Tr\left(\rho P\right)=\sum_{\psi_{j}\in S}r_{jj}\textrm{ ,}\label{BornRule.mixed-1}
\end{eqnarray}
which is basically the same as equation (\ref{eq:Radon-Nikodym}).

The generalized Radon-Nikodym theorem is called Gleason's theorem~\cite{HollandJr1970,Redhead1987-REDINA}
which states that, in Hilbert spaces of dimension $d\geq3$, given
a probability measure $\mu$, there exist a mixed state~$\rho$ that
induces such a measure using the Born rule~\cite{gleason1957,Redhead1987-REDINA,peres1995quantum}.
...
probability measure]\label{Three-dimensional-three-value} Given
...
probability measure{]} Given a three dimensional Hilbert space with
...
\item Let $\ket{\psi\left(\theta,\gamma\right)}=\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$
and $\ket{\phi\left(\theta,\gamma\right)}=\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$,
where $0\le\theta<\frac{\pi}{2}$ and $0\le\gamma<\pi$. we define
$\bar{\mu}(\proj{\psi\left(\theta,\gamma\right)})=\ket{\phi\left(\theta,\gamma\right)}=\likely$
and $\bar{\mu}(\mathbb{1}-\proj{\psi\left(\theta,\gamma\right)})=\bar{\mu}(\mathbb{1}-\proj{\phi\left(\theta,\gamma\right)})=\unlikely$.
\item Otherwise, $\bar{\mu}(\proj{\psi})=\unlikely$ and $\bar{\mu}(\mathbb{1}-\proj{\psi})=\likely$.
\end{enumerate}
$\bar{\mu}$ cannot correspond to any quantum probability measure
because of the exact same reason as in example~\ref{Three-dimensional-three-value}.
...
\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}}\right) & \subseteq & \bar{\mu}\left(\proj{\psi_{2}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{3}}\right)\\
...
We can enumerate all possible cases as follow. 
\begin{enumerate}
\item When $\ket{\psi_{1}}$ is $\ket{0}$, then $\ket{\psi_{2}}$ must
be parallel to $\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$
for $0\le\theta\le\frac{\pi}{2}$ and $0\le\gamma<2\pi$. Then,
\[
\ket{\psi_{1}}\times\ket{\psi_{2}}=\begin{pmatrix}1\\
0\\
0
...
\end{pmatrix}
\]
WLOG, we can pick $\ket{\psi_{2}}=\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$
for $0\le\theta<\frac{\pi}{2}$ and $0\le\gamma<\pi$, and $\ket{\psi_{3}}=\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$
for $0\le\theta<\frac{\pi}{2}$ and $0\le\gamma<\pi$ 
\[
\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}\times\psi_{2}}\right)=\likely=\imposs\tensor*[_{\mathscr{I}}]{+}{}\likely=\bar{\mu}\left(\proj{\psi_{1}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{2}}\right)
\]
\[
\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}}\right)=\necess\subseteq\likely\tensor*[_{\mathscr{I}}]{+}{}\unlikely=\bar{\mu}\left(\proj{\psi_{2}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{1}\times\psi_{2}}\right)
\]
If ($0<\theta<\frac{\pi}{2}$ and $\pi\le\gamma<2\pi$) or $\theta=\frac{\pi}{2}$
, then 
\[
\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}\times\psi_{2}}\right)=\unlikely=\imposs\tensor*[_{\mathscr{I}}]{+}{}\unlikely=\bar{\mu}\left(\proj{\psi_{1}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{2}}\right)
\]
Similarly, when $\ket{\psi_{1}}$ is $\ket{\ps}$, equation (\ref{eq:non-additive-vectors})
holds.
\item When $\proj{\psi_{1}}\tensor*[_{\mathscr{O}}]{+}{}\proj{\psi_{2}}=\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{0}$,
then Similarly, when $\proj{\psi_{1}}\tensor*[_{\mathscr{O}}]{+}{}\proj{\psi_{2}}=\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\ps}$,
