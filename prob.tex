...
to the counting measure, or just called the Radon-Nikodym derivative
of $\pmeas$ because we are always with respect to the counting measure.
The Radon-Nikodym theorem states that the converse is also true.
...
Because a probability measure~$\pmeas:\events\rightarrow[0,1]$ can
be simplified to its Radon-Nikodym derivative~$\rho:\Omega\rightarrow[0,1]$
by the Radon-Nikodym theorem, people have tried to find a Radon-Nikodym
Theorem for the interval-valued probability measure. One possible
approach is to consider the inverse Möbius transform of $\bar{\mu}:\events\rightarrow\mathscr{I}$~\cite{GilboaSchmeidler1994,Goodman2013}.
Although the inverse Möbius transform can always be defined, the domain
of the inverse Möbius transform of of $\bar{\mu}$ is still the event
space~$\events$ so that it does not simplify an interval-valued
probability measure to a function from the same space~$\Omega$.
The other approach tried to only consider the Radon-Nikodym derivative
between two interval-valued probability measures with a special relation
called the strong decomposition property~\cite{Choquet1954,Graf1980,Goodman2013}.
However, only the real-valued probability measures could have a Radon-Nikodym
derivative with respect to the counting measure\footnote{\yutsung{Need to verify a concrete example.}}.
In summary, there is no satisfactory Radon-Nikodym Theorem to simplify
interval-valued probability measures~$\bar{\mu}:\events\rightarrow\mathscr{I}$
to their Radon-Nikodym derivative~$\bar{\rho}:\Omega\rightarrow\mathscr{I}$.
