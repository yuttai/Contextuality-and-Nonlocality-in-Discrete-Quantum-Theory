...
If we adopt the frequency interpretation as in section~\ref{subsec:Measuring-Probabilities:-Buffon},
and measure the angular momentum for a spin-one particle in the Stern-Gerlach
experiment~\cite{FeynmanLeightonSands1965,Stern1988,peres1995quantum,544199}.
...
corresponding to $\ket{+Z}$, $\ket{0Z}$, and $\ket{-Z}$. In reality,
the local field strength cannot prefect directs to to the $z$-axis,
and all particles come from a hot oven cannot have the same velocity.
As the result, spots will be broader and more washed out so that it
may sometimes hard to decide a particle corresponding to which state\footnote{\yutsung{Citation here?}}.
