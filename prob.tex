...
space $\Hilb$, a function~$\mu:\events\rightarrow[0,1]$ is a probability
measure if and only if $\mu$ satisfies the following conditions:
\begin{itemize}
\item $\mu(\mathbb{0})=0$, 
\item $\mu(\mathbb{1})=1$, 
\item $\mu\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P\right)=1\tensor*[_{\mathbb{R}}]{-}{}\mu\left(P\right)$,
and 
\item for mutually orthogonal projections $P_{i}$, we have $\mu\left(\tensor*[_{\mathscr{O}}]{\sum}{_{i}}P_{i}\right)=\tensor*[_{\mathbb{R}}]{\sum}{_{i}}\mu\left(P_{i}\right)$. 
\end{itemize}
\qed\end{lemma}
...
$\ket{\phi}$ induces a probability measure $\tilde{\mu}_{\phi}$
as follows: 
\[
\tilde{\mu}_{\phi}(E)=\ip{\phi}{E\phi}
\]
...
measure introduced by $\rho$ is the combination of $\tilde{\mu}_{\phi_{j}}$
with respect to probability $q_{j}$~\cite{peres1995quantum,544199,RiederSvozil2007}:
\begin{eqnarray}
\tilde{\mu}_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\tilde{\mu}_{\phi_{j}}\left(E\right)\textrm{ .}\label{BornRule.mixed}
\end{eqnarray}
...
\begin{cor}\label{cor:Gleason's}Given a Hilbert space $\Hilb$,
for any function~$\mu:\events\rightarrow[0,1]$ satisfies the conditions
listed in lemma~\ref{lem:quantumProbabilityMeasure}, there exist
an unique mixed state~$\rho$ such that $\mu=\tilde{\mu}_{\rho}$.\end{cor}
...
\tilde{\mu}_{\ket{0}}(E) & \textrm{, otherwise.}
\end{cases}
\]
On one hand, $\mu$ is a probability measure. Because $\mu$ is almost
the same as a probability measure~$\tilde{\mu}_{\ket{0}}$, we only
...
\]
However, $\tilde{\mu}_{\rho}(E)=1$ if and only if $\rho$ represents
a pure state and $\rho=E$. \qed\end{example}
...
probability measure} is a function~$\mu:\events\rightarrow\mathscr{I}$
such that: 
\begin{itemize}
\item $\mu(\mathbb{0})=\left[0,0\right]$, 
\item $\mu(\mathbb{1})=\left[1,1\right]$, 
\item $\mu\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}P\right)=1\tensor*[_{\mathscr{I}}]{-}{}\mu\left(P\right)$,
and 
\item for mutually orthogonal projections $P_{i}$, we have $\mu\left(\tensor*[_{\mathscr{O}}]{\sum}{_{i}}P_{i}\right)\subseteq\tensor*[_{\mathscr{I}}]{\sum}{_{i}}\mu\left(P_{i}\right)$. 
\end{itemize}
\qed\end{definition}
...
for any quantum probability measure~$\tensor*[_{\left[0,1\right]}]{\mu}{}:\events\rightarrow\left[0,1\right]$,
we can define a corresponding quantum \emph{$\mathscr{I}$}-interval-valued
probability measure~$\tensor*[_{\mathscr{I}}]{\mu}{}:\events\rightarrow\mathscr{I}$
by $\tensor*[_{\mathscr{I}}]{\mu}{}\left(E\right)=\left[\tensor*[_{\left[0,1\right]}]{\mu}{}\left(E\right),\tensor*[_{\left[0,1\right]}]{\mu}{}\left(E\right)\right]$.
...
measure~$\tensor*[_{\left[0,1\right]}]{\mu}{}:\events\rightarrow\left[0,1\right]$,
we can define a quantum interval-valued probability measure~$\tensor*[_{\mathscr{I}}]{\mu}{}:\events\rightarrow\mathscr{I}$
by 
\[
\tensor*[_{\mathscr{I}}]{\mu}{}(E)=\iota\left(\tensor*[_{\left[0,1\right]}]{\mu}{}(E)\right)\textrm{ ,}
\]
...
By applying these two properties, it is easy to verify $\tensor*[_{\mathscr{I}}]{\mu}{}$
...
a quantum interval-valued probability measure~$\tensor*[_{\mathscr{I}}]{\mu}{}:\events\rightarrow\mathscr{I}$
defined by 
\begin{eqnarray*}
\tensor*[_{\mathscr{I}}]{\mu}{}(\mathbb{0}) & = & \imposs\textrm{ ,}\\
\tensor*[_{\mathscr{I}}]{\mu}{}(\mathbb{1}) & = & \necess\textrm{ ,}\\
\tensor*[_{\mathscr{I}}]{\mu}{}(\proj{\psi}) & = & \begin{cases}
...
so we have
\[
\tensor*[_{\mathscr{I}}]{\mu}{}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi}\right)=\tensor*[_{\mathscr{I}}]{\mu}{}\left(\proj{\psi^{\perp}}\right)=1\tensor*[_{\mathscr{I}}]{-}{}\tensor*[_{\mathscr{I}}]{\mu}{}\left(\proj{\psi}\right)\textrm{ .}
\]
Hence, $\tensor*[_{\mathscr{I}}]{\mu}{}$ is a quantum $\mathscr{I}$-interval-valued
probability measure. Notice that for all projection~$E$, $\tensor*[_{\left[0,1\right]}]{\tilde{\mu}}{_{\ket{0}}}(E)\in\tensor*[_{\mathscr{I}}]{\mu}{}(E)$
implies that $\tensor*[_{\mathscr{I}}]{\mu}{}(E)$ is one of quantum
$\mathscr{I}$-interval-valued probability measures corresponding
to $\tensor*[_{\left[0,1\right]}]{\tilde{\mu}}{_{\ket{0}}}(E)$. However,
$\tensor*[_{\left[0,1\right]}]{\tilde{\mu}}{_{\ket{0}}}(E)$ can correspond
...
Even if there is a natural map from quantum probability measures to
quantum interval-valued probability measures, there are also some
quantum interval-valued probability measures do not come from quantum
probability measures.

\begin{example}[Three-dimensional quantum interval-valued probability
...
as before. Let
\begin{eqnarray*}
\mu(\mathbb{0}) & = & \imposs\textrm{ ,}\\
\mu(\mathbb{1}) & = & \necess\textrm{ ,}\\
\mu(\proj{0}) & = & \necess\textrm{ ,}\\
\mu(\proj{\ps}) & = & \necess\textrm{ ,}\\
\mu(\proj{\psi}) & = & \imposs\textrm{, for }\ip{\psi}{0}=0\textrm{ or }\ip{\psi}{\ps}=0\textrm{ ,}\\
\mu(1-\proj{0}) & = & \imposs\textrm{ ,}\\
\mu(1-\proj{\ps}) & = & \imposs\textrm{ ,}\\
\mu(1-\proj{\psi}) & = & \necess\textrm{, for }\ip{\psi}{0}=0\textrm{ or }\ip{\psi}{\ps}=0\textrm{ ,}\\
\mu(P) & = & \unknown\textrm{, otherwise. }
\end{eqnarray*}
\qed\end{example}

\amr{ The idea will be the following. First describe quantum probability
spaces conventionally. Then talk about the following: 
\begin{itemize}
\item the dimension of the Hilbert space is a parameter that is like the
number of needles; it gives an upper bound on the accuracy of the
numbers that are relevant in expressing probabilities 
\item the intervals will come from two things: the fact that states can
only be prepared to a certain accuracy so when we say the state is
$\ket{\psi}$ we really mean a neighborhood of states close to $\ket{\psi}$ 
\item similarly when we do an experiment with $\proj{\phi}$ we are really
testing a family of projections that are near $\proj{\phi}$; this
fuziness will cause the probability to only be specifiable as intervals 
\end{itemize}
}
