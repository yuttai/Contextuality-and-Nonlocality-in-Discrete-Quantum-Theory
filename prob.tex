...
In previous section, we assume the probability of each event is known
a priori. In reality, we seldom know much about events, but we assume
each event~$E$ has a probability~$\pmeas(E)$. $\pmeas(E)$ could
be approximated by the relative frequency~$\frac{1}{N}\sum_{i}x_{i}$,
where $x_{i}$ is the outcome of $i$-th trial within total $N$ independent
trials. This fact is called by the law of large numbers~\cite{Bernoulli1713,Kolmogorov1950,Uspensky1937,Shafer1976,544199}.
...
theorem. However, the definition diverges, and none of the source I have checked 
support our definition, yet. We may need to check more papers to find a source
to support our definition or to find the majority definition (if there is one)...}
