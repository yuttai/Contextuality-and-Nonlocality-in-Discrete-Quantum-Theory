...
% abbreviate $1-\left[a,b\right]$, so the probability measure $\pmeas$
% should satisfy:
% \begin{itemize}
% \item $\pmeas(\emptyset)=[0,0]$, 
% \item $\pmeas(\Omega)=[1,1]$, and 
% \item $\pmeas\left(\Omega\backslash E\right)=1-\pmeas\left(E\right)$
% \end{itemize}
% However, if we want $\pmeas(\Omega)=[1,1]$, and the probability assignment
% for singleton sets $\pmeas(\{HH\})=\unlikely$, $\pmeas(\{HT\})=\imposs$,
% $\pmeas(\{TH\})=\likely$, and $\pmeas(\{TT\})=\imposs$ as in example~\ref{Two-coin-set-valued-again},
% we only have $\pmeas(\Omega)\subsetneq\pmeas(\{HH\})+\pmeas(\{HT\})+\pmeas(\{TH\})+\pmeas(\{TT\})$.
...
% the whole. Therefore, $\pmeas\left(\bigcup_{i}E_{i}\right)$ is a
% subset of $\sum_{i}\pmeas\left(E_{i}\right)$, but may not equal.
% In our example, 
% \begin{eqnarray*}
%  &  & \pmeas(\{HH\})+\pmeas(\{HT\})+\pmeas(\{TH\})+\pmeas(\{TT\})\\
...
% \end{eqnarray*}
% The above equation told us $\pmeas(\Omega)\subseteq\left[\frac{1}{2},\frac{3}{2}\right]$.
% However, because of $\pmeas(\emptyset)=\imposs$, we have $\pmeas\left(\Omega\right)=1-\pmeas\left(\emptyset\right)=\necess$,
% but $\pmeas(\emptyset)=\imposs$ cannot be used to reasoning the probability
...
% \begin{enumerate}
% \item $1\in\pmeas(\Omega)$, and 
% \item for a collection $E_{i}$ of pairwise disjoint events, $\pmeas(\bigcup_{i}E_{i})=\sum_{i}\pmeas(E_{i})$. 
% \end{enumerate}
...
%%%%%
\subsection{Quantum Interval-valued Probabilities Measures}

\yutsung{In \cite{Vourdas2014,Vourdas2015}, Vourdas started from
the additivity property of Kolmogorov probabilities 
\begin{equation}
\delta\left(E_{1},E_{2}\right)=\pmeas\left(E_{1}\vee E_{2}\right)-\pmeas\left(E_{1}\right)-\pmeas\left(E_{2}\right)+\pmeas\left(E_{1}\wedge E_{2}\right)=0\label{eq:Kolmogorov}
\end{equation}
which is intuitive because the relative frequencies of disjoint events
should be additive if the measurement is accurate. While $\delta\left(E_{1},E_{2}\right)$
is always zero for Kolmogorov probabilities, the additivity property
of the Dempster-Shafer theory is relaxed to $\delta\left(E_{1},E_{2}\right)\ge0$
for the lower probability or belief and $\delta\left(E_{1},E_{2}\right)\le0$
for the upper probability or plausibility. This modification is also
intuitive because the imprecision of whether an event happens or not
is taken into account, but we can still decide whether an instance
belongs to different events concurrently. 

Vourdas observed that equation (\ref{eq:Kolmogorov}) is similar to
\[
\mathfrak{D}\left(S_{1},S_{2}\right)=\Pi\left(S_{1}\vee S_{2}\right)-\Pi\left(S_{1}\right)-\Pi\left(S_{2}\right)+\Pi\left(S_{1}\wedge S_{2}\right)\textrm{ ,}
\]
where $S_{1}$ and $S_{2}$ are arbitrary linear subspaces, and $\Pi$
maps a linear subspace to its projection operator. After apply the
Born rule on both sides, we get an equation about the probabilities
\begin{equation}
\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)=\Tr\left(\rho\Pi\left(S_{1}\vee S_{2}\right)\right)-\Tr\left(\rho\Pi\left(S_{1}\right)\right)-\Tr\left(\rho\Pi\left(S_{2}\right)\right)+\Tr\left(\rho\Pi\left(S_{1}\wedge S_{2}\right)\right)\textrm{ .}\label{eq:quantum-additivity}
\end{equation}
Given the density matrix~$\rho$, the sign of $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$
is then analog to the sign of $\delta\left(E_{1},E_{2}\right)$ since
the non-additivity operator~$\mathfrak{D}\left(S_{1},S_{2}\right)$
is non-zero when $S_{1}$ and $S_{2}$ does not commute so that $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$
is not always zero. Vourdas then considered the pair $\Tr\left(\rho\Pi\left(S_{1}\right)\right)$
and $\Tr\left(\rho\Pi\left(S_{2}\right)\right)$ are lower or upper
probabilities depending on the sign of $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$.
Although Vourdas' idea is clear, the approach does not exactly fit
the idea. First, when we measure a physical quantity, we consider
the eigenspaces of an observable which always commute. According to
Vourdas' definition, $\Tr\left(\rho\Pi\left(S_{1}\right)\right)$
is then always an Kolmogorov probability within one measurement process.
Second, if we adopt the frequency interpretation as in section \ref{subsec:Measuring-Probabilities:-Buffon},
and implement our one-qubit example in the Stern-Gerlach experiment~\cite{Stern1988,peres1995quantum,544199,Griffiths2003}.
Let $S_{1}=\left\{ \lambda\ket{0}\middle|\lambda\in\C\right\} $ and
$S_{2}=\left\{ \lambda\ket{\ps}\middle|\lambda\in\C\right\} $.

\begin{example}[Two-qubit quantum probability space] Consider
a two-qubit Hilbert space with the following subspaces:
\begin{eqnarray*}
S_{00} & = & \mathrm{span}\left(\left\{ \ket{0}\otimes\ket{0}\right\} \right)\\
S_{0+} & = & \mathrm{span}\left(\left\{ \ket{0}\otimes\ket{+}\right\} \right)\\
S_{00}\wedge S_{0+} & = & \mathcal{O}=\left\{ 0\right\} \\
S_{00}\vee S_{0+} & = & \mathrm{span}\left(\left\{ \ket{0}\right\} \right)\otimes\C^{2}
\end{eqnarray*}
We can implement the first qubit by the spin of a particle and the
second qubit in path 

 Then, their projection and non-additivity operators are 
\begin{eqnarray*}
\Pi\left(S_{00}\right) & = & \proj{0}\otimes\proj{0}\\
\Pi\left(S_{0+}\right) & = & \proj{0}\otimes\proj{+}\\
\Pi\left(S_{00}\wedge S_{0+}\right) & = & \mathbb{0}\otimes\mathbb{0}\\
\Pi\left(S_{00}\vee S_{0+}\right) & = & \proj{0}\otimes\mathbb{1}\\
\mathfrak{D}\left(S_{1},S_{2}\right) & = & \proj{0}\otimes\left(-\sigma_{x}-\sigma_{z}\right)
\end{eqnarray*}
We also consider the density matrix $\rho=a\proj{0}\otimes\proj{0}+\left(1-a\right)\proj{0}\otimes\proj{1}$
for $a\in\left[0,1\right]$. According to the Born rule, their probabilities
are
\begin{eqnarray*}
\Tr\left(\rho\Pi\left(S_{00}\right)\right) & = & a\\
\Tr\left(\rho\Pi\left(S_{0+}\right)\right) & = & \frac{1}{2}\left[a+\left(1-a\right)\right]=\frac{1}{2}\\
\Tr\left(\rho\Pi\left(S_{00}\wedge S_{0+}\right)\right) & = & 0\\
\Tr\left(\rho\Pi\left(S_{00}\vee S_{0+}\right)\right) & = & a+\left(1-a\right)=1
\end{eqnarray*}
and the non-additivity quantity~$\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)=\frac{1}{2}\left[-a+\left(1-a\right)\right]=\frac{1}{2}-a$.
However, I still don't have much intuition on why the probabilities
should have what kind of relations?\qed\end{example}

However, his section VI is confusing. For example, the second sentence
in section IV: 
\begin{itemize}
\item If $\mathfrak{d}\left(H_{1},H_{2}\middle|\rho\right)\ge0$, then $p\left(H_{1}\middle|\rho\right)$
and $p\left(H_{2}\middle|\rho\right)$ are lower probabilities. Also,
according to Eq. (22), in this case $\mathfrak{d}\left(H_{1}^{\perp},H_{2}^{\perp}\middle|\rho\right)\le0$,
and therefore the $p\left(H_{1}^{\perp}\middle|\rho\right)$ and $p\left(H_{2}^{\perp}\middle|\rho\right)$
are upper probabilities. 
\end{itemize}
I assume Vourdas means that $p\left(H_{1}\middle|\rho\right)$ and
$p\left(H_{1}^{\perp}\middle|\rho\right)$ are the lower and upper
probability respectively for measuring subspace~$H_{1}$ on the system~$\rho$.
Consider the simplest example the subspace $H_{1}$ is the zero space
$\mathcal{O}$ which commutes with every other subspaces, so $\mathfrak{d}\left(\mathcal{O},H_{2}\middle|\rho\right)=0$.
Then, for measuring $\mathcal{O}$ on $\rho$, the lower and upper
probabilities are $p\left(\mathcal{O}\middle|\rho\right)=0$ and $p\left(\mathcal{O}^{\perp}\middle|\rho\right)=p\left(\mathcal{I}\middle|\rho\right)=1$,
respectively. The lower probability is $0$ makes sense, but I don't
understand why the upper probability is $1$. Because both the lower
and upper probabilities of the empty set are $0$ in classical Dempster-Shafer
theory.}

\amr{
The idea will be the following. First describe quantum probability
spaces conventionally. Then talk about the following:
\begin{itemize}
\item the dimension of the Hilbert space is a parameter that is like
  the number of needles; it gives an upper bound on the accuracy of
  the numbers that are relevant in expressing probabilities
\item the intervals will come from two things: the fact that states
  can only be prepared to a certain accuracy so when we say the state
  is $\ket{\psi}$ we really mean a neighborhood of states close to $\ket{\psi}$
\item similarly when we do an experiment with $\proj{\phi}$ we are
  really testing a family of projections that are near $\proj{\phi}$;
  this fuziness will cause the probability to only be specifiable as intervals
\end{itemize}
}
...
on the space of events defined for any event $E\in\qevents$ as follows~\cite{Born1983,Mermin2007}:
\begin{equation}
