...
In order to simplify the discussion, we will focus on $\mathscr{L}_{2}=\left\{ \textrm{impossible},\textrm{possible}\right\} $
where impossible and possible means $\left\{ 0\right\} $ and $\left(0,1\right]$
respectively while our result can be easily extend to other values
with the form $\left\{ \left\{ 0\right\} ,\left(0,\frac{1}{n}\right],\ldots,\left(\frac{n-1}{n},1\right]\right\} $. 

Because $\mathscr{L}_{2}$ is a partition of $\left[0,1\right]$~\cite{Artin1991,Wechler1992,PurdomBrown1995,DummitFoote2004},
there is a natural surjective map~$\iota:\left[0,1\right]\rightarrow\mathscr{L}_{2}$
defined by $\iota\left(0\right)=\textrm{impossible}$ and $\iota\left(x\right)=\textrm{possible}$
for $x\ne0$. By applying $\iota$, we can define the corresponding
Born rule~$\bar{\mu}$ by $\bar{\mu}_{\psi}(E)=\iota\left(\mu_{\psi}(E)\right)$,
and the corresponding probability measure~$\hat{\mu}$ should satisfy: 
\begin{itemize}
\item $\hat{\mu}(\mathbb{1})=\textrm{possible}$, and 
\item for a collection of pairwise orthogonal $E_{i}$, we have $\hat{\mu}(\sum_{i}E_{i})=\bigvee_{i}\hat{\mu}(E_{i})$,
where $x\vee y=\textrm{impossible}$ if and only if $x=y=\textrm{impossible}$.
\end{itemize}
