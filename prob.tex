...
and $w\left(\sigma_{x}^{3}\right)$ must be odd.
...
and $w\left(\sigma_{y}^{j}\right)$ for these probabilities. Notice
that the nature should predetermine the same values of $w\left(\sigma_{x}^{j}\right)$
and $w\left(\sigma_{y}^{j}\right)$ no matter we measure along $x$
axes or $y$ axes because the nature does not know which axes we are
going to measure.
