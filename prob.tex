...
and $\ket{\psi}\in S^{2}$, then the \emph{symmetrization} of $f$
with respect to $\ket{\psi}$ is the function~$g:S^{2}\rightarrow\mathbb{R}$
defined by
\[
g\left(\ket{\phi}\right)=f\left(\ket{\phi}\right)+f\left(\sigma\ket{\phi}\right)
\]
where $\sigma$ is clockwise rotation by $\pi/2$ about $\ket{\psi}$.\qed\end{definition}
...
$m=\inf f$ or $m=\sup f$, and $W=\left(W_{l}+W_{r}\right)/2$. Let
$Q$ be the quadratic form given by
\[
Q\left(\ket{\phi}\right)=W-m+\left(3m-W\right)\ip{\psi}{\phi}^{2}=2m\ip{\psi}{\phi}^{2}+\left(W-m\right)\left(1-\ip{\psi}{\phi}^{2}\right)
\]
For each $\delta>0$, there is $\eta>0$ such that if
\begin{itemize}
\item $\ket{\psi}\in S^{2}$such that $\left|f\left(\ket{\psi}\right)-m\right|<\eta$,
and 
\item $g$ is the symmetrization of $f$ with respect to $\ket{\psi}$.
\end{itemize}
then $\left|Q\left(\ket{\phi}\right)-g\left(\ket{\phi}\right)\right|\le\delta+3\left(W_{r}-W_{l}\right)\left[12\left\lceil \frac{\pi^{2}}{\varDelta_{f}\left(\frac{\delta}{36}\right)}\right\rceil +1\right]$,
...
of weight~$\left(W_{l},W_{r}\right)$, $m=\min f=f\left(\ket{\psi}\right)$
or $m=\max f=f\left(\ket{\psi}\right)$, and $W=\left(W_{l}+W_{r}\right)/2$.
Let $Q$ be the quadratic form given by
\[
Q\left(\ket{\phi}\right)=W-m+\left(3m-W\right)\ip{\psi}{\phi}^{2}=2m\ip{\psi}{\phi}^{2}+\left(W-m\right)\left(1-\ip{\psi}{\phi}^{2}\right)
\]
If $g$ is the symmetrization of $f$ with respect to $\ket{\psi}$,
then $\left|Q\left(\ket{\phi}\right)-g\left(\ket{\phi}\right)\right|\le3\left(W_{r}-W_{l}\right)$.\qed\end{cor}
...
for all $\ket{\phi}$ on the sphere.\qed\end{thm}
...
and there is a point~$\ket{\psi}\in S^{2}$, then the oscillation
on any latitude of the symmetrization of $f$ with respect to $\ket{\psi}$
is?\qed\end{lemma}
\begin{proof}
Consider $\ket{\psi}$ to be the north pole of $S^{2}$. Let $g$
be the symmetrization of $f$ with respect to $\ket{\psi}$, and $\eta=f\left(\ket{\psi}\right)$.
Let 
\begin{eqnarray*}
\omega_{f}\left(\delta\right) & = & \sup_{\left\Vert \ket{\psi}-\ket{\phi}\right\Vert <\delta}\left|f\left(\ket{\psi}\right)-f\left(\ket{\phi}\right)\right|\\
\omega_{g}\left(\delta\right) & = & \sup_{\left\Vert \ket{\psi}-\ket{\phi}\right\Vert <\delta}\left|g\left(\ket{\psi}\right)-g\left(\ket{\phi}\right)\right|=2\omega_{f}\left(\delta\right)
\end{eqnarray*}
Then, for any latitude $\theta_{0}$, the oscillation of $g$ on the
polar cap of latitude $2\theta_{0}-\frac{\pi}{2}$ is less than $\omega_{g}\left(2\sin2\theta_{0}\right)=2\omega_{f}\left(2\sin2\theta_{0}\right)$.

Let $\ket{\phi}$ and $\ket{\phi'}$ be points in the northern hemisphere
with the same latitude~$\theta$. If $\theta>2\theta_{0}-\frac{\pi}{2}$,
both $\ket{\phi}$ and $\ket{\phi'}$ are in the polar cap, so $\left|g\left(\ket{\phi}\right)-g\left(\ket{\phi'}\right)\right|<\omega_{g}\left(2\sin2\theta_{0}\right)=2\omega_{f}\left(2\sin2\theta_{0}\right)$.
Otherwise, consider $\theta\le2\theta_{0}-\frac{\pi}{2}$. Let $a$
be the difference in longitudes of $\ket{\phi}$ and $\ket{\phi'}$.
Take $n$ steps along east-west great circles to go from $\ket{\phi'}$
to a point $\ket{\phi_{s}}$ due south of $\ket{\phi}$. The difference
in latitude between $\ket{\phi}$ and $\ket{\phi_{s}}$ is at most
$\frac{a^{2}}{n}$, while, from (Lemma 16 to be typed),
\[
g\left(\ket{\phi_{s}}\right)\ge g\left(\ket{\phi'}\right)-n\left[2f\left(\ket{\psi}\right)+4\left(W_{r}-W_{l}\right)\right]=g\left(\ket{\phi'}\right)-2n\eta-4n\left(W_{r}-W_{l}\right)
\]
When the difference in latitude between $\ket{\phi}$ and $\ket{\phi_{s}}$
is at most $\frac{a^{2}}{n}$, the length between $\ket{\phi}$ and
$\ket{\phi_{s}}$ is at most $2-2\cos\left(\frac{a^{2}}{n}\right)$,
and hence 
\[
g\left(\ket{\phi}\right)\ge g\left(\ket{\phi_{s}}\right)-\omega_{g}\left(2-2\cos\left(\frac{a^{2}}{n}\right)\right)\ge g\left(\ket{\phi'}\right)-2n\eta-4n\left(W_{r}-W_{l}\right)-2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)
\]
for $\frac{\pi^{2}}{n}\le\frac{\pi}{2}\Leftrightarrow2\pi\le n$.
\end{proof}
