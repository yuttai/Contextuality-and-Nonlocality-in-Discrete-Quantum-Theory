...
explicitly specifies $a_{i}\in\mathbb{R}$. We may drop the preposing
...
measures~\cite{Dempster1967,Shafer1976,Weichselberger2000,JamisonLodwick2004}
...
and may be dropped clearly by content. Given
...
and may be dropped when there is no ambiguity. 
...
is also a projection operator, called complement. It is orthogonal to $P$, has
...
\item Projection operators $P_{1}$ and $P_{2}$ commute if $P_{1}P_{2}=P_{2}P_{1}$.
...
sources:
\begin{enumerate}
\item In Swart's~\cite{Swart2013}, two $*$-algebras~$\mathscr{A}_{1}$ and
$\mathscr{A}_{2}$ which commute with each other are \emph{logically
independent} if for all probability measure~$\mu_{1}$ on $\mathscr{A}_{1}$ and
$\mu_{2}$ on $\mathscr{A}_{2}$, there is an unique probability measure~$\mu_{1}\otimes\mu_{2}$
on $\mathscr{A}_{1}\otimes\mathscr{A}_{2}$ such that
$\left(\mu_{1}\otimes\mu_{2}\right)\left(A_{1}\otimes
...
with product state. It is good that independence is tied up with Bell
inequality, but the problem for us is that this defines the independent of
algebras, but not events\ldots
\item In Yuan's~\cite{Yuan2012}, given a $\mu$ on an $*$-algebra~$\mathscr{A}$,
two sub-$*$-algebra $\mathscr{A}_{1}$ and $\mathscr{A}_{2}$ are independent
if $\mu\left(A_{1}A_{2}\right)=\mu\left(A_{1}\right)\mu\left(A_{2}\right)$
for all $A_{1}\in\mathscr{A}_{1}$ and $A_{2}\in\mathscr{A}_{2}$.
The same problem as the previous one: this defines the
independent of algebras, but not events\ldots
\item In order to understand the definition of independence in~\cite{GuehneKleinmannCabelloEtAl2010},
we need first understand their notation because they focused on the
...
measuring $A$ first, then $B$, and finally $C$. We assume the outcome
of an observable is deterministic by $\lambda$. For example, the
outcome of $B_{2}$ from the preceding sequence is denoted by $v\left(B_{2}\middle|A_{1}B_{2}C_{3}\right)$.
Then, the outcome of $A$ is independent of whether $B$ is measured
...
HV~$\lambda$, the HV model is non-contextual.\\
...
and the definition of independence is tied up with contextuality.
...
$P_{1}$ and $P_{2}$ are independent if $\mu\left(P_{1}P_{2}\right)=\mu\left(P_{1}\right)\mu\left(P_{2}\right)$.
...
\end{eqnarray*}
Therefore, $P_{1}$ and $P_{3}$ commute, but not independent. On
...
\begin{eqnarray*}
\ket{\phi^{\pm}} & = & \frac{1}{\sqrt{2}}\left(\ket{00}\pm\ket{11}\right)\textrm{ ,}\\
...
\]
With the probability measure~$\mu\left(P\right)=\melement{\psi}{P}$,
$\proj{\phi^{+}}+\proj{\psi^{+}}$ and $\proj{\phi^{+}}+\proj{\psi^{-}}$
are independent as we expect.\qed\end{example}

The definition of independence is interesting, and we definitely need
to discuss it when we want to discuss Bell's theorem and the Kochen-Specker
theorem. However, the definition diverges, and our definition didn't come from
any sources directly. We may need to check more papers to find a source or
adopt one of the majority definition (if there is one)...}
