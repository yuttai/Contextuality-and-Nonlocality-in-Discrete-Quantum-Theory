...
maps a linear space to its projection operator. In the Dempster-Shafer
theory, the additivity property is relaxed such that $\delta\left(E_{1},E_{2}\right)\ge0$
...
is then analog to $\Tr\left(\rho\mathfrak{D}\left(S_{1},S_{2}\right)\right)$
given the density matrix~$\rho$ since the non-additivity operator~$\mathfrak{D}\left(S_{1},S_{2}\right)$
is non-zero when $S_{1}$ and $S_{2}$ does not commute. 
