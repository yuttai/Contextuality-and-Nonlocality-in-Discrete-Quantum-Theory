...
for all $E_{1},E_{2}\in\events$~\cite{Shapley1971,GilboaSchmeidler1994,Marinacci1999,Teran2014,Grabisch2016}.\footnote{\yutsung{In order to simplify the discussion, let $l$ be the left-end
...
\begin{equation}
l\left(E_{1}\cup E_{2}\right)+l\left(E_{1}\cap E_{2}\right)\ge l\left(E_{1}\right)+l\left(E_{2}\right)\textrm{ ,}\label{eq:convex-game}
\end{equation}
which is a special case of the Dempster-Shafer condition~\cite{Shafer1976,Grabisch2016}
\begin{equation}
l\left(\bigcup_{i=1}^{N}E_{i}\right)\ge\sum_{I\subseteq\left\{ 1,\ldots,N\right\} }\left(-1\right)^{\left|I\right|+1}l\left(\bigcap_{i\in I}E_{i}\right)\textrm{ .}\label{eq:Dempster-Shafer-game}
\end{equation}
...
Instead, they introduce a basic belief assignment~$m\left(E\right)=\sum_{E'\subseteq E}\left(-1\right)^{\left|E\setminus E'\right|}l\left(E'\right)$
first, and $m\left(E\right)$ represents the belief committed to the
event~$E$ and only to it, in the sense that it could not be committed
to any proper subset of $A$. Then, equation~(\ref{eq:Dempster-Shafer-game})
is equivalent to $m\left(E\right)\ge0$ for all $E$, and the non-negative
of $m\left(E\right)$ is intuitive.
...
again, and the symmetric answer doesn't really work. Hence, currently
only back-track randomized program still searches IVPM with a extremely
slow speed...
...
satisfies equation~(\ref{eq:convex-game}) when $E_{1}\cap E_{2}=\emptyset$
or $E_{1}\cup E_{2}=\Omega$. This condition is much weaker than convex,
and I would be surprise that this condition is strong enough to imply
...
but not $\ket{1}$, and it is no longer valid for more precise measurement
equipment as well. In general, we believe if the measurement equipment
is more and more precise, its interval-valued probability measure
will be closer and closer to the the Born rule. In the limit case,
...
half of those states need special treatment, i.e., when where $0\le\theta<\frac{\pi}{2}$
...
\begin{enumerate}
\item When $\ket{\psi_{1}}$ is $\ket{0}$, then either $\ket{\psi_{2}}$
or $\ket{\psi_{3}}$ must be $\ket{0_{\theta,\gamma}^{\perp}}$ for
...
Then, we have 
\begin{equation}
\begin{aligned}\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{0_{\theta,\gamma}^{\perp}}\right)=\unlikely & \subseteq\unlikely\tensor*[_{\mathscr{I}}]{+}{}\unlikely=\bar{\mu}\left(\proj{\psi_{2}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{3}}\right)\\
...
verified. 
\begin{equation}
\bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}}\right)=\likely\subseteq\unlikely\tensor*[_{\mathscr{I}}]{+}{}\unlikely=\bar{\mu}\left(\proj{\psi_{2}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{3}}\right)\label{eq:case3}
\end{equation}
\end{enumerate}
...
In order to verify example~\ref{fig:three-dimensional-4-value} cannot
hold with more precise intervals. Consider $\mathscr{I}=\left\{ \imposs,\left[l_{1},r_{1}\right],\left[1-r_{1},1-l_{1}\right],\necess\right\} $,
where $\left[l_{1},r_{1}\right]$ and $\left[1-r_{1},1-l_{1}\right]$
will be used to replace every $\unlikely$ and $\likely$ in example~\ref{fig:three-dimensional-4-value},
respectively. In order to make the equations corresponding to equation~(\ref{eq:case1}),
(\ref{eq:case2}), and (\ref{eq:case3}) hold, we could estimate the
bound of $l_{1}$ and $r_{1}$.

When we replace $\unlikely$ and $\likely$ by $\left[l_{1},r_{1}\right]$
and $\left[1-r_{1},1-l_{1}\right]$, equation~(\ref{eq:case1}) will
become

\begin{equation}
\begin{aligned}\necess & \subseteq\left[1-r_{1},1-l_{1}\right]\tensor*[_{\mathscr{I}}]{+}{}\left[l_{1},r_{1}\right]\\
\left[l_{1},r_{1}\right] & \subseteq\left[l_{1},r_{1}\right]\tensor*[_{\mathscr{I}}]{+}{}\imposs\\
\left[1-r_{1},1-l_{1}\right] & \subseteq\imposs\tensor*[_{\mathscr{I}}]{+}{}\left[1-r_{1},1-l_{1}\right]
\end{aligned}
\label{eq:case1-1}
\end{equation}
...
\left[1-r_{1},1-l_{1}\right] & \subseteq\left[l_{1},r_{1}\right]\tensor*[_{\mathscr{I}}]{+}{}\left[1-r_{1},1-l_{1}\right]
\end{aligned}
\label{eq:case2-1}
\end{equation}
...
become
\begin{equation}
\left[1-r_{1},1-l_{1}\right]\subseteq\left[l_{1},r_{1}\right]\tensor*[_{\mathscr{I}}]{+}{}\left[l_{1},r_{1}\right]\label{eq:case3-1}
\end{equation}
which implies $\frac{1}{2}\le r_{1}\le1$. Therefore, $l_{1}=0$ and
$r_{1}=\frac{1}{2}$ provides the most precise intervals which make
example~\ref{fig:three-dimensional-4-value} work, and in this case,
