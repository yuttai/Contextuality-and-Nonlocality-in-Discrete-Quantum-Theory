...
\begin{definition}Given a vector~$\ket{\psi}\in\mathbb{C}^{3}$,
$\sigma_{\ket{\psi}}$ is the clockwise rotation by $\frac{\pi}{2}$
about $\ket{\psi}$. More explicitly, for any vector~$\ket{\phi}\in\mathbb{C}S^{2}$,
$\sigma_{\ket{\psi}}\ket{\phi}=\frac{\ket{\psi}\times\ket{\phi}}{\left\Vert \ket{\psi}\times\ket{\phi}\right\Vert }$.\qed\end{definition}
...
by
\[
g_{f,\ket{0}}\left(\ket{\psi}\right)=f\left(\ket{\psi}\right)+f\left(\sigma_{\ket{0}}\ket{\psi}\right)\textrm{ .}
\]
\qed\end{definition}
...
\item $\ket{2}=\sigma_{\ket{0}\times\ket{\bar{0}}}\ket{0}$.
\item Choose $\ket{1}\in S^{2}$ such that $\ip{1}{0}=\ip{1}{2}=0$. For
example, we may choose $\ket{1}=\ket{2}\times\ket{0}$.
\item $Q\left(\ket{\psi}\right)=M\ip{\psi}{0}\ip{0}{\psi}+\left(W-M-m\right)\ip{\psi}{1}\ip{1}{\psi}+m\ip{\psi}{2}\ip{2}{\psi}$,
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\sup_{\theta\in\left[0,\frac{\pi}{2}\right)}\min\left\{ \min_{n\ge n\left(\theta\right)}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right),2\omega_{f}\left(2\cos\theta\right)\right\} $,
and 
...
for all $\ket{\psi}\in S^{2}$.\qed\end{thm}
\begin{proof}
Let $\ket{\bar{2}}=\sigma_{\ket{0}\times\ket{\bar{0}}}\ket{\bar{0}}$.
Then 
\[
...
\begin{eqnarray*}
\sigma_{\ket{0}}\left(x,y,z\right) & = & \left(x,-z,y\right)\\
\sigma_{\ket{2}}\left(x,y,z\right) & = & \left(-y,x,z\right)
\end{eqnarray*}
Note that 
\begin{eqnarray*}
Q\left(\ket{\psi}\right)+Q\left(\sigma_{\ket{0}}\ket{\psi}\right) & = & 2Mx^{2}+\left(W-M\right)\left(y^{2}+z^{2}\right)\\
Q\left(\ket{\psi}\right)+Q\left(\sigma_{\ket{2}}\ket{\psi}\right) & = & 2mz^{2}+\left(W-m\right)\left(x^{2}+y^{2}\right)
\end{eqnarray*}
...
we should have 
\begin{eqnarray*}
\left|f'\left(\ket{\psi}\right)+f'\left(\sigma_{\ket{0}}\ket{\psi}\right)\right| & = & \left|g_{f,\ket{0}}-\left(2Mx^{2}+\left(W-M\right)\left(y^{2}+z^{2}\right)\right)\right|\le9\delta\left(0,W_{r}-W_{l},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)\\
\left|f'\left(\ket{\psi}\right)+f'\left(\sigma_{\ket{2}}\ket{\psi}\right)\right| & = & \left|g_{f,\ket{2}}-\left(2mz^{2}+\left(W-m\right)\left(x^{2}+y^{2}\right)\right)\right|\le9\delta\left(W_{r}-W_{l},W_{r}-W_{l},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)
\end{eqnarray*}
...
\yutsung{\begin{thm}\label{thm:analogTheorem3.5Gleason1957}~
\begin{itemize}
...
\item $\ket{2}=\sigma_{\ket{0}\times\ket{\bar{0}}}\ket{0}$, $\ket{1}=\ket{2}\times\ket{0}$, 
\item $Q\left(\ket{\psi}\right)=M\ip{\psi}{0}\ip{0}{\psi}+\left(W-M-m\right)\ip{\psi}{1}\ip{1}{\psi}+m\ip{\psi}{2}\ip{2}{\psi}$,
\item $\delta\left(\eta,W_{\Delta},\omega_{f}\right)=\sup_{\theta\in\left[0,\frac{\pi}{2}\right)}\min\left\{ \min_{n\ge n\left(\theta\right)}2n\eta+4nW_{\Delta}+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right),2\omega_{f}\left(2\cos\theta\right)\right\} $,
and 
\item $f'\left(\ket{\psi}\right)=f\left(\ket{\psi}\right)-Q\left(\ket{\psi}\right)$.
...
\yutsung{
\begin{proof}
For every vector~$\ket{\psi}\in\mathbb{C}S^{1}$, consider the real
vector space spanned by $\ket{0'}=\lambda_{0}\ket{0}$, $\ket{\bar{0}'}=\lambda_{\bar{0}}\ket{\bar{0}}$,
$\ket{\psi'}=\lambda_{\psi}\ket{\psi}$, where $\lambda_{0}$, $\lambda_{\bar{0}}$,
and $\lambda_{\psi}\in\mathbb{C}$, and denote its unit sphere as
$S^{2}$. Then, 
\begin{eqnarray*}
f\left(\ket{0'}\right)=f\left(\ket{0}\right)=\max_{\ket{\phi}\in\mathbb{C}S^{2}}f\left(\ket{\phi}\right)\ge\max_{\ket{\phi}\in S^{2}}f\left(\ket{\phi}\right)\ge f\left(\ket{0'}\right) & \Rightarrow & \max_{\ket{\phi}\in S^{2}}f\left(\ket{\phi}\right)=f\left(\ket{0'}\right)\\
f\left(\ket{\bar{0}'}\right)=f\left(\ket{\bar{0}}\right)=\min_{\ket{\phi}\in\mathbb{C}S^{2}}f\left(\ket{\phi}\right)\le\min_{\ket{\phi}\in S^{2}}f\left(\ket{\phi}\right)\le f\left(\ket{\bar{0}'}\right) & \Rightarrow & \min_{\ket{\phi}\in S^{2}}f\left(\ket{\phi}\right)=f\left(\ket{\bar{0}'}\right)\textrm{ .}
\end{eqnarray*}
And let 
\begin{eqnarray*}
\ket{2'} & = & \sigma_{\ket{0'}\times\ket{\bar{0}'}}\ket{0'}=\frac{\left[\left(\lambda_{0}\ket{0}\right)\times\left(\lambda_{\bar{0}}\ket{\bar{0}}\right)\right]\times\left(\lambda_{0}\ket{0}\right)}{\left\Vert \left[\left(\lambda_{0}\ket{0}\right)\times\left(\lambda_{\bar{0}}\ket{\bar{0}}\right)\right]\times\left(\lambda_{0}\ket{0}\right)\right\Vert }=\frac{\lambda_{0}\lambda_{\bar{0}}\lambda_{0}^{*}\left[\left(\ket{0}\times\ket{\bar{0}}\right)\times\left(\ket{0}\right)\right]}{\left\Vert \lambda_{0}\lambda_{\bar{0}}\lambda_{0}^{*}\left[\left(\ket{0}\times\ket{\bar{0}}\right)\times\left(\ket{0}\right)\right]\right\Vert }=\sigma_{\ket{0}\times\ket{\bar{0}'}}\ket{0}\frac{\lambda_{\bar{0}}}{\left|\lambda_{\bar{0}}\right|}=\ket{2}\frac{\lambda_{\bar{0}}}{\left|\lambda_{\bar{0}}\right|}
\end{eqnarray*}
Is $\ket{0}\times\ket{\bar{0}'}\in S^{2}$? Can't we find a rotate
$\ket{0'}$ in $S^{2}$? And then $\ket{2'}$ will be obviously in
$S^{2}$? Then, the problem will be whether $\ket{2'}$ parallel to
$\ket{2}$? Use Mathematica to compute some example?
\begin{eqnarray*}
 &  & \left|Q\left(\ket{\psi}\right)-f\left(\ket{\psi}\right)\right|\\
 & = & \left|Q\left(\ket{\psi}\right)-f\left(\sum_{i=0}^{2}\ket{i}\ip{i}{\psi}\right)\right|\\
 & = & \left|Q\left(\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\ket{\psi}\right)-f\left(\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\sum_{i=0}^{2}\ket{i}\ip{i}{\psi}\right)\right|\\
 & = & \left|Q\left(\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\ket{\psi}\right)-f\left(\ket{0}\left|\ip{0}{\psi}\right|+\sum_{i=1}^{2}\ket{i}\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\frac{\ip{i}{\psi}}{\left|\ip{i}{\psi}\right|}\left|\ip{i}{\psi}\right|\right)\right|
\end{eqnarray*}
In order to estimate the last equation, we need to verify the conditions
listed in theorem~\ref{thm:analogTheorem23RichmanBridges1999}.

First, $\ket{0}$, $\ket{1'}=\ket{1}\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\frac{\ip{1}{\psi}}{\left|\ip{1}{\psi}\right|}$,
and $\ket{2'}=\ket{2}\frac{\left|\ip{0}{\psi}\right|}{\ip{0}{\psi}}\frac{\ip{2}{\psi}}{\left|\ip{2}{\psi}\right|}$
form an orthonormal basis, and the restriction of $f$ on the real
vector space spanned by $\ket{0}$, $\ket{1'}$, and $\ket{2'}$ is
still a approximation frame function of weight~$\left(W_{l},W_{r}\right)$.
Because $f\left(\ket{0}\right)=M$, we must also have $f\left(\ket{0}\right)=\max_{\ket{\psi}\in S^{2}}f\left(\ket{\psi}\right)$. 
\end{proof}
}
