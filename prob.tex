...
and $\ket{\ps'}=\frac{1}{\sqrt{2}}(\ket{0}+\ket{2})$. Let 
\begin{eqnarray*}
...
\]
so that no quantum probability measure~$\mu$ such that $\bar{\mu}(P)=\iota\left(\mu(P)\right)$.\qed\end{example}
...
\yutsung{Is there a quantum interval-value probability measure which
corresponds to no quantum probability measure for $\mathscr{I}=\left\{ \imposs,\unlikely,\likely,\necess\right\} $?
This is still important because \emph{$\mathscr{I}=\left\{ \necess,\imposs,\unknown\right\} $}
is way to small for a general case.}
