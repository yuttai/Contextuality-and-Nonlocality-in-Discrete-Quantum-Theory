...
\item When $\ket{\psi}$ is parallel to $\rme^{\rmi\gamma}\sin\theta\ket{1}+\cos\theta\ket{2}$
or $-\rme^{\rmi\gamma}\sin\theta\ket{\ms}+\cos\theta\ket{2}$, where
$0\le\theta<\frac{\pi}{2}$ and $0\le\gamma<\pi$, we define $\bar{\mu}(\proj{\psi})=\likely$
and $\bar{\mu}(\mathbb{1}-\proj{\psi})=\unlikely$.
...
To verify $\bar{\mu}$ is a quantum interval-valued probability measure,
it is sufficient to check $\bar{\mu}\left(\proj{\psi_{1}}\tensor*[_{\mathscr{O}}]{+}{}\proj{\psi_{2}}\right)\subseteq\bar{\mu}\left(\proj{\psi_{1}}\right)\tensor*[_{\mathscr{I}}]{+}{}\bar{\mu}\left(\proj{\psi_{2}}\right)$
for orthogonal $\ket{\psi_{1}}$ and $\ket{\psi_{2}}$. We can enumerate
all possible cases as follow. 
\begin{enumerate}
...
\begin{eqnarray*}
 &  & \bar{\mu}\left(\proj{\psi_{1}}\tensor*[_{\mathscr{O}}]{+}{}\proj{\psi_{2}}\right)\\
 & = & \bar{\mu}\left(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\psi_{1}\times\psi_{2}}\right)
\end{eqnarray*}
\end{enumerate}
\qed\end{example}
