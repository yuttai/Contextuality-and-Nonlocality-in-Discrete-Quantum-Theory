...
\begin{document}
\title{Discrete Probability for Discrete Quantum Theories}
\author{}
\date{\today}
\maketitle
...
several variations that avoid using the real interval $[0,1]$.
...
resources. We expand on this observation and then consider two
approaches from the literature: set-valued probability
...
We analyze this situation in the
mathematical framework of probability spaces paying special attention
to the resources needed to estimate the probability computationally or
experimentally.
...
of sequences of characters of length $N$ where each
character is either $X$ to indicate a needle crossing a line or $-$ to
indicate a needle not crossing a line. If $N=3$, the event that
exactly 2 needles cross lines is $\{-XX,X-X,XX-\}$: it has
...
experiment with $3408$ needles, the probability was
$0.6366197193098592$. 
\\
\yutsung{Add $\ldots$ at the end? }\\
Comparing the values to the idealized
mathematical value, we see that the observed probability approaches
$\frac{2}{\pi}$ but only if \emph{larger and larger resources} are
expended. 
...
allocated for the measurement process.
...
that exactly $M$ needles cross lines, we might instead talk about the
event that $M-\delta$ needles evidently cross lines and $M+\delta'$
needles plausibly cross lines. This fuzzy notion of events leads us to
consider intervals of probability reflecting the certainty of the
event and its plausibility. 
...
The singleton closed interval $[0,0]$ is a required spacial value
...
must therefore have probability $\left[1-b,1-a\right]$ which we abbreviate
$1-\left[a,b\right]$, so the probability measure $\pmeas$ should
satisfy:
\begin{itemize}
...
\begin{example}{[}Two-coin probability space with four intervals{]}
...
\end{array}\end{array}
\]
This probability measure is more informative than the one in Ex.~\ref{ex2}:
not only does it reveal that the second coin is double-headed but
it also reveals the bias in the first coin. \qed\end{example}
