...
consider subsets~$\mathscr{L}$ of $[0,1]$ and only consider probability
measures up to set membership with intuitive addition\footnote{Notice that the set-valued measure in~\cite{Artstein1972,PuriRalescu1983},
only require $I_{1}+I_{2}=\left\{ a_{1}+a_{2}\middle|a_{1}\in I_{1},a_{2}\in I_{2}\right\} $
because they focused on measure theory. In probability thoery, $2\in\pmeas\left(E\right)$
doesn't really make sense so that we consider intersect $[0,1]$ in
our definition.} 
\[
I_{1}+I_{2}=\left\{ a_{1}+a_{2}\middle|a_{1}\in I_{1},a_{2}\in I_{2}\right\} \cap[0,1]\textrm{ .}
\]
Then, a probability measure in this case is modified to a function
...
\end{itemize}
The simplest such situation is to partition the interval $[0,1]$
into $\{0\}$ (which we will call $\imposs$) and the half-open interval
$(0,1]$ (which we will call $\poss$). We will call the resulting
set $\left\{ \imposs,\poss\right\} $, the set $\mathscr{L}_{2}$. 
...
for singlton sets $\pmeas(\{HH\})=\unlikely$, $\pmeas(\{HT\})=\imposs$,
