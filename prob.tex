...
or $m=\max f=f\left(\ket{\psi}\right)$, and $W=\frac{W_{l}+W_{r}}{2}$.
...
$2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$
for some integer $n\ge2\pi$. The exact $n$ could be computed to
minimize the oscillation when the function $f$ is given.\qed\end{lemma}
\begin{proof}
...
of $\ket{\phi}$ and $\ket{\phi'}$. Take $n$ steps along east-west
great circles to go from $\ket{\phi'}$ to a point $\ket{\phi_{s}}$
due south of $\ket{\phi}$. The difference in latitude between $\ket{\phi}$
and $\ket{\phi_{s}}$ is at most $\frac{a^{2}}{n}$, while, from (Lemma
16 to be typed),
\[
g\left(\ket{\phi_{s}}\right)\ge g\left(\ket{\phi'}\right)-n\left[2f\left(\ket{\psi}\right)+4\left(W_{r}-W_{l}\right)\right]=g\left(\ket{\phi'}\right)-2n\eta-4n\left(W_{r}-W_{l}\right)\textrm{ .}
\]
...
\item $g$ be the symmetrization of $f$ with respect to $\ket{0}$, and 
\item $Q\left(\ket{\phi}\right)=W\left(1-\ip{0}{\phi}^{2}\right)$. 
\end{itemize}
Then, $\left|Q\left(\ket{\phi}\right)-g\left(\ket{\phi}\right)\right|\le$.\qed\end{thm}
\begin{proof}
