...
\begin{definition}\cite{WheedenZygmund1977}~For any continuous
function~$f:S^{d-1}\rightarrow\mathbb{R}$, the modulus of continuity
of $f$ is given by $\omega_{f}\left(\delta\right)=\sup_{\left\Vert \ket{\psi}-\ket{\phi}\right\Vert <\delta}\left|f\left(\ket{\psi}\right)-f\left(\ket{\phi}\right)\right|$.\qed\end{definition}

\begin{definition}\cite{RichmanBridges1999}~If $f:S^{2}\rightarrow\mathbb{R}$
is a frame function of weight~$\left(W_{l},W_{r}\right)$, and $\ket{\psi}\in S^{2}$,
then the \emph{symmetrization} of $f$ with respect to $\ket{\psi}$
...
\begin{thm}Let $f$ be an bounded continuous frame function of weight~$\left(W_{l},W_{r}\right)$,
...
\end{itemize}
then $\left|Q\left(\ket{\phi}\right)-g\left(\ket{\phi}\right)\right|\le\delta+3\left(W_{r}-W_{l}\right)\left[12\left\lceil \frac{\pi^{2}}{\omega_{f}^{-1}\left(\frac{\delta}{36}\right)}\right\rceil +1\right]$,
