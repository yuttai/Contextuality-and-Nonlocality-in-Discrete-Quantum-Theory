...
$\ket{\phi}$ induces a probability measure $\mu^B_{\phi}$
as follows: 
\[
\mu^B_{\phi}(E)=\ip{\phi}{E\phi}
\]
...
\begin{eqnarray}
\mu^B_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\mu^B_{\phi_{j}}\left(E\right)\textrm{ .}\label{BornRule.mixed}
\end{eqnarray}
...
\]
However, $\mu^B_{\rho}(E)=1$ if and only if $\rho$ represents
...
by $\bar{\mu}\left(E\right)=\left[\mu\left(E\right),\mu\left(E\right)\right]$.
...
by 
\[
\bar{\mu}(E)=\iota\left(\mu(E)\right)\textrm{ ,}
\]
...
probability measure. Notice that for all projection~$E$, $\mu^B_{\ket{0}}(E)\in\bar{\mu}(E)$
implies that $\bar{\mu}(E)$ is one of quantum
$\mathscr{I}$-interval-valued probability measures corresponding
to $\mu^B_{\ket{0}}(E)$. However,
$\mu^B_{\ket{0}}(E)$ can correspond
...
\begin{equation}
\mu^B_{\psi}(E)=\frac{\melement{\psi}{E}}{\ip{\psi}{\psi}}\label{eq:Born}
\end{equation}
...
\begin{eqnarray}
\mu^B_{\rho}\left(E\right) & = & \Tr\left(\rho E\right)=\sum_{j=1}^{N}q_{j}\mu^B_{\Psi_{j}}\left(E\right)\textrm{ .}\label{BornRule.mixed}
\end{eqnarray}
...
\item $\mu^B_{\psi}\left(E\right)=\mu^B_{\mathbf{U}\ket{\psi}}\left(\mathbf{U}E\mathbf{U}^{\dagger}\right)\textrm{ ,}$where
...
a mixed state ~$\rho'$, such that $\mu'^B_{\phi}\left(E\right)=\Tr\left(\rho'E\right)=\sum_{j=1}^{N}q_{j}\mu^B_{\psi_{j}}\left(E\right)$
for all event $E$. 

Consider the event $E'=\mathbb{1}-\proj{\phi}$, we have 
\begin{eqnarray*}
0 & \overset{\textrm{Condition 3}}{=} & \mu^B_{\phi}\left(E'\right)\\
 & = & \sum_{j=1}^{N}q_{j}\mu^B_{\psi_{j}}\left(E'\right)
\end{eqnarray*}
Because $q_{j}>0$, we have $\mu^B_{\psi_{j}}\left(E\right)=0$,
