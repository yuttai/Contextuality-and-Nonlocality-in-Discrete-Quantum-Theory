...
where $0\le\theta\le\frac{\pi}{2}$ and $0\le\gamma<2\pi$, as the
red and green circles in figure~\ref{fig:three-dimensional-4-value}.
...
we must have is $\mu_{\rho}^{B}(\proj{0})=\mu_{\rho}^{B}(\proj{\ps})=0$
so that $\mu_{\rho}^{B}=\mu_{\ket{2}}^{B}$. However, 
\[
\mu_{\ket{2}}^{B}(\proj{2})=1\notin\unlikely=\bar{\mu}(\proj{2})\textrm{ .}
\]
A contradiction! Hence, we have $\mu(P)\notin\bar{\mu}(P)$ for all
quantum probability measure~$\mu$.
