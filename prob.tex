...
due south of $\ket{\psi}$\footnote{Double check how many step at least from $\ket{\psi'}$ to a point
$\ket{\psi_{s}}$.}. The difference in latitude between $\ket{\psi}$ and $\ket{\psi_{s}}$
is at most $\frac{a^{2}}{n}$, while, from Lemma \footnote{Analog to Lemma 16 in \cite{RichmanBridges1999}... To be typed...},
\[
...
\begin{itemize}
\item Let $f$ be a nonnegative approximation frame function of weight~$\left(W_{l},W_{r}\right)$,
...
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g\left(\ket{\psi}\right)\right|\le?$.\qed\end{thm}
\begin{proof}
Consider $\ket{0}$ to be the north pole of $S^{2}$. Note that $g$
is a nonnegative frame function of weight~$\left(2W_{l},2W_{r}\right)$,
and that $W_{l}\le\eta+g\left(\ket{\psi}\right)\le W_{r}$ for all
$\ket{\psi}$ on the equator. From Lemma~\ref{lemma:analogLemma20RichmanBridges1999},
...
For each $\varphi$ define $F_{\varphi}$ on $\left[0,1\right]$ by
setting $F_{\varphi}\left(\ell\left(\ket{\psi}\right)\right)=W-g\left(\ket{\psi}\right)$
where $\ket{\psi}$ is a point in the closed northern hemisphere with
longitude~$\varphi$. Then, $\left|F_{\varphi}\left(a_{i}\right)-\left(W-g\left(\ket{\psi_{i}}\right)\right)\right|\le2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$
\end{proof}
