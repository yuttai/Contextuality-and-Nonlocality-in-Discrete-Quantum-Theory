...
\rme^{\rmi\left(\pi+\gamma\right)}\sin\left(\frac{\pi}{2}-\theta\right)\\
\cos\left(\frac{\pi}{2}-\theta\right)
\end{pmatrix}=\sqrt{2}\rme^{-\rmi\gamma}\ket{\ps_{\frac{\pi}{2}-\theta,\pi+\gamma}^{\perp}}
\]
...
Although example~\ref{ex:three-dimensional-three-value} and \ref{ex:three-dimensional-4-value}
rely on $\imposs$ to deduce that an interval-valued probability measure
has an empty core, it is not necessary. The following example is modified
from example~\ref{ex:three-dimensional-4-value}, but has the property
that $\bar{\mu}\left(P\right)=\imposs$ if and only if $P=\mathbb{0}$.

\begin{example}[Another three-dimensional quantum 4-value interval-valued
probability measure]\label{ex:three-dimensional-4-value-2} Consider\emph{
}$\mathscr{I}=\left\{ \imposs,\unlikely,\likely,\necess\right\} $,
and $\ket{0}$, $\ket{1}$, $\ket{2}$, $\ket{\ps}$, $\ket{\ms}$,
and $\bar{\mu}$ are defined as in example~\ref{ex:three-dimensional-4-value}
except 
\begin{eqnarray*}
\bar{\mu}(\proj{0})=\bar{\mu}(\proj{\ps}) & = & \unlikely\\
\bar{\mu}(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{0})=\bar{\mu}(\mathbb{1}\tensor*[_{\mathscr{O}}]{-}{}\proj{\ps}) & = & \likely\textrm{ .}
\end{eqnarray*}
By following the same as process as in example~\ref{ex:three-dimensional-4-value},
it is easy to verify that $\bar{\mu}$ is an interval-valued probability
measure.
...
and $0\le\gamma<\pi$, we have $\mu_{\rho}^{B}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)=\frac{1}{2}$.
On one hand, $\mu_{\rho}^{B}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)\in\bar{\mu}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)=\likely$
implies $\mu_{\rho}^{B}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)\ge\frac{1}{2}$.
...
these is always a state~$\ket{\psi}$ such that $\mu_{\rho}^{B}\left(\proj{\psi}\right)\in\bar{\mu}\left(\proj{\psi}\right)=\unlikely$,
i.e., $\mu_{\rho}^{B}\left(\proj{\psi}\right)\le\frac{1}{2}$. Because
$\mu_{\rho}^{B}$ is continuous, we have $\mu_{\rho}^{B}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)=\frac{1}{2}$.
Similarly, $\mu_{\rho}^{B}\left(\proj{\ps_{\theta,\gamma}^{\perp}}\right)=\frac{1}{2}$.

Consider a general density matrix~ $\rho=\tensor*[_{\mathscr{O}}]{\sum}{_{i=0}^{2}}\tensor*[_{\mathscr{O}}]{\sum}{_{j=0}^{2}}r_{ij}\ket{i}\bra{j}$.
Then, we have 
\begin{eqnarray*}
\frac{1}{2} & = & \mu_{\rho}^{B}\left(\proj{0_{\theta,\gamma}^{\perp}}\right)\\
...
and $0\le\gamma<\pi$, we must have $r_{12}=r_{21}=0$ and $r_{11}=r_{22}=\frac{1}{2}$.
We also have
\begin{eqnarray*}
\frac{1}{2} & = & \mu_{\rho}^{B}\left(\proj{\ps_{\theta,\gamma}^{\perp}}\right)\\
 ...
 & = & -\frac{\rme^{-\rmi\gamma}\sin\theta}{\sqrt{2}}\left(-\frac{r_{00}\rme^{\rmi\gamma}\sin\theta}{\sqrt{2}}+\frac{r_{01}\rme^{\rmi\gamma}\sin\theta}{\sqrt{2}}+r_{02}\cos\theta\right)+\frac{\rme^{-\rmi\gamma}\sin\theta}{\sqrt{2}}\left(-\frac{r_{10}\rme^{\rmi\gamma}\sin\theta}{\sqrt{2}}+\frac{\rme^{\rmi\gamma}\sin\theta}{2\sqrt{2}}\right)\\
 &  & +\cos\theta\left(-\frac{r_{20}\rme^{\rmi\gamma}\sin\theta}{\sqrt{2}}+\frac{\cos\theta}{2}\right)\\
 & = & \left(\frac{r_{00}\sin^{2}\theta}{2}-\frac{r_{01}\sin^{2}\theta}{2}-\frac{r_{02}\rme^{-\rmi\gamma}\sin\theta\cos\theta}{\sqrt{2}}\right)+\left(-\frac{r_{10}\sin^{2}\theta}{2}+\frac{\sin^{2}\theta}{4}\right)+\left(-\frac{r_{20}\rme^{\rmi\gamma}\cos\theta\sin\theta}{\sqrt{2}}+\frac{\cos^{2}\theta}{2}\right)\\
 & = & \left(\frac{r_{00}}{2}-\frac{r_{01}}{2}-\frac{r_{10}}{2}+\frac{1}{4}\right)\sin^{2}\theta+\frac{\cos^{2}\theta}{2}-\left(r_{02}\rme^{-\rmi\gamma}+r_{20}\rme^{\rmi\gamma}\right)\frac{\cos\theta\sin\theta}{\sqrt{2}}
\end{eqnarray*}
Because of the same reason, we have $r_{02}=r_{20}=0$ and $r_{00}=\frac{1}{2}+r_{01}+r_{10}$.
Moreover, because $\rho$ is a density matrix, we should have $\Tr\left(\rho\right)=1$
and $\rho^{\dagger}=\rho$ so that $r_{00}=0$, $r_{01}=-\frac{1}{4}+\rmi b$,
and $r_{01}=-\frac{1}{4}-\rmi b$, where $b\in\mathbb{R}$. However,
it is easy to verify that $\rho$ will have a negative eigenvalue.
This contradicts to the fact that $\rho$ as a a density matrix must
be a positive operator.\qed\end{example} 
\end{document}
