...
\item Conversely, every projection~$P$ can be expressed as $\sum_{j}\proj{\psi_{j}}$,
where $P$ actually projects onto the linear subspace~$S$ which
has an orthonormal basis~$\{\ket{\psi_{j}}\}$. 
\end{itemize}
\end{definition}

Comparing to the classical events, the empty set~$\emptyset$ corresponds
to the empty projection~$\mathbb{0}$, the event of whole sample
space~$\Omega$ corresponds to the identity projection~$\mathbb{1}$.
