...
first, and $m\left(E\right)$ expresses the proportion of all relevant
and available evidence that supports the claim that the actual state
belongs to $A$ but to no particular subset of $A$. Then, equation~(\ref{eq:Dempster-Shafer-game})
is equivalent to $m\left(E\right)\ge0$ for all $E$~\cite{Grabisch2016},
and the later statement is intuitive.}}
\end{itemize}
\qed\end{definition}
...
equipment reveals too little information. 
