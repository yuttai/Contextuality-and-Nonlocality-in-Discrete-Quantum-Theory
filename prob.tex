...
between events. 
\item ~\\ \yutsung{The definition of independence is different from different
sources:\\
1. In Swart's~\cite{Swart2013}, given two $*$-algebras~$\mathscr{A}_{1}$ and
$\mathscr{A}_{2}$ which commute with each other. They are \emph{logically
...
with product state. The problem for us is that this defines the independent of
algebras, but not events\ldots\\
2. In Yuan's~\cite{Yuan2012}, given a $\mu$ on an $*$-algebra~$\mathscr{A}$,
...
for all $A_{1}\in\mathscr{A}_{1}$ and $A_{2}\in\mathscr{A}_{2}$.
The problem for us is the same as the previous one: this defines the
independent of algebras, but not events\ldots\\
3. In order to understand the definition of independence in~\cite{GuehneKleinmannCabelloEtAl2010},
we need first understand their notation. \cite{GuehneKleinmannCabelloEtAl2010}
focus on the hidden-variable (HV) models, but their notation may be
applied to the usual quantum model. \\
}
\end{itemize}
\qed\end{definition}
