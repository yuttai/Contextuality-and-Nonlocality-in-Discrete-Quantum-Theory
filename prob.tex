...
\subsection{Measuring Quantum Probabilities: Ideal Case}
...
if we pick a basis~$\left\{ \ket{\psi_{j}}\right\} _{j=1}^{d}$,
only consider the events~$\events'$ with the form $P=\tensor*[_{\mathscr{O}}]{\sum}{_{\ket{\psi_{j}}\in S}}\proj{\psi_{j}}$,
where $S$ is the linear subspace $P$ projects onto, and fix a density
matrix~$\rho$ representing in the same basis, i.e., $\rho=\sum_{i=1}^{d}\sum_{j=1}^{d}r_{ij}\ket{\psi_{i}}\bra{\psi_{j}}$.
The tuple~$\left(\left\{ \ket{\psi_{j}}\right\} _{j=1}^{d},\events',\rho\right)$
is essentially the same as an classical probability space~\cite{Maassen2010}.
...
\subsection{Measuring Quantum Probabilities: In Reality}

In reality, we need a lot of resource to keep the local field of strength
directing to the $z$-axis precisely, to keep the atoms having almost
the same velocity, and to point out the exact position each particle
landed on.
...
an interval-valued probability~$\left[l,r\right]$. Therefore, we
should plug in the definition of interval-valued probability measure
into the definition of quantum probability space for each ideal measurement.

More specifically, for classical probability measures, definition~\ref{def:ClassicalProbabilitySpace}
