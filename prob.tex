...
\begin{itemize}
\item Let $f$ be a nonnegative continuous\footnote{\yutsung{We need this ``continuous'' to apply Lemma~19 in \cite{RichmanBridges1999},
...
\begin{itemize}
\item Let $f$ be a bounded continuous approximation frame function of weight~$\left(W_{l},W_{r}\right)$,
$W=\frac{W_{l}+W_{r}}{2}$, $m=\min_{\ket{\psi}\in S^{2}}f\left(\ket{\psi}\right)$,
$\eta=f\left(\ket{0}\right)-m$, 
\item $g$ be the symmetrization of $f$ with respect to $\ket{0}$,
\item $Q\left(\ket{\psi}\right)=W-m+\left(3m-W\right)\ip{0}{\psi}^{2}=2m\ip{0}{\psi}^{2}+\left(W-m\right)\left(1-\ip{0}{\psi}^{2}\right)$,
and 
\item $\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)=\min_{n\ge2\pi}2n\eta+4n\left(W_{r}-W_{l}\right)+2\omega_{f}\left(2-2\cos\left(\frac{\pi^{2}}{n}\right)\right)$.
\end{itemize}
Then, $\left|Q\left(\ket{\psi}\right)-g\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$.\qed\end{cor}
\begin{proof}
...
and $g'=g-2m$ be the symmetrization of $f'$ with respect to $\ket{0}$
. Let $Q'\left(\ket{\psi}\right)=Q\left(\ket{\psi}\right)-2m=W'\left(1-\ip{0}{\psi}^{2}\right)$,
and we have $\left|Q\left(\ket{\psi}\right)-g\left(\ket{\psi}\right)\right|=\left|Q'\left(\ket{\psi}\right)-g'\left(\ket{\psi}\right)\right|\le9\delta\left(\eta,W_{l},W_{r},\omega_{f}\right)+3\left(W_{r}-W_{l}\right)$
...
\begin{thm}Let $f$ be a bounded continuous approximation frame function
of weight~$\left(W_{l},W_{r}\right)$ on $S^{2}$. Then there is
a quadratic form $Q$ on $\mathbb{R}^{3}$ so that $\left|f\left(x\right)-Q\left(x\right)\right|\le?$
for all $\ket{\psi}\in S^{2}$.\qed\end{thm}
\begin{proof}
Let $M=\max_{\ket{\psi}\in S^{2}}f\left(\ket{\psi}\right)=f\left(\ket{0}\right)$
and $m=\min_{\ket{\psi}\in S^{2}}f\left(\ket{\psi}\right)=f\left(\ket{0'}\right)$.
Let $\ket{2}$ and $\ket{2'}$ be the images of $\ket{\psi_{M}}$
and $\ket{\psi_{m}}$ under $90\textdegree$ rotation of the great
circle joining them. Then 
\[
\left|\left(f\left(\ket{\psi_{M}}\right)+f\left(\ket{\psi_{M}'}\right)\right)-\left(f\left(\ket{\psi_{m}}\right)+f\left(\ket{\psi_{m}'}\right)\right)\right|\le W_{r}-W_{l}
\]
so
\[
f\left(\ket{\psi_{m}'}\right)-f\left(\ket{\psi_{M}'}\right)\ge f\left(\ket{\psi_{M}}\right)-f\left(\ket{\psi_{m}}\right)-\left(W_{r}-W_{l}\right)=M-m-\left(W_{r}-W_{l}\right)
\]
whence $f\left(\ket{\psi_{M}'}\right)\le m+W_{r}-W_{l}$ and $f\left(\ket{\psi_{m}'}\right)\ge M-\left(W_{r}-W_{l}\right)$.
\end{proof}
