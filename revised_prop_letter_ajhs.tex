...
consider $x\le0$ case in Eq.~(\ref{eq:interval-times}) because the minus sign
appears in
Eq.~(\ref{eq:complement}).}  With the understanding that
the union of classical sets $E_1\cup E_2$ is replaced by
$P_0+P_1-P_0P_1$ in the case of quantum projection
...
$\eventsC$ of mutually commuting, \yutsung{Since ``mutually commuting'' is the
object of the proposition ``of'', is ``mutually commuting'' a noun?}
...
$\events'$ as a \emph{subspace} of a set of events {\color{purple}if} $\events$ if
...
uncountable range $[0,1]$. A QIVPM constructed by blurring \yutsung{Should we
change this ``blurring''?} such a
