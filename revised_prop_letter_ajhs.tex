...
\NewDocumentCommand{\newInterval}{o o o o}{
 \newcommand{#1}{\text{\wesa{{#2}}}}
 % \newcommand{#1}{\ensuremath{\left[{#3}\right]}}
 % \newcommand{#1}{\ensuremath{I\left({#4}\right)}}
}
\newInterval[\imposs][impossible][0,0][0]
\newInterval[\likely][likely][\tfrac{1}{2},1][\tfrac{1}{2},1]
\newInterval[\unlikely][unlikely][0,\tfrac{1}{2}][0,\tfrac{1}{2}]
\newInterval[\necess][certain][1,1][1]
\newInterval[\unknown][unknown][0,1][0,1]
\newInterval[\midd][middle][\tfrac{1}{4},\tfrac{3}{4}][\tfrac{1}{4},\tfrac{3}{4}]
...
\NewDocumentEnvironment{compare}{}{\begin{widetext}\end{widetext}}{\begin{widetext}\end{widetext}}
\NewDocumentEnvironment{oldText}{m}{#1{The following is old text.}\noindent\begin{minipage}[t]{1\columnwidth}}{#1{The old text end here.}\end{minipage}\newpage}
\NewDocumentEnvironment{newText}{m}{\noindent\begin{minipage}[t]{1\columnwidth}#1{The following is new text.}}{\end{minipage}#1{The new text end here.}}
...
\begin{compare}
\begin{oldText}{\yutsung}
To that end, we construct a family of QIVPMs parameterized by \emph{blurring}
functions $f:\left[0,1\right]\rightarrow\mathscr{I}$ that formalize
how the real-valued probabilities are mapped to intervals. We also
introduce a notion of norm that quantifies the distance between (pure
or mixed) states. The norm of a pure state $\rho=\proj{\psi}$ is
...
Then, a finite-precision extension of Gleason's theorem can be stated
as follows.
...
$f$ be a blurring function and let $f\circ\muB_{\rho}$ be a QIVPM,
where $\muB_{\rho}$ is the probability measure induced by the Born
rule for a given state~$\rho$. Let $\alpha$ be the maximum length
of intervals in its image. If a state $\rho'$ is consistent with
$f\circ\muB_{\rho}$ on all events, i.e., $\rho'\in\coreBorn\left(f\circ\muB_{\rho},\events\right)$,
...
$\unknown$. We also introduce a notion of norm that quantifies the
...
\begin{thm}[Finite-Precision Extension of the Gleason Theorem]\label{thm:Finite-precision-Gleason-1}Let
...
\end{tabular}
\end{table*}
It is Meyer's great insight to realize that the Kochen-Specker theorem
can be nullified by replacing the field of the reals by rational numbers.
In other words, Meyer exclusively attributes finite-precision errors
to the description of the states defined in a general Hilbert space.
The response of Mermin argues against Meyer's interpretation but accepts
...
theorem requires a different approach altogether (see for example
Ax and Kochen's communication cited by Cabello~\cite{Cabello2002}).
Our approach is agnostic about attributing elements of reality to
the state function (as Meyer did). We simply attribute lack of certainty
to the uncertain results obtained through the use of a measuring instrument.
Thus, the parameter~$\delta$ in our framework reflects insufficient
knowledge of the experimenter, which could be due to a variety of
reasons related to imperfections of devices. For example, a typical
question that an experimenter may not be able to answer accurately
would be ``did an electron land in the left half or the right half
of the screen?'' There are cases in which the electron would land
too close to the middle of the screen for the experimenter to be able
to determine with certainty that it was left or right of the center.
We simply record this imprecision in the probability we assign to
events, and use the axioms of probability theory to argue that a sharp
