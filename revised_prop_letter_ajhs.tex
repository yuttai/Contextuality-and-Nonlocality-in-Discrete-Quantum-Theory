...
guarantee} that there exists {\color{red}a state} $\rho$ that satisfies
...
$\events'$ as a \emph{subspace} of a set of {\color{red}events~$\events$} if
...
\begin{definition}[Expectation Value of Observables over QIVPMs] Let
  ...
  ${\color{red}\sum}_{i=1}^d \lambda_iP_i$. Let $\events'$ be the minimal
...
measurements~{\color{red}\cite{PhysRevLett.83.3751,HKSS1999apsrev4,Mermin1999,Kent1999,SimonBruknerZeilinger2001,Cabello2002,Larsson2002,Appleby2002,BarrettKent2004,Appleby_2005,Spekkens2005,GuehneKleinmannCabelloEtAl2010,MazurekPuseyKunjwalEtAl2016}}.
...
$\bar{\mu}{\color{red}\left(\sum_{j=0}^{d-1}{\proj{\psi_j}}\right)}=\necess$ and by
...
\begin{thm}[0-Deterministic Variant of the Kochen-Specker
  ...
  dimension $d\ge3$, there is no {\color{red}$0$}-deterministic measure~$\bar{\mu}$
...
\begin{lemma}\label{lemma:delta2zero} 
  ...
  $\bar{\mu}^{\color{red}D}:\events\rightarrow\left\{
    \imposs,\necess\right\}$ defined as follows:
\begin{equation}
\bar{\mu}^{\color{red}D}\left(P\right)=\begin{cases}
...
condition for $\bar{\mu}^{\color{red}D}$. By Lemma~\ref{lemma:verify-convexity}, it is sufficient
to verify the following equation for orthogonal projectors $P_{0}$
and~$P_{1}$,
\begin{equation}
\bar{\mu}^{\color{red}D}\left(P_{0}+P_{1}\right)=\bar{\mu}^{\color{red}D}\left(P_{0}\right)+\bar{\mu}^{\color{red}D}\left(P_{1}\right)\,, \label{eq:QuantumInterval-valuedProbability-Equal}
\end{equation}
for two conditions, which we now examine in detail.

When one of $\bar{\mu}^{\color{red}D}\left(P_{0}\right)$ and $\bar{\mu}^{\color{red}D}\left(P_{1}\right)$
is $\necess$, say $\bar{\mu}^{\color{red}D}\left(P_{0}\right)=\imposs$
and $\bar{\mu}^{\color{red}D}\left(P_{1}\right)=\necess$, we have $\bar{\mu}\left(P_{0}\right)\subseteq\left[0,\delta\right]$
...
which implies $\bar{\mu}^{\color{red}D}\left(P_{0}+P_{1}\right)$ is also
$\necess$, thus satisfying Eq.~(\ref{eq:QuantumInterval-valuedProbability-Equal}).

When both
$\bar{\mu}^{\color{red}D}\left(P_{0}\right)$ and $\bar{\mu}^{\color{red}D}\left(P_{1}\right)$
...
$\bar{\mu}^{\color{red}D}\left(P_{0}+P_{1}\right)=\imposs$, and hence
...
experiment {\color{red}$m$} times and count the number of times we witness the
...
\yutsung{The above new text seems not very smooth. How about the following?}
\noindent If this is true, then $\ip{0}{\phi_{j}}=\ip{\ps}{\phi_{j}}=\ip{\ps'}{\phi_{j}}=0$
for all~$j$. This follows that
\begin{subequations}
\begin{eqnarray}
...
theorem is proved.
\yutsung{End of the suggested text.}
...
To that end, we {\color{red}introduce the conept of \emph{interval maps} which
we can use to construct a consistent family of QIVPM.}
A{\color{red}n} interval map $f:\left[0,1\right]\rightarrow\mathscr{I}$
...
{\color{red}(this is unrelated to the interval-valued probability
$\unknown$).} \yutsung{How about change to ``(this should not be confused with
the interval-valued probability $\unknown$)''?}
We also need a notion of {\color{red}\emph{norm}} to quantify the
...
{\color{red} For any given Hermitian operator~$A$, we choose}
...
th{\color{red}is norm is the eigenvalue with maximum absolute value}. Then, a
