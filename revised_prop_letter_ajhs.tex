...
\NewDocumentEnvironment{oldText}{m}{\noindent\begin{minipage}[b]{1\columnwidth}#1{The following is old text.}}{#1{The old text end here.}\end{minipage}\newpage}
\NewDocumentEnvironment{newText}{m}{\noindent\begin{minipage}[b]{1\columnwidth}#1{The following is new text.}}{#1{The new text end here.}\end{minipage}}
...
consequence {\color{blue}of this fact} is that known properties of classical IVPMs
...
$\bar{\mu}\left(P\right)$. However, if {\color{blue}the set of events is} mutually
...
attempts at proving Gleason's theorem by extending the Born measure
$\muB_{\rho}\left(P\right)$ to QIVPMs
$\bar{\mu}\left(P\right)$. However, if {\color{blue}we restrict ourselves to the set
$\eventsC$ of} mutually
commuting, the situation reverts to the classical case in which
probabilities always determine at least one state.
\end{newText}
...
\begin{definition}[Consistency] We say a QIVPM $\bar{\mu}$
is \emph{consistent} with a state $\rho$ and a projector~$P$ if the
...
$\events'$ contains the projector $\mathbb{0}$ and $\mathbb{1}$ and is closed
under complements, sums, and products.  In particular, for any
