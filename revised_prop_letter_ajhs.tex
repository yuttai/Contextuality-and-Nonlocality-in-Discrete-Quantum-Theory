...
\yutsung{The new text seems not very smooth. How about the following?}
...
\end{subequations}
so
that$\ket{\phi_{j}}=\ket{0}\ip{0}{\phi_{j}}+\ket{1}\ip{1}{\phi_{j}}+\ket{2}\ip{2}{\phi_{j}}=0$.
This violates the fact that $\ket{\phi_{j}}$ is a normalized state, and thus the
theorem is proved.
...
$\unknown$.} We also need a notion of {\color{red}\emph{norm}} to quantify the
...
measure $\mu$, it is the case that if $f \circ \mu$ is a QIVPM, then the {\color{red}interval
...
Can this relate it to the following theorem:
\begin{thm}Any ($\rho \rightarrow \rho$)-computable function~$f$ mapping between
continuums, like unit interval $[0,1]$, is continuous
\cite{Ziegler2007,weihrauch2012computable}.\end{thm}
The idea is that when we mapping between continuums, only continuous functions
could be approximated, and discontinuous functions might be too wild to be approximated...
However, I don't know how to fit this idea with other part of the paper...
